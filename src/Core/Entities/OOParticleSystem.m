...
- (id)initWithPosition:(HPVector)pos
    ...
    if ((self = [super init])) {
        ...
        for (unsigned i = 0; i < count; i++) {
            GLfloat speed =
                minSpeed + 0.5f * (randf() + randf()) * (maxSpeed - minSpeed);  // speed tends toward middle of range
            _particleVelocity[i] = vector_multiply_scalar(OORandomUnitVector(), speed);

            Vector color = make_vector(baseColor[0] * 0.1f * (9.5f + randf()),
                                       baseColor[1] * 0.1f * (9.5f + randf()),
                                       baseColor[2] * 0.1f * (9.5f + randf()));
...
- (void)drawImmediate:(bool)immediate translucent:(bool)translucent {
    ...
    if ([UNIVERSE reducedDetail]) {
        ...
        float distanceThreshold =
            collision_radius *
            2.0f;  // Distance between player and middle of effect where we start to transition to "non-fast rendering."
        ...
        if (distanceSq > thresholdSq) {
            ...
            */
            ...
            for (i = 0; i < count; i++) {
                OOGLPushModelView();
                OOGLTranslateModelView(particlePosition[i]);
                OOGLMultModelView(OOMatrixForBillboard(
                    HPvector_add(selfPosition,
                                 vectorToHPVector(vector_multiply_scalar(particlePosition[i], individuality))),
                    viewPosition));
...
- (id)initFragmentBurstFrom:(HPVector)fragPosition velocity:(Vector)fragVelocity size:(GLfloat)size {
    enum { kMinSpeed = 100, kMaxSpeed = 400 };
    ...
    if ((self = [super initWithPosition:fragPosition
...
+ (id)fragmentBurstFromEntity:(Entity *)entity {
    return [[[self alloc] initFragmentBurstFrom:[entity position]
                                       velocity:[entity velocity]
                                           size:[entity collisionRadius]] autorelease];
}
...
+ (id)fragmentBurstFromEntity:(Entity *)entity {
    return [[[self alloc] initFragmentBurstFrom:[entity position]
                                       velocity:vector_multiply_scalar([entity velocity], 0.85)
                                           size:[entity collisionRadius]] autorelease];
}
