...
- (BOOL)setUpFromDictionary:(NSDictionary *)shipDict {
    ...
    max_flight_yaw = [shipDict oo_floatForKey:@ "max_flight_yaw"
                                 defaultValue:max_flight_pitch];  // Note by default yaw == pitch
    ...
    if (afterburner_speed_factor < 1.0) {
        ...
        OOLog(@ "ship.setup.injectorSpeed",
    ...
    // Each new ship should start in seemingly good operating condition, unless specifically told not to - this does not
    // affect the ship's energy levels
    ...
    if (weapon_facings & WEAPON_FACING_FORWARD)
        forward_weapon_type = OOWeaponTypeFromString([shipDict oo_stringForKey:@ "forward_weapon_type"
                                                                  defaultValue:@ "EQ_WEAPON_NONE"]);
    if (weapon_facings & WEAPON_FACING_AFT)
        aft_weapon_type = OOWeaponTypeFromString([shipDict oo_stringForKey:@ "aft_weapon_type"
                                                              defaultValue:@ "EQ_WEAPON_NONE"]);
    if (weapon_facings & WEAPON_FACING_PORT)
        port_weapon_type = OOWeaponTypeFromString([shipDict oo_stringForKey:@ "port_weapon_type"
                                                               defaultValue:@ "EQ_WEAPON_NONE"]);
    if (weapon_facings & WEAPON_FACING_STARBOARD)
        starboard_weapon_type = OOWeaponTypeFromString([shipDict oo_stringForKey:@ "starboard_weapon_type"
                                                                    defaultValue:@ "EQ_WEAPON_NONE"]);

    cloaking_device_active = NO;
    military_jammer_active = NO;
    cloakPassive = [shipDict oo_boolForKey:@ "cloak_passive"
                              defaultValue:YES];  // Nikos - switched passive cloak default to YES 20120523
    ...
    /* TODO: The following initializes the missile list to be blank, which prevents a crash caused by
       hasOneEquipmentItem trying to access a missile list previously initialized but then released.  See issue #204. We
       need to investigate further the cause of the missile list being released.
                    - kanthoney 10/03/2017
            Update 20170818: The issue seems to have been resolved properly using the fix below and the problem was
       apparently an access of the missile_list array elements before their initialization and while we were checking
       whether equipment can be added or not. There is probably not much more that can be done here, unless someone
       would like to have a go at refactoring the entire ship initialization code. In any case, the crash is no more and
       the applied solution is both simple and logical - Nikos
    */
    unsigned i;
    ...
    missile_load_time =
        fmax(0.0, [shipDict oo_doubleForKey:@ "missile_load_time" defaultValue:0.0]);  // no negative load times
    ...
    if ([shipDict oo_fuzzyBooleanForKey:@ "has_cloaking_device"])
    ...
    if ([shipDict oo_floatForKey:@ "has_energy_bomb"] > 0) {
        ...
        */
        if ([shipDict oo_fuzzyBooleanForKey:@ "has_energy_bomb"]) {
            if (max_missiles == missiles && max_missiles < SHIPENTITY_MAX_MISSILES &&
                [shipDict objectForKey:@ "max_missiles"] == nil) {
                max_missiles++;
            }
    ...
    if ([shipDict oo_fuzzyBooleanForKey:@ "has_fuel_injection"])
        [self addEquipmentItem:@ "EQ_FUEL_INJECTION" inContext:@ "npc"];

#if USEMASC
    if ([shipDict oo_fuzzyBooleanForKey:@ "has_military_jammer"])
        [self addEquipmentItem:@ "EQ_MILITARY_JAMMER" inContext:@ "npc"];
    if ([shipDict oo_fuzzyBooleanForKey:@ "has_military_scanner_filter"])
    ...
    hyperspaceMotorSpinTime = [shipDict oo_floatForKey:@ "hyperspace_motor_spin_time"
                                          defaultValue:DEFAULT_HYPERSPACE_SPIN_TIME];
    ...
    if (isWeaponNone(forward_weapon_type)) {
        ...
        if (isWeaponNone(weapon_type) &&
            hasTurrets) { /* safety for ships only equipped with turrets
    ...
    forwardWeaponOffset = [[self getWeaponOffsetFrom:shipDict withKey:@ "weapon_position_forward"
                                              inMode:weaponMountMode] retain];
    aftWeaponOffset = [[self getWeaponOffsetFrom:shipDict withKey:@ "weapon_position_aft"
                                          inMode:weaponMountMode] retain];
    portWeaponOffset = [[self getWeaponOffsetFrom:shipDict withKey:@ "weapon_position_port"
                                           inMode:weaponMountMode] retain];
    starboardWeaponOffset = [[self getWeaponOffsetFrom:shipDict
                                               withKey:@ "weapon_position_starboard"
                                                inMode:weaponMountMode] retain];
...
- (BOOL)setUpShipFromDictionary:(NSDictionary *)shipDict {
    ...
    // scan class settings. 'scanClass' is in common usage, but we could also have a more standard 'scan_class' key with
    // higher precedence. Kaks 20090810 let's see if scan_class is set...
    ...
    if (weapon_damage == 0.0) {
        weapon_damage_override = weapon_damage = [shipDict oo_floatForKey:@ "weapon_energy"
                                                             defaultValue:0];  // any damage value for missiles/bombs
    } else {
    ...
    fuel = [shipDict
        oo_unsignedShortForKey:@ "fuel"];  // Does it make sense that this defaults to 0? Should it not be 70? -- Ahruman
    ...
    if (cargoString != nil) {
        if ([cargoString isEqualToString:@ "SCARCE_GOODS"]) {
            ...
            if ([scanner scanInt:&c_amount]) {
                [scanner ooliteScanCharactersFromSet:[NSCharacterSet whitespaceCharacterSet]
                                          intoString:NULL];  // skip whitespace
    ...
    [roleSet release];
    roleSet =
        [[[OORoleSet roleSetWithString:[shipDict oo_stringForKey:@ "roles"]] roleSetWithRemovedRole:@ "player"] retain];
    [primaryRole release];
    primaryRole = nil;
    ...
    // these are the colors used for the "lollipop" of the ship. Any of the two (or both, for flash effect) can be
    // defined. nil means use default from shipData.
    ...
    for (i = 0, j = 0; i < missiles; i++) {
        ...
        // could loop forever (if missile_role is badly defined, selectMissile might return nil in some cases) . Try 3
        // times, and if no luck, skip
    ...
    if (accuracy < -5.0f || accuracy > 10.0f) {
        ...
        if (accuracy < 0.0f &&
            (scanClass == CLASS_MILITARY ||
             scanClass == CLASS_POLICE)) {  // police and military pilots have a better average skill.
            accuracy = -accuracy;
        }
    }
    if (scanClass == CLASS_MISSILE) {  // missile accuracy range is 0 to 10
...
- (BOOL)setUpSubEntities {
    ...
    for (i = 0; i < [plumes count]; i++) {
        ...
        OOExhaustPlumeEntity *exhaust = [OOExhaustPlumeEntity exhaustForShip:self
                                                              withDefinition:definition
                                                                    andScale:_scaleFactor];
...
- (NSString *)descriptionComponents {
    if (![self isSubEntity]) {
        ...
        if ([self behaviour] == BEHAVIOUR_TRACK_AS_TURRET)
            subtype = @ "(turret)";
        else
            subtype = @ "(subentity)";

        return [NSString
            stringWithFormat:@ "\"%@\" position: %@ %@", [self name], HPVectorDescription([self position]), subtype];
    }
}
...
- (void)setSubEntityTakingDamage:(ShipEntity *)sub {
    ...
    if (sub != nil) {
        if (![self hasSubEntity:sub]) {
            OOLog(@ "ship.subentity.sanityCheck.failed.details",
                  @ "Attempt to set subentity taking damage of %@ to %@, which is not a subentity.",
                  [self shortDescription],
                  sub);
            sub = nil;
        } else if (![sub isShip]) {
            OOLog(@ "ship.subentity.sanityCheck.failed",
...
- (void)wasAddedToUniverse {
    ...
    if (universalID != NO_TARGET) {
        // set up escorts
        if (([self status] == STATUS_IN_FLIGHT || [self status] == STATUS_LAUNCHING) &&
            _pendingEscortCount != 0)  // just popped into existence
        {
            [self setUpEscorts];
        } else {
...
- (HPVector)absoluteTractorPosition {
    return HPvector_add(position,
                        vectorToHPVector(quaternion_rotate_vector([self normalOrientation], tractor_position)));
}
...
- (id<OOHUDBeaconIcon>)beaconDrawable {
    if (_beaconDrawable == nil) {
        ...
        if (length > 1) {
            ...
            if (iconData != nil)
                _beaconDrawable = [[OOPolygonSprite alloc] initWithDataArray:iconData outlineWidth:0.5 name:beaconCode];
        }
...
- (void)setUpEscorts {
    ...
    if ([self isEscort]) {
        OOLogWARN(
            ...
            @ "Ship %@ requested escorts, when it is an escort ship itself. Avoiding possible circular reference overflow by ignoring escort setup.",
            self);
        return;
    }
    ...
    if (_maxEscortCount < _pendingEscortCount) {
        if ([self hasPrimaryRole:@ "police"] || [self hasPrimaryRole:@ "hunter"]) {
            ...
            _pendingEscortCount =
                _maxEscortCount;  // other ships can only get what's defined inside their 'escorts' key.
        }
    }
    ...
    if (escortRole == nil) escortRole = [shipinfoDictionary oo_stringForKey:@ "escort-role" defaultValue:defaultRole];
    ...
    if (escortShipKey == nil) escortShipKey = [shipinfoDictionary oo_stringForKey:@ "escort-ship"];
    ...
    while (_pendingEscortCount > 0 && ([self isThargoid] || currentEscortCount < _maxEscortCount)) {
        ...
        [self setUpOneEscort:escorter
                     inGroup:escortGroup
                    withRole:escortRole
                  atPosition:ex_pos
                    andCount:currentEscortCount];
...
- (void)setUpMixedEscorts {
    ...
    if (escortRoles == nil) {
        OOLogWARN(@ "eship.setUp.escortShipRoles", @ "Ship %@ has bad escort_roles definition.", self);
        return;
    }
...
- (void)setUpOneEscort:(ShipEntity *)escorter
    ...
    if ([self bounty] == 0)
        [escorter setBounty:0 withReason:kOOLegalStatusReasonSetup];  // Avoid dirty escorts for clean mothers
    ...
    if ([self status] == STATUS_DOCKED) {
        ...
        [escortAI setState:@ "FLYING_ESCORT"];  // Begin escort flight. (If the AI doesn't define FLYING_ESCORT, this has
                                               // no effect.)
        [escorter doScriptEvent:OOJSID("spawnedAsEscort") withArgument:self];
    }

    if ([escorter heatInsulation] < [self heatInsulation])
        [escorter setHeatInsulation:[self heatInsulation]];  // give escorts same protection as mother.
    if (([escorter maxFlightSpeed] < cruiseSpeed) && ([escorter maxFlightSpeed] > cruiseSpeed * 0.3))
        cruiseSpeed =
            [escorter maxFlightSpeed] * 0.99;  // adapt patrolSpeed to the slowest escort but ignore the very slow ones.
...
ShipEntity *doOctreesCollide(ShipEntity *prime, ShipEntity *other) {
    ...
    Vector relative_position_of_other =
        resolveVectorInIJK(HPVectorToVector(HPvector_between(prime_position, other_position)), prime_ijk);
    ...
    if (prime_subs) {
        ...
        for (i = 0; i < n_subs; i++) {
            ...
            if ([se isShip] && [se canCollide] && doOctreesCollide((ShipEntity *)se, other)) return other;
        }
    }
    ...
    if (other_subs) {
        ...
        for (i = 0; i < n_subs; i++) {
            ...
            if ([se isShip] && [se canCollide] && doOctreesCollide(prime, (ShipEntity *)se)) return (ShipEntity *)se;
        }
    }
...
- (BOOL)checkCloseCollisionWith:(Entity *)other {
    ...
    if (otherShip != nil && trackCloseContacts) {
        ...
        if (![closeContactsInfo objectForKey:other_key] &&
            ...
            Vector rpos =
                make_vector(dot_product(dpos, v_right), dot_product(dpos, v_up), dot_product(dpos, v_forward));
            [closeContactsInfo setObject:[NSString stringWithFormat:@ "%f %f %f", rpos.x, rpos.y, rpos.z]
                                  forKey:other_key];
    ...
    if (zero_distance > CLOSE_COLLISION_CHECK_MAX_RANGE2)	// don't work too hard on entities that are far from the
    player return YES;
    */
...
- (ShipEntity *)launchPodWithCrew:(NSArray *)podCrew {
    ...
    if (!pod) {
        ...
        pod = [UNIVERSE newShipWithRole:[shipinfoDictionary oo_stringForKey:@ "escape_pod_model"
                                                               defaultValue:@ "escape-capsule"]];
        if (!pod) {
            pod = [UNIVERSE newShipWithRole:@ "escape-capsule"];
            OOLog(@ "shipEntity.noEscapePod",
...
- (void)update:(OOTimeDelta)delta_t {
    ...
    if (!isSubEnt) {
        ...
        if (trackCloseContacts) {
            // in checkCloseCollisionWith: we check if some thing has come within touch range (origin within our
            // collision_radius) here we check if it has gone outside that range
            ...
            foreachkey(other_key, closeContactsTemp) {
                ...
                if ((other != nil) && (other->isShip)) {
                    if (HPdistance2(position, other->position) >
                        collision_radius * collision_radius)  // moved beyond our sphere!
                    {
                        // calculate position with respect to our own position and orientation
                        Vector dpos = HPVectorToVector(HPvector_between(position, other->position));
                        Vector pos1 = make_vector(
                            dot_product(dpos, v_right), dot_product(dpos, v_up), dot_product(dpos, v_forward));
                        Vector pos0 = {0, 0, 0};
                        ...
                        if ((pos0.x < 0.0) && (pos1.x > 0.0)) {
                            [self doScriptEvent:OOJSID("shipTraversePositiveX")
                                       withArgument:other
                                andReactToAIMessage:@ "POSITIVE X TRAVERSE"];
                        }
                        if ((pos0.x > 0.0) && (pos1.x < 0.0)) {
                            [self doScriptEvent:OOJSID("shipTraverseNegativeX")
                                       withArgument:other
                                andReactToAIMessage:@ "NEGATIVE X TRAVERSE"];
                        }
                        if ((pos0.y < 0.0) && (pos1.y > 0.0)) {
                            [self doScriptEvent:OOJSID("shipTraversePositiveY")
                                       withArgument:other
                                andReactToAIMessage:@ "POSITIVE Y TRAVERSE"];
                        }
                        if ((pos0.y > 0.0) && (pos1.y < 0.0)) {
                            [self doScriptEvent:OOJSID("shipTraverseNegativeY")
                                       withArgument:other
                                andReactToAIMessage:@ "NEGATIVE Y TRAVERSE"];
                        }
                        if ((pos0.z < 0.0) && (pos1.z > 0.0)) {
                            [self doScriptEvent:OOJSID("shipTraversePositiveZ")
                                       withArgument:other
                                andReactToAIMessage:@ "POSITIVE Z TRAVERSE"];
                        }
                        if ((pos0.z > 0.0) && (pos1.z < 0.0)) {
                            [self doScriptEvent:OOJSID("shipTraverseNegativeZ")
                                       withArgument:other
                                andReactToAIMessage:@ "NEGATIVE Z TRAVERSE"];
                        }
                        _primaryTarget = temp;
    ...
    if (!isSubEnt) {
        ...
        if (external_temp > heatThreshold && external_temp > ship_temperature)
            ship_temperature +=
                (external_temp - ship_temperature) * delta_t * SHIP_INSULATION_FACTOR / [self heatInsulation];
        else {
            if (ship_temperature > SHIP_MIN_CABIN_TEMP) {
                ship_temperature += (external_temp - heatThreshold - ship_temperature) * delta_t * SHIP_COOLING_FACTOR /
                                    [self heatInsulation];
    ...
    if (!haveExecutedSpawnAction) {
        ...
        if (script != nil &&
            (status == STATUS_IN_FLIGHT || status == STATUS_LAUNCHING || status == STATUS_BEING_SCOOPED ||
...
- (BOOL)hasAllEquipment:(id)equipmentKeys includeWeapons:(BOOL)includeWeapons whileLoading:(BOOL)loading {
    ...
    if ([equipmentKeys isKindOfClass:[NSString class]])
        equipmentKeys = [NSArray arrayWithObject:equipmentKeys];
    else if (![equipmentKeys isKindOfClass:[NSArray class]] && ![equipmentKeys isKindOfClass:[NSSet class]])
        return NO;
...
- (BOOL)canAddEquipment:(NSString *)equipmentKey inContext:(NSString *)context {
    ...
    if ([equipmentKey hasSuffix:@ "MISSILE"] || [equipmentKey hasSuffix:@ "MINE"] ||
        ([self isThargoid] && ([lcEquipmentKey hasPrefix:@ "thargon"] || [lcEquipmentKey hasSuffix:@ "thargon"]))) {
    ...
    if (facing & weapon_facings) {
        switch (facing) {
            case WEAPON_FACING_FORWARD:
                ...
                // if no forward weapon, and not carrying out a strict check, see if subentities have forward weapons,
...
- (NSArray *)missilesList {
    ...
    return missile_list[0] != nil ? [NSArray arrayWithObjects:missile_list count:missiles] : [NSArray array];
}
...
- (OOEquipmentType *)generateMissileEquipmentTypeFrom:(NSString *)role {
    ...
    */
    NSArray *itemInfo =
        [NSArray arrayWithObjects:@ "100",
                                  @ "100000",
                                  @ "Missile",
                                  role,
                                  @ "Unidentified missile type.",
                                  [NSDictionary dictionaryWithObjectsAndKeys:@ "true", @ "is_external_store", nil],
                                  nil];
...
- (BOOL)equipmentValidToAdd:(NSString *)equipmentKey whileLoading:(BOOL)loading inContext:(NSString *)context {
    ...
    if ([self availableCargoSpace] < [eqType requiredCargoSpace] && !validationForDamagedEquipment && !loading)
        return NO;
    if ([eqType requiresEquipment] != nil && ![self hasAllEquipment:[eqType requiresEquipment]
                                                     includeWeapons:YES
                                                       whileLoading:loading])
        return NO;
    if ([eqType requiresAnyEquipment] != nil && ![self hasEquipmentItem:[eqType requiresAnyEquipment]
                                                         includeWeapons:YES
                                                           whileLoading:loading])
        return NO;
    if ([eqType incompatibleEquipment] != nil && [self hasEquipmentItem:[eqType incompatibleEquipment]
                                                         includeWeapons:YES
                                                           whileLoading:loading])
        return NO;
    ...
    if (!loading) {
        ...
        if (condition_script != nil) {
            ...
            if (condScript != nil)  // should always be non-nil, but just in case
            {
                ...
                jsval result;
                jsval args[] = {OOJSValueFromNativeObject(JScontext, equipmentKey),
                                OOJSValueFromNativeObject(JScontext, self),
                                OOJSValueFromNativeObject(JScontext, context)};
...
- (BOOL)addEquipmentItem:(NSString *)equipmentKey withValidation:(BOOL)validateAddition inContext:(NSString *)context {
    ...
    if ([equipmentKey hasSuffix:@ "_DAMAGED"]) {
        eqType = [OOEquipmentType
            equipmentTypeWithIdentifier:[equipmentKey substringToIndex:[equipmentKey length] - [@ "_DAMAGED" length]]];
    } else {
        eqType = [OOEquipmentType equipmentTypeWithIdentifier:equipmentKey];
...
- (void)removeEquipmentItem:(NSString *)equipmentKey {
    ...
    if ([eqType isMissileOrMine] ||
        ([self isThargoid] && ([lcEquipmentKey hasSuffix:@ "thargon"] || [lcEquipmentKey hasPrefix:@ "thargon"]))) {
...
- (OOEquipmentType *)verifiedMissileTypeFromRole:(NSString *)role {
    ...
    if (eqRole == nil) {
        ...
        if (!missile) {
            ...
            if (isRandomMissile)
                return [self verifiedMissileTypeFromRole:role];
            else
                return nil;
        }
        ...
        if (isRandomMissile) {
            ...
            if (![missileType isMissileOrMine]) {
                role = shipKey;  // unique identifier to use in lieu of a valid equipment type if none are defined
                                 // inside the generic missile roleset.
            }
        }
        ...
        if (!missileType) {
            OOLogWARN(@ "ship.setUp.missiles",
                      @ "%@ \"%@\" used in ship \"%@\" needs a valid %@.plist entry.%@",
                      (isRandomMissile ? @ "random missile" : @ "missile_role"),
                      role,
                      [self name],
                      @ "equipment",
        ...
        if ([eqRole isEqualToString:@ ""]) {
            ...
            if (isRandomMissile)
                return [self verifiedMissileTypeFromRole:role];  // try and find a valid missile with role 'missile'.
            return nil;
        }
...
- (OOEquipmentType *)selectMissile {
    ...
    if (missileType == nil)
        OOLogERR(@ "ship.setUp.missiles",
                 @ "could not resolve missile / mine type for ship \"%@\". Original missile role:\"%@\".",
                 [self name],
                 _missileRole);
    ...
    if (thargoidMissile || (!thargoidMissile && [missileType isMissileOrMine])) {
        ...
        OOLogWARN(@ "ship.setUp.missiles",
                  @ "missile_role \"%@\" is not a valid missile / mine type for ship \"%@\".%@",
                  [missileType identifier],
                  [self name],
                  @ " No missile selected.");
        return nil;
    }
...
- (void)behaviour_tractored:(double)delta_t {
    ...
    if ((hauler) && ([hauler isShip])) {
        ...
        if ([self status] == STATUS_BEING_SCOOPED) {
            BOOL lost_contact =
                (distance > hauler->collision_radius + collision_radius + 250.0f);  // 250m range for tractor beam
            if ([hauler isPlayer]) {
...
- (void)behaviour_intercept_target:(double)delta_t {
    ...
    if (behaviour == BEHAVIOUR_INTERCEPT_TARGET) {
        ...
        if (desired_speed > maxFlightSpeed) {  // never use injectors for scooping
            desired_speed = maxFlightSpeed;
        }
        ...
        {
            frustration -= delta_t;
            if (frustration < 0.0) frustration = 0.0;
        } else {
...
- (void)behaviour_attack_break_off_target:(double)delta_t {
    ...
    if (range > 3000.0 || ([target isShip] && [(ShipEntity *)target primaryTarget] != self) ||
        frustration - floor(frustration) > fmin(1.6 / max_flight_roll, aspect)) {
    ...
    } else if (frustration > 10.0 && [self approachAspectToPrimaryTarget] < 0.85 &&
               forward_weapon_temp < COMBAT_AI_WEAPON_TEMP_READY) {
        frustration = 0.0;
...
- (void)behaviour_attack_target:(double)delta_t {
    ...
    if (isWeaponNone(forward_weapon_real_type)) {
        ...
        if (isWeaponNone(forward_weapon_real_type) && hasTurrets) {  // safety for ships only equipped with turrets
    ...
    if ([forward_weapon_real_type isTurretLaser]) {
        ...
        BOOL aft_weapon_ready =
            !isWeaponNone(aft_weapon_type) && (aft_weapon_temp < COMBAT_AI_WEAPON_TEMP_READY) && in_good_range;
        BOOL forward_weapon_ready =
            !isWeaponNone(forward_weapon_real_type) &&
            (forward_weapon_real_temp < COMBAT_AI_WEAPON_TEMP_READY);  // does not require in_good_range
        BOOL port_weapon_ready =
            !isWeaponNone(port_weapon_type) && (port_weapon_temp < COMBAT_AI_WEAPON_TEMP_READY) && in_good_range;
        BOOL starboard_weapon_ready = !isWeaponNone(starboard_weapon_type) &&
                                      (starboard_weapon_temp < COMBAT_AI_WEAPON_TEMP_READY) && in_good_range;
        ...
        if (!forward_weapon_ready && !aft_weapon_ready && !port_weapon_ready && !starboard_weapon_ready) {
            weapons_heating = YES;
            aft_weapon_ready =
                !isWeaponNone(aft_weapon_type) && (aft_weapon_temp < COMBAT_AI_WEAPON_TEMP_USABLE) && in_good_range;
            forward_weapon_ready =
                !isWeaponNone(forward_weapon_real_type) &&
                (forward_weapon_real_temp < COMBAT_AI_WEAPON_TEMP_USABLE);  // does not require in_good_range
            port_weapon_ready =
                !isWeaponNone(port_weapon_type) && (port_weapon_temp < COMBAT_AI_WEAPON_TEMP_USABLE) && in_good_range;
            starboard_weapon_ready = !isWeaponNone(starboard_weapon_type) &&
                                     (starboard_weapon_temp < COMBAT_AI_WEAPON_TEMP_USABLE) && in_good_range;
        }

        Entity *target = [self primaryTarget];
        double aspect = [self approachAspectToPrimaryTarget];

        if (!forward_weapon_ready && !aft_weapon_ready && !port_weapon_ready &&
            !starboard_weapon_ready) {  // no usable weapons! Either not fitted or overheated

            // if unarmed
            if (isWeaponNone(forward_weapon_real_type) && isWeaponNone(aft_weapon_type) &&
                isWeaponNone(port_weapon_type) && isWeaponNone(starboard_weapon_type)) {
        ...
        // if our current target isn't targeting us, and we have some idea of how to fight, and our weapons are running
        // hot, and we're fairly nearby
        else if (weapons_heating && accuracy >= COMBAT_AI_ISNT_AWFUL && [target isShip] &&
                 [(ShipEntity *)target primaryTarget] != self && range < COMBAT_OUT_RANGE_FACTOR * weaponRange) {
            ...
            if (nearby && aft_weapon_ready) {
                ...
                if (!pitching_over)  // don't change jink in the middle of a sharp turn.
                {
                    /*
                            ...
                            need a different jink.z for optimal collision avoidance at high speed approach and low speed
                       dogfighting. The COMBAT_JINK_OFFSET intentionally over-compensates the range for collision radii
                       to send ships towards the target at low speeds.
                    */
                    float relativeSpeed = magnitude(vector_subtract([self velocity], [target velocity]));
                    [self setEvasiveJink:(range + COMBAT_JINK_OFFSET - relativeSpeed / max_flight_pitch)];
                }
...
- (void)behaviour_attack_broadside:(double)delta_t {
    ...
    if (range < COMBAT_BROADSIDE_IN_RANGE_FACTOR * weaponRange) {
        ...
        jink = kZeroVector;
        if (weapon_damage == 0.0) {  // safety in case side lasers no longer exist
            behaviour = BEHAVIOUR_ATTACK_TARGET;
...
- (void)behaviour_attack_broadside_target:(double)delta_t leftside:(BOOL)leftside {
    ...
    double slow_down_range =
        currentWeaponRange * COMBAT_WEAPON_RANGE_FACTOR * ((isUsingAfterburner) ? 3.0 * [self afterburnerFactor] : 1.0);
    //	double target_speed = [target speed];
    if (range <= slow_down_range)
        desired_speed = fmin(0.8 * maxFlightSpeed,
                             fmax((2.0 - frustration) * maxFlightSpeed,
                                  0.1 * maxFlightSpeed));  // within the weapon's range slow down to aim
    else
    ...
    if (weapon_temp > COMBAT_AI_WEAPON_TEMP_USABLE) {  // will probably have more luck with the other laser or picking a
...
- (void)behaviour_fly_to_target_six:(double)delta_t {
    ...
    double slow_down_range =
        weaponRange * COMBAT_WEAPON_RANGE_FACTOR * ((isUsingAfterburner) ? 3.0 * [self afterburnerFactor] : 1.0);
    if (closeQuickly) {
        slow_down_range = weaponRange * COMBAT_OUT_RANGE_FACTOR;
    }
    double back_off_range =
        weaponRange * COMBAT_OUT_RANGE_FACTOR * ((isUsingAfterburner) ? 3.0 * [self afterburnerFactor] : 1.0);
    Entity *rawTarget = [self primaryTarget];
    if (![rawTarget isShip]) {
        [self noteLostTargetAndGoIdle];
        return;
    ...
    if (behaviour == BEHAVIOUR_ATTACK_FLY_TO_TARGET_TWELVE) {
        if ([forward_weapon_type isTurretLaser]) {
            ...
            if (entity_personality & 1) {  // half at random
                offset = -offset;
            }
    ...
    if (success_factor > last_success_factor || confidenceFactor < 0.85)
        frustration += delta_t;
    else if (frustration > 0.0)
        frustration -= delta_t * 0.75;
...
- (void)behaviour_attack_fly_to_target:(double)delta_t {
    ...
    double back_off_range =
        10000 * COMBAT_OUT_RANGE_FACTOR * ((isUsingAfterburner) ? 3.0 * [self afterburnerFactor] : 1.0);
    double target_speed = [target speed];
    double aspect = [self approachAspectToPrimaryTarget];
    ...
    if (range <= slow_down_range) {
        if (range < back_off_range) {
            if (accuracy < COMBAT_AI_IS_SMART || ([target primaryTarget] == self && aspect > 0.8) ||
                aim_tolerance * range > COMBAT_AI_CONFIDENCE_FACTOR) {
                if (accuracy >= COMBAT_AI_FLEES_BETTER && aspect > 0.8) {
                    ...
                    desired_speed =
                        fmax(target_speed * 1.05, 0.25 * maxFlightSpeed);  // within the weapon's range match speed
                }
            } else {  // smart, and not being shot at right now - slow down to attack
                desired_speed = fmax(0.1 * target_speed, 0.1 * maxFlightSpeed);
            }
        } else {
            if (accuracy < COMBAT_AI_IS_SMART || ([target isShip] && [(ShipEntity *)target primaryTarget] == self) ||
        ...
        if (closeQuickly) {
            ...
            desired_speed = fmax(
                maxFlightSpeed, fmin(3.0 * target_speed, max_available_speed));  // possibly use afterburner to approach
        }
    }
    ...
    if (weapon_temp > COMBAT_AI_WEAPON_TEMP_USABLE && accuracy >= COMBAT_AI_ISNT_AWFUL &&
        ...
        // lasers effectively
        if (aspect < 0 || !isWeaponNone(aft_weapon_type) || !isWeaponNone(port_weapon_type) ||
        ...
        if ([target behaviour] != BEHAVIOUR_FLEE_TARGET && [target behaviour] != BEHAVIOUR_FLEE_EVASIVE_ACTION) {
            if ((aspect > 0.99999 && !isWeaponNone([target weaponTypeForFacing:WEAPON_FACING_FORWARD strict:NO])) ||
                (aspect < -0.999 && !isWeaponNone([target weaponTypeForFacing:WEAPON_FACING_AFT strict:NO]))) {
                frustration = 0.0;
                behaviour = BEHAVIOUR_EVASIVE_ACTION;
            }
        }
...
- (void)behaviour_attack_fly_from_target:(double)delta_t {
    ...
    {
        frustration += delta_t;
    } else {  // not getting away fast enough?
    ...
    if (frustration > 10.0) {
        if (randf() < 0.3) {
            desired_speed =
                maxFlightSpeed * (([self hasFuelInjection] && (fuel > MIN_FUEL)) ? [self afterburnerFactor] : 1);
    ...
    if (cloakAutomatic) [self activateCloakingDevice];
    if ([self hasProximityAlertIgnoringTarget:YES]) [self avoidCollision];
...
- (void)behaviour_fly_range_from_destination:(double)delta_t {
    ...
    if (distance < desired_range) {
        ...
        if (desired_speed < maxFlightSpeed) {
            desired_speed = maxFlightSpeed;  // Not all AI define speed when flying away. Start with max speed to stay
                                             // compatible with such AI's, but allow faster flight if it's (e.g.) used
                                             // to flee from coordinates rather than entity
        }
    } else {
...
- (void)behaviour_face_destination:(double)delta_t {
    ...
    if (desired_range > 1.0 && distance > desired_range) {
        max_cos = sqrt(
            1 -
            0.90 * desired_range * desired_range /
                (distance *
                 distance));  // Head for a point within 95% of desired_range (must match the value in trackDestination)
    }
    double confidenceFactor = [self trackDestination:delta_t:NO];
    ...
    /* 2009-7-18 Eric: the condition check below is intended to eliminate the flippering between two positions for fast
       turning ships during low FPS conditions. This flippering is particular frustrating on slow computers during
       docking. But with my current computer I can't induce those low FPS conditions so I can't properly test if it
       helps. I did try with the TAF time acceleration that also generated larger frame jumps and than it seemed to
       help.
    */
    if (flightSpeed == 0 && frustration > 5 && confidenceFactor > 0.5 &&
        ((flightPitch > 0 && old_pitch < 0) || (flightPitch < 0 && old_pitch > 0))) {
...
- (void)behaviour_formation_form_up:(double)delta_t {
    ...
    if (distance < last_distance)  // improvement
    {
        ...
        if (frustration > 15.0) {
            if (!leadShip)
                [self noteFrustration:@ "BEHAVIOUR_FORMATION_FORM_UP"];  // escorts never reach their destination when
                                                                        // following leader.
            else if (distance > 0.5 * scannerRange && !pitching_over) {
                pitching_over = YES;  // Force the ship in a 180 degree turn. Do it here to allow escorts to break out
                                      // formation for some seconds.
            }
            frustration = 0;
        }
    }
...
- (void)behaviour_fly_to_destination:(double)delta_t {
    double distance = [self rangeToDestination];
    // double desiredRange = (dockingInstructions != nil) ? 1.2 * desired_range : desired_range; // stop a bit earlyer
    // when docking.
    if (distance < desired_range)  // + collision_radius)
    {
        ...
        /*	2009-07-19 Eric: Estimated Time of Arrival (eta) should also take the "angle to target" into account
           (confidenceFactor = cos(angle to target)) and should not fuss about that last meter and use "distance + 1"
           instead of just "distance". trackDestination already did pitch regulation, use confidence here only for
           cutting down to high speeds. This should prevent ships crawling to their destination when they try to pull up
           close to their destination.

                To prevent ships circling around their target without reaching destination I added a limitation based on
           turnrate, speed and distance to target. Formula based on satelite orbit: orbitspeed = turnrate (rad/sec) *
           radius (m)   or   flightSpeed = max_flight_pitch * 2 Pi * distance Speed must be significant lower when not
           flying in direction of target (low confidenceFactor) or it can never approach its destination and the ships
           runs the risk flying in circles around the target. (exclude active escorts)
        */
        GLfloat eta =
            ((distance + 1) - desired_range) /
            (0.51 * flightSpeed * confidenceFactor);  // 2% safety margin assuming an average of half current speed
        GLfloat slowdownTime = (thrust > 0.0) ? flightSpeed / (thrust) : 4.0;
        GLfloat minTurnSpeedFactor = 0.05 * max_flight_pitch * max_flight_roll;  // faster turning implies higher speeds
        ...
        if (((eta < slowdownTime) && (flightSpeed > maxFlightSpeed * minTurnSpeedFactor)) ||
            (flightSpeed > max_flight_pitch * 5 * confidenceFactor * distance)) {
            desired_speed = flightSpeed * 0.50;  // cut speed by 50% to a minimum minTurnSpeedFactor of speed
        }

        /* Flight correction block to prevent one possible form of
         * crashes in late docking process */
        if (docking_match_rotation && confidenceFactor >= MAX_COS && dockingInstructions != nil &&
            [dockingInstructions oo_intForKey:@ "docking_stage"] >= 7) {
        ...
        if (distance < last_distance)  // improvement
        {
            ...
            frustration += delta_t;
            if ((frustration > slowdownTime * 10.0 && slowdownTime > 0) ||
                (frustration > 15.0))  // 10x slowdownTime or 15s of frustration
            {
...
- (void)behaviour_fly_thru_navpoints:(double)delta_t {
    ...
    HPVector xp =
        make_HPvector(ref.y * rel.z - ref.z * rel.y, ref.z * rel.x - ref.x * rel.z, ref.x * rel.y - ref.y * rel.x);
...
- (HPVector)calculateTargetPosition {
    ...
    return HPvector_add(HPvector_add(trackingCurveCoeffs[0], HPvector_multiply_scalar(trackingCurveCoeffs[1], t)),
                        HPvector_multiply_scalar(trackingCurveCoeffs[2], t * t));
}
...
- (void)updateTrackingCurve {
    ...
    if (EXPECT_NOT([target isShip] && [(ShipEntity *)target isCloaked])) {
        ...
        trackingCurvePositions[0] = HPvector_add(
            [target position], OOHPVectorRandomSpatial([(ShipEntity *)target flightSpeed] * reactionTime * 0.02));
    } else {
        trackingCurvePositions[0] = [target position];
    }
...
- (void)calculateTrackingCurve {
    ...
    double t1 = trackingCurveTimes[2] - trackingCurveTimes[1], t2 = trackingCurveTimes[3] - trackingCurveTimes[1];
    trackingCurveCoeffs[0] = trackingCurvePositions[1];
    trackingCurveCoeffs[1] =
        HPvector_add(HPvector_add(HPvector_multiply_scalar(trackingCurvePositions[1], -(t1 + t2) / (t1 * t2)),
    ...
    trackingCurveCoeffs[2] =
        HPvector_add(HPvector_add(HPvector_multiply_scalar(trackingCurvePositions[1], 1 / (t1 * t2)),
    ...
    if (!immediate)  // TODO: is this relevant any longer?
    {
        ...
        if ([self subEntityCount] > 0) {
            ...
            foreach (subEntity, [self subEntities]) {
                NSAssert3([subEntity owner] == self,
                          @ "Subentity ownership broke - %@ should be owned by %@ but is owned by %@.",
                          subEntity,
                          self,
                          [subEntity owner]);
...
- (void)drawDebugStuff {
    ...
    if (0 && reportAIMessages) {
        ...
        OODebugDrawColoredLine(
            HPVectorToVector([self position]), HPVectorToVector(_destination), [OOColor colorWithWhite:0.15 alpha:1.0]);

        Entity *pTarget = [self primaryTarget];
        if (pTarget != nil) {
            OODebugDrawPoint(HPVectorToVector([pTarget position]), [OOColor redColor]);
            OODebugDrawColoredLine(HPVectorToVector([self position]),
                                   HPVectorToVector([pTarget position]),
...
- (GLfloat *)scannerDisplayColorForShip:(ShipEntity *)
    ...
    if (isHostile) {
        ...
        if (scannerDisplayColorH1 || scannerDisplayColorH2) {
            if (scannerDisplayColorH1 && !scannerDisplayColorH2) {
                [scannerDisplayColorH1 getRed:&scripted_color[0]
            ...
            if (!scannerDisplayColorH1 && scannerDisplayColorH2) {
                [scannerDisplayColorH2 getRed:&scripted_color[0]
                                        green:&scripted_color[1]
                                         blue:&scripted_color[2]
                                        alpha:&scripted_color[3]];
            }
    ...
    if (scannerDisplayColor1 || scannerDisplayColor2) {
        if (scannerDisplayColor1 && !scannerDisplayColor2) {
            [scannerDisplayColor1 getRed:&scripted_color[0]
                                   green:&scripted_color[1]
                                    blue:&scripted_color[2]
                                   alpha:&scripted_color[3]];
        }
        ...
        if (!scannerDisplayColor1 && scannerDisplayColor2) {
            [scannerDisplayColor2 getRed:&scripted_color[0]
                                   green:&scripted_color[1]
                                    blue:&scripted_color[2]
                                   alpha:&scripted_color[3]];
        }
    ...
    switch (scanClass) {
        ...
        case CLASS_MINE:
            if (flash)
                ...
                return hostile_color;
        default:
            if (isHostile) return hostile_color;
    }
    return neutral_color;
}
...
- (void)setScannerDisplayColor1:(OOColor *)color {
    ...
    if (color == nil)
        color = [OOColor colorWithDescription:[[self shipInfoDictionary] objectForKey:@ "scanner_display_color1"]];
...
- (void)setScannerDisplayColor2:(OOColor *)color {
    ...
    if (color == nil)
        color = [OOColor colorWithDescription:[[self shipInfoDictionary] objectForKey:@ "scanner_display_color2"]];
...
- (void)setScannerDisplayColorHostile1:(OOColor *)color {
    ...
    if (color == nil)
        color =
            [OOColor colorWithDescription:[[self shipInfoDictionary] objectForKey:@ "scanner_hostile_display_color1"]];
...
- (void)setScannerDisplayColorHostile2:(OOColor *)color {
    ...
    if (color == nil)
        color =
            [OOColor colorWithDescription:[[self shipInfoDictionary] objectForKey:@ "scanner_hostile_display_color2"]];
...
- (void)applyThrust:(double)delta_t {
    ...
    // check for speed
    if (desired_speed > max_available_speed) desired_speed = max_available_speed;
...
- (void)applyRoll:(GLfloat)roll1 climb:(GLfloat)climb1 andYaw:(GLfloat)yaw1 {
    if ((roll1 == 0.0) && (climb1 == 0.0) && (yaw1 == 0.0) && (!hasRotated)) return;

    Quaternion q1 = kIdentityQuaternion;

    if (roll1) quaternion_rotate_about_z(&q1, -roll1);
    if (climb1) quaternion_rotate_about_x(&q1, -climb1);
    if (yaw1) quaternion_rotate_about_y(&q1, -yaw1);
...
- (void)avoidCollision {
    if (scanClass == CLASS_MISSILE) return;  // missiles are SUPPOSED to collide!
...
- (void)setProximityAlert:(ShipEntity *)other {
    ...
    if ((other->isStation) && ([self status] == STATUS_LAUNCHING || dockingInstructions != nil)) {
        return;
    }

    if (!crew)  // Ships without pilot (cargo, rocks, missiles, buoys etc) will not get alarmed. (escape-pods have
                // pilots)
        return;
    ...
    double cr2 = collision_radius * 2.0 + other->collision_radius;
    cr2 *= cr2;  // check with twice the combined radius
    ...
    if (behaviour == BEHAVIOUR_AVOID_COLLISION)  //	already avoiding something
    {
        ...
        if ((prox) && (prox != other)) {
            ...
            GLfloat sa_other =
                other->collision_radius * other->collision_radius / HPdistance2(position, other->position);
...
- (OORoleSet *)roleSet {
    ...
    return [[roleSet roleSetWithAddedRoleIfNotSet:primaryRole
                                      probability:1.0] roleSetWithAddedRoleIfNotSet:[self shipDataKeyAutoRole]
                                                                        probability:1.0];
}
...
- (void)addRole:(NSString *)role withProbability:(float)probability {
    if (![self hasRole:role]) {
        ...
        if (roleSet != nil)
            newRoles = [roleSet roleSetWithAddedRole:role probability:probability];
        else
            newRoles = [OORoleSet roleSetWithRole:role probability:probability];
        if (newRoles != nil) {
            [roleSet release];
...
- (BOOL)isUnpiloted {
    return [self isExplicitlyUnpiloted] || [self isHulk] || [self scanClass] == CLASS_ROCK ||
...
- (Entity<OOStellarBody> *)findNearestStellarBody {
    ...
    if (sun != nil) {
        if (match == nil || SurfaceDistanceSqared(self, sun) < SurfaceDistanceSqared(self, match)) {
            match = sun;
        }
    }
...
- (OOPlanetEntity *)findNearestPlanetExcludingMoons {
    ...
    for (i = 0; i < [bodies count]; i++) {
        ...
        if ([planet planetType] == STELLAR_TYPE_NORMAL_PLANET) planets = [planets arrayByAddingObject:planet];
    }
...
- (OOAegisStatus)checkForAegis {
    ...
    isNearPlanetSurface =
        (d2 - cr2) < (250000.0f + 1000.0f * cr);  // less than 500m from the surface: (a+b)*(a+b) = a*a+b*b +2*a*b
    ...
    } else if (EXPECT_NOT(isNearPlanetSurface ||
                          d2 < cr2 * 9.0f))  // to 3x radius of any planet/moon - or 500m of tiny ones,
    {
    ...
    */
    if (EXPECT(!suppressAegisMessages)) {
        ...
        if (EXPECT_NOT(result == AEGIS_IN_DOCKING_RANGE && aegis_status != result)) {
            ...
            if ([self lastAegisLock] == nil &&
                !sunGoneNova)  // With small main planets the station aegis can come before planet aegis
            {
        ...
        // approaching..
        else if (EXPECT_NOT((result == AEGIS_CLOSE_TO_ANY_PLANET || result == AEGIS_CLOSE_TO_MAIN_PLANET) &&
                            [self lastAegisLock] != nearest)) {
            ...
            if (EXPECT_NOT([nearest isSun])) {
                ...
                if (EXPECT(result == AEGIS_CLOSE_TO_MAIN_PLANET)) {
                    // It's been years since 1.71 - it should be safe enough to comment out the line below for 1.77/1.78
                    // -- Kaks 20120917
                    //[shipAI message:@"AEGIS_CLOSE_TO_PLANET"];	    // fires only for main planets, kept for
                    // compatibility with pre-1.72 AI plists.
...
- (void)setSingleCrewWithRole:(NSString *)crewRole {
    if (![self isUnpiloted]) {
        OOCharacter *crewMember = [OOCharacter randomCharacterWithRole:crewRole andOriginalSystem:[self homeSystem]];
    ...
    if (script == nil) {
        ...
        if (actions) {
            OOStandardsDeprecated(
                [NSString stringWithFormat:@ "The launch_actions ship key is deprecated on %@.", [self displayName]]);
        ...
        if (actions) {
            OOStandardsDeprecated(
                [NSString stringWithFormat:@ "The script_actions ship key is deprecated on %@.", [self displayName]]);
            if (!OOEnforceStandards()) {
        ...
        if (actions) {
            OOStandardsDeprecated(
                [NSString stringWithFormat:@ "The death_actions ship key is deprecated on %@.", [self displayName]]);
            if (!OOEnforceStandards()) {
        ...
        if (actions) {
            OOStandardsDeprecated(
                [NSString stringWithFormat:@ "The setup_actions ship key is deprecated on %@.", [self displayName]]);
            if (!OOEnforceStandards()) {
...
- (OOFuelQuantity)fuelCapacity {
    // FIXME: shipdata.plist can allow greater fuel quantities (without extending hyperspace range). Need some
    // consistency here.
...
- (void)applySticks:(double)delta_t {
    ...
    if (accuracy >= COMBAT_AI_TRACKS_CLOSER) {
        if (stick_roll == 0.0) rate1 *= 2.0;   // faster correction
        if (stick_pitch == 0.0) rate2 *= 2.0;  // faster correction
        if (stick_yaw == 0.0) rate3 *= 2.0;    // faster correction
    }
...
- (void)setBounty:(OOCreditsQuantity)amount withReason:(OOLegalStatusReason)reason {
    if ([self isSubEntity]) {
        ...
        if ((scanClass == CLASS_THARGOID || scanClass == CLASS_STATION) && reason != kOOLegalStatusReasonSetup &&
            reason != kOOLegalStatusReasonByScript) {
...
- (int)legalStatus {
    if (scanClass == CLASS_THARGOID) return 5 * collision_radius;
    if (scanClass == CLASS_ROCK) return 0;
...
- (void)setCommodityForPod:(OOCommodityType)co_type andAmount:(OOCargoQuantity)co_amount {
    ...
    if (unit == UNITS_TONS && co_amount > 1)
        co_amount = 1;
    else if (unit == UNITS_KILOGRAMS && co_amount > 1000)
        co_amount = 1000;
    else if (unit == UNITS_GRAMS && co_amount > 1000000)
        co_amount = 1000000;
        ...
        if (num > 0) {
            switch (cargo_flag) {
                case CARGO_FLAG_FULL_UNIFORM:
                    newCargo = [UNIVERSE getContainersOfCommodity:[shipinfoDictionary oo_stringForKey:@ "cargo_carried"
                    ]:num];
                    break;
...
- (void)dealEnergyDamageWithinDesiredRange {
    ...
    NSArray *targets =
        [UNIVERSE entitiesWithinRange:(desired_range < SCANNER_MAX_RANGE ? desired_range : SCANNER_MAX_RANGE)
                             ofEntity:self];
    if ([targets count] > 0) {
        unsigned i;
        for (i = 0; i < [targets count]; i++) {
...
- (void)noteTakingDamage:(double)amount from:(Entity *)entity type:(OOShipDamageType)type {
    ...
    if ([entity isShip]) {
        ...
        if ([self hasHostileTarget] && accuracy >= COMBAT_AI_IS_SMART &&
            (randf() * 10.0 < accuracy || desired_speed < 0.5 * maxFlightSpeed) &&
            behaviour != BEHAVIOUR_EVASIVE_ACTION && behaviour != BEHAVIOUR_FLEE_EVASIVE_ACTION &&
    ...
    for (i = 0; i < n_jetsam; i++) {
        if (Ranrot() % 100 < cargo_chance)  //  chance of any given piece of cargo surviving decompression
        {
            ...
            if ([containerAI hasSuspendedStateMachines])  // check if new or recycled cargo.
            {
                ...
                [container
                    setThrust:[container maxThrust]];  // restore old value. Was set to zero on previous scooping.
...
- (void)becomeExplosion {
    ...
    @try {
        ...
        if (!suppressExplosion &&
            ([self isVisible] || HPdistance2([self position], [PLAYER position]) < SCANNER_MAX_RANGE2)) {
            ...
            BOOL add_debris =
                (UNIVERSE->n_entities < 0.95 * UNIVERSE_MAX_ENTITIES) && ([UNIVERSE getTimeDelta] < 0.125);  // FPS > 8
            ...
            if (add_debris) {
                ...
                if ([self hasRole:@ "asteroid"] || [self isBoulder]) {
                    if (!noRocks && (being_mined || randf() < 0.20)) {
                        ...
                        NSString *debrisRole = [[self shipInfoDictionary] oo_stringForKey:@ "debris_role"
                        ...
                        for (i = 0; i < n_rocks; i++) {
                            ...
                            if (rock) {
                                ...
                                float cr = (collision_radius < rock->collision_radius) ? collision_radius
                                ...
                                HPVector rpos = HPvector_add(
                                    xposition, vectorToHPVector(vector_multiply_scalar(vector_normal(v), cr)));
                ...
                if ((n_alloys && canFragment) || canReleaseSubWreckage) {
                    ...
                    for (i = 0; i < n_wreckage; i++) {
                        ...
                        ShipEntity *wreck = [UNIVERSE addWreckageFrom:self
                                                             withRole:@ "wreckage"
                                                                   at:rpos
                                                                scale:1.0
                                                             lifetime:lifetime / 2];

                        [wreck setVelocity:vector_add([wreck velocity],
                                                      vector_multiply_scalar(vector_normal(dir),
                                                                             randf() * [wreck collisionRadius]))];
                    }
        ...
        if (self != PLAYER)  // was if !isPlayer - but I think this may cause ghosts (Who's "I"? -- Ahruman)
        {
            if (isPlayer) {
#ifndef NDEBUG
                OOLog(@ "becomeExplosion.suspectedGhost.confirm",
                      @ "%@",
...
- (GLfloat)laserHeatLevelForward {
    ...
    if (isWeaponNone(forward_weapon_type)) {  // must check subents
...
- (void)checkScanner {
    ...
    //
    scan = z_previous;
    while ((scan) && (scan->isShip == NO)) scan = scan->z_previous;  // skip non-ships
    ...
    //
    scan = z_next;
    while ((scan) && (scan->isShip == NO)) scan = scan->z_next;  // skip non-ships
    while ((scan) && (scan->position.z < position.z + scannerRange) && (n_scanned_ships < MAX_SCAN_NUMBER)) {
        if (scan->isShip && ![(ShipEntity *)scan isCloaked] && [self isValidTarget:scan]) {
            distance2_scanned_ships[n_scanned_ships] = HPdistance2(position, scan->position);
            if (distance2_scanned_ships[n_scanned_ships] < scannerRange2)
...
- (void)checkScannerIgnoringUnpowered {
    ...
    while ((scan) && (scan->position.z > position.z - scannerRange) && (n_scanned_ships < MAX_SCAN_NUMBER)) {
        ...
        scan = scan->z_previous;
        while ((scan) &&
               ((scan->isShip == NO) || (scan->scanClass == CLASS_ROCK) || (scan->scanClass == CLASS_CARGO))) {
            scan = scan->z_previous;  // skip non-ships
        }
    }
    //
 ...
 * - track cloaked ships once primary targeted (but no missiles, and can't keep as a mere defense target, and probably
...
- (id)primaryTarget {
    ...
    if ((result == nil && _primaryTarget != nil) || ![self isValidTarget:result]) {
        DESTROY(_primaryTarget);
        return nil;
...
- (BOOL)isFriendlyTo:(ShipEntity *)otherShip {
    ...
    if ((otherShip == self) || ([self isPolice] && [otherShip isPolice]) ||
...
- (void)noteLostTarget {
    ...
    if (target == nil)
        [shipAI message:@ "TARGET_LOST"];  // stale target? no major urgency.
    else
        [shipAI reactToMessage:@ "TARGET_LOST" context:@ "flight updates"];  // execute immediately otherwise.
}
...
- (HPVector)distance_six:(GLfloat)dist {
    ...
    six.x -= dist * v_forward.x;
    six.y -= dist * v_forward.y;
    six.z -= dist * v_forward.z;
    return six;
}

- (HPVector)distance_twelve:(GLfloat)dist withOffset:(GLfloat)offset {
    HPVector twelve = position;
    twelve.x += dist * v_up.x;
    twelve.y += dist * v_up.y;
    twelve.z += dist * v_up.z;
    twelve.x += offset * v_right.x;
    twelve.y += offset * v_right.y;
    twelve.z += offset * v_right.z;
    return twelve;
}
...
- (void)trackOntoTarget:(double)delta_t withDForward:(GLfloat)dp {
    ...
    if (dp > max_cos) return;  // ON TARGET!
...
- (double)trackPrimaryTarget:(double)delta_t:(BOOL)retreat {
     ...
     * CIM */
    if (scanClass == CLASS_MISSILE) return [self missileTrackPrimaryTarget:delta_t];
    ...
    if (retreat)  // calculate jink position when flying away from target.
    {
        ...
        if (range2 <
            collision_radius * target->collision_radius * 100.0)  // Check direction within 10 * collision radius.
        {
            Vector targetDirection = kBasisZVector;
            if (!vector_equal(relPos, kZeroVector)) targetDirection = vector_normal(relPos);
            avoidCollision =
                (dot_product(targetDirection, v_forward) > -0.1);  // is flying toward target or only slightly outward.
        }
        ...
        if (accuracy >= COMBAT_AI_FLEES_BETTER) {
            ...
            if (jink.x == 0.0 && behaviour != BEHAVIOUR_RUNNING_DEFENSE) {  // test for zero jink and correct
                [self setEvasiveJink:400.0];
            }
        }
    ...
    if (!vector_equal(relPos, kZeroVector))
        relPos = vector_normal(relPos);
    else
        relPos.z = 1.0;
    ...
    if ((d_forward < max_cos) || (retreat))  // not on course so we must adjust controls..
    {
        ...
        if (d_up > min_d) {
            ...
            if (d_right > min_d) stick_roll = -max_flight_roll * r_max_factor * factor;   // note#
            if (d_right < -min_d) stick_roll = +max_flight_roll * r_max_factor * factor;  // note#
        }
        if (d_up < -min_d) {
            int factor = sqrt(fabs(d_right) / fabs(min_d));
        ...
        if (stick_roll == 0.0) {
            ...
            if (factor > max_factor) factor = max_factor;
            if (d_up > min_d) stick_pitch = -max_flight_pitch * reverse * r_max_factor * factor;
            if (d_up < -min_d) stick_pitch = +max_flight_pitch * reverse * r_max_factor * factor;
        }
    ...
    /*	#  note
            Eric 9-9-2010: Removed the "reverse" variable from the stick_roll calculation. This was mathematical wrong
       and made the ship roll in the wrong direction, preventing the ship to fly away in a straight line from the
       target. This means all the places were a jink was set, this jink never worked correctly. The main reason a ship
       still managed to turn at close range was probably by the fail-safe mechanisme with the "pitching_over" variable.
            The jink was programmed to do nothing within 500 meters of the ship and just fly away in direct line from
       the target in that range. Because of the bug the ships always rolled to the wrong side needed to fly away in
       direct line resulting in making it a difficult target. After fixing the bug, the ship realy flew away in direct
       line during the first 500 meters, making it a easy target for the player. All jink settings are retested and
       changed to give a turning behaviour that felt like the old situation, but now more deliberately set.
     */
    ...
    if (retreat) d_forward *= d_forward;  // make positive AND decrease granularity

    if (d_forward < 0.0) return 0.0;
...
- (double)trackSideTarget:(double)delta_t:(BOOL)leftside {
    ...
    if ((d_right * reverse < max_cos))  // not on course so we must adjust controls..
    {
        ...
        if (d_forward > min_d) {
            int factor = sqrt(fabs(d_up) / fabs(min_d));
            if (factor > max_factor) factor = max_factor;
            if (d_up > min_d) stick_pitch = +max_flight_pitch * r_max_factor * factor;   // note#
            if (d_up < -min_d) stick_pitch = -max_flight_pitch * r_max_factor * factor;  // note#
        }
        if (d_forward < -min_d) {
            int factor = sqrt(fabs(d_up) / fabs(min_d));
        ...
        if (fabs(stick_pitch) == 0.0 || fabs(d_forward) > 0.5) {
            ...
            if (factor > max_factor) factor = max_factor;
            if (d_forward > min_d) stick_yaw = -max_flight_yaw * reverse * r_max_factor * factor;
            if (d_forward < -min_d) {
...
- (double)missileTrackPrimaryTarget:(double)delta_t {
    ...
    if (pitching_over) pitching_over = (stick_pitch != 0.0);

    if ((d_forward < -pitch_tolerance) && (!pitching_over)) {
        pitching_over = YES;
        if (d_up >= 0) stick_pitch = -max_flight_pitch;
        if (d_up < 0) stick_pitch = max_flight_pitch;
    }
    ...
    // apply damping
    if (flightRoll < 0) flightRoll += (flightRoll < -damping) ? damping : -flightRoll;
    if (flightRoll > 0) flightRoll -= (flightRoll > damping) ? damping : flightRoll;
    if (flightPitch < 0) flightPitch += (flightPitch < -damping) ? damping : -flightPitch;
    if (flightPitch > 0) flightPitch -= (flightPitch > damping) ? damping : flightPitch;
...
- (double)trackDestination:(double)delta_t:(BOOL)retreat {
    ...
    double precision =
        we_are_docking
            ? 0.25
            : 0.9025;  // lower values force a direction closer to the target. (resp. 50% and 95% within range)

    if (retreat) reverse = -reverse;
    ...
    /*	2009-7-18 Eric: We need to aim well inide the desired_range sphere round the target and not at the surface of
       the sphere. Because of the framerate most ships normally overshoot the target and they end up flying clearly on a
       path through the sphere. Those ships give no problems, but ships with a very low turnrate will aim close to the
       surface and will than have large trouble with reaching their destination. When those ships enter the slowdown
       range, they have almost no speed vector in the direction of the target. I now used 95% of desired_range to aim
       at, but a smaller value might even be better.
    */
    ...
    if ((d_forward < max_cos) || (retreat))  // not on course so we must adjust controls..
    {
        if (d_forward <= -max_cos ||
            (retreat && d_forward >= max_cos))  // hack to avoid just flying away from the destination
        {
            d_up = min_d * 2.0;
        }
        ...
        if (d_up > min_d) {
            ...
            if (d_right < -min_d) stick_roll = +max_flight_roll * reversePlayer * 0.125 * factor;
            if (fabs(d_right) < fabs(stick_roll) * delta_t)
        ...
        if (stick_roll == 0.0) {
            int factor = sqrt(fabs(d_up) / fabs(min_d));
            if (factor > 8) factor = 8;
            if (d_up > min_d) stick_pitch = -max_flight_pitch * reverse * 0.125 * factor;  // pitch_pitch * reverse;
            if (d_up < -min_d) stick_pitch = +max_flight_pitch * reverse * 0.125 * factor;
            if (fabs(d_up) < fabs(stick_pitch) * delta_t)
    ...
    if (we_are_docking && docking_match_rotation && (d_forward > max_cos)) {
        ...
        if ((station_for_docking) && (station_for_docking->isStation)) {
            stick_roll = [self rollToMatchUp:[station_for_docking portUpVectorForShip:self]
                                    rotating:[station_for_docking flightRoll]];
        }
    }
...
- (GLfloat)currentAimTolerance {
    ...
    if (currentWeaponFacing == WEAPON_FACING_AFT && accuracy < COMBAT_AI_ISNT_AWFUL) {  // bad shots with aft lasers
        basic_aim *= 1.3;
    } else if (currentWeaponFacing == WEAPON_FACING_PORT ||
               currentWeaponFacing == WEAPON_FACING_STARBOARD) {  // everyone a bit worse with side lasers
        if (accuracy < COMBAT_AI_ISNT_AWFUL) {                    // especially these
            basic_aim *= 1.3 + randf();
        } else {
            basic_aim *= 1.3;
        }
    ...
    if (isSunlit) {
        OOSunEntity *sun = [UNIVERSE sun];
        if (sun) {
            GLfloat sunGlareAngularSize =
                atan([sun radius] / HPdistance([self position], [sun position])) * SUN_GLARE_MULT_FACTOR +
                (SUN_GLARE_ADD_FACTOR);
            GLfloat glareLevel =
                [self lookingAtSunWithThresholdAngleCos:cos(sunGlareAngularSize)] * (1.0f - [self sunGlareFilter]);
            if (glareLevel > 0.1f) {
                // looking towards sun can seriously mess up aim (glareLevel 0..1)
                basic_aim *= (1.0 + glareLevel * 3.0);
                //				OOLog(@"aim.debug",@"Sun glare affecting aim: %f for
                //%@",glareLevel,self);
                if (glareLevel > 0.5f) {
...
- (GLfloat)lookingAtSunWithThresholdAngleCos:(GLfloat)thresholdAngleCos {
    ...
    if (thresholdAngleCos <= measuredCosAbs &&
        measuredCosAbs <= 1.0f)  // angle from viewpoint to sun <= desired threshold
    {
...
- (BOOL)onTarget:(OOWeaponFacing)direction withWeapon:(OOWeaponType)weapon_type {
    // initialize dq to a value that would normally return NO; dq is handled inside the defaultless switch(direction)
    // statement and should alaways be recalculated anyway. Initialization here needed to silence compiler warning -
    // Nikos 20120526
    GLfloat dq = -1.0f;
...
- (BOOL)fireWeapon:(OOWeaponType)weapon_type direction:(OOWeaponFacing)direction range:(double)range {
    ...
    if (![weapon_type isTurretLaser]) {  // thargoid laser may just pick secondary target in this case
...
- (BOOL)fireMainWeapon:(double)range {
    ...
    if (isWeaponNone(forward_weapon_type)) {
        ...
        if (isWeaponNone(weapon_type) &&
            hasTurrets) { /* no forward weapon but has turrets, so set up range calculations accordingly
                             note: this was hard-coded to 10000.0, although turrets have a notably
                             shorter range. We are using a multiplier of 1.667 in order to not change
                             something that already works, but probably it would be best to use
                             TURRET_SHOT_RANGE * COMBAT_WEAPON_RANGE_FACTOR here
...
- (BOOL)fireTurretCannon:(double)range {
    ...
    if ([root isPlayer] && ![PLAYER weaponsOnline]) return NO;
    ...
    origin = HPvector_add(
        origin,
        vectorToHPVector(vector_multiply_scalar(vel, collision_radius + 0.5)));  // Start just outside collision sphere
...
- (BOOL)fireSubentityLaserShot:(double)range {
    ...
    NSAssert([parent isShipWithSubEntityShip:self],
    ...
    ShipEntity *victim = [UNIVERSE firstShipHitByLaserFromShip:self
                                                   inDirection:direction
                                                        offset:kZeroVector
                                             gettingRangeFound:&hitAtRange];
    ...
    if (victim != nil) {
        ...
        if (subent != nil && [victim isFrangible]) {
            ...
            [victim takeEnergyDamage:0.01 * weapon_damage
                                from:self
                           becauseOf:parent
                    weaponIdentifier:[[self weaponTypeForFacing:WEAPON_FACING_FORWARD strict:YES] identifier]];
            victim = subent;
        }

        if (hitAtRange < weaponRange) {
            [victim takeEnergyDamage:weapon_damage
                                from:self
                           becauseOf:parent
                    weaponIdentifier:[[self weaponTypeForFacing:WEAPON_FACING_FORWARD
                                                         strict:YES] identifier]];  // a very palpable hit

            [shot setRange:hitAtRange];
            Vector vd = vector_forward_from_quaternion([shot orientation]);
            HPVector flash_pos =
                HPvector_add([shot position], vectorToHPVector(vector_multiply_scalar(vd, hitAtRange)));
        ...
        if (![parent isCloaked]) {
            ...
            Vector victimDirection =
                vector_normal(HPVectorToVector(HPvector_subtract([victim position], [parent position])));
...
- (BOOL)fireDirectLaserShotAt:(Entity *)my_target {
    ...
    q_laser.x +=
        acc_factor * (randf() - 0.5);  // randomise aim a little (+/- 0.005 at accuracy 0, never miss at accuracy 10)
    ...
    ShipEntity *victim = [UNIVERSE firstShipHitByLaserFromShip:self
                                                   inDirection:WEAPON_FACING_FORWARD
                                                        offset:kZeroVector
                                             gettingRangeFound:&hit_at_range];
    [self setShipHitByLaser:victim];
    ...
    if (victim != nil) {
        ...
        if (hit_at_range * hit_at_range < range_limit2) {
            ...
            Vector vd = vector_forward_from_quaternion([shot orientation]);
            HPVector flash_pos =
                HPvector_add([shot position], vectorToHPVector(vector_multiply_scalar(vd, hit_at_range)));
            [UNIVERSE addLaserHitEffectsAt:flash_pos against:victim damage:weapon_damage color:laser_color];
        }
    }
    ...
    for (i = 0; i < barrels; i++) {
        ...
        if (victim != nil) {
            ...
            */
            ...
            if (subent != nil && [victim isFrangible]) {
                // do 1% bleed-through damage...
                [victim takeEnergyDamage:0.01 * effective_damage
                                    from:self
                               becauseOf:self
            ...
            if (hit_at_range * hit_at_range < range_limit2) {
                [victim takeEnergyDamage:effective_damage
                                    from:self
                               becauseOf:self
                        weaponIdentifier:weaponIdentifier];  // a very palpable hit
            ...
            if (![self isCloaked]) {
                ...
                if ([victim isShip])  // it might not be - fixes crash bug
                {
                    ...
                    if (dot_product(vector_forward_from_quaternion([shot orientation]),
                                    vector_normal([self vectorTo:victim])) > 0.995) {
...
- (void)throwSparks {
    ...
    HPVector origin =
        HPvector_add(position, vectorToHPVector(quaternion_rotate_vector([self normalOrientation], offset)));
...
- (Vector)missileLaunchPosition {
    ...
    if (start.x == 0.0f && start.y == 0.0f && start.z <= 0.0f)  // The kZeroVector as start is illegal also.
    {
        OOLog(
            ...
            @ "***** ERROR: The missile_launch_position defines a position %@ behind the %@. In future versions such missiles may explode on launch because they have to travel through the ship.",
            VectorDescription(start),
            self);
        start.x = 0.0f;
...
- (BOOL)isMissileFlagSet {
    return isMissile;  // were we created using fireMissile? (for tracking submunitions and preventing collisions at
                       // launch)
}

- (void)setIsMissileFlag:(BOOL)newValue {
    isMissile =
        !!newValue;  // set the isMissile flag, used for tracking submunitions and preventing collisions at launch.
}
...
- (void)noticeECM {
    if (accuracy >= COMBAT_AI_ISNT_AWFUL && missiles > 0 && [[missile_list[0] identifier] isEqualTo:@ "EQ_MISSILE"]) {
        ...
        missile_launch_time =
            [UNIVERSE getTime] + fmax(2.0, missile_load_time);  // set minimum launchtime for the next missile.
    }
}
...
- (ShipEntity *)launchEscapeCapsule {
    ...
    */
    ...
    for (i = 1; i < n_pods; i++) {
        ...
        passenger =
            [self launchPodWithCrew:[NSArray arrayWithObject:[OOCharacter randomCharacterWithRole:@ "passenger"
                                                                                andOriginalSystem:gen_rnd_number()]]];
...
- (ShipEntity *)dumpCargoItem:(OOCommodityType)preferred {
    ...
    if (([cargo count] > 0) && ([UNIVERSE getTime] - cargo_dump_time > 0.5))  // space them 0.5s or 10m apart
    {
        ...
        if (jetto != nil) {
            ...
            [cargo removeObjectAtIndex:i];
            [self broadcastAIMessage:@ "CARGO_DUMPED"];  // goes only to 16 nearby ships in range, but that should be
                                                        // enough.
            unsigned i;
...
- (void)manageCollisions {
    ...
    while ([collidingEntities count] > 0) {
        // EMMSTRAN: investigate if doing this backwards would be more efficient. (Not entirely obvious, NSArray is
        // kinda funky.) -- Ahruman 2011-02-12
...
- (BOOL)collideWithShip:(ShipEntity *)other {
    ...
    if (universalID == NO_TARGET) return NO;
    if (other->universalID == NO_TARGET) return NO;
    ...
    if (magnitude2(v) <=
        0.1)  // virtually no relative velocity - we must provide at least 1m/s to avoid conjoined objects
    {
...
- (BOOL)canScoop:(ShipEntity *)other {
    ...
    if ([self isPlayer] && dot_product(v_up, HPVectorToVector(loc)) > 0.0f)
        return NO;  // player has to scoop on underside, give more flexibility to NPCs
...
- (void)takeEnergyDamage:(double)amount
    ...
    if (hunter != nil && [self owner] != hunter)  // our owner could be the same entity as the one responsible for our
                                                  // taking damage in the case of submunitions
    {
        if ([hunter isCloaked]) {
    ...
    if (suppressExplosion)
        damageType = kOODamageTypeRemoved;
    else if (energyMine)
        damageType = kOODamageTypeCascadeWeapon;
    ...
    if (energy <= 0.0) {
        ...
        if ((energy < maxEnergy * 0.125 || (energy < 64 && energy < amount * 2)) && [self hasEscapePod] &&
            (ranrot_rand() & 3) == 0)  // 25% chance he gets to an escape pod
        {
            [self abandonShip];
        }
    }
...
- (BOOL)abandonShip {
    ...
    if (![self hasEscapePod]) {
        OOLog(@ "ShipEntity.abandonShip.failed",
              @ "Ship abandonment was requested for %@, but this ship does not carry escape pod(s).",
              self);
        return OK;
    }

    if (EXPECT([self launchEscapeCapsule] !=
               NO_TARGET))  // -launchEscapeCapsule takes care of everything for the player
    {
        if (![self isPlayer]) {
            OK = YES;
...
- (void)enterWormhole:(WormholeEntity *)w_hole replacing:(BOOL)replacing {
    ...
    [shipAI reactToMessage:@ "WITCHSPACE OKAY"
                   context:@ "performHyperSpaceExit"];  // must be a reaction, the ship is about to disappear
...
- (BOOL)canAcceptEscort:(ShipEntity *)potentialEscort {
    ...
    if (scanClass !=
        [potentialEscort scanClass])  // this makes sure that wingman can only select police, thargons only thargoids.
    {
        return NO;
    }
...
- (BOOL)acceptAsEscort:(ShipEntity *)other_ship {
    ...
    if ([self canAcceptEscort:other_ship]) {
        ...
        if (_maxEscortCount == 0 && ([self hasPrimaryRole:@ "police"] || [self hasPrimaryRole:@ "hunter"] ||
                                     [self hasRole:@ "thargoid-mothership"])) {
            _maxEscortCount = MAX_ESCORTS;
        }
        ...
        if (escortCount < maxEscorts) {
            ...
            } else if ([self group] != escortGroup)
...
- (HPVector)coordinatesForEscortPosition:(unsigned)idx {
    ...
    */
    ...
    return HPvector_add(self->position,
                        vectorToHPVector(quaternion_rotate_vector([self normalOrientation], _escortPositions[idx])));
}
...
- (void)setTargetToNearestStationIncludingHostiles:(BOOL)includeHostiles {
    ...
    for (i = 0; i < ent_count; i++)
        if (uni_entities[i]->isStation) my_entities[station_count++] = [uni_entities[i] retain];  //	retained
    //
    ...
    for (i = 0; i < station_count; i++) [my_entities[i] release];  //	released
    //
    if (station) {
...
// Exposed to AI
- (void)abortDocking {
    [[UNIVERSE findEntitiesMatchingPredicate:IsStationPredicate parameter:nil inRange:-1
                                    ofEntity:nil] makeObjectsPerformSelector:@selector(abortDockingForShip:)
                                                                  withObject:self];
}
...
- (void)broadcastThargoidDestroyed {
    [[UNIVERSE findShipsMatchingPredicate:HasRolePredicate
                                parameter:@ "tharglet"
                                 ...
                                 ofEntity:self] makeObjectsPerformSelector:@selector(sendAIMessage:)
                                                                withObject:@ "THARGOID_DESTROYED"];
}
...
static BOOL AuthorityPredicate(Entity *entity, void *parameter) {
    ...
    if ([entity scanClass] == CLASS_POLICE && HPdistance2([victim position], [entity position]) < SCANNER_MAX_RANGE2) {
        return YES;
    }
...
- (void)broadcastHitByLaserFrom:(ShipEntity *)aggressor_ship {
    ...
    if ((scanClass == CLASS_NEUTRAL) || (scanClass == CLASS_STATION) || (scanClass == CLASS_BUOY) ||
        (scanClass == CLASS_POLICE) || (scanClass == CLASS_MILITARY) ||
        (scanClass == CLASS_PLAYER))  // only for active ships...
    {
        ...
        authorities = [UNIVERSE findShipsMatchingPredicate:AuthorityPredicate parameter:self inRange:-1 ofEntity:nil];
...
- (void)sendMessage:(NSString *)message_text
    ...
    if (d2 > scannerRange * scannerRange) return;  // out of comms range
...
- (void)sendExpandedMessage:(NSString *)message_text toShip:(ShipEntity *)other_ship {
    if (!other_ship || !crew) return;  // nobody to receive or send the signal
    ...
    NSDictionary *specials = [NSDictionary dictionaryWithObjectsAndKeys:[self displayName],
                                                                        @ "[self:name]",
                                                                        [other_ship identFromShip:self],
                                                                        @ "[target:name]",
                                                                        nil];
    NSString *expandedMessage = OOExpandDescriptionString(
        OOStringExpanderDefaultRandomSeed(), message_text, specials, nil, nil, kOOExpandNoOptions);
...
- (void)broadcastMessage:(NSString *)message_text withUnpilotedOverride:(BOOL)unpilotedOverride {
    ...
    if (!crew && !unpilotedOverride) return;  // nobody to send the signal and no override for unpiloted craft is set
...
- (void)setCommsMessageColor {
    ...
    if (scanClass == CLASS_THARGOID) [[UNIVERSE commLogGUI] setTextColor:[OOColor greenColor]];
    if (scanClass == CLASS_POLICE) [[UNIVERSE commLogGUI] setTextColor:[OOColor cyanColor]];
}
...
- (BOOL)markForFines {
    if (being_fined) return NO;  // can't mark twice
...
- (void)interpretAIMessage:(NSString *)ms {
    if ([ms hasPrefix:AIMS_AGGRESSOR_SWITCHED_TARGET]) {
        ...
        if ((switcher == [self primaryAggressor]) && (switcher == [self primaryTarget]) && (switcher) && (rescuer) &&
            (rescuer->isShip) && ([self thankedShip] != rescuer) && (scanClass != CLASS_THARGOID)) {
            ...
            if (scanClass == CLASS_POLICE) {
                ...
                [rescueShip setBounty:[rescueShip bounty] * 0.80
                           withReason:kOOLegalStatusReasonAssistingPolice];  // lower bounty by 20%
            } else {
...
- (void)spawn:(NSString *)roles_number {
    ...
    if ([tokens count] != 2) {
        OOLog(
            kOOLogSyntaxAddShips, @ "***** Could not spawn: \"%@\" (must be two tokens, role and number)", roles_number);
        return;
    }
...
- (int)checkShipsInVicinityForWitchJumpExit {
    ...
    for (i = 0; (i < ship_count) && (result == NO_TARGET); i++) {
        ...
        if (![ship isPlayer] || ![PLAYER isDocked]) {  // player doesn't block if docked
            if ((k * [ship mass] > d2) &&
                (d2 < SCANNER_MAX_RANGE2))  // if you go off (typical) scanner from a blocker - it ceases to block
...
- (BOOL)isShipWithSubEntityShip:(Entity *)other {
    ...
    if (![self hasSubEntity:(ShipEntity *)other]) {
        OOLogERR(@ "ship.subentity.sanityCheck.failed",
                 @ "%@ thinks it's a subentity of %@, but the supposed parent does not agree. %@",
...
NSDictionary *OODefaultShipShaderMacros(void) {
    ...
    if (macros == nil) {
        macros = [[[ResourceManager materialDefaults] oo_dictionaryForKey:@ "ship-prefix-macros"
                                                             defaultValue:[NSDictionary dictionary]] retain];
    }
...
BOOL OOUniformBindingPermitted(NSString *propertyName, id bindingTarget) {
    ...
    if (entityWhitelist == nil) {
        ...
        playerShipWhitelist =
            [[NSSet alloc] initWithArray:[wlDict oo_arrayForKey:@ "shader_player_ship_binding_methods"]];
        visualEffectWhitelist =
            [[NSSet alloc] initWithArray:[wlDict oo_arrayForKey:@ "shader_visual_effect_binding_methods"]];
    }
...
GLfloat getWeaponRangeFromType(OOWeaponType weapon_type) { return [weapon_type weaponRange]; }
