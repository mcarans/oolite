...
#if OO_SHADERS
enum { kShaderModeOff, kShaderModeOn, kShaderModeUnknown };
#endif
...
- (id)init {
    ...
    if (hasPointSprites) {
        texture =
            [[OOTexture textureWithName:@ "oolite-particle-dust.png"
                               inFolder:@ "Textures"
...
- (void)updateCameraRelativePosition {
    ...
    cameraRelativePosition = make_vector((OOScalar)-fmod(c_pos.x, DUST_SCALE),
                                         (OOScalar)-fmod(c_pos.y, DUST_SCALE),
                                         (OOScalar)-fmod(c_pos.z, DUST_SCALE));
}
...
- (void)update:(OOTimeDelta)delta_t {
    ...
    int vi;
    for (vi = 0; vi < DUST_N_PARTICLES; vi++) {
        while (vertices[vi].x - offset.x < -half_scale) vertices[vi].x += DUST_SCALE;
        while (vertices[vi].x - offset.x > half_scale) vertices[vi].x -= DUST_SCALE;

        while (vertices[vi].y - offset.y < -half_scale) vertices[vi].y += DUST_SCALE;
        while (vertices[vi].y - offset.y > half_scale) vertices[vi].y -= DUST_SCALE;

        while (vertices[vi].z - offset.z < -half_scale) vertices[vi].z += DUST_SCALE;
        while (vertices[vi].z - offset.z > half_scale) vertices[vi].z -= DUST_SCALE;
    }
}
...
- (OOShaderProgram *)shader {
    if (shader == nil) {
        NSString *prefix = [NSString stringWithFormat:@ "#define OODUST_SCALE_MAX    (float(%g))\n"
...
- (Vector)offsetPlayerPosition {
    ...
    HPVector c_pos = [PLAYER viewpointPosition];
    Vector offset = make_vector(
        (OOScalar)fmod(c_pos.x, DUST_SCALE), (OOScalar)fmod(c_pos.y, DUST_SCALE), (OOScalar)fmod(c_pos.z, DUST_SCALE));
