...
- (void)showScenarioDetails {
    ...
    if ([key hasPrefix:@ "Scenario"]) {
        ...
        if (scenario) {
            [gui addLongText:OOExpand([scenario oo_stringForKey:@ "description"])
                startingAtRow:GUI_ROW_SCENARIOS_DETAIL
...
- (void)saveCommanderInputHandler {
    ...
    if ([self handleGUIUpDownArrowKeys]) {
        ...
        if (guiSelectedRow != MOREROW && guiSelectedRow != BACKROW) {
            [self showCommanderShip:idx];
            if ([(NSDictionary *)[cdrDetailArray objectAtIndex:idx]
                    oo_boolForKey:@ "isSavedGame"])  // don't show things that aren't saved games
                commanderNameString = [[cdrDetailArray oo_dictionaryAtIndex:idx]
                    oo_stringForKey:@ "player_save_name"
                       defaultValue:[[cdrDetailArray oo_dictionaryAtIndex:idx] oo_stringForKey:@ "player_name"]];
            else
    ...
    [gui setText:[NSString stringWithFormat:DESC(@ "savescreen-commander-name-@"), commanderNameString] forRow:INPUTROW];
    ...
    if (((([gameView isDown:gvMouseDoubleClick] || [self checkKeyPress:n_key_gui_select]) &&
          [gui selectedRow] == BACKROW) ||
         ([self checkKeyPress:n_key_gui_arrow_left] || [self checkKeyPress:n_key_gui_page_up])) &&
        [[gui keyForRow:BACKROW] isEqual:GUI_KEY_OK]) {
        currentPage--;
        [self lsCommanders:gui directory:dir pageNumber:currentPage highlightName:nil];
        [gameView suppressKeysUntilKeyUp];
    ...
    //
    if (((([gameView isDown:gvMouseDoubleClick] || [self checkKeyPress:n_key_gui_select]) &&
          [gui selectedRow] == MOREROW) ||
         ([self checkKeyPress:n_key_gui_arrow_right] || [self checkKeyPress:n_key_gui_page_down])) &&
        [[gui keyForRow:MOREROW] isEqual:GUI_KEY_OK]) {
        currentPage++;
        [self lsCommanders:gui directory:dir pageNumber:currentPage highlightName:nil];
        [gameView suppressKeysUntilKeyUp];
...
- (void)overwriteCommanderInputHandler {
    ...
    if (([self checkKeyPress:n_key_gui_select] && ([gui selectedRow] == SAVE_OVERWRITE_YES_ROW)) ||
        [gameView isDown:cYes] || [gameView isDown:cYes - 32]) {
        pollControls = YES;
        [self nativeSavePlayer:commanderNameString];
    ...
    if (([self checkKeyPress:n_key_gui_select] && ([gui selectedRow] == SAVE_OVERWRITE_NO_ROW)) ||
        [gameView isDown:27] || [gameView isDown:cNo] || [gameView isDown:cNo - 32]) {
    ...
    */
    ...
    if (loadedOK) {
        ...
        if (shipDict == nil) {
            loadedOK = NO;
            if (shipKey != nil)
                fail_reason = [NSString
                    stringWithFormat:DESC(
                                         @ "loadfailed-could-not-find-ship-type-@-please-reinstall-the-appropriate-OXP"),
                                     shipKey];
            else
                fail_reason = DESC(@ "loadfailed-invalid-saved-game-no-ship-specified");
        }
    }
    ...
    if (loadedOK) {
        ...
        OOLog(@ "load.failed",
              ...
              [fileToOpen lastPathComponent],
              fail_reason ? fail_reason : (NSString *)@ "unknown error");
    ...
    [self setGuiToStatusScreen];
    if (loadedOK)
        [self doWorldEventUntilMissionScreen:OOJSID("missionScreenOpportunity")];  // trigger missionScreenOpportunity
...
- (void)writePlayerToPath:(NSString *)path {
    ...
    if (dict == nil)
        errDesc = @ "could not construct commander data dictionary.";
    else
        didSave = [dict writeOOXMLToFile:path atomically:YES errorDescription:&errDesc];
    if (didSave) {
        ...
        [NSException raise:@ "OoliteException" format:@ "Attempt to save game to file '%@' failed: %@", path, errDesc];
    }
    [[UNIVERSE gameView] suppressKeysUntilKeyUp];
    [self setGuiToStatusScreen];
}
...
- (void)setGuiToOverwriteScreen:(NSString *)cdrName {
    ...
    [gui
        setText:[NSString stringWithFormat:DESC(@ "overwritescreen-commander-@-already-exists-overwrite-query"), cdrName]
         forRow:SAVE_OVERWRITE_WARN_ROW
    ...
    [UNIVERSE
        enterGUIViewModeWithMouseInteraction:NO];  // FIXME: should be YES, but was NO before introducing new mouse mode
                                                   // stuff. If set to YES, choices can be selected but not activated.
}
...
- (void)lsCommanders:(GuiDisplayGen *)gui
    ...
    if (!cdrDetailArray)
        cdrDetailArray =
            [[NSMutableArray alloc] init];  // alloc retains this so the retain further on in the code was unnecessary
    else
        [cdrDetailArray removeAllObjects];

    [cdrDetailArray
        addObject:[NSMutableDictionary dictionaryWithObjectsAndKeys:@ "YES",
                                                                    @ "isParentFolder",
            ...
            if (isDirectory && ![[path lastPathComponent] hasPrefix:@ "."]) {
                [cdrDetailArray
                    addObject:[NSDictionary
                                  dictionaryWithObjectsAndKeys:@ "YES", @ "isFolder", path, @ "saved_game_path", nil]];
            }
        }
    }
    ...
    if (page) {
        ...
        [gui setArray:[NSArray arrayWithObjects:DESC(@ "gui-back"), @ " <-- ", nil] forRow:STARTROW - 1];
    ...
    if (firstIndex + NUMROWS >= [cdrDetailArray count]) {
        ...
        [gui setArray:[NSArray arrayWithObjects:DESC(@ "gui-more"), @ " --> ", nil] forRow:ENDROW];
    ...
    for (i = firstIndex; i < lastIndex; i++) {
        ...
        if ([cdr oo_boolForKey:@ "isSavedGame"]) {
            ...
            [gui setArray:[NSArray arrayWithObjects:
                                       [NSString
                                           stringWithFormat:@ " %@ ",
                                                            [cdr oo_stringForKey:@ "player_save_name"
                                                                    defaultValue:[cdr oo_stringForKey:@ "player_name"]]],
                                       [NSString stringWithFormat:@ " %@ ", ratingDesc],
                                       nil]
                   forRow:row];
            if ([[self lastsaveName] isEqualToString:[cdr oo_stringForKey:@ "player_save_name"
                                                             defaultValue:[cdr oo_stringForKey:@ "player_name"]]]) {
        ...
        if ([cdr oo_boolForKey:@ "isParentFolder"]) {
            [gui setArray:[NSArray arrayWithObjects:[NSString stringWithFormat:@ " (..) %@ ",
...
- (void)showCommanderShip:(int)cdrArrayIndex {
    ...
    if ([cdr oo_boolForKey:@ "isFolder"]) {
        NSString *folderDesc =
            [NSString stringWithFormat:DESC(@ "loadsavescreen-hold-@-and-press-return-to-open-folder-@"),
    ...
    if ([cdr oo_boolForKey:@ "isParentFolder"]) {
        NSString *folderDesc =
            [NSString stringWithFormat:DESC(@ "loadsavescreen-hold-@-and-press-return-to-open-parent-folder-@"),
                                       @COMMAND_MODIFIER_KEY,
                                       [[cdr oo_stringForKey:@ "saved_game_path"] lastPathComponent]];
        [gui setColor:[OOColor orangeColor] forRow:CDRDESCROW];
        [gui addLongText:folderDesc startingAtRow:CDRDESCROW align:GUI_ALIGN_LEFT];
    ...
    NSString *locationTL =
        [cdr objectForKey:@ "current_system_techlevel"]
            ? [NSString stringWithFormat:@ "%u", [cdr oo_unsignedIntForKey:@ "current_system_techlevel"] + 1]
            : nil;
    if (locationTL) {
        locationGov = [NSString stringWithFormat:@ "%c", [cdr oo_unsignedCharForKey:@ "current_system_government"]];
        locationEco =
            [NSString stringWithFormat:@ " %c", (7 - [cdr oo_unsignedCharForKey:@ "current_system_economy"]) + 16];
    } else
        locationTL = @ "";
    ...
    cdrDesc = [NSString
        stringWithFormat:
            DESC(
                @ "loadsavescreen-commander-@-rated-@-has-@-legal-status-@-ship-@-location-@-g-@-eco-@-gov-@-tl-@-timestamp-@"),
...
- (int)findIndexOfCommander:(NSString *)cdrName {
    ...
    for (i = 0; i < [cdrDetailArray count]; i++) {
        NSString *currentName = [[cdrDetailArray oo_dictionaryAtIndex:i]
            oo_stringForKey:@ "player_save_name"
               defaultValue:[[cdrDetailArray oo_dictionaryAtIndex:i] oo_stringForKey:@ "player_name"]];
