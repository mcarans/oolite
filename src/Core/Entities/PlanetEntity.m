...
static const Vector kUntexturedVertices[] = {{+0.000000, -0.850664, -0.525710},
...
static const Vector kTexturedVertices[] = {{+0.525731, +0.000000, +0.850651},
...
- (id)initAsAtmosphereForPlanet:(PlanetEntity *)planet dictionary:(NSDictionary *)dict {
    ...
    int percent_land = 100 - [dict oo_intForKey:@ "percent_cloud"
...
#define POLAR_CLOUD_ALPHA 0.75
    ...
    if (polarClearSkyColor != nil) {
        [polarClearSkyColor getRed:&amb_polar_land[0]
    ...
    if (polarCloudColor != nil) {
        [polarCloudColor getRed:&amb_polar_sea[0]
    ...
    if (planet->planet_type == STELLAR_TYPE_MINIATURE)
        collision_radius = planet->collision_radius +
                           ATMOSPHERE_DEPTH * PLANET_MINIATURE_FACTOR * 2.0;  // not to scale: invisible otherwise
    ...
    rotational_velocity = [dict
        oo_floatForKey:@ "atmosphere_rotational_velocity"
          defaultValue:[planet rotationalVelocity] * (0.9 + (randf() * 0.2))];  // 90-110% of planet rotation speed
...
- (void)miniaturize {
    ...
    [self scaleVertices];
    if (atmosphere != nil) {
        atmosphere->collision_radius =
            collision_radius + ATMOSPHERE_DEPTH * PLANET_MINIATURE_FACTOR * 2.0;  // not to scale: invisible otherwise
...
- (id)initFromDictionary:(NSDictionary *)dict withAtmosphere:(BOOL)atmo andSeed:(Random_Seed)p_seed {
    ...
    int techlevel = [dict oo_intForKey:KEY_TECHLEVEL defaultValue:[planetInfo oo_intForKey:KEY_TECHLEVEL]];
    ...
    last_launch_time =
        [UNIVERSE getTime] + 30.0 - shuttle_launch_interval;  // debug - launch 30s after player enters universe
    ...
    if (isTextureImage) {
        ...
        land_hsb.x = 0.0;
        land_hsb.y = 0.0;
        land_hsb.z = 1.0;  // non-saturated fully bright (white)
        sea_hsb.x = 0.0;
        sea_hsb.y = 1.0;
        sea_hsb.z = 1.0;  // fully-saturated fully bright (red)
        // override the mainPlanet texture colour...
        [self setTextureColorForPlanet:!![dict objectForKey:@ "mainForLocalSystem"]
                              inSystem:[dict oo_boolForKey:@ "mainForLocalSystem" defaultValue:NO]];
    } else {
        // random land & sea colours.
        land_hsb.x = gen_rnd_number() / 256.0;
        land_hsb.y = gen_rnd_number() / 256.0;
        land_hsb.z = 0.5 + gen_rnd_number() / 512.0;
        sea_hsb.x = gen_rnd_number() / 256.0;
        sea_hsb.y = gen_rnd_number() / 256.0;
        sea_hsb.z = 0.5 + gen_rnd_number() / 512.0;
        ...
        land_polar_hsb.x = land_hsb.x;
        land_polar_hsb.y = (land_hsb.y / 4.0);
        land_polar_hsb.z = 1.0 - (land_hsb.z / 10.0);
        sea_polar_hsb.x = sea_hsb.x;
        sea_polar_hsb.y = (sea_hsb.y / 4.0);
        sea_polar_hsb.z = 1.0 - (sea_hsb.z / 10.0);

        OOColor *amb_land_color = [OOColor colorWithHue:land_hsb.x
                                             saturation:land_hsb.y
                                             brightness:land_hsb.z
                                                  alpha:1.0];
        OOColor *amb_sea_color = [OOColor colorWithHue:sea_hsb.x saturation:sea_hsb.y brightness:sea_hsb.z alpha:1.0];
        OOColor *amb_polar_land_color = [OOColor colorWithHue:land_polar_hsb.x
                                                   saturation:land_polar_hsb.y
                                                   brightness:land_polar_hsb.z
                                                        alpha:1.0];
        OOColor *amb_polar_sea_color = [OOColor colorWithHue:sea_polar_hsb.x
                                                  saturation:sea_polar_hsb.y
                                                  brightness:sea_polar_hsb.z
                                                       alpha:1.0];
    ...
    if ([dict objectForKey:@ "rotational_velocity"]) {
        rotational_velocity = [dict oo_floatForKey:@ "rotational_velocity"
                                      defaultValue:0.01f * randf()];  // 0.0 .. 0.01 avr 0.005
    } else {
        rotational_velocity = [planetInfo oo_floatForKey:@ "rotation_speed"
                                            defaultValue:0.002 * (0.5 + 0.5 * randf())];  // 0.001 .. 0.002 avr 0.0015
    ...
    if (_texture != nil)
        atmoDict = [NSDictionary
            dictionaryWithObjectsAndKeys:@ "0",
                                         @ "percent_cloud",
                                         [NSNumber numberWithFloat:[planetInfo oo_floatForKey:@ "cloud_alpha"
                                                                                 defaultValue:1.0]],
                                         @ "cloud_alpha",
                                         nil];
...
- (NSString *)descriptionComponents {
    ...
    switch (planet_type) {
        case STELLAR_TYPE_MINIATURE:
            typeString = @ "STELLAR_TYPE_MINIATURE";
            break;
        case STELLAR_TYPE_NORMAL_PLANET:
            typeString = @ "STELLAR_TYPE_NORMAL_PLANET";
            break;
        case STELLAR_TYPE_ATMOSPHERE:
            typeString = @ "STELLAR_TYPE_ATMOSPHERE";
            break;
        case STELLAR_TYPE_MOON:
            typeString = @ "STELLAR_TYPE_MOON";
            break;

        default:
            typeString = @ "UNKNOWN";
    }
    return [NSString stringWithFormat:@ "ID: %u position: %@ type: %@ radius: %.3fkm",
                                      [self universalID],
                                      HPVectorDescription([self position]),
                                      typeString,
                                      0.001 * [self radius]];
}
...
- (BOOL)checkCloseCollisionWith:(Entity *)other {
#ifndef NDEBUG
    if (gDebugFlags & DEBUG_COLLISIONS) OOLog(@ "planet.collide", @ "PLANET Collision!");
#endif
    ...
    if (other->isShip) {
        ...
        if ([ship reportAIMessages]) {
            ...
            OOLog(@ "planet.collide.shipHit",
                  @ "DEBUG: %@ %d collided with planet at (%.1f,%.1f,%.1f)",
                  [ship name],
                  [ship universalID],
                  p1.x,
                  p1.y,
    ...
    switch (planet_type) {
        case STELLAR_TYPE_NORMAL_PLANET:
            // we have atmosphere in any case.
            {
                ...
                double atmo = 10.0 * (atmosphere->collision_radius -
                                      collision_radius);  // effect starts at 10x the height of the clouds
                ...
                if ((alt > 0) && (alt <= atmo)) {
                    ...
                    [UNIVERSE setSkyColorRed:0.0f green:0.0f blue:0.0f alpha:0.0f];
                }
            }
        case STELLAR_TYPE_MOON: {
    ...
    if (isMain) {
        if (isLocal)
            ...
            ScanVectorFromString(
                [[UNIVERSE generateSystemData:[PLAYER target_system_seed]] objectForKey:@ "texture_hsb_color"],
                &land_hsb);
    }

    land_polar_hsb.x = land_hsb.x;
    land_polar_hsb.y = (land_hsb.y / 5.0);
    land_polar_hsb.z = 1.0 - (land_hsb.z / 10.0);

    amb_sea[0] = amb_land[0] = [[OOColor colorWithHue:land_hsb.x saturation:land_hsb.y brightness:land_hsb.z
                                                alpha:1.0] redComponent];
    amb_sea[1] = amb_land[1] = [[OOColor colorWithHue:land_hsb.x saturation:land_hsb.y brightness:land_hsb.z
                                                alpha:1.0] blueComponent];
    amb_sea[2] = amb_land[2] = [[OOColor colorWithHue:land_hsb.x saturation:land_hsb.y brightness:land_hsb.z
                                                alpha:1.0] greenComponent];
    amb_sea[3] = amb_land[3] = 1.0;
    amb_polar_sea[0] = amb_polar_land[0] = [[OOColor colorWithHue:land_polar_hsb.x
                                                       saturation:land_polar_hsb.y
                                                       brightness:land_polar_hsb.z
                                                            alpha:1.0] redComponent];
    amb_polar_sea[1] = amb_polar_land[1] = [[OOColor colorWithHue:land_polar_hsb.x
                                                       saturation:land_polar_hsb.y
                                                       brightness:land_polar_hsb.z
                                                            alpha:1.0] blueComponent];
    amb_polar_sea[2] = amb_polar_land[2] = [[OOColor colorWithHue:land_polar_hsb.x
                                                       saturation:land_polar_hsb.y
                                                       brightness:land_polar_hsb.z
                                                            alpha:1.0] greenComponent];
...
- (BOOL)setUpPlanetFromTexture:(NSString *)fileName {
    ...
    NSDictionary *atmo_dictionary = [NSDictionary dictionaryWithObjectsAndKeys:@ "0",
                                                                               @ "percent_cloud",
                                                                               [NSNumber numberWithFloat:oldCloudAlpha],
                                                                               @ "cloud_alpha",
                                                                               nil];
    [atmosphere autorelease];
    atmosphere =
        [self hasAtmosphere] ? [[PlanetEntity alloc] initAsAtmosphereForPlanet:self dictionary:atmo_dictionary] : nil;
...
- (void)drawModelWithVertexArraysAndSubdivision:(int)subdivide {
    OOGL(glDrawElements(
        GL_TRIANGLES, 3 * n_triangles[subdivide], GL_UNSIGNED_INT, &vertexdata.index_array[triangle_start[subdivide]]));
}
...
static unsigned baseVertexIndexForEdge(GLushort va, GLushort vb, BOOL textured) {
    ...
    if (num != 0) {
        ...
        NSCAssert(vindex < sizeof base_vertex_array / sizeof *base_vertex_array,
...
- (void)paintVertex:(unsigned)vi:(int)seed {
    ...
    for (i = 0; i < 3; i++) {
        double cv =
            (ranrot_rand() % 100) *
            0.01;  //  0..1 ***** DON'T CHANGE THIS LINE, '% 100' MAY NOT BE EFFICIENT BUT THE PATTERNING IS GOOD.
...
- (void)scaleVertices {
    ...
    for (vi = 0; vi < next_free_vertex; vi++) {
        ...
        vertexdata.vertex_array[vi] =
            make_vector(v.x * collision_radius, v.y * collision_radius, v.z * collision_radius);
...
- (void)loadTexture:(NSDictionary *)configuration {
    [_texture release];
    _texture = [OOTexture textureWithConfiguration:configuration
                                      extraOptions:kOOTextureAllowCubeMap | kOOTextureRepeatS];
    [_texture retain];
...
- (OOTexture *)planetTextureWithInfo:(NSDictionary *)info {
    ...
    fillRanNoiseBuffer();
    if (![TextureStore getPlanetTextureNameFor:info intoData:&data width:&width height:&height]) {
        return nil;
    }

    OOPixMap pm = OOMakePixMap(data, width, height, kOOPixMapRGBA, 0, 0);
    OOTextureGenerator *loader =
        [[OOPixMapTextureLoader alloc] initWithPixMap:pm
...
- (OOTexture *)cloudTextureWithCloudColor:(OOColor *)cloudColor
    ...
    fillRanNoiseBuffer();
    if (![TextureStore getCloudTextureNameFor:
                                   cloudColor:cloud_impress:cloud_bias
                                     intoData:&data
                                        width:&width
                                       height:&height]) {
        return nil;
