...
- (void)abortAllDockings {
    ...
    if ([player getTargetDockStation] == self &&
...
- (NSDictionary *)dockingInstructionsForShip:(ShipEntity *)ship {
    ...
    if ([ship isPlayer] && [ship legalStatus] > 50)  // note: non-player fugitives dock as normal
    {
        ...
        return OOMakeDockingInstructions(
            self, [ship position], 0, 100, @ "DOCKING_REFUSED", @ "[station-docking-refused-to-fugitive]", NO, -1);
    }

    if (magnitude2(velocity) > 1.0 || fabs(flightPitch) > 0.01 || fabs(flightYaw) > 0.01) {
        // no docking while station is moving, pitching or yawing
        return [self holdPositionInstructionForShip:ship];
    }
    PlayerEntity *player = PLAYER;
    BOOL player_is_ahead =
        (![ship isPlayer] && [player getDockingClearanceStatus] == DOCKING_CLEARANCE_STATUS_REQUESTED &&
    ...
    for (subEnum = [self dockSubEntityEnumerator]; (sub = [subEnum nextObject]);) {
        ...
        if (sub != player_reserved_dock || [ship isPlayer]) {
            ...
            if ([docking isEqualToString:@ "DOCK_CLOSED"]) {
                ...
                jsval args[] = {OOJSValueFromNativeObject(context, sub), OOJSValueFromNativeObject(context, ship)};
                JSBool tempreject = NO;

                BOOL OK = [[self script] callMethod:OOJSID("willOpenDockingPortFor")
                                          inContext:context
                                      withArguments:args
                                              count:2
                                             result:&rval];
...
- (BOOL)setUpShipFromDictionary:(NSDictionary *)dict {
    ...
    hasNPCTraffic = [dict oo_fuzzyBooleanForKey:@ "has_npc_traffic"
                                   defaultValue:(maxFlightSpeed == 0)];  // carriers default to NO
    ...
    requiresDockingClearance = [dict oo_boolForKey:@ "requires_docking_clearance"
                                      defaultValue:[UNIVERSE dockingClearanceProtocolActive]];
    ...
    for (subEnum = [self shipSubEntityEnumerator]; (subEntity = [subEnum nextObject]);) {
        if ([subEntity isStation]) {
            OOLog(
                ...
                @ "Subentity %@ (%@) of station %@ is itself a StationEntity. This is an internal error - please report it. ",
                subEntity,
                [subEntity shipDataKey],
                [self displayName]);
        }
    }
#endif
...
- (void)update:(OOTimeDelta)delta_t {
    ...
    if (isDockingStation && [player status] == STATUS_IN_FLIGHT) {
        if ([player getDockingClearanceStatus] >= DOCKING_CLEARANCE_STATUS_GRANTED) {
            if (last_launch_time - 30 < unitime &&
                [player getDockingClearanceStatus] != DOCKING_CLEARANCE_STATUS_TIMING_OUT) {
                ...
                [player setDockingClearanceStatus:DOCKING_CLEARANCE_STATUS_NONE];  // Docking clearance for player has
                                                                                   // expired.
        ...
        else if ([player getDockingClearanceStatus] == DOCKING_CLEARANCE_STATUS_REQUESTED && [self hasClearDock]) {
            DockEntity *dock = [self selectDockForDocking];
            last_launch_time = unitime + DOCKING_CLEARANCE_WINDOW;
            if ([self hasMultipleDocks]) {
                [self sendExpandedMessage:[NSString
                                              stringWithFormat:DESC(@ "station-docking-clearance-granted-in-@-until-@"),
                                                               [dock displayName],
                                                               ClockToString(
                                                                   [player clockTime] + DOCKING_CLEARANCE_WINDOW, NO)]
                                   toShip:player];
            } else {
                [self sendExpandedMessage:[NSString
                                              stringWithFormat:DESC(@ "station-docking-clearance-granted-until-@"),
                                                               ClockToString(
                                                                   [player clockTime] + DOCKING_CLEARANCE_WINDOW, NO)]
                                   toShip:player];
            }
...
- (BOOL)hasEligibleDock {
    ...
    for (subEnum = [self dockSubEntityEnumerator]; (sub = [subEnum nextObject]);) {
        ...
        if ([sub allowsDocking] && ([[sub canAcceptShipForDocking:PLAYER] isEqualToString:@ "DOCKING_POSSIBLE"] ||
                                    [[sub canAcceptShipForDocking:PLAYER] isEqualToString:@ "TRY_AGAIN_LATER"])) {
            return YES;
        }
    }
    return NO;
...
- (BOOL)fitsInDock:(ShipEntity *)ship andLogNoFit:(BOOL)logNoFit {
    ...
    if (logNoFit)
        OOLog(@ "station.launchShip.failed",
    ...
    if (isDockingStation && [player status] == STATUS_IN_FLIGHT &&
        ...
        if (![self hasClearDock]) {
            ...
            if ([self currentlyInDockingQueues]) {
                [self sendExpandedMessage:
                          [NSString stringWithFormat:DESC(@ "station-docking-clearance-holding-d-ships-approaching"),
                                                     [self currentlyInDockingQueues] + 1]
                                   toShip:player];
            } else if ([self currentlyInLaunchingQueues]) {
                [self sendExpandedMessage:
                          [NSString stringWithFormat:DESC(@ "station-docking-clearance-holding-d-ships-departing"),
                                                     [self currentlyInLaunchingQueues] + 1]
                                   toShip:player];
            }
        }
    }
...
- (void)addShipToStationCount:(ShipEntity *)ship {
    if ([ship isShuttle])
        docked_shuttles++;
    else if ([ship isTrader] && ![ship isPlayer])
        docked_traders++;
    else if (([ship isPolice] && ![ship isEscort]) || [ship hasPrimaryRole:@ "defense_ship"]) {
        if (0 < defenders_launched) defenders_launched--;
    } else if ([ship hasPrimaryRole:@ "scavenger"] ||
               [ship hasPrimaryRole:@ "miner"])  // treat miners and scavengers alike!
    {
...
- (BOOL)hasHostileTarget {
    return [super hasHostileTarget] || ([self primaryTarget] != nil && ((alertLevel == STATION_ALERT_LEVEL_YELLOW) ||
                                                                        (alertLevel == STATION_ALERT_LEVEL_RED)));
}
...
- (ShipEntity *)launchIndependentShip:(NSString *)role {
    if (![self hasLaunchDock]) {
        OOLog(@ "station.launchShip.impossible",
    ...
    if (ship) {
        ...
        if (trader || ship->scanClass == CLASS_NOT_SET)
            [ship setScanClass:CLASS_NEUTRAL];  // keep defined scanclasses for non-traders.
        ...
        if (trader) {
            ...
            if (sunskimmer) {
                ...
                //				if ([ship hasRole:@"sunskim-trader"]) [UNIVERSE makeSunSkimmer:ship
                // andSetAI:NO];
            }
        }
        ...
        // Eric: Escorts are defined both as _group and as _escortGroup because friendly attacks are only handled within
        // _group.
...
- (NSArray *)launchPolice {
    ...
    for (i = 0; (i < 4) && (defenders_launched < max_police); i++) {
        ...
        if (police_ship && [self fitsInDock:police_ship]) {
            ...
            if ([police_ship scanClass] == CLASS_NOT_SET) [police_ship setScanClass:CLASS_POLICE];
...
- (ShipEntity *)launchDefenseShip {
    ...
    NSString *defense_ship_key = nil, *defense_ship_role = nil, *default_defense_ship_role = nil;
    ...
    if (scanClass == CLASS_ROCK) default_defense_ship_role = @ "hermit-ship";
    ...
    if (!defense_ship) {
        defense_ship_role = [shipinfoDictionary oo_stringForKey:@ "defense_ship_role"
                                                   defaultValue:default_defense_ship_role];
...
- (ShipEntity *)launchScavenger {
    ...
    unsigned scavs = [UNIVERSE countShipsWithPrimaryRole:@ "scavenger" inRange:SCANNER_MAX_RANGE ofEntity:self] +
                     [self countOfShipsInLaunchQueueWithPrimaryRole:@ "scavenger"];
    ...
    if (scavenger_ship) {
        ...
        [scavenger_ship setGroup:[self stationGroup]];  // who's your Daddy -- FIXME: should we have a separate group
                                                        // for non-escort auxiliaires?
...
- (ShipEntity *)launchMiner {
    ...
    int n_miners = [UNIVERSE countShipsWithPrimaryRole:@ "miner" inRange:SCANNER_MAX_RANGE ofEntity:self] +
                   [self countOfShipsInLaunchQueueWithPrimaryRole:@ "miner"];
    ...
    if (miner_ship) {
        ...
        if ([miner_ship heatInsulation] < [self heatInsulation]) [miner_ship setHeatInsulation:[self heatInsulation]];
        [miner_ship setGroup:[self stationGroup]];  // who's your Daddy -- FIXME: should we have a separate group for
                                                    // non-escort auxiliaires?
...
- (ShipEntity *)launchPirateShip {
    ...
    if (defenders_launched >= max_defense_ships)
        return nil;  // shuttles are to rockhermits what police ships are to stations
    ...
    if (pirate_ship) {
        ...
        if ([pirate_ship heatInsulation] < [self heatInsulation]) [pirate_ship setHeatInsulation:[self heatInsulation]];
...
- (ShipEntity *)launchPatrol {
    ...
    if (defenders_launched < max_police) {
        ...
        if (patrol_ship) {
            ...
            if ([patrol_ship scanClass] == CLASS_NOT_SET) [patrol_ship setScanClass:CLASS_POLICE];
...
- (void)becomeExplosion {
    ...
    PlayerEntity *player = PLAYER;
    if ((player) && ([player status] == STATUS_DOCKED || [player status] == STATUS_DOCKING) &&
        ([player dockedStation] == self)) {
        // undock the player!
...
- (NSString *)acceptDockingClearanceRequestFrom:(ShipEntity *)other {
        ...
        switch ([player getDockingClearanceStatus]) {
            case DOCKING_CLEARANCE_STATUS_TIMING_OUT:
                if (!no_docking_while_launching) {
                    last_launch_time = timeNow + DOCKING_CLEARANCE_WINDOW;
                    [self
                        sendExpandedMessage:[NSString
                                                stringWithFormat:DESC(@ "station-docking-clearance-extended-until-@"),
                                                                 ClockToString(
                                                                     [player clockTime] + DOCKING_CLEARANCE_WINDOW, NO)]
                                     toShip:other];
                    [player setDockingClearanceStatus:DOCKING_CLEARANCE_STATUS_GRANTED];
    ...
    if (result == nil && self == [UNIVERSE station] &&
        [other bounty] > 50)  // do not grant docking clearance to fugitives
    {
        [self sendExpandedMessage:@ "[station-docking-clearance-H-clearance-refused]" toShip:other];
        if ([other isPlayer]) [player setDockingClearanceStatus:DOCKING_CLEARANCE_STATUS_NONE];
    ...
    } else if (![self hasClearDock])  // skip check if at least one dock clear
    {
        ...
        // the player
        if (result == nil &&
            (([self currentlyInDockingQueues] && last_launch_time < timeNow) ||
             (![other isPlayer] && [player getDockingClearanceStatus] == DOCKING_CLEARANCE_STATUS_REQUESTED))) {
            [self sendExpandedMessage:
                      [NSString stringWithFormat:DESC(@ "station-docking-clearance-acknowledged-d-ships-approaching"),
                                                 [self currentlyInDockingQueues] + 1]
                               toShip:other];
        ...
        if (result == nil && [self currentlyInLaunchingQueues]) {
            [self sendExpandedMessage:
                      [NSString stringWithFormat:DESC(@ "station-docking-clearance-acknowledged-d-ships-departing"),
                                                 [self currentlyInLaunchingQueues] + 1]
                               toShip:other];
            // No need to set status to REQUESTED as we've already done that earlier.
        ...
        if (result == nil) {
            ...
            for (subEnum = [self dockSubEntityEnumerator]; (sub = [subEnum nextObject]);) {
                ...
                if ([docking isEqualToString:@ "DOCK_CLOSED"]) {
                    JSContext *context = OOJSAcquireContext();
                    jsval rval = JSVAL_VOID;
                    jsval args[] = {OOJSValueFromNativeObject(context, sub), OOJSValueFromNativeObject(context, other)};
                    JSBool tempreject = NO;
    ...
    if (result == nil) {
        ...
        if ([self hasMultipleDocks] && [other isPlayer]) {
            [self
                sendExpandedMessage:[NSString stringWithFormat:DESC(@ "station-docking-clearance-granted-in-@-until-@"),
                                                               [player_reserved_dock displayName],
                                                               ClockToString(
                                                                   [player clockTime] + DOCKING_CLEARANCE_WINDOW, NO)]
                             toShip:other];
        } else {
            [self sendExpandedMessage:[NSString stringWithFormat:DESC(@ "station-docking-clearance-granted-until-@"),
                                                                 ClockToString(
                                                                     [player clockTime] + DOCKING_CLEARANCE_WINDOW, NO)]
                               toShip:other];
        }
...
- (BOOL)isRotatingStation {
    ...
    return [[shipinfoDictionary objectForKey:@ "roles"] rangeOfString:@ "rotating-station"].location !=
           NSNotFound;  // legacy
}
...
- (BOOL)hasShipyard {
    ...
    if (!determinant) determinant = [shipinfoDictionary objectForKey:@ "hasShipyard"];
...
- (void)generateShipyard:(OOTechLevelID)stationTechLevel {
    ...
    if ([self localShipyard] == nil) {
        [self setLocalShipyard:[UNIVERSE shipsForSaleForSystem:[UNIVERSE currentSystemID]
                                                        withTL:stationTechLevel
...
#define ADD_FLAG_IF_SET(x)      \
    ...
    if ([self isRotatingStation]) {
        [flags addObject:@ "rotatingStation"];
    }
    if (![self dockingCorridorIsEmpty]) {
        [flags addObject:@ "dockingCorridorIsBusy"];
    }
    ...
    if ([_shipsOnHold count] > 0) {
        ...
        unsigned i = 1;
        while ((ship = [onHoldEnum nextObject])) {
            OOLog(@ "dumpState.stationEntity",
                  @ "Nr %i: %@ at distance %g with role: %@",
                  i++,
                  [ship displayName],
                  HPdistance([self position], [ship position]),
                  [ship primaryRole]);
        }
        OOLogOutdent();
    }
