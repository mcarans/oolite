...
- (void)displayFunctionList:(GuiDisplayGen *)gui skip:(NSUInteger)skip;
...
- (void)setGuiToStickMapperScreen:(unsigned)skip resetCurrentRow:(BOOL)resetCurrentRow {
    ...
    for (i = 0; i < stickCount; i++) {
        ...
        [gui setArray:[NSArray arrayWithObjects:stickNameForThisRow,
                                                @ "",  // skip one column
                                                stickNameAdditional,
                                                nil]
...
- (void)stickMapperInputHandler:(GuiDisplayGen *)gui view:(MyOpenGLView *)gameView {
    OOJoystickManager *stickHandler = [OOJoystickManager sharedStickHandler];
    ...
    if (waitingForStickCallback) {
        if ([gameView isDown:27]) {
            ...
            [gui setArray:[NSArray arrayWithObjects:@ "Function setting aborted.", nil] forRow:GUI_ROW_INSTRUCT];
    ...
    if ([gameView isDown:13]) {
        if ([key hasPrefix:@ "More:"]) {
            ...
            if ([[UNIVERSE gui] selectedRow] < 0) [[UNIVERSE gui] setSelectedRow:GUI_ROW_FUNCSTART];
            if (from_function == 0) [[UNIVERSE gui] setSelectedRow:GUI_ROW_FUNCSTART + MAX_ROWS_FUNCTIONS - 1];
            return;
        }

        NSDictionary *entry = [stickFunctions objectAtIndex:selFunctionIdx];
        int hw = [(NSNumber *)[entry objectForKey:KEY_ALLOWABLE] intValue];
        [stickHandler setCallback:@selector(updateFunction:) object:self hardware:hw];
...
- (void)displayFunctionList:(GuiDisplayGen *)gui skip:(NSUInteger)skip {
    ...
    [gui setArray:[NSArray arrayWithObjects:@ "Function", @ "Assigned to", @ "Type", nil] forRow:GUI_ROW_HEADING];
        ...
        for (i = 0; i < (n_functions - skip) && (int)i < n_rows; i++) {
            ...
            if ([entry objectForKey:KEY_HEADER]) {
                ...
                switch (allowable) {
                    case HW_AXIS:
                        allowedThings = @ "Axis";
                        assignment = [self describeStickDict:[assignedAxes objectForKey:axFuncKey]];
                        break;
                    case HW_BUTTON:
                        allowedThings = @ "Button";
                        int bf = [butFuncKey integerValue];
                        if (bf < 10000) {
                            assignment = [self describeStickDict:[assignedButs objectForKey:butFuncKey]];
                        } else {
                            NSString *key = CUSTOMEQUIP_BUTTONACTIVATE;
                            bf -= 10000;
                            if (bf >= 10000) {
                            ...
                            assignment =
                                [self describeStickDict:[[customEquipActivation objectAtIndex:bf] objectForKey:key]];
                        }
                        break;
                    default:
                        ...
                        // axis has priority
                        assignment = [self describeStickDict:[assignedAxes objectForKey:axFuncKey]];
                        if (!assignment) assignment = [self describeStickDict:[assignedButs objectForKey:butFuncKey]];
                }
                ...
                [gui
                    setArray:[NSArray arrayWithObjects:[entry objectForKey:KEY_GUIDESC], assignment, allowedThings, nil]
                      forRow:i + start_row];
...
- (NSString *)describeStickDict:(NSDictionary *)stickDict {
    NSString *desc = nil;
    if (stickDict) {
        int thingNumber = [(NSNumber *)[stickDict objectForKey:STICK_AXBUT] intValue];
        int stickNumber = [(NSNumber *)[stickDict objectForKey:STICK_NUMBER] intValue];
        // Button or axis?
        if ([(NSNumber *)[stickDict objectForKey:STICK_ISAXIS] boolValue]) {
            desc = [NSString stringWithFormat:@ "Stick %d axis %d", stickNumber + 1, thingNumber + 1];
            ...
            desc = [NSString stringWithFormat:@ "Stick %d hat %d %s",
                                              stickNumber + 1,
                                              (thingNumber - MAX_REAL_BUTTONS) / 4 + 1,
                                              dir[thingNumber & 3]];
        } else {
            desc = [NSString stringWithFormat:@ "Stick %d button %d", stickNumber + 1, thingNumber + 1];
        }
    }
    return desc;
}
...
- (NSArray *)stickFunctionList {
    ...
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-roll")
                                     allowable:HW_AXIS
                                        axisfn:AXIS_ROLL
                                         butfn:STICK_NOFUNCTION]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-pitch")
                                     allowable:HW_AXIS
                                        axisfn:AXIS_PITCH
                                         butfn:STICK_NOFUNCTION]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-yaw")
                                     allowable:HW_AXIS
                                        axisfn:AXIS_YAW
                                         butfn:STICK_NOFUNCTION]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-increase-thrust")
                                     allowable:HW_AXIS | HW_BUTTON
                                        axisfn:AXIS_THRUST
                                         butfn:BUTTON_INCTHRUST]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-decrease-thrust")
                                     allowable:HW_AXIS | HW_BUTTON
                                        axisfn:AXIS_THRUST
                                         butfn:BUTTON_DECTHRUST]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-fuel-injection")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_FUELINJECT]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-hyperspeed")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_HYPERSPEED]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-hyperdrive")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_HYPERDRIVE]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-gal-hyperdrive")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_GALACTICDRIVE]];

    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-roll/pitch-precision-toggle")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
    ...
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-compass-mode-next")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_COMPASSMODE]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-compass-mode-prev")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_COMPASSMODE_PREV]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-scanner-zoom")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_SCANNERZOOM]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-scanner-unzoom")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_SCANNERUNZOOM]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-view-forward")
                                     allowable:HW_AXIS | HW_BUTTON
                                        axisfn:AXIS_VIEWY
                                         butfn:BUTTON_VIEWFORWARD]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-view-aft")
                                     allowable:HW_AXIS | HW_BUTTON
                                        axisfn:AXIS_VIEWY
                                         butfn:BUTTON_VIEWAFT]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-view-port")
                                     allowable:HW_AXIS | HW_BUTTON
                                        axisfn:AXIS_VIEWX
                                         butfn:BUTTON_VIEWPORT]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-view-starboard")
                                     allowable:HW_AXIS | HW_BUTTON
                                        axisfn:AXIS_VIEWX
                                         butfn:BUTTON_VIEWSTARBOARD]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-ext-view-cycle")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_EXTVIEWCYCLE]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-toggle-ID")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_ID]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-docking-clearance")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_DOCKINGCLEARANCE]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-dockcpu")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_DOCKCPU]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-dockcpufast")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_DOCKCPUFAST]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-docking-music")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
    ...
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-weapons-online-toggle")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_WEAPONSONLINETOGGLE]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-primary-weapon")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_FIRE]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-secondary-weapon")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_LAUNCHMISSILE]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-arm-secondary")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_ARMMISSILE]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-disarm-secondary")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_UNARM]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-target-nearest-incoming-missile")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_TARGETINCOMINGMISSILE]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-cycle-secondary")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_CYCLEMISSILE]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-next-target")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_NEXTTARGET]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-previous-target")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
    ...
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-ECM")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_ECM]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-jettison")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_JETTISON]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-rotate-cargo")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_ROTATECARGO]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-escape-pod")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_ESCAPE]];
    ...
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-mfd-select-next")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_MFDSELECTNEXT]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-mfd-select-prev")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_MFDSELECTPREV]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-mfd-cycle-next")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_MFDCYCLENEXT]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-mfd-cycle-prev")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_MFDCYCLEPREV]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-prime-equipment")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_PRIMEEQUIPMENT]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-prime-prev-equipment")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_PRIMEEQUIPMENT]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-activate-equipment")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_ACTIVATEEQUIPMENT]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-mode-equipment")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_MODEEQUIPMENT]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-fastactivate-a")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_CLOAK]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-fastactivate-b")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
    ...
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-snapshot")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_SNAPSHOT]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-pause")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_PAUSE]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-toggle-hud")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_TOGGLEHUD]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-comms-log")
                                     allowable:HW_BUTTON
                                        axisfn:STICK_NOFUNCTION
                                         butfn:BUTTON_COMMSLOG]];
#if OO_FOV_INFLIGHT_CONTROL_ENABLED
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-increase-field-of-view")
                                     allowable:HW_AXIS | HW_BUTTON
                                        axisfn:AXIS_FIELD_OF_VIEW
                                         butfn:BUTTON_INC_FIELD_OF_VIEW]];
    [funcList addObject:[self makeStickGuiDict:DESC(@ "stickmapper-decrease-field-of-view")
                                     allowable:HW_AXIS | HW_BUTTON
                                        axisfn:AXIS_FIELD_OF_VIEW
    ...
    if ([customEquipActivation count] > 0) {
        ...
        for (i = 0; i < [customEquipActivation count]; i++) {
            [funcList
                addObject:[self makeStickGuiDict:[NSString stringWithFormat:@ "Activate '%@'",
                                       ...
                                       allowable:HW_BUTTON
                                          axisfn:STICK_NOFUNCTION
                                           butfn:(i + 10000)]];
            [funcList
                addObject:[self makeStickGuiDict:[NSString stringWithFormat:@ "Mode '%@'",
                                                                            [[customEquipActivation objectAtIndex:i]
                                                                                oo_stringForKey:CUSTOMEQUIP_EQUIPNAME]]
                                       allowable:HW_BUTTON
                                          axisfn:STICK_NOFUNCTION
...
- (NSDictionary *)makeStickGuiDict:(NSString *)what allowable:(int)allowable axisfn:(int)axisfn butfn:(int)butfn {
    ...
    [guiDict setObject:[NSNumber numberWithInt:allowable] forKey:KEY_ALLOWABLE];
    if (axisfn >= 0) [guiDict setObject:[NSNumber numberWithInt:axisfn] forKey:KEY_AXISFN];
    if (butfn >= 0) [guiDict setObject:[NSNumber numberWithInt:butfn] forKey:KEY_BUTTONFN];
    return guiDict;
}
