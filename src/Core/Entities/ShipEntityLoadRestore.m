...
- (NSDictionary *)savedShipDictionaryWithContext:(NSMutableDictionary *)context {
    ...
    */
    ...
    // FIXME: AI.
    // Eric: I think storing the AI name should be enough. On entering a wormhole, the stack is cleared so there are no
    // preserved AI states. Also the AI restarts itself with the GLOBAL state, so no need to store any old state.
...
- (void)simplifyShipdata:(NSMutableDictionary *)data andGetDeletes:(NSArray **)deletes {
    ...
    *deletes = nil;
    ...
    NSMutableDictionary *referenceData = [NSMutableDictionary
        dictionaryWithDictionary:[[OOShipRegistry sharedRegistry] shipInfoForKey:[self shipDataKey]]];
    ...
    // after rev3010 this loop was using cycles without doing anything - commenting this whole loop out for now. -- kaks
    // 20100207
    /*
...
static void StripIgnoredKeys(NSMutableDictionary *dict) {
    ...
    if (ignoredKeys == nil)
        ignoredKeys = [[NSArray alloc] initWithObjects:@ "ai_type",
                                                       @ "has_ecm",
                                                       @ "has_scoop",
                                                       @ "has_escape_pod",
                                                       @ "has_energy_bomb",
