...
OOINLINE NSString *CurrentScriptDesc(void) { return CurrentScriptNameOr(@ "<anonymous actions>"); }
...
static void PerformActionStatment(NSArray *statement, Entity *target) {
    ...
    */
    ...
    if (argumentString != nil) {
        ...
        expandedString = OOExpandDescriptionString(
            OOStringExpanderDefaultRandomSeed(), argumentString, nil, locals, nil, kOOExpandNoOptions);
...
OOINLINE OOEntityStatus RecursiveRemapStatus(OOEntityStatus status) {
    ...
    if (status == STATUS_DOCKING || status == STATUS_LAUNCHING || status == STATUS_ENTERING_WITCHSPACE ||
...
- (void)runUnsanitizedScriptActions:(NSArray *)actions
...
#define DOUBLEVAL(x) ((x != nil) ? [x doubleValue] : 0.0)
        ...
        if (comparator == COMPARISON_ONEOF) {
            ...
            switch (comparator) {
                ...
                case COMPARISON_ONEOF:
                    ...
                    OOLog(@ "script.error.unexpectedOperator",
                          @ "***** SCRIPT ERROR: in %@, operator %@ is not valid for numbers, evaluating to false.",
                          CurrentScriptDesc(),
        ...
        switch (comparator) {
            ...
            case COMPARISON_ONEOF:
                ...
                OOLog(@ "script.error.unexpectedOperator",
                      @ "***** SCRIPT ERROR: in %@, operator %@ is not valid for booleans, evaluating to false.",
                      CurrentScriptDesc(),
                      OOComparisonTypeToString(comparator));
                return NO;
        }
    ...
    OOLog(@ "script.error.fallthrough",
          @ "***** SCRIPT ERROR: in %@, unhandled condition '%@' (%@). %@",
          CurrentScriptDesc(),
          [scriptCondition objectAtIndex:1],
          scriptCondition,
...
- (void)setMissionVariable:(NSString *)value forKey:(NSString *)key {
    if (key != nil) {
        if (value != nil)
            [mission_variables setObject:value forKey:key];
        else
...
- (NSArray *)missionsList {
    ...
    if ([passengerManifest count] > 0) {
        [result2 addObject:[[NSArray arrayWithObject:DESC(@ "manifest-passengers")]
                               arrayByAddingObjectsFromArray:passengerManifest]];
    }

    if ([parcelManifest count] > 0) {
        [result2 addObject:[[NSArray arrayWithObject:DESC(@ "manifest-parcels")]
                               arrayByAddingObjectsFromArray:parcelManifest]];
    }

    if ([contractManifest count] > 0) {
        [result2 addObject:[[NSArray arrayWithObject:DESC(@ "manifest-contracts")]
                               arrayByAddingObjectsFromArray:contractManifest]];
    }
            ...
            if ([vars isKindOfClass:[NSString class]]) {
                ...
                foreach (element, result2) {
                    if ([[element oo_stringAtIndex:0] isEqualToString:[(NSArray *)vars oo_stringAtIndex:0]]) {
                        ...
                        [result2 addObject:[element arrayByAddingObjectsFromArray:[(NSArray *)vars
                                                                                      subarrayWithRange:notTheHeader]]];
                        found = YES;
                        break;
                    }
                }
...
- (NSString *)replaceVariablesInString:(NSString *)args {
    ...
    for (i = 0; i < [tokens count]; i++) {
        ...
        if ([valueString hasPrefix:@ "mission_"] && [mission_variables objectForKey:valueString]) {
            [resultString replaceOccurrencesOfString:valueString
                                          withString:[mission_variables objectForKey:valueString]
                                             options:NSLiteralSearch
                                               range:NSMakeRange(0, [resultString length])];
        } else if ([locals objectForKey:valueString]) {
            [resultString replaceOccurrencesOfString:valueString
                                          withString:[locals objectForKey:valueString]
                                             options:NSLiteralSearch
                                               range:NSMakeRange(0, [resultString length])];
        } else if (([valueString hasSuffix:@ "_number"]) || ([valueString hasSuffix:@ "_bool"]) ||
                   ([valueString hasSuffix:@ "_string"])) {
            SEL valueselector = NSSelectorFromString(valueString);
            if ([self respondsToSelector:valueselector]) {
                [resultString
                    replaceOccurrencesOfString:valueString
                                    withString:[NSString stringWithFormat:@ "%@", [self performSelector:valueselector]]
                                       options:NSLiteralSearch
                                         range:NSMakeRange(0, [resultString length])];
            }
...
- (void)setMissionDescription:(NSString *)textKey forMission:(NSString *)key {
    ...
    if (!text) {
        OOLogERR(kOOLogScriptMissionDescNoText,
                 @ "in %@, no mission text set for key '%@' [UNIVERSE missiontext] is:\n%@ ",
                 CurrentScriptDesc(),
                 textKey,
...
- (NSString *)dockedStationName_string  // returns 'NONE' if the player isn't docked, [station name] if it is, 'UNKNOWN'
                                        // otherwise (?)
{
    NSString *result = nil;
...
- (void)awardCredits:(NSString *)valueString {
    ...
    */
    ...
    award *= 10;
    if (award < 0 && credits < (OOCreditsQuantity)-award)
        credits = 0;
    else
        credits += award;
}
...
- (void)awardEquipment:(NSString *)equipString  // eg. EQ_NAVAL_ENERGY_UNIT
{
    ...
    if ([eqType isMissileOrMine]) {
        ...
        OOLog(kOOLogSyntaxAwardEquipment,
              @ "***** SCRIPT ERROR: in %@, CANNOT award undamaged weapon:'%@'. Damaged weapons can be awarded instead.",
              CurrentScriptDesc(),
              equipString);
    } else if ([equipString hasSuffix:@ "_DAMAGED"] &&
               [self hasEquipmentItem:[equipString substringToIndex:[equipString length] - [@ "_DAMAGED" length]]]) {
        OOLog(kOOLogSyntaxAwardEquipment,
              @ "***** SCRIPT ERROR: in %@, CANNOT award damaged equipment:'%@'. Undamaged version already equipped.",
              CurrentScriptDesc(),
              equipString);
...
- (void)removeEquipment:(NSString *)equipKey  // eg. EQ_NAVAL_ENERGY_UNIT
{
    ...
    if ([equipKey isEqualToString:@ "EQ_CARGO_BAY"] && [self hasEquipmentItem:equipKey] &&
        ...
        OOLog(kOOLogSyntaxRemoveEquipment,
              @ "***** SCRIPT ERROR: in %@, CANNOT remove cargo bay. Too much cargo.",
              CurrentScriptDesc());
        return;
    }
...
- (void)setPlanetinfo:(NSString *)key_valueString  // uses key=value format
{
    ...
    if ([tokens count] != 2) {
        OOLog(kOOLogSyntaxSetPlanetInfo,
              @ "***** SCRIPT ERROR: in %@, CANNOT setPlanetinfo: '%@' (bad parameter count)",
              CurrentScriptDesc(),
              key_valueString);
        return;
    }
...
- (void)setSpecificPlanetInfo:(NSString *)key_valueString  // uses galaxy#=planet#=key=value
{
    ...
    if ([tokens count] != 4) {
        OOLog(kOOLogSyntaxSetPlanetInfo,
              @ "***** SCRIPT ERROR: in %@, CANNOT setSpecificPlanetInfo: '%@' (bad parameter count)",
              CurrentScriptDesc(),
              key_valueString);
        return;
    }
    ...
    [UNIVERSE setSystemDataForGalaxy:gnum
                              planet:pnum
                                 key:keyString
                               value:valueString
                        fromManifest:@ ""
...
- (void)awardCargo:(NSString *)amount_typeString {
    ...
    if ([tokens count] != 2) {
        OOLog(kOOLogSyntaxAwardCargo,
              @ "***** SCRIPT ERROR: in %@, CANNOT awardCargo: '%@' (%@)",
              CurrentScriptDesc(),
              amount_typeString,
              @ "bad parameter count");
        return;
...
- (void)removeAllCargo:(BOOL)forceRemoval {
    ...
    if ([self status] != STATUS_DOCKED && !forceRemoval) {
        OOLogWARN(kOOLogRemoveAllCargoNotDocked,
                  @ "%@removeAllCargo only works when docked.",
                  [NSString stringWithFormat:@ " in %@, ", CurrentScriptDesc()]);
        return;
    }
...
- (void)awardFuel:(NSString *)valueString  // add to fuel up to 7.0 LY
{
    ...
    if (delta < 0 && scriptTargetFuelBeforeAward < (unsigned)-delta)
...
- (void)messageShipAIs:(NSString *)roles_message {
    ...
    if ([tokens count] < 2) {
        OOLog(kOOLogSyntaxMessageShipAIs,
              @ "***** SCRIPT ERROR: in %@, CANNOT messageShipAIs: '%@' (bad parameter count)",
              CurrentScriptDesc(),
              roles_message);
        return;
    }
...
- (void)addShips:(NSString *)roles_number {
    ...
    if ([tokens count] != 2) {
        OOLog(kOOLogSyntaxAddShips,
              @ "***** SCRIPT ERROR: in %@, CANNOT addShips: '%@' (expected <role> <count>)",
              CurrentScriptDesc(),
              roles_number);
        return;
    }
    ...
    if (number < 0) {
        OOLog(kOOLogSyntaxAddShips,
              @ "***** SCRIPT ERROR: in %@, can't add %i ships -- that's less than zero, y'know..",
              CurrentScriptDesc(),
              number);
        return;
    }
    ...
    while (number--) [UNIVERSE witchspaceShipWithPrimaryRole:roleString];
}
...
- (void)addSystemShips:(NSString *)roles_number_position {
    ...
    if ([tokens count] != 3) {
        OOLog(kOOLogSyntaxAddShips,
              @ "***** SCRIPT ERROR: in %@, CANNOT addSystemShips: '%@' (expected <role> <count> <position>)",
              CurrentScriptDesc(),
              roles_number_position);
        return;
    }
    ...
    OOLog(kOOLogNoteAddShips,
          @ "DEBUG: Going to add %d ships with role '%@' at a point %.3f along route1",
          number,
          roleString,
          posn);

    while (number--) [UNIVERSE addShipWithRole:roleString nearRouteOneAt:posn];
}
...
- (void)addShipsAt:(NSString *)roles_number_system_x_y_z {
    ...
    if ([tokens count] != 6) {
        OOLog(
            ...
            @ "***** SCRIPT ERROR: in %@, CANNOT addShipsAt: '%@' (expected <role> <count> <coordinate-system> <x> <y> <z>)",
            CurrentScriptDesc(),
            roles_number_system_x_y_z);
        return;
    }
    ...
    if (number < 1) {
        OOLog(kOOLogSyntaxAddShips,
              @ "----- WARNING in %@  Tried to add %i ships -- no ship added.",
              CurrentScriptDesc(),
              number);
        return;
    }
    ...
    OOLog(kOOLogNoteAddShips,
          @ "DEBUG: Going to add %d ship(s) with role '%@' at point (%.3f, %.3f, %.3f) using system %@",
          number,
          roleString,
          posn.x,
          posn.y,
    ...
    if (![UNIVERSE addShips:number withRole:roleString nearPosition:posn withCoordinateSystem:systemString]) {
        OOLog(kOOLogScriptAddShipsFailed,
...
- (void)addShipsAtPrecisely:(NSString *)roles_number_system_x_y_z {
    ...
    if ([tokens count] != 6) {
        OOLog(
            kOOLogSyntaxAddShips,
            @ "***** SCRIPT ERROR: in %@,* CANNOT addShipsAtPrecisely: '%@' (expected <role> <count> <coordinate-system> <x> <y> <z>)",
            CurrentScriptDesc(),
            roles_number_system_x_y_z);
        return;
    }
    ...
    if (number < 1) {
        OOLog(kOOLogSyntaxAddShips,
              @ "----- WARNING: in %@, Can't add %i ships -- no ship added.",
              CurrentScriptDesc(),
              number);
        return;
    }
    ...
    OOLog(kOOLogNoteAddShips,
          @ "DEBUG: Going to add %d ship(s) with role '%@' precisely at point (%.3f, %.3f, %.3f) using system %@",
          number,
          roleString,
          posn.x,
          posn.y,
...
- (void)addShipsWithinRadius:(NSString *)roles_number_system_x_y_z_r {
    ...
    if ([tokens count] != 7) {
        OOLog(
            kOOLogSyntaxAddShips,
            @ "***** SCRIPT ERROR: in %@, CANNOT 'addShipsWithinRadius: %@' (expected <role> <count> <coordinate-system> <x> <y> <z> <radius>))",
            CurrentScriptDesc(),
            roles_number_system_x_y_z_r);
        return;
    }
    ...
    OOLog(
        kOOLogNoteAddShips,
        @ "DEBUG: Going to add %d ship(s) with role '%@' within %.2f radius about point (%.3f, %.3f, %.3f) using system %@",
        number,
        roleString,
        r,
        x,
    ...
    if (![UNIVERSE addShips:number
                        withRole:roleString
                    nearPosition:posn
            withCoordinateSystem:systemString
                    withinRadius:r]) {
        OOLog(kOOLogScriptAddShipsFailed,
              @ "***** SCRIPT ERROR :in %@, %@ could not add %u ships with role \"%@\"",
              CurrentScriptDesc(),
...
- (void)spawnShip:(NSString *)ship_key {
    if ([UNIVERSE spawnShip:ship_key]) {
        ...
        OOLog(kOOLogScriptAddShipsFailed,
              @ "***** SCRIPT ERROR: in %@, could not spawn ship with shipdata key '%@'.",
              CurrentScriptDesc(),
              ship_key);
    }
}
...
- (void)set:(NSString *)missionvariable_value {
    ...
    if ([tokens count] < 2) {
        OOLog(
            kOOLogSyntaxSet,
            @ "***** SCRIPT ERROR: in %@, CANNOT SET '%@' (expected mission_variable or local_variable followed by value expression)",
            CurrentScriptDesc(),
            missionvariable_value);
        return;
    }
    ...
    if (!hasMissionPrefix && !hasLocalPrefix) {
        OOLog(kOOLogSyntaxSet,
              @ "***** SCRIPT ERROR: in %@, IDENTIFIER '%@' DOES NOT BEGIN WITH 'mission_' or 'local_'",
              CurrentScriptDesc(),
              missionVariableString);
        return;
    }
...
- (void)reset:(NSString *)missionvariable {
    NSString *missionVariableString =
        [missionvariable stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    BOOL hasMissionPrefix, hasLocalPrefix;
...
- (void)increment:(NSString *)missionVariableString {
    ...
    if (hasMissionPrefix) {
        ...
        value++;
        [self setLocalVariable:[NSString stringWithFormat:@ "%d", value]
                        forKey:missionVariableString
                    andMission:sCurrentMissionKey];
    } else {
        OOLog(kOOLogSyntaxIncrement,
              @ "***** SCRIPT ERROR: in %@, IDENTIFIER '%@' DOES NOT BEGIN WITH 'mission_' or 'local_'",
              CurrentScriptDesc(),
              missionVariableString);
    }
...
- (void)add:(NSString *)missionVariableString_value {
    ...
    if ([tokens count] < 2) {
        OOLog(kOOLogSyntaxAdd,
              @ "***** SCRIPT ERROR: in %@, CANNOT ADD: '%@'",
              CurrentScriptDesc(),
              missionVariableString_value);
        return;
    }
    ...
    if (hasMissionPrefix) {
        ...
        value += [valueString doubleValue];
        [self setLocalVariable:[NSString stringWithFormat:@ "%f", value]
                        forKey:missionVariableString
                    andMission:sCurrentMissionKey];
    } else {
        OOLog(
            kOOLogSyntaxAdd,
            @ "***** SCRIPT ERROR: in %@, CANNOT ADD: '%@' -- IDENTIFIER '%@' DOES NOT BEGIN WITH 'mission_' or 'local_'",
            CurrentScriptDesc(),
            missionVariableString_value,
            missionVariableString_value);
    }
...
- (void)subtract:(NSString *)missionVariableString_value {
    ...
    if ([tokens count] < 2) {
        OOLog(kOOLogSyntaxSubtract,
              @ "***** SCRIPT ERROR: in %@, CANNOT SUBTRACT: '%@'",
              CurrentScriptDesc(),
              missionVariableString_value);
        return;
    }
    ...
    if (hasMissionPrefix) {
        ...
        OOLog(
            ...
            @ "***** SCRIPT ERROR: in %@, CANNOT SUBTRACT: '%@' -- IDENTIFIER '%@' DOES NOT BEGIN WITH 'mission_' or 'local_'",
            CurrentScriptDesc(),
            missionVariableString_value,
            missionVariableString_value);
    }
...
- (void)setMissionChoicesDictionary:(NSDictionary *)choicesDict {
    ...
    for (i = 0; i < [choiceKeys count]; i++) {
        if (![[choiceKeys objectAtIndex:i] isKindOfClass:[NSString class]]) {
            OOLog(@ "test.script.error",
                  @ "Choices list in mission screen has non-string value %@",
                  [choiceKeys objectAtIndex:i]);
            keysOK = false;
        }
    }
    if (keysOK) {
    ...
    if ((end_row + 1) < [choiceKeys count]) {
        OOLogERR(
            kOOLogException,
            @ "in mission.runScreen choices: number of choices defined (%llu) is greater than available lines (%llu). Check HUD settings for allowBigGui.",
            [choiceKeys count],
            (end_row + 1));
        keysCount = end_row + 1;
    }
...
- (void)launchFromStation {
    ...
    if ([self status] == STATUS_DOCKING)
        [self setStatus:STATUS_DOCKED];  // needed here to prevent the normal update from continuing with docking.
...
- (void)blowUpStation {
    ...
    if (mainStation != nil) {
        ...
        [mainStation takeEnergyDamage:500000000.0
                                 from:nil
                            becauseOf:nil
                     weaponIdentifier:@ ""];  // 500 million should do it!
    }
}
...
- (void)sendAllShipsAway {
    ...
    for (i = 1; i < ent_count; i++) {
        ...
        if ([e1 isShip]) {
            ...
            if (((e_class == CLASS_NEUTRAL) || (e_class == CLASS_POLICE) || (e_class == CLASS_MILITARY) ||
                 (e_class == CLASS_THARGOID)) &&
                ...
                [se1 hasHyperspaceMotor])  // exclude non jumping ships. Escorts will still be able to follow a mother.
            {
                ...
                [se1 setPrimaryRole:@ "oolite-none"];  // prevents new ship from appearing at witchpoint when this one
                                                      // leaves!
            }
        }
    }
...
- (OOPlanetEntity *)addPlanet:(NSString *)planetKey {
    ...
    if (!UNIVERSE) return nil;
    NSDictionary *dict = [[UNIVERSE systemManager] getPropertiesForSystemKey:planetKey];
    if (!dict) {
        OOLog(@ "script.error.addPlanet.keyNotFound",
              @ "***** ERROR: could not find an entry in planetinfo.plist for '%@'",
              planetKey);
        return nil;
    }
    ...
    OOPlanetEntity *planet =
        [[[OOPlanetEntity alloc] initFromDictionary:dict
                                     withAtmosphere:YES
                                            andSeed:[[UNIVERSE systemManager] getRandomSeedForCurrentSystem]
                                          forSystem:system_id] autorelease];
    ...
    if (![dict objectForKey:@ "position"]) {
        OOLog(@ "script.error.addPlanet.noPosition",
              @ "***** ERROR: you must specify a position for scripted planet '%@' before it can be created",
              planetKey);
        return nil;
    }
    ...
    if (posn.x || posn.y || posn.z) {
        OOLog(kOOLogDebugAddPlanet,
              @ "planet position (%.2f %.2f %.2f) derived from %@",
              posn.x,
              posn.y,
              posn.z,
              positionString);
...
- (OOPlanetEntity *)addMoon:(NSString *)moonKey {
    ...
    if (![dict objectForKey:@ "position"]) {
        OOLog(@ "script.error.addPlanet.noPosition",
              @ "***** ERROR: you must specify a position for scripted moon '%@' before it can be created",
              moonKey);
        return nil;
    }
    ...
    if (posn.x || posn.y || posn.z) {
        OOLog(kOOLogDebugAddPlanet,
              @ "moon position (%.2f %.2f %.2f) derived from %@",
              posn.x,
              posn.y,
              posn.z,
              positionString);
...
- (void)refreshMissionScreenTextEntry {
    ...
    [gui setText:[NSString stringWithFormat:DESC(@ "mission-screen-text-prompt-@"), [gameView typedString]]
          forRow:end_row
...
- (void)setGuiToMissionScreenWithCallback:(BOOL)callback {
    ...
    if (gui) gui_screen = GUI_SCREEN_MISSION;
...
- (BOOL)processSceneDictionary:(NSDictionary *)couplet atOffset:(Vector)off {
    ...
    NSArray *actions = nil;
    if ([couplet objectForKey:@ "do"]) actions = [NSArray arrayWithObject:[couplet objectForKey:@ "do"]];
    NSArray *else_actions = nil;
    if ([couplet objectForKey:@ "else"]) else_actions = [NSArray arrayWithObject:[couplet objectForKey:@ "else"]];
    BOOL success = YES;
    if (conditions == nil) {
        OOLog(@ "script.scene.couplet.badConditions",
    ...
    if ((success) && (actions) && [actions count]) [self addScene:actions atOffset:off];

    // perform unsuccessful actions
    if ((!success) && (else_actions) && [else_actions count]) [self addScene:else_actions atOffset:off];
...
- (BOOL)processSceneString:(NSString *)item atOffset:(Vector)off {
    ...
    if ([i_key isEqualToString:@ "scene"]) {
        ...
        ScanVectorFromString([[i_info subarrayWithRange:NSMakeRange(2, 3)] componentsJoinedByString:@ " "],
                             &scene_offset);
        scene_offset.x += off.x;
        scene_offset.y += off.y;
        scene_offset.z += off.z;
    ...
    if ([i_key isEqualToString:@ "ship"] || [i_key isEqualToString:@ "model"] || [i_key isEqualToString:@ "role"]) {
        ...
        ScanVectorAndQuaternionFromString(
            [[i_info subarrayWithRange:NSMakeRange(2, 7)] componentsJoinedByString:@ " "], &model_p0, &model_q);
    ...
    if ([i_key isEqualToString:@ "player"]) {
        ...
        ScanVectorAndQuaternionFromString(
            [[i_info subarrayWithRange:NSMakeRange(1, 7)] componentsJoinedByString:@ " "], &model_p0, &model_q);

        Vector model_offset =
            positionOffsetForShipInRotationToAlignment(doppelganger, model_q, (NSString *)[i_info objectAtIndex:8]);
    ...
    if ([i_key isEqualToString:@ "local-planet"] || [i_key isEqualToString:@ "target-planet"]) {
        ...
        if (info_system_id & 8) {
            _sysInfoLight =
                (info_system_id & 2) ? (Vector){-10000.0, 4000.0, -10000.0} : (Vector){-12000.0, -5000.0, -10000.0};
        } else {
            _sysInfoLight =
                (info_system_id & 2) ? (Vector){6000.0, -5000.0, -10000.0} : (Vector){6000.0, 4000.0, -10000.0};
        }
        ...
        NSMutableDictionary *planetInfo =
            [NSMutableDictionary dictionaryWithDictionary:[UNIVERSE generateSystemData:target_system_seed]];
        ...
        if ([i_key isEqualToString:@ "local-planet"] && [UNIVERSE sun]) {
            ...
            if (texture != nil) {
                ...
                //[planetInfo oo_setQuaternion:[mainPlanet orientation] forKey:@"orientation"]; // the orientation is
                // overwritten later on, without regard for the real planet's orientation.
            }
        }

        doppelganger = [[OOPlanetEntity alloc] initFromDictionary:planetInfo
                                                   withAtmosphere:YES
                                                          andSeed:target_system_seed];
        ...
        // miniature radii are roughly between 60 and 120. Place miniatures with a radius bigger than 60 a bit futher
        // away.
         ...
         *     - necessary to duplicate the rotation already performed in PlanetEntity.m since we reset the orientation
         * above. */
...
- (BOOL)addEqScriptForKey:(NSString *)eq_key {
    ...
    OOLog(@ "player.equipmentScript",
          @ "Added equipment %@, with the following script property: '%@'.",
          eq_key,
          scriptName);
...
- (void)removeEqScriptForKey:(NSString *)eq_key {
    ...
    for (i = 0; i < count; i++) {
        ...
        if ([key isEqualToString:eq_key]) {
            ...
            if (i == primedEquipment)
                primedEquipment = count;  // primed-none
            else if (i < primedEquipment)
                primedEquipment--;                            // track the primed equipment
            if (count == primedEquipment) primedEquipment--;  // the array has shrunk by one!

            OOLog(@ "player.equipmentScript",
                  @ "Removed equipment %@, with the following script property: '%@'.",
                  eq_key,
...
- (void)setGalacticHyperspaceBehaviourTo:(NSString *)galacticHyperspaceBehaviourString {
    OOGalacticHyperspaceBehaviour ghBehaviour =
        OOGalacticHyperspaceBehaviourFromString(galacticHyperspaceBehaviourString);
    if (ghBehaviour == GALACTIC_HYPERSPACE_BEHAVIOUR_UNKNOWN) {
        OOLog(@ "player.setGalacticHyperspaceBehaviour.invalidInput",
              @ "setGalacticHyperspaceBehaviourTo: called with unknown behaviour %@.",
