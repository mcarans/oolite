...
- (void)setMissionChoice:(NSString *)newChoice keyPress:(NSString *)keyPress withEvent:(BOOL)withEvent {
    BOOL equal =
        [newChoice isEqualToString:missionChoice] || (newChoice == missionChoice);  // Catch both being nil as well
    if (!equal) {
        if (newChoice == nil) {
...
- (NSDictionary *)passengerContractMarker:(OOSystemID)system {
    return [[[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:system],
                                                        @ "system",
                                                        MISSION_DEST_LEGACY,
                                                        @ "name",
                                                        @ "orangeColor",
...
- (NSDictionary *)validatedMarker:(NSDictionary *)marker {
    ...
    return
        [[[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:dest],
                                                     @ "system",
                                                     group,
                                                     @ "name",
                                                     [marker oo_stringForKey:@ "markerColor" defaultValue:@ "redColor"],
                                                     @ "markerColor",
                                                     [marker oo_stringForKey:@ "markerShape" defaultValue:@ "MARKER_X"],
                                                     @ "markerShape",
                                                     [NSNumber numberWithFloat:[marker oo_floatForKey:@ "markerScale"
                                                                                         defaultValue:1.0]],
                                                     @ "markerScale",
                                                     nil] retain] autorelease];
}
...
- (NSString *)getKeyBindingDescription:(NSArray *)keyList {
    ...
    for (i = 0; i < [keyList count]; i++) {
        ...
        // 0 = key not set
        if (k_int != 0) {
            if ([[def objectForKey:@ "mod2"] boolValue] == YES)
                final = [NSMutableString stringWithFormat:@ "%@%@+", final, keyMod2Text];
            if ([[def objectForKey:@ "mod1"] boolValue] == YES)
                final = [NSMutableString stringWithFormat:@ "%@%@+", final, keyMod1Text];
            if ([[def objectForKey:@ "shift"] boolValue] == YES)
