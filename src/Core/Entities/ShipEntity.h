...
@class OOColor, StationEntity, WormholeEntity, AI, Octree, OOMesh, OOScript, OOJSScript, OORoleSet, OOShipGroup,
    OOEquipmentType, OOWeakSet, OOExhaustPlumeEntity, OOFlasherEntity;
...
@interface ShipEntity : OOEntityWithDrawable <OOSubEntity, OOBeaconEntity> {
   ...
   @protected
    ...
    GLfloat max_flight_pitch;  // maximum pitch rate   (1.0 for player)	(1.5 for fast raider) also radians/sec for (*
                               // turrets *)
    ...
    unsigned military_jammer_active : 1,  // military_jammer
        ...
        // scripting
        scripted_misjump : 1, haveExecutedSpawnAction : 1, haveStartedJSAI : 1, noRocks : 1, _lightsActive : 1;
    ...
    OOCargoQuantity
        equipment_weight;      // amount of equipment using cargo space (excluding passenger_berth & extra_cargo_bay)
    ...
    GLfloat forward_weapon_temp, aft_weapon_temp, port_weapon_temp,
        starboard_weapon_temp;  // current weapon temperatures
    ...
    NSArray *forwardWeaponOffset, *aftWeaponOffset, *portWeaponOffset, *starboardWeaponOffset;
    ...
    // Stuff for the target tracking curve.  The ship records the position of the target every reactionTime/2 seconds,
    // then fits a curve to the last three recorded positions.  Instead of tracking the primary target's actual position
    // it uses the curve to calculate the target's position. This introduces a small amount of lag to the target
    // tracking making the NPC more human.
    float reactionTime;
...
- (void)
    clearSubEntities;  // Releases and clears subentity array, after making sure subentities don't think ship is owner.
...
- (BOOL)hasEquipmentItem:(id)equipmentKeys
          includeWeapons:(BOOL)includeWeapons
            whileLoading:(BOOL)loading;  // This can take a string or an set or array of strings. If a collection,
                                         // returns YES if ship has _any_ of the specified equipment. If includeWeapons
                                         // is NO, missiles and primary weapons are not checked.
...
- (BOOL)hasAllEquipment:(id)equipmentKeys
         includeWeapons:(BOOL)includeWeapons
           whileLoading:
               (BOOL)loading;  // Like hasEquipmentItem:includeWeapons:, but requires _all_ elements in collection.
...
- (BOOL)canAddEquipment:(NSString *)equipmentKey
              inContext:
                  (NSString *)
                      context;  // Test ability to add equipment, taking equipment-specific constriants into account.
- (BOOL)equipmentValidToAdd:(NSString *)equipmentKey
                  inContext:(NSString *)context;  // Actual test if equipment satisfies validation criteria.
...
- (void)setTotalVelocity:
    (Vector)vel;  // Set velocity to vel - thrustVector, effectively setting the instanteneous velocity to vel.
...
// Dispatch shipDied() and possibly shipKilledOther() events. This is only for use by getDestroyedBy:damageType:, but
// needs to be visible to PlayerEntity's version.
...
- (void)overrideScriptInfo:(NSDictionary *)override;  // Add items from override (if not nil) to scriptInfo, replacing
                                                      // in case of duplicates. Used for subentities.
...
OOWeaponType OOWeaponTypeFromEquipmentIdentifierSloppy(
    NSString *string) PURE_FUNC;  // Uses suffix match for backwards compatibility.
