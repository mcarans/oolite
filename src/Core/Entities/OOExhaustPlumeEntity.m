...
- (Frame)frameAtTime:(double)t_frame
           fromFrame:(Frame)frame_zero;  // t_frame is relative to now ie. -0.5 = half a second ago.
...
- (id)initForShip:(ShipEntity *)ship withDefinition:(NSArray *)definition andScale:(float)scaleFactor {
    ...
    if ((self = [super init])) {
        [self setOwner:ship];
        HPVector pos = {[definition oo_floatAtIndex:0] * scaleFactor,
                        [definition oo_floatAtIndex:1] * scaleFactor,
                        [definition oo_floatAtIndex:2] * scaleFactor};
        [self setPosition:pos];
        // scale.z is special and *not* multiplied by scaleFactor
        Vector scale = {[definition oo_floatAtIndex:3] * scaleFactor,
                        [definition oo_floatAtIndex:4] * scaleFactor,
                        [definition oo_floatAtIndex:5]};
        [self setScale:scale];
    }
...
- (void)update:(OOTimeDelta)delta_t {
    ...
    [[ship exhaustEmissiveColor] getRed:&ex_emissive[0]
                                  green:&ex_emissive[1]
                                   blue:&ex_emissive[2]
                                  alpha:&ex_emissive[3]];
    ...
    Frame zero = {.timeframe = [UNIVERSE getTime], .orientation = shipQrotation, .k = [ship forwardVector]};
    int dam = [ship damage];
    ...
    if ((int)(ranrot_rand() % 50) < dam - 50)  // flicker the damaged engines
        red_factor = 0.0;
    if ((int)(ranrot_rand() % 40) < dam - 60) green_factor = 0.0;
    if ((int)(ranrot_rand() % 25) < dam - 75) flare_factor = 0.0;
    ...
    for (i = 0; i < 8; i++) {
        vertex =
            HPvector_add(f01.position,
                         vectorToHPVector(vector_add(
                             b01, vector_add(vector_multiply_scalar(i1, s1[i]), vector_multiply_scalar(j1, c1[i])))));
    ...
    (void)iv;
    (void)ci;  // Suppress Clang static analyzer warnings.
}
...
- (void)drawSubEntityImmediate:(bool)immediate translucent:(bool)translucent {
    ...
    GLfloat texCoords[68] = {
        ...
        0.1, phase + pA[1], 0.1, phase + pA[1], 0.1, phase + pA[1], 0.1, phase + pA[1],
        0.9, phase + pA[1], 0.9, phase + pA[1], 0.9, phase + pA[1], 0.9, phase + pA[1],

        0.1, phase + pA[2], 0.1, phase + pA[2], 0.1, phase + pA[2], 0.1, phase + pA[2],
        0.9, phase + pA[2], 0.9, phase + pA[2], 0.9, phase + pA[2], 0.9, phase + pA[2],

        0.1, phase + pA[3], 0.1, phase + pA[3], 0.1, phase + pA[3], 0.1, phase + pA[3],
        0.9, phase + pA[3], 0.9, phase + pA[3], 0.9, phase + pA[3], 0.9, phase + pA[3],

        0.1, phase + pA[4], 0.1, phase + pA[4], 0.1, phase + pA[4], 0.1, phase + pA[4],
        0.9, phase + pA[4], 0.9, phase + pA[4], 0.9, phase + pA[4], 0.9, phase + pA[4],
        ...
        sPlumeTexture = [[OOTexture textureWithName:@ "oolite-exhaust-blur.png"
                                           inFolder:@ "Textures"
                                            options:kOOTextureMinFilterMipMap | kOOTextureMagFilterLinear |
                                                    kOOTextureAlphaMask | kOOTextureRepeatT | kOOTextureRepeatS
