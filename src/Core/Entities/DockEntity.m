...
- (NSUInteger)pruneAndCountShipsOnApproach {
    ...
    foreach (idObj, [shipsOnApproach allKeys]) {
         ...
         */
        if (ship == nil || HPmagnitude2(HPvector_subtract([ship position], [self absolutePositionForSubentity])) >
                               SCANNER_MAX_RANGE2) {
...
- (void)abortAllDockings {
    ...
    if (isDockingStation && [player status] == STATUS_IN_FLIGHT &&
        ...
        if (HPmagnitude2(HPvector_subtract([player position], [self absolutePositionForSubentity])) >
            2250000)  // within 1500m of the dock
        {
            ...
            playerExtraTime =
                10;  // when very close to the port, give the player a few seconds to react on the abort message.
            int seconds = round(playerExtraTime);
            [station sendExpandedMessage:OOExpandKey(@ "station-docking-clearance-abort-cancelled-in-time", seconds)
                                  toShip:player];
...
- (NSString *)canAcceptShipForDocking:(ShipEntity *)ship {
    ...
    BOOL OK = [[self script] callMethod:OOJSID("acceptDockingRequestFrom")
                              inContext:context
                          withArguments:args
                                  count:1
                                 result:&rval];
...
- (NSDictionary *)dockingInstructionsForShip:(ShipEntity *)ship {
    ...
    if (![shipsOnApproach objectForKey:shipID]) {
        ...
        if (ship_distance > SCANNER_MAX_RANGE) {
            ...
            return OOMakeDockingInstructions(
                station, [self absolutePositionForSubentity], [ship maxFlightSpeed], 10000, @ "APPROACH", nil, NO, -1);
        }
        ...
        if (ship_distance < 1000.0 + [station collisionRadius] + ship->collision_radius)  // too close - back off
            return OOMakeDockingInstructions(
                station, [self absolutePositionForSubentity], [ship maxFlightSpeed], 5000, @ "BACK_OFF", nil, NO, -1);
        ...
        if (dot < 0)  // approaching from the wrong side of the station - construct a vector to the side of the station.
        {
            ...
            approachVector = HPcross_product(
                launchVector, approachVector);  // vector, 90 degr rotated from launchVector towards target.
            return OOMakeDockingInstructions(
                station,
                OOHPVectorTowards(
                    [self absolutePositionForSubentity], approachVector, [station collisionRadius] + 5000),
                [ship maxFlightSpeed],
                1000,
                @ "APPROACH",
                nil,
    ...
    if (![shipsOnApproach objectForKey:shipID]) {
        ...
        OOLogERR(@ "station.issueDockingInstructions.failed",
    ...
    double max_allowed_range =
        2.0f * rangeAdvised +
        ship->collision_radius;  // maximum distance permitted from control point - twice advised range
    HPVector delta = HPvector_subtract(ship->position, coords);

    if (HPmagnitude2(delta) >
        max_allowed_range * max_allowed_range)  // too far from the coordinates - do not remove them from the stack!
    {
        if ((docking_stage == 1) && (HPmagnitude2(delta) < 1000000.0))  // 1km*1km
            speedAdvised *= 0.5;                                        // half speed

        return OOMakeDockingInstructions(
            station, coords, speedAdvised, rangeAdvised, @ "APPROACH_COORDINATES", nil, NO, docking_stage);
    }
        ...
        [id_lock[docking_stage + 2] weakRefUnderlyingObject] == nil)  // check three stages ahead
    {
        ...
        return OOMakeDockingInstructions(
            station, coords, speedAdvised, rangeAdvised, @ "APPROACH_COORDINATES", nil, match_rotation, docking_stage);
    }
...
- (void)addShipToShipsOnApproach:(ShipEntity *)ship {
    ...
    point1.x +=
        c * upVector.x * corridor_offset[corridor_count - 1] + s * rightVector.x * corridor_offset[corridor_count - 1];
    point1.y +=
        c * upVector.y * corridor_offset[corridor_count - 1] + s * rightVector.y * corridor_offset[corridor_count - 1];
    point1.z +=
        c * upVector.z * corridor_offset[corridor_count - 1] + s * rightVector.z * corridor_offset[corridor_count - 1];
    alt1.x -=
        c * upVector.x * corridor_offset[corridor_count - 1] + s * rightVector.x * corridor_offset[corridor_count - 1];
    alt1.y -=
        c * upVector.y * corridor_offset[corridor_count - 1] + s * rightVector.y * corridor_offset[corridor_count - 1];
    ...
    for (i = corridor_count - 1; i >= 0; i--) {
        ...
        if ((i == corridor_count - 1) && [self countOfShipsInDockingQueue]) {
            ...
            //			OOLog(@"docking.debug",@"Adjusted coordinates by %f x %f x %f",c2 * ssize,s2 *
            // ssize,ssize * ((float)offset_id3 / 4.0));
        }
        ...
        if ([player status] == STATUS_IN_FLIGHT &&
                ...
                2250000)  // within 1500m of the dock
            {
                ...
                int seconds =
                    10;  // when very close to the port, give the player a few seconds to react on the abort message.
                [[self parentEntity]
                    sendExpandedMessage:OOExpandKey(@ "station-docking-clearance-abort-cancelled-in-time", seconds)
                                 toShip:player];
                [player setDockingClearanceStatus:DOCKING_CLEARANCE_STATUS_TIMING_OUT];
            }
        }
...
- (BOOL)shipIsInDockingCorridor:(ShipEntity *)ship {
    ...
    if ([ship isPlayer] && (gDebugFlags & DEBUG_DOCKING)) {
        ...
        OOLog(@ "docking.debug",
              ...
              hh * 2.0,
              dd,
              VectorDescription(arbb.min),
              VectorDescription(arbb.max),
              inLane ? "in lane" : "out of lane",
              laneFlags,
              range);
    }
#endif
    ...
    if ((arbb.max.x < ww * 3.0) && (arbb.min.x > -ww * 3.0) && (arbb.max.y < hh * 3.0) && (arbb.min.y > -hh * 3.0)) {
        if ([station requiresDockingClearance] && [ship isPlayer] && [ship status] != STATUS_LAUNCHING &&
            [ship status] != STATUS_AUTOPILOT_ENGAGED &&
    ...
    if ((arbb.max.x < ww) && (arbb.min.x > -ww) && (arbb.max.y < hh) && (arbb.min.y > -hh)) {
        if ([ship status] != STATUS_LAUNCHING && !allow_docking_thisship) {  // launch-only dock: will collide!
            if (arbb.min.z < dd) {
            ...
            if (arbb.max.z < 0.0) {  // give some warning before exploding...
                return NO;
            }
        }
    ...
    if ((arbb.min.x > -safety * ww) && (arbb.max.x < safety * ww) && (arbb.min.y > -safety * hh) &&
        (arbb.max.y < safety * hh)) {
        if (arbb.min.z < 0.0)  // got our nose inside
        {
            ...
            [ship takeScrapeDamage:5 * [UNIVERSE getTimeDelta] * [ship flightSpeed] from:station];
            [station doScriptEvent:OOJSID("shipCollided")
                      withArgument:ship];  // no COLLISION message to station AI, carriers would move away!
            [ship doScriptEvent:OOJSID("shipCollided")
                   withArgument:station];  // no COLLISION message to ship AI, dockingAI.plist would abort.
...
- (void)launchShip:(ShipEntity *)ship {
    ...
    if ((port_dimensions.x < port_dimensions.y) ^ (bb.max.x - bb.min.x < bb.max.y - bb.min.y)) {
        quaternion_rotate_about_axis(
            &q1, launchVector, M_PI * 0.5);  // to account for the slot being at 90 degrees to vertical
    }
    [ship setNormalOrientation:q1];
    // launch position
    [ship setPosition:launchPos];
    if ([ship pendingEscortCount] > 0)
        [ship setPendingEscortCount:0];  // Make sure no extra escorts are added after launch. (e.g. for miners etc.)
    ...
    last_launch_time = [UNIVERSE getTime];
    double delay =
        (port_corridor + 2 * port_dimensions.z) / launchSpeed;  // pause until 2 portlengths outside of the station.
    ...
    [station doScriptEvent:OOJSID("stationLaunchedShip")
               withArgument:ship
        andReactToAIMessage:@ "STATION_LAUNCHED_SHIP"];
}
...
- (BOOL)allowsLaunchingOf:(ShipEntity *)ship {
    ...
    BOOL OK = [[self script] callMethod:OOJSID("acceptLaunchingRequestFrom")
                              inContext:context
                          withArguments:args
                                  count:1
                                 result:&rval];
...
- (BOOL)dockingCorridorIsEmpty {
    ...
    for (i = 0; i < ent_count; i++) {
        ...
        if ([uni_entities[i] isShip] &&
            ([station alertLevel] < STATION_ALERT_LEVEL_RED || ![uni_entities[i] isPlayer]) &&
            [uni_entities[i] mass] > 1000) {
    ...
    for (i = 0; (i < ship_count) && (isEmpty); i++) {
        ...
        if ((ship != station) && (d2 < 25000000) && ([ship status] != STATUS_DOCKED))  // within 5km
        {
            ...
            if (d2 < 4000000)  // within 2km of the port entrance
            {
                ...
                HPVector r_pos =
                    make_HPvector(ship->position.x - ppos.x, ship->position.y - ppos.y, ship->position.z - ppos.z);
        ...
        if ((ship != station) && (d2 < 25000000) && ([ship status] != STATUS_DOCKED))  // within 5km
        {
            ...
            do {
                ...
                if (d2 < 4000000)  // within 2km of the port entrance
                {
                    ...
                    Vector r_pos =
                        make_vector(ship->position.x - ppos.x, ship->position.y - ppos.y, ship->position.z - ppos.z);
                    if (r_pos.x || r_pos.y || r_pos.z)
...
- (void)setDimensionsAndCorridor:(BOOL)docking:(BOOL)ddc:(BOOL)launching {
    ...
    while ((start.x > stbb.min.x) && (start.x < stbb.max.x) && (start.y > stbb.min.y) && (start.y < stbb.max.y) &&
