...
static void DrawWormholeCorona(
    GLfloat inner_radius, GLfloat outer_radius, int step, GLfloat z_distance, GLfloat *col4v1);
...
- (WormholeEntity *)initWithDict:(NSDictionary *)dict {
    ...
    if ((self = [self init])) {
        ...
        destinationCoords = [[UNIVERSE systemManager] getCoordinatesForSystem:destination
        ...
        while ((currShipDict = [shipDicts nextObject]) != nil) {
            ...
            if (shipInfo != nil) {
                ...
                if (ship != nil) {
                    [shipsInTransit
                        addObject:[NSDictionary dictionaryWithObjectsAndKeys:ship,
                                                                             @ "ship",
                                                                             [currShipDict objectForKey:@ "time_delta"],
                                                                             @ "time",
                                                                             nil]];
                } else {
                    OOLog(@ "wormhole.load.warning",
                          @ "Wormhole ship \"%@\" failed to initialize - missing OXP or old-style saved wormhole data.",
                          [shipInfo oo_stringForKey:@ "ship_key"]);
                }
            }
        }
        [pool release];
...
- (WormholeEntity *)initWormholeTo:(OOSystemID)s fromShip:(ShipEntity *)ship {
    ...
    if ((self = [self init])) {
        ...
        destinationCoords = [[UNIVERSE systemManager] getCoordinatesForSystem:destination
                                                                     inGalaxy:[PLAYER galaxyNumber]];
        distance =
            distanceBetweenPlanetPositions(originCoords.x, originCoords.y, destinationCoords.x, destinationCoords.y);
        distance = fmax(distance, 0.1);
        witch_mass = 200000.0;  // MKW 2010.11.21 - originally the ship's mass was added twice - once here and once in
                                // suckInShip.  Instead, we give each wormhole a minimum mass.
        if ([ship isPlayer]) witch_mass += [ship mass];  // The player ship never gets sucked in, so add its mass here.

        if (sun && ([sun willGoNova] || [sun goneNova]) && [ship mass] > 240000)
            shrink_factor = [ship mass] / 240000;  // don't allow longstanding wormholes in nova systems. (60 sec *
                                                   // WORMHOLE_SHRINK_RATE = 240 000)
        else
            shrink_factor = 1;
...
- (void)setMisjump {
    ...
    if (!_misjump) {
        double distance =
            distanceBetweenPlanetPositions(originCoords.x, originCoords.y, destinationCoords.x, destinationCoords.y);
        double time_adjust =
            distance * distance * (3600 - 2700);  // NB: Time adjustment is calculated using original distance. Formula
                                                  // matches the one in [PlayerEntity witchJumpTo]
...
- (void)setMisjumpWithRange:(GLfloat)range {
    ...
    if (!_misjump) {
        ...
        // formulae for time and distance by mwerle at
        // http://developer.berlios.de/pm/task.php?func=detailtask&project_task_id=4703&group_id=3577&group_project_id=1753
        arrival_time -= time_adjust;
        travel_time -= time_adjust;
...
- (BOOL)suckInShip:(ShipEntity *)ship {
    ...
    /* CIM: removed test. Not valid for wormholes which last longer than their travel time. Most likely for short
     * distances e.g. zero-distance doubles. equal_seeds test above should cover it, with expiry_time test for safety.
     */
    /*	if (now > arrival_time)
                    return NO;	// far end of the wormhole! */
    if (now > expiry_time) return NO;
    ...
    if (d > 0.0f) {
        ...
        if (leader && (leader != ship)) {
            afterburnerFactor =
                [leader hasFuelInjection] && [leader fuel] > MIN_FUEL ? [leader afterburnerFactor] : 1.0;
    ...
    [shipsInTransit
        addObject:[NSDictionary
                      dictionaryWithObjectsAndKeys:ship,
                                                   @ "ship",
                                                   [NSNumber numberWithDouble:now + travel_time - arrival_time],
                                                   @ "time",
                                                   [ship beaconCode],
                                                   @ "shipBeacon",  // in case a beacon code has been set, nil otherwise
                                                   nil]];
...
- (void)disgorgeShips {
    ...
    foreach (shipInfo, shipsInTransit) {
        ...
        if (ship_arrival_time > now) {
            ...
            if (hasExitPosition && (!containsPlayer || useExitXYScatter)) {
                ...
                if (containsPlayer) {  // reset the player's speed to the new speed
            ...
            // - MKW 2011.03.10
            if (!_misjump)
                [ship setBounty:[ship bounty] / 2
                     withReason:kOOLegalStatusReasonNewSystem];  // adjust legal status for new system
            ...
            if (!hasExitPosition) {
                ...
                [ship update:time_passed];     // do this only for one ship or the next ships might appear at very
                                               // different locations.
                position = [ship position];    // e.g. when the player docks first before following, time_passed is
                                               // already > 10 minutes.
            } else if (time_passed >
                       1)  // Only update the ship position if it was some time ago, otherwise we're in 'real time'.
            {
                if (hasShiftedExitPosition) {
                    // only update the time delay to the lead ship. Sign is not correct but updating gives a small
    ...
    if (containsPlayer) {
        ...
        position =
            HPvector_add([PLAYER position], vectorToHPVector(vector_multiply_scalar([PLAYER forwardVector], -500.0f)));
        containsPlayer = NO;
    }
...
- (void)update:(OOTimeDelta)delta_t {
    ...
    if (now > expiry_time) {
        scanClass =
            CLASS_NO_DRAW;  // witch_mass not certain to be limiting factor on extremely short jumps, so make sure now
...
- (void)drawImmediate:(bool)immediate translucent:(bool)translucent {
    ...
    if (cam_zero_distance > no_draw_distance) return;  // TOO FAR AWAY TO SEE

    if (witch_mass <= 0.0) return;

    if (collision_radius <= 0.0) return;

    if ([self scanClass] == CLASS_NO_DRAW) return;
...
static void DrawWormholeCorona(
    ...
    for (i = 0; i < 360; i += step) {
        ...
        glColor4f(col4v1[0] * OO_WORMHOLE_COLOR_BOOST,
                  col4v1[1] * OO_WORMHOLE_COLOR_BOOST,
                  col4v1[2] * OO_WORMHOLE_COLOR_BOOST,
                  col4v1[3] * rv0);
...
- (NSDictionary *)getDict {
    ...
    while ((currShipDict = [ships nextObject]) != nil) {
        ...
        [shipArray addObject:[NSDictionary
                                 dictionaryWithObjectsAndKeys:[NSNumber numberWithDouble:[currShipDict
                                                                                             oo_doubleForKey:@ "time"]],
                                                              @ "time_delta",
                                                              [ship savedShipDictionaryWithContext:context],
                                                              @ "ship_info",
                                                              nil]];
    }
...
- (NSString *)scanInfoString {
    switch (scan_info) {
        case WH_SCANINFO_NONE:
            return @ "WH_SCANINFO_NONE";
        case WH_SCANINFO_SCANNED:
            return @ "WH_SCANINFO_SCANNED";
        case WH_SCANINFO_COLLAPSE_TIME:
            return @ "WH_SCANINFO_COLLAPSE_TIME";
        case WH_SCANINFO_ARRIVAL_TIME:
            return @ "WH_SCANINFO_ARRIVAL_TIME";
        case WH_SCANINFO_DESTINATION:
            return @ "WH_SCANINFO_DESTINATION";
        case WH_SCANINFO_SHIP:
            return @ "WH_SCANINFO_SHIP";
    }
...
- (void)dumpSelfState {
    ...
    for (i = 0; i < [shipsInTransit count]; ++i) {
        ...
        OOLog(@ "dumpState.wormholeEntity.ships",
              @ "Ship %d: %@  mass %.2f  arrival time %@",
              i + 1,
              ship,
              [ship mass],
              ClockToString(ship_arrival_time, false));
    }
}
