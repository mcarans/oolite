...
- (void)initCheckingDictionary {
    ...
    [nav_keys release];
    nav_keys = [[NSArray alloc] initWithObjects:@ "key_roll_left",
                                                @ "key_roll_right",
                                                @ "key_pitch_forward",
                                                @ "key_pitch_back",
                                                @ "key_yaw_left",
    ...
    [camera_keys release];
    camera_keys = [[NSArray alloc] initWithObjects:@ "key_custom_view_zoom_out",
                                                   @ "key_custom_view_zoom_in",
                                                   @ "key_custom_view_roll_left",
                                                   @ "key_custom_view_roll_right",
                                                   @ "key_custom_view_pan_left",
                                                   @ "key_custom_view_pan_right",
                                                   @ "key_custom_view_rotate_up",
                                                   @ "key_custom_view_rotate_down",
                                                   @ "key_custom_view_pan_down",
                                                   @ "key_custom_view_pan_up",
                                                   @ "key_custom_view_rotate_left",
                                                   @ "key_custom_view_rotate_right",
                                                   nil];
}
...
- (void)setGuiToKeyMapperScreen:(unsigned)skip resetCurrentRow:(BOOL)resetCurrentRow {
    ...
    [gui setArray:[NSArray arrayWithObjects:DESC(@ "oolite-keyconfig-keyboard"), [self keyboardDescription:kbd], nil]
...
- (void)keyMapperInputHandler:(GuiDisplayGen *)gui view:(MyOpenGLView *)gameView {
    ...
    if (selectKeyPress) {
        ...
        if (![self entryIsDictCustomEquip:selected_entry]) {
            key_list =
                [[NSMutableArray alloc] initWithArray:(NSArray *)[keyconfig2_settings
                                                          objectForKey:[selected_entry objectForKey:KEY_KC_DEFINITION]]
                                            copyItems:YES];
        } else {
            key_list = [[NSMutableArray alloc]
                initWithArray:[self getCustomEquipArray:[selected_entry oo_stringForKey:KEY_KC_DEFINITION]]];
        }
...
- (BOOL)entryIsCustomEquip:(NSString *)entry {
    BOOL result = NO;
    if ([entry hasPrefix:@ "activate_"] || [entry hasPrefix:@ "mode_"]) result = YES;
    return result;
}
...
- (void)setGuiToKeyConfigScreen:(BOOL)resetSelectedRow {
    ...
    [gui setArray:[NSArray arrayWithObjects:DESC(@ "oolite-keyconfig-update-function"),
                                            [selected_entry objectForKey:KEY_KC_GUIDESC],
                                            nil]
    ...
    if (validate) {
        for (i = 0; i < [keyFunctions count]; i++) {
            if ([[[keyFunctions objectAtIndex:i] objectForKey:KEY_KC_DEFINITION] isEqualToString:validate]) {
                [gui setText:[NSString stringWithFormat:DESC(@ "oolite-keyconfig-update-validation-@"),
                                                        (NSString *)[[keyFunctions objectAtIndex:i]
                                                            objectForKey:KEY_KC_GUIDESC]]
                      forRow:GUI_ROW_KC_VALIDATION
...
- (void)outputKeyDefinition:(NSString *)key
    ...
    [gui setArray:[NSArray arrayWithObjects:(skiprows == 0 ? DESC(@ "oolite-keyconfig-update-key")
                                                           : DESC(@ "oolite-keyconfig-update-alternate")),
                                            key,
                                            nil]
    ...
    if (![nav_keys containsObject:[selected_entry objectForKey:KEY_KC_DEFINITION]]) {
        if (![key isEqualToString:DESC(@ "oolite-keycode-unset")]) {
            [gui setArray:[NSArray arrayWithObjects:DESC(@ "oolite-keyconfig-update-shift"), shift, nil]
            ...
            if (![camera_keys containsObject:[selected_entry objectForKey:KEY_KC_DEFINITION]]) {
                [gui setArray:[NSArray arrayWithObjects:DESC(@ "oolite-keyconfig-update-mod1"), mod1, nil]
                       forRow:GUI_ROW_KC_MOD1 + skiprows];
                [gui setKey:GUI_KEY_OK forRow:GUI_ROW_KC_MOD1 + skiprows];
            } else {
                [gui setArray:[NSArray
                                  arrayWithObjects:DESC(@ "oolite-keyconfig-update-mod1"), DESC(@ "not-applicable"), nil]
                       forRow:GUI_ROW_KC_MOD1 + skiprows];
            }

#if OOLITE_MAC_OS_X
            [gui setArray:[NSArray arrayWithObjects:DESC(@ "oolite-keyconfig-update-mod2-mac"), mod2, nil]
                   forRow:GUI_ROW_KC_MOD2 + skiprows];
#else
            [gui setArray:[NSArray arrayWithObjects:DESC(@ "oolite-keyconfig-update-mod2-pc"), mod2, nil]
                   forRow:GUI_ROW_KC_MOD2 + skiprows];
#endif
...
- (void)setGuiToKeyConfigEntryScreen {
    ...
    if ([key_list count] < (key_index + 1)) {
        ...
        NSMutableDictionary *key1 = [[NSMutableDictionary alloc] initWithObjectsAndKeys:@ "",
                                                                                        @ "key",
    ...
    [gui addLongText:DESC(@ "oolite-keyconfig-update-entry-info")
        startingAtRow:GUI_ROW_KC_ENTRY_INFO
                align:GUI_ALIGN_LEFT];

    [gui setText:[NSString stringWithFormat:DESC(@ "Key: %@"), [gameView typedString]]
          forRow:end_row
           align:GUI_ALIGN_LEFT];
...
- (void)handleKeyConfigEntryKeys:(GuiDisplayGen *)gui view:(MyOpenGLView *)gameView {
    ...
    [gui setText:[NSString stringWithFormat:DESC(@ "Key: %@"), [gameView typedString]] forRow:end_row];
    [gui setColor:[OOColor cyanColor] forRow:end_row];
...
- (void)updateKeyDefinition:(NSString *)keystring index:(NSUInteger)index {
    NSMutableDictionary *key_def =
        [[NSMutableDictionary alloc] initWithDictionary:(NSDictionary *)[key_list objectAtIndex:index] copyItems:YES];
    [key_def setObject:keystring forKey:@ "key"];
    // auto=turn on shift if the entered key was shifted

    if (last_shift && [keystring length] == 1 &&
        ![nav_keys containsObject:[selected_entry objectForKey:KEY_KC_DEFINITION]]) {
...
- (void)setGuiToConfirmClearScreen {
    ...
    [gui addLongText:[NSString stringWithFormat:@ "%@", DESC(@ "oolite-keyconfig-clear-overrides")]
        startingAtRow:GUI_ROW_KC_CONFIRMCLEAR
                align:GUI_ALIGN_LEFT];
...
- (void)handleKeyMapperConfirmClearKeys:(GuiDisplayGen *)gui view:(MyOpenGLView *)gameView {
    ...
    // Translation issue: we can't confidently use raw Y and N ascii as shortcuts. It's better to use the
    // load-previous-commander keys.
    id valueYes = [[[UNIVERSE descriptions] oo_stringForKey:@ "load-previous-commander-yes"
                                               defaultValue:@ "y"] lowercaseString];
    id valueNo = [[[UNIVERSE descriptions] oo_stringForKey:@ "load-previous-commander-no"
                                              defaultValue:@ "n"] lowercaseString];
    ...
    if ((selectKeyPress && ([gui selectedRow] == GUI_ROW_KC_CONFIRMCLEAR_YES)) || [gameView isDown:cYes] ||
        [gameView isDown:cYes - 32]) {
        [self deleteAllKeySettings];
        [gameView suppressKeysUntilKeyUp];
        [self setGuiToKeyMapperScreen:0 resetCurrentRow:YES];
    }

    if ((selectKeyPress && ([gui selectedRow] == GUI_ROW_KC_CONFIRMCLEAR_NO)) || [gameView isDown:27] ||
        [gameView isDown:cNo] || [gameView isDown:cNo - 32]) {
...
- (void)displayKeyFunctionList:(GuiDisplayGen *)gui skip:(NSUInteger)skip {
    ...
    [gui setArray:[NSArray arrayWithObjects:@ "Function", @ "Assigned to", @ "Overrides", nil] forRow:GUI_ROW_KC_HEADING];
    ...
    if (skip >= n_functions) skip = n_functions - 1;
    ...
    if (n_functions > 0) {
        ...
        for (i = 0; i < (n_functions - skip) && (int)i < n_rows; i++) {
            ...
            if ([entry objectForKey:KEY_KC_HEADER]) {
                ...
                if (![self entryIsDictCustomEquip:entry]) {
                    ...
                    override =
                        ([overrides objectForKey:[entry objectForKey:KEY_KC_DEFINITION]]
                             ? @ "Yes"
                             : @ "");  // work out whether this assignment is overriding the setting in keyconfig2.plist
                    validate = [self validateKey:[entry objectForKey:KEY_KC_DEFINITION]
                                       checkKeys:(NSArray *)[keyconfig2_settings
                                                     objectForKey:[entry objectForKey:KEY_KC_DEFINITION]]];
                } else {
                    NSString *custom_keytype =
                    ...
                    assignment = [PLAYER getKeyBindingDescription:[[customEquipActivation objectAtIndex:idx]
                                                                      oo_arrayForKey:custom_keytype]];
                    OOEquipmentType *item =
                        [OOEquipmentType equipmentTypeWithIdentifier:[[customEquipActivation objectAtIndex:idx]
                                                                         oo_stringForKey:CUSTOMEQUIP_EQUIPKEY]];
                    bool result = true;
                    int j, k;
                    ...
                    for (j = 0; j < [defArray count]; j++) {
                        for (k = 0; k < [compArray count]; k++) {
                            if (![self compareKeyEntries:[defArray objectAtIndex:j]
                                                  second:[compArray objectAtIndex:k]]) {
                                result = false;
                                break;
                            }
                        }
                    ...
                    override = (!result ? @ "Yes" : @ "");
                    validate = [self validateKey:[entry objectForKey:KEY_KC_DEFINITION]
                                       checkKeys:(NSArray *)[[customEquipActivation objectAtIndex:idx]
                                                     oo_arrayForKey:custom_keytype]];
                }
                if (assignment == nil) {
                    assignment = @ "   -   ";
                ...
                [gui setArray:[NSArray arrayWithObjects:[entry objectForKey:KEY_KC_GUIDESC], assignment, override, nil]
                       forRow:i + start_row];
        ...
        [gui setSelectableRange:NSMakeRange(GUI_ROW_KC_SELECTKBD,
                                            (i + start_row - GUI_ROW_KC_FUNCSTART) +
                                                (GUI_ROW_KC_FUNCSTART - GUI_ROW_KC_SELECTKBD))];
    }
}
...
- (NSArray *)keyFunctionList {
    ...
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_gui_screen_options")
                                      keyDef:@ "key_gui_screen_options"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_gui_screen_equipship")
                                      keyDef:@ "key_gui_screen_equipship"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_gui_screen_interfaces")
                                      keyDef:@ "key_gui_screen_interfaces"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_gui_screen_status")
                                      keyDef:@ "key_gui_screen_status"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_gui_chart_screens")
                                      keyDef:@ "key_gui_chart_screens"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_gui_system_data")
                                      keyDef:@ "key_gui_system_data"]];
    ...
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_galactic_hyperspace")
                                      keyDef:@ "key_galactic_hyperspace"]];

    [funcList addObject:[self makeKeyGuiDictHeader:DESC(@ "oolite-keydesc-header-navigation")]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_next_compass_mode")
                                      keyDef:@ "key_next_compass_mode"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_prev_compass_mode")
                                      keyDef:@ "key_prev_compass_mode"]];
    ...
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_docking_clearance_request")
    ...
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_weapons_online_toggle")
                                      keyDef:@ "key_weapons_online_toggle"]];
    ...
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_untarget_missile")
                                      keyDef:@ "key_untarget_missile"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_target_incoming_missile")
                                      keyDef:@ "key_target_incoming_missile"]];
    ...
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_previous_target")
                                      keyDef:@ "key_previous_target"]];
    ...
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_launch_escapepod")
                                      keyDef:@ "key_launch_escapepod"]];
    ...
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_cycle_previous_mfd")
                                      keyDef:@ "key_cycle_previous_mfd"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_switch_next_mfd")
                                      keyDef:@ "key_switch_next_mfd"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_switch_previous_mfd")
                                      keyDef:@ "key_switch_previous_mfd"]];

    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_prime_next_equipment")
                                      keyDef:@ "key_prime_next_equipment"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_prime_previous_equipment")
                                      keyDef:@ "key_prime_previous_equipment"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_activate_equipment")
                                      keyDef:@ "key_activate_equipment"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_mode_equipment") keyDef:@ "key_mode_equipment"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_fastactivate_equipment_a")
                                      keyDef:@ "key_fastactivate_equipment_a"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_fastactivate_equipment_b")
                                      keyDef:@ "key_fastactivate_equipment_b"]];

    [funcList addObject:[self makeKeyGuiDictHeader:DESC(@ "oolite-keydesc-header-chart-screen")]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_advanced_nav_array_next")
                                      keyDef:@ "key_advanced_nav_array_next"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_advanced_nav_array_previous")
    ...
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_map_next_system")
                                      keyDef:@ "key_map_next_system"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_map_previous_system")
                                      keyDef:@ "key_map_previous_system"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_chart_highlight")
                                      keyDef:@ "key_chart_highlight"]];
    ...
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_system_next_system")
                                      keyDef:@ "key_system_next_system"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_system_previous_system")
                                      keyDef:@ "key_system_previous_system"]];

    [funcList addObject:[self makeKeyGuiDictHeader:DESC(@ "oolite-keydesc-header-market-screen")]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_market_filter_cycle")
                                      keyDef:@ "key_market_filter_cycle"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_market_sorter_cycle")
                                      keyDef:@ "key_market_sorter_cycle"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_market_buy_one") keyDef:@ "key_market_buy_one"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_market_sell_one")
                                      keyDef:@ "key_market_sell_one"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_market_buy_max") keyDef:@ "key_market_buy_max"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_market_sell_max")
                                      keyDef:@ "key_market_sell_max"]];
    ...
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_mouse_control_roll")
                                      keyDef:@ "key_mouse_control_roll"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_mouse_control_yaw")
                                      keyDef:@ "key_mouse_control_yaw"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_hud_toggle") keyDef:@ "key_hud_toggle"]];
#if OO_FOV_INFLIGHT_CONTROL_ENABLED
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_inc_field_of_view")
                                      keyDef:@ "key_inc_field_of_view"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_dec_field_of_view")
                                      keyDef:@ "key_dec_field_of_view"]];
#endif
    ...
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_custom_view_zoom_in")
                                      keyDef:@ "key_custom_view_zoom_in"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_custom_view_zoom_out")
                                      keyDef:@ "key_custom_view_zoom_out"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_custom_view_roll_left")
                                      keyDef:@ "key_custom_view_roll_left"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_custom_view_roll_right")
                                      keyDef:@ "key_custom_view_roll_right"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_custom_view_pan_left")
                                      keyDef:@ "key_custom_view_pan_left"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_custom_view_pan_right")
                                      keyDef:@ "key_custom_view_pan_right"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_custom_view_pan_up")
                                      keyDef:@ "key_custom_view_pan_up"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_custom_view_pan_down")
                                      keyDef:@ "key_custom_view_pan_down"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_custom_view_rotate_left")
                                      keyDef:@ "key_custom_view_rotate_left"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_custom_view_rotate_right")
                                      keyDef:@ "key_custom_view_rotate_right"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_custom_view_rotate_up")
                                      keyDef:@ "key_custom_view_rotate_up"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_custom_view_rotate_down")
                                      keyDef:@ "key_custom_view_rotate_down"]];

    [funcList addObject:[self makeKeyGuiDictHeader:DESC(@ "oolite-keydesc-header-oxz-manager")]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_oxzmanager_setfilter")
                                      keyDef:@ "key_oxzmanager_setfilter"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_oxzmanager_showinfo")
                                      keyDef:@ "key_oxzmanager_showinfo"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_oxzmanager_extract")
                                      keyDef:@ "key_oxzmanager_extract"]];
    ...
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_gui_arrow_right")
                                      keyDef:@ "key_gui_arrow_right"]];
    ...
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_dump_target_state")
                                      keyDef:@ "key_dump_target_state"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_dump_entity_list")
                                      keyDef:@ "key_dump_entity_list"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_debug_full") keyDef:@ "key_debug_full"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_debug_collision")
                                      keyDef:@ "key_debug_collision"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_debug_console_connect")
                                      keyDef:@ "key_debug_console_connect"]];
    [funcList addObject:[self makeKeyGuiDict:DESC(@ "oolite-keydesc-key_debug_bounding_boxes")
                                      keyDef:@ "key_debug_bounding_boxes"]];
    ...
    if ([customEquipActivation count] > 0) {
        ...
        for (i = 0; i < [customEquipActivation count]; i++) {
            [funcList
                addObject:[self makeKeyGuiDict:[NSString stringWithFormat:@ "Activate '%@'",
                                                                          [[customEquipActivation objectAtIndex:i]
                                                                              oo_stringForKey:CUSTOMEQUIP_EQUIPNAME]]
                                        keyDef:[NSString stringWithFormat:@ "activate_%@",
                                                                          [[customEquipActivation objectAtIndex:i]
                                                                              oo_stringForKey:CUSTOMEQUIP_EQUIPKEY]]]];
            [funcList
                addObject:[self makeKeyGuiDict:[NSString stringWithFormat:@ "Mode '%@'",
                                                                          [[customEquipActivation objectAtIndex:i]
                                                                              oo_stringForKey:CUSTOMEQUIP_EQUIPNAME]]
                                        keyDef:[NSString stringWithFormat:@ "mode_%@",
                                                                          [[customEquipActivation objectAtIndex:i]
                                                                              oo_stringForKey:CUSTOMEQUIP_EQUIPKEY]]]];
        }
    }
...
#endif
    NSDictionary *kmap = [NSDictionary dictionaryWithDictionary:[ResourceManager dictionaryFromFilesNamed:map
                                                                                                 inFolder:@ "Config"
                                                                                                mergeMode:MERGE_BASIC
                                                                                                    cache:NO]];
...
- (NSArray *)keyboardLayoutList {
    ...
    for (i = 0; i < [keys count]; i++) {
        if (![[keys objectAtIndex:i] isEqualToString:@ "default"]) {
            [kbdList addObject:[[NSDictionary alloc]
                                   initWithObjectsAndKeys:[keys objectAtIndex:i],
                                                          @ "key",
                                                          [self keyboardDescription:[keys objectAtIndex:i]],
                                                          @ "description",
                                                          //([[keys objectAtIndex:i] isEqualToString:kbd] ? @"Current" :
                                                          //@""), @"selected",
                                                          nil]];
        } else {
            // key the "default" item separate, so we can add it at the top of the list, rather than getting it sorted
            def = [[NSDictionary alloc]
                initWithObjectsAndKeys:[keys objectAtIndex:i],
                                       @ "key",
                                       [self keyboardDescription:[keys objectAtIndex:i]],
                                       @ "description",
...
- (NSArray *)validateAllKeys {
    ...
    for (i = 0; i < [keyFunctions count]; i++) {
        ...
        validate =
            [self validateKey:[entry objectForKey:KEY_KC_DEFINITION]
                    checkKeys:(NSArray *)[keyconfig2_settings objectForKey:[entry objectForKey:KEY_KC_DEFINITION]]];
        if (validate) {
...
- (NSString *)validateKey:(NSString *)key checkKeys:(NSArray *)check_keys {
    ...
    NSArray *gui_keys = [NSArray arrayWithObjects:@ "key_gui_arrow_left",
                                                  @ "key_gui_arrow_right",
                                                  @ "key_gui_arrow_up",
                                                  @ "key_gui_arrow_down",
                                                  @ "key_gui_page_up",
    ...
    NSArray *debug_keys = [NSArray arrayWithObjects:@ "key_dump_target_state",
                                                    @ "key_dump_entity_list",
                                                    @ "key_debug_full",
                                                    @ "key_debug_collision",
                                                    @ "key_debug_console_connect",
                                                    @ "key_debug_bounding_boxes",
                                                    @ "key_debug_shaders",
                                                    @ "key_debug_off",
                                                    nil];
    ...
    NSArray *customview_keys = [NSArray arrayWithObjects:@ "key_custom_view",
                                                         @ "key_custom_view_zoom_out",
                                                         @ "key_custom_view_zoom_in",
                                                         @ "key_custom_view_roll_left",
                                                         @ "key_custom_view_pan_left",
                                                         @ "key_custom_view_roll_right",
    ...
    NSMutableArray *inflight_keys = [NSMutableArray arrayWithObjects:@ "key_roll_left",
                                                                     @ "key_roll_right",
                                                                     @ "key_pitch_forward",
                                                                     @ "key_pitch_back",
                                                                     @ "key_yaw_left",
                                                                     ...
                                                                     @ "key_next_missile",
                                                                     @ "key_ecm",
                                                                     @ "key_prime_next_equipment",
                                                                     @ "key_prime_previous_equipment",
                                                                     ...
                                                                     @ "key_snapshot",
                                                                     @ "key_cycle_next_mfd",
                                                                     @ "key_cycle_previous_mfd",
                                                                     @ "key_switch_next_mfd",
                                                                     @ "key_switch_previous_mfd",
                                                                     @ "key_next_target",
                                                                     @ "key_previous_target",
                                                                     @ "key_comms_log",
    ...
    if ([self entryIsCustomEquip:key]) {
        NSUInteger i;
        for (i = 0; i < [customEquipActivation count]; i++) {
            [inflight_keys addObject:[NSString stringWithFormat:@ "activate_%@",
                                                                [[customEquipActivation objectAtIndex:i]
                                                                    oo_stringForKey:CUSTOMEQUIP_EQUIPKEY]]];
            [inflight_keys addObject:[NSString stringWithFormat:@ "mode_%@",
                                                                [[customEquipActivation objectAtIndex:i]
                                                                    oo_stringForKey:CUSTOMEQUIP_EQUIPKEY]]];
        }
    }
    ...
    NSArray *docking_keys = [NSArray arrayWithObjects:@ "key_docking_music", @ "key_autopilot", @ "key_pausebutton", nil];
    ...
    NSArray *docked_keys = [[NSArray arrayWithObjects:@ "key_launch_ship",
                                                      @ "key_gui_screen_options",
                                                      @ "key_gui_screen_equipship",
                                                      @ "key_gui_screen_interfaces",
                                                      @ "key_gui_screen_status",
                                                      @ "key_gui_chart_screens",
                                                      @ "key_gui_system_data",
                                                      @ "key_gui_market",
                                                      nil] arrayByAddingObjectsFromArray:gui_keys];
    ...
    NSArray *paused_keys = [[[NSArray arrayWithObjects:@ "key_pausebutton",
                                                       @ "key_gui_screen_options",
                                                       @ "key_hud_toggle",
                                                       @ "key_show_fps",
                                                       @ "key_mouse_control_roll",
                                                       @ "key_mouse_control_yaw",
                                                       nil] arrayByAddingObjectsFromArray:debug_keys]
    ...
    NSArray *chart_keys = [NSArray arrayWithObjects:@ "key_advanced_nav_array_next",
                                                    @ "key_advanced_nav_array_previous",
                                                    @ "key_map_home",
                                                    @ "key_map_end",
                                                    @ "key_map_info",
                                                    @ "key_map_zoom_in",
    ...
    NSArray *sysinfo_keys = [NSArray arrayWithObjects:@ "key_system_home",
                                                      @ "key_system_end",
                                                      @ "key_system_next_system",
                                                      @ "key_system_previous_system",
                                                      @ "key_launch_ship",
                                                      @ "key_gui_screen_options",
                                                      @ "key_gui_screen_equipship",
                                                      @ "key_gui_screen_interfaces",
    ...
    NSArray *market_keys = [NSArray arrayWithObjects:@ "key_market_filter_cycle",
...
- (NSString *)searchArrayForMatch:(NSArray *)search_list key:(NSString *)key checkKeys:(NSArray *)check_keys {
    ...
    for (i = 0; i < [search_list count]; i++) {
        ...
        if (![search isEqualToString:key]) {
            ...
            // we need to compare all entries to each other to look for any match, as any match would indicate a
            // conflict
            NSArray *current = nil;
            ...
            for (j = 0; j < [current count]; j++) {
                for (k = 0; k < [check_keys count]; k++) {
                    if ([self compareKeyEntries:[current objectAtIndex:j] second:[check_keys objectAtIndex:k]])
                        return search;
                }
            }
        }
...
- (BOOL)compareKeyEntries:(NSDictionary *)first second:(NSDictionary *)second {
    if ([(NSString *)[first objectForKey:@ "key"] integerValue] ==
        [(NSString *)[second objectForKey:@ "key"] integerValue]) {
...
- (void)saveKeySetting:(NSString *)key {
    ...
    if ([[(NSDictionary *)[key_list objectAtIndex:0] objectForKey:@ "key"] integerValue] == 0) {
        if ([key_list count] == 1 || ([key_list count] > 1 && [[(NSDictionary *)[key_list objectAtIndex:1]
                                                                  objectForKey:@ "key"] isEqualToString:@ ""])) {
    ...
    if (![self entryIsCustomEquip:key]) {
        ...
        NSMutableDictionary *keyconf =
            [NSMutableDictionary dictionaryWithDictionary:[defaults objectForKey:KEYCONFIG_OVERRIDES]];
    ...
    if ([customEquipActivation count] > 0) {
        ...
        for (i = 0; i < [customEquipActivation count]; i++) {
            ...
            if ([item defaultActivateKey])
                [[customEquipActivation objectAtIndex:i] setObject:[item defaultActivateKey]
                                                            forKey:CUSTOMEQUIP_KEYACTIVATE];
            else
