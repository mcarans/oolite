...
- (NSString *)processEscapePods  // removes pods from cargo bay and treats categories of characters carried
{
    ...
    if ([UNIVERSE inInterstellarSpace])
        government = 1;  // equivalent to Feudal. I'm assuming any station in interstellar space is military. -- Ahruman
                         // 2008-05-29
        ...
        if ([rescuee script]) {
            ...
            if (government > (Ranrot() & 7) || reward >= insurance) {
                ...
                [result appendFormat:DESC(@ "capture-reward-for-@@-@-credits-@-alt"),
                                     [rescuee name],
                                     [rescuee shortDescription],
                                     OOStringFromDeciCredits(reward, YES, NO),
                                     OOStringFromDeciCredits(insurance, YES, NO)];
                [self doScriptEvent:OOJSID("playerRescuedEscapePod")
                      withArguments:[NSArray arrayWithObjects:[NSNumber numberWithUnsignedInteger:reward],
                                                              @ "bounty",
                                                              [rescuee infoForScripting],
                                                              nil]];
            } else {
                ...
                [result appendFormat:DESC(@ "rescue-reward-for-@@-@-credits-@-alt"),
                                     [rescuee name],
                                     [rescuee shortDescription],
                                     OOStringFromDeciCredits(insurance - reward, YES, NO),
                                     OOStringFromDeciCredits(reward, YES, NO)];
            ...
            [result appendFormat:DESC(@ "rescue-reward-for-@@-@-credits"),
                                 [rescuee name],
                                 [rescuee shortDescription],
                                 OOStringFromDeciCredits([rescuee insuranceCredits] * 10, YES, NO)];
            credits += 10 * [rescuee insuranceCredits];
            [self doScriptEvent:OOJSID("playerRescuedEscapePod")
                  withArguments:[NSArray
                                    arrayWithObjects:[NSNumber
                                                         numberWithUnsignedInteger:(10 * [rescuee insuranceCredits])],
                                                     @ "insurance",
                                                     [rescuee infoForScripting],
                                                     nil]];
            ...
            [self doScriptEvent:OOJSID("playerRescuedEscapePod")
                  withArguments:[NSArray arrayWithObjects:[NSNumber numberWithUnsignedInteger:0],
                                                          @ "slave",
                                                          [rescuee infoForScripting],
                                                          nil]];
        }
        if ((i < [rescuees count] - 1) && added_entry) [result appendString:@ "\n"];
        added_entry = NO;
    }
    ...
    for (i = 0; i < [passengers count]; i++) {
        ...
        if (system_id == dest) {
            ...
            if (dest_eta > 0) {
                ...
                while ((randf() < 0.75) &&
                       (dest_eta > 3600))  // delivered with more than an hour to spare and a decent customer?
                {
                    fee *= 110;  // tip + 10%
                    fee /= 100;
                    dest_eta *= 0.5;
                ...
                [result appendFormatLine:DESC(@ "passenger-delivered-okay-@-@-@"),
                                         passenger_name,
                                         OOIntCredits(fee),
                                         passenger_dest_name];
                ...
                [self increasePassengerReputation:RepForRisk([passenger_info oo_unsignedIntForKey:CONTRACT_KEY_RISK
                                                                                     defaultValue:0])];
                [passengers removeObjectAtIndex:i--];
                [self doScriptEvent:OOJSID("playerCompletedContract")
                      withArguments:[NSArray arrayWithObjects:@ "passenger",
                                                              @ "success",
                                                              [NSNumber numberWithUnsignedInteger:(10 * fee)],
                                                              passenger_info,
                                                              nil]];
            } else {
                // but we're late!
                ...
                [result appendFormatLine:DESC(@ "passenger-delivered-late-@-@-@"),
                                         passenger_name,
                                         OOIntCredits(fee),
                                         passenger_dest_name];
                if ([passenger_info oo_unsignedIntForKey:CONTRACT_KEY_RISK defaultValue:0] > 0) {
            ...
            if (dest_eta < 0) {
                ...
                [self decreasePassengerReputation:RepForRisk([passenger_info oo_unsignedIntForKey:CONTRACT_KEY_RISK
                                                                                     defaultValue:0])];
                [passengers removeObjectAtIndex:i--];
                [self doScriptEvent:OOJSID("playerCompletedContract")
                      withArguments:[NSArray arrayWithObjects:@ "passenger",
    ...
    for (i = 0; i < [parcels count]; i++) {
        ...
        if (system_id == dest) {
            ...
            if (dest_eta > 0) {
                ...
                while ((randf() < 0.75) &&
                       (dest_eta > 86400))  // delivered with more than a day to spare and a decent customer?
                {
                ...
                [self increaseParcelReputation:RepForRisk([parcel_info oo_unsignedIntForKey:CONTRACT_KEY_RISK
                                                                               defaultValue:0])];
                ...
                [self doScriptEvent:OOJSID("playerCompletedContract")
                      withArguments:[NSArray arrayWithObjects:@ "parcel",
                                                              @ "success",
                                                              [NSNumber numberWithUnsignedInteger:10 * fee],
                                                              parcel_info,
                                                              nil]];
            ...
            if (dest_eta < 0) {
                ...
                [self decreaseParcelReputation:RepForRisk([parcel_info oo_unsignedIntForKey:CONTRACT_KEY_RISK
                                                                               defaultValue:0])];
                [parcels removeObjectAtIndex:i--];
                [self doScriptEvent:OOJSID("playerCompletedContract")
                      withArguments:[NSArray arrayWithObjects:@ "parcel",
        ...
        if (system_id == dest) {
            ...
            if (dest_eta > 0) {
                ...
                if (quantity_on_hand >= contract_amount) {
                    ...
                    [self doScriptEvent:OOJSID("playerCompletedContract")
                          withArguments:[NSArray arrayWithObjects:@ "cargo",
                                                                  @ "success",
                                                                  [NSNumber numberWithUnsignedInteger:fee],
                                                                  contract_info,
                                                                  nil]];
                    ...
                    if (percent_delivered >= acceptable_ratio) {
                        ...
                        [result appendFormatLine:DESC(@ "cargo-delivered-short-@-@-d"),
                                                 contract_cargo_desc,
                                                 OOCredits(payment),
                                                 shortfall];
                        ...
                        [self doScriptEvent:OOJSID("playerCompletedContract")
                              withArguments:[NSArray arrayWithObjects:@ "cargo",
                                                                      @ "short",
                                                                      [NSNumber numberWithUnsignedInteger:payment],
                                                                      contract_info,
                                                                      nil]];
    ...
    for (i = 0; i < [names count]; i++) {
        ...
        if (dest_eta < 0) {
            ...
            for (j = 0; j < [passengers count]; j++) {
                ...
                if ([[passenger_info objectForKey:PASSENGER_KEY_NAME] isEqual:[names objectAtIndex:i]]) on_board = YES;
            }
            if (!on_board) {
...
- (void)increasePassengerReputation:(unsigned)amount {
    ...
    for (unsigned i = 0; i < amount; i++) {
        if (bad > 0) {
            ...
            bad--;
            if (unknown < MAX_CONTRACT_REP) unknown++;
        } else {
            // shift a bean from unknown to good
            if (unknown > 0) unknown--;
            if (good < MAX_CONTRACT_REP) good++;
        }
    }
...
- (BOOL)addPassenger:(NSString *)Name
         ...
         destination:(unsigned)Destination
             ...
             advance:(double)advance
                risk:(unsigned)risk {
    NSDictionary *passenger_info =
        [NSDictionary dictionaryWithObjectsAndKeys:Name,
                                                   PASSENGER_KEY_NAME,
                                                   [NSNumber numberWithInt:start],
                                                   CONTRACT_KEY_START,
                                                   [NSNumber numberWithInt:Destination],
                                                   CONTRACT_KEY_DESTINATION,
                                                   [NSNumber numberWithDouble:[PLAYER clockTime]],
                                                   CONTRACT_KEY_DEPARTURE_TIME,
                                                   [NSNumber numberWithDouble:eta],
                                                   CONTRACT_KEY_ARRIVAL_TIME,
                                                   [NSNumber numberWithDouble:fee],
                                                   CONTRACT_KEY_FEE,
                                                   [NSNumber numberWithDouble:advance],
                                                   CONTRACT_KEY_PREMIUM,
                                                   [NSNumber numberWithUnsignedInt:risk],
                                                   CONTRACT_KEY_RISK,
    ...
    [self doScriptEvent:OOJSID("playerEnteredContract")
          withArguments:[NSArray arrayWithObjects:@ "passenger", passenger_info, nil]];
...
- (BOOL)addParcel:(NSString *)Name
    ...
    NSDictionary *parcel_info =
        [NSDictionary dictionaryWithObjectsAndKeys:Name,
                                                   ...
                                                   [NSNumber numberWithDouble:fee],
                                                   CONTRACT_KEY_FEE,
                                                   [NSNumber numberWithDouble:premium],
                                                   CONTRACT_KEY_PREMIUM,
                                                   [NSNumber numberWithUnsignedInt:risk],
                                                   CONTRACT_KEY_RISK,
                                                   NULL];
    ...
    [self doScriptEvent:OOJSID("playerEnteredContract")
          withArguments:[NSArray arrayWithObjects:@ "parcel", parcel_info, nil]];
...
- (BOOL)awardContract:(unsigned)qty
            commodity:(OOCommodityType)type
                start:(unsigned)start
          destination:(unsigned)Destination
                  eta:(double)eta
                  fee:(double)fee
    ...
    NSDictionary *cargo_info =
        [NSDictionary dictionaryWithObjectsAndKeys:cargo_ID,
                                                   CARGO_KEY_ID,
                                                   type,
                                                   CARGO_KEY_TYPE,
                                                   [NSNumber numberWithInt:qty],
                                                   CARGO_KEY_AMOUNT,
                                                   [UNIVERSE describeCommodity:type amount:qty],
    ...
    [self doScriptEvent:OOJSID("playerEnteredContract")
          withArguments:[NSArray arrayWithObjects:@ "cargo", cargo_info, nil]];

    return YES;
}

- (BOOL)removeContract:(OOCommodityType)type
           destination:(unsigned)dest  // removes the first match found, returns NO if none found
{
...
- (NSArray *)contractsListFromArray:(NSArray *)contracts_array forCargo:(BOOL)forCargo forParcels:(BOOL)forParcels {
    ...
    NSString *formatString =
        (forCargo || forParcels) ? @ "oolite-manifest-item-delivery" : @ "oolite-manifest-person-travelling";
    unsigned i;
    for (i = 0; i < [contracts_array count]; i++) {
        ...
        // the system name can change via script. The following PASSENGER_KEYs are identical to the corresponding
        // CONTRACT_KEYs
...
#define SET_MANIFEST_ROW(obj, color, row)   \
    ([self setManifestScreenRow:obj         \
                        inColor:color       \
                         forRow:row         \
                         ofRows:max_rows    \
                      andOffset:page_offset \
                    inMultipage:multi_page])
...
- (void)setGuiToManifestScreen {
    ...
    // GUI stuff
    {
        ...
        BOOL multi_page = NO;
        //		NSUInteger total_rows = cargoRowCount + MAX(1U,[passengerManifest count]) +
        // MAX(1U,[contractManifest count]) + mmRows + MAX(1U,[parcelManifest count]) + 5;
...
- (void)setGuiToDockingReportScreen {
    ...
    [dockingReport
        setString:[dockingReport stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
    ...
    // GUI stuff
    {
        ...
        while (([dockingReport length] > 0) && (text_row < 18)) {
            if ([dockingReport rangeOfString:@ "\n"].location != NSNotFound) {
                ...
                [dockingReport
                    setString:[dockingReport
                                  stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
            } else {
                text_row = [gui addLongText:[NSString stringWithString:dockingReport]
                              startingAtRow:text_row
                                      align:GUI_ALIGN_LEFT];
                [dockingReport setString:@ ""];
            }
        }

        [gui setText:[NSString stringWithFormat:DESC_PLURAL(@ "contracts-cash-@-load-d-of-d-passengers-d-of-d-berths",
                                                            max_passengers),
                                                OOCredits(credits),
                                                current_cargo,
...
- (void)setGuiToShipyardScreen:(NSUInteger)skip {
    ...
    // GUI stuff
    {
        ...
        if (shipCount <= MAX_ROWS_SHIPS_FOR_SALE)
            ...
            if (skip + rowCount < shipCount) rowCount -= 1;
        }

        if (shipCount > 0) {
            [gui setColor:[gui colorFromSetting:kGuiShipyardHeadingColor defaultValue:[OOColor greenColor]]
                   forRow:GUI_ROW_SHIPYARD_LABELS];
            [gui setArray:[NSArray arrayWithObjects:DESC(@ "shipyard-shiptype"),
                                                    DESC(@ "shipyard-price-label"),
                                                    DESC(@ "shipyard-cargo-label"),
                                                    DESC(@ "shipyard-speed-label"),
                                                    nil]
                   forRow:GUI_ROW_SHIPYARD_LABELS];

            if (skip > 0) {
                [gui setColor:[gui colorFromSetting:kGuiShipyardScrollColor defaultValue:[OOColor greenColor]]
                ...
                [gui
                    setArray:[NSArray
                                 arrayWithObjects:
                                     [NSString
                                         stringWithFormat:@ " %@ ",
                                                          [[ship_info oo_dictionaryForKey:SHIPYARD_KEY_SHIP]
                                                              oo_stringForKey:@ "display_name"
                                                                 defaultValue:[[ship_info
                                                                                  oo_dictionaryForKey:SHIPYARD_KEY_SHIP]
                                                                                  oo_stringForKey:KEY_NAME]]],
            ...
            [gui setColor:[gui colorFromSetting:kGuiShipyardNoshipColor defaultValue:[OOColor greenColor]]
                   forRow:GUI_ROW_NO_SHIPS];
...
- (void)showShipyardInfoForSelection {
    ...
    if (info) {
        ...
        if (i - 1 >= GUI_ROW_MARKET_CASH - 1) {
            [gui setColor:[gui colorFromSetting:kGuiShipyardDescriptionColor defaultValue:[OOColor greenColor]]
                   forRow:i - 1];
            [gui setColor:[gui colorFromSetting:kGuiShipyardDescriptionColor defaultValue:[OOColor greenColor]]
...
- (void)showTradeInInformationFooter {
    ...
    [gui setText:OOExpandKey(@ "shipyard-total-available-with-trade-in", shipType, total, credits, tradeIn)
          forRow:GUI_ROW_MARKET_CASH];
}
...
- (void)showShipyardModel:(NSString *)shipKey shipData:(NSDictionary *)shipData personality:(uint16_t)personality {
    ...
    [self showShipModelWithKey:shipKey
                      shipData:shipData
...
- (OOCreditsQuantity)tradeInValue {
    ...
    */
    ...
    value = cunningFee(((value * 75 * ship_trade_in_factor) + 5000) / 10000,
                       0.005);  // Multiply by two percentages, divide by 100*100. The +5000 is to get normal rounding.
    return value * 10;
}
...
- (BOOL)buySelectedShip {
    ...
    if ([key hasPrefix:@ "More:"]) {
        ...
        NSMutableArray *row_info =
            [NSMutableArray arrayWithArray:(NSArray *)[gui objectForRow:GUI_ROW_SHIPYARD_LABELS]];
        while ([row_info count] < 4) {
            [row_info addObject:@ ""];
        }
        [row_info replaceObjectAtIndex:2 withObject:@ ""];
    ...
    if (credits + tradeIn < price * 10) return NO;  // you can't afford it!
    ...
    [self doScriptEvent:OOJSID("playerWillBuyNewShip")
          withArguments:[NSArray arrayWithObjects:[shipInfo oo_stringForKey:SHIPYARD_KEY_SHIPDATA_KEY],
                                                  [[[self dockedStation] localShipyard]
                                                      objectAtIndex:selectedRow - GUI_ROW_SHIPYARD_START],
                                                  [NSNumber numberWithUnsignedLongLong:price],
                                                  [NSNumber numberWithUnsignedLongLong:(tradeIn / 10)],
                                                  nil]];
...
- (BOOL)replaceShipWithNamedShip:(NSString *)shipKey {
    ...
    NSArray *extras = [NSMutableArray
        arrayWithArray:[[ship_info oo_dictionaryForKey:KEY_STANDARD_EQUIPMENT] oo_arrayForKey:KEY_EQUIPMENT_EXTRAS]];
...
- (void)newShipCommonSetup:(NSString *)shipKey
    ...
    if (available_facings & WEAPON_FACING_STARBOARD)
        starboard_weapon_type =
            OOWeaponTypeFromEquipmentIdentifierSloppy([shipDict oo_stringForKey:@ "starboard_weapon_type"]);
    else
        starboard_weapon_type = OOWeaponTypeFromEquipmentIdentifierSloppy(@ "EQ_WEAPON_NONE");

    if (available_facings & WEAPON_FACING_FORWARD)
        forward_weapon_type =
            OOWeaponTypeFromEquipmentIdentifierSloppy([shipDict oo_stringForKey:@ "forward_weapon_type"]);
    else
