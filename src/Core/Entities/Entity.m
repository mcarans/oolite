...
- (NSString *)descriptionComponents {
    return [NSString stringWithFormat:@ "position: %@ scanClass: %@ status: %@",
                                      HPVectorDescription([self position]),
                                      OOStringFromScanClass([self scanClass]),
...
- (void)addToLinkedLists {
#ifndef NDEBUG
    if (gDebugFlags & DEBUG_LINKED_LISTS) OOLog(kOOLogEntityAddToList, @ "DEBUG adding entity %@ to linked lists", self);
#endif
    //
    // insert at the start
    if (UNIVERSE) {
    ...
    if (gDebugFlags & DEBUG_LINKED_LISTS) {
        if (![self checkLinkedLists]) {
            OOLog(kOOLogEntityAddToListError,
                  @ "DEBUG LINKED LISTS - problem encountered while adding %@ to linked lists",
                  self);
...
- (void)removeFromLinkedLists {
    ...
    if (UNIVERSE) {
        if ((UNIVERSE->x_list_start == self) && (x_next)) UNIVERSE->x_list_start = x_next;
        if ((UNIVERSE->y_list_start == self) && (y_next)) UNIVERSE->y_list_start = y_next;
        if ((UNIVERSE->z_list_start == self) && (z_next)) UNIVERSE->z_list_start = z_next;
    }
    //
    ...
    if (gDebugFlags & DEBUG_LINKED_LISTS) {
        if (![self checkLinkedLists]) {
            OOLog(kOOLogEntityRemoveFromListError,
                  @ "DEBUG LINKED LISTS - problem encountered while removing %@ from linked lists",
                  self);
            [UNIVERSE debugDumpEntities];
        }
    }
...
- (void)updateLinkedLists {
    if (!UNIVERSE) return;                               // not in the UNIVERSE - don't do this!
    if ((x_next == nil) && (x_previous == nil)) return;  // not in the lists - don't do this!

#ifndef NDEBUG
    if (gDebugFlags & DEBUG_LINKED_LISTS) {
        if (![self checkLinkedLists]) {
            OOLog(kOOLogEntityVerificationError,
                  @ "DEBUG LINKED LISTS problem encountered before updating linked lists for %@",
                  self);
            [UNIVERSE debugDumpEntities];
        }
    }
    ...
    if (gDebugFlags & DEBUG_LINKED_LISTS) {
        if (![self checkLinkedLists]) {
            OOLog(kOOLogEntityUpdateError,
                  @ "DEBUG LINKED LISTS problem encountered after updating linked lists for %@",
                  self);
            [UNIVERSE debugDumpEntities];
        }
    }
...
- (void)warnAboutHostiles {
    ...
    OOLog(@ "general.error.subclassResponsibility.Entity-warnAboutHostiles",
          @ "%@",
...
- (void)updateCameraRelativePosition {
    cameraRelativePosition =
        HPVectorToVector(HPvector_subtract([self absolutePositionForSubentity], [PLAYER viewpointPosition]));
}
...
- (void)applyRoll:(GLfloat)roll andClimb:(GLfloat)climb {
    if ((roll == 0.0) && (climb == 0.0) && (!hasRotated)) return;

    if (roll) quaternion_rotate_about_z(&orientation, -roll);
    if (climb) quaternion_rotate_about_x(&orientation, -climb);

    [self orientationChanged];
}

- (void)applyRoll:(GLfloat)roll climb:(GLfloat)climb andYaw:(GLfloat)yaw {
    if ((roll == 0.0) && (climb == 0.0) && (yaw == 0.0) && (!hasRotated)) return;

    if (roll) quaternion_rotate_about_z(&orientation, -roll);
    if (climb) quaternion_rotate_about_x(&orientation, -climb);
    if (yaw) quaternion_rotate_about_y(&orientation, -yaw);
...
- (NSString *)descriptionForObjDump {
    ...
    result = [result stringByAppendingFormat:@ " range: %g (visible: %@)",
                                             HPdistance([self position], [PLAYER position]),
