...
- (BOOL)setSunColor:(OOColor *)sun_color {
    ...
    */
    ...
    color = [OOColor colorWithRed:[color redComponent] * _sunBrightnessFactor
                            green:[color greenComponent] * _sunBrightnessFactor
                             blue:[color blueComponent] * _sunBrightnessFactor
...
- (id)initSunWithColor:(OOColor *)sun_color andDictionary:(NSDictionary *)dict {
    ...
    corona_stage = 0.0;
    for (i = 0; i < SUN_CORONA_SAMPLES; i++) rvalue[i] = randf();
...
- (NSString *)descriptionComponents {
    NSString *result = [NSString stringWithFormat:@ "ID: %u position: %@ radius: %.3fkm",
                                                  [self universalID],
        ...
        if (_novaCountdown >= 0.0)  // countdown
        {
            ...
            if (_novaExpansionTimer <= 60.0)  // expand for a minute
            {
                ...
                if (sky_bri == 1.0) {
                    ...
                    [UNIVERSE setSystemDataKey:@ "sun_gone_nova"
                                         value:[NSNumber numberWithBool:YES]
                                  fromManifest:@ "org.oolite.oolite"];
                    [UNIVERSE setSystemDataKey:@ "corona_flare"
                                         value:[NSNumber numberWithFloat:0.3]
                                  fromManifest:@ "org.oolite.oolite"];
                    [UNIVERSE setSystemDataKey:@ "corona_hues"
                                         value:[NSNumber numberWithFloat:0.05]
                                  fromManifest:@ "org.oolite.oolite"];
                ...
                discColor[0] = 1.0 * _sunBrightnessFactor;
                discColor[1] = 1.0 * _sunBrightnessFactor;
                discColor[2] = 1.0 * _sunBrightnessFactor;
                _novaExpansionTimer += delta_t;
                [UNIVERSE setSystemDataKey:@ "sun_radius"
                                     value:[NSNumber numberWithFloat:collision_radius + delta_t * _novaExpansionRate]
                              fromManifest:@ "org.oolite.oolite"];
            } else {
...
- (void)drawOpaqueParts {
    ...
    if ([UNIVERSE reducedDetail]) {
        ...
        float drawRatio2 =
            drawFactor * effective_radius / sqrt_zero_distance;  // equivalent to size on screen in pixels
        ...
        */
        ...
        [self calculateGLArrays:effective_radius width:effective_cor16k zDistance:sqrt_zero_distance];
        OOGL(glDisable(GL_BLEND));
...
- (void)calculateGLArrays:(GLfloat)inner_radius width:(GLfloat)width zDistance:(GLfloat)z_distance {
    ...
    GLfloat activity[8] = {0.84, 0.74, 0.64, 0.54, 0.3, 0.4, 0.7, 0.8};
    ...
    for (j = 0; j <= 4; j++) {
        ...
        for (i = 0; i < 360; i++) {
            GLfloat rm = 1.0;
            if (j >= 1 && j < 4) {
                rm = 1.0 + ((0.04 / j) * (pt0 * (rvalue[i] + rvalue[i + 1] + rvalue[i + 2]) +
                                          pt1 * (rvalue[i + 360] + rvalue[i + 361] + rvalue[i + 362]))) /
                               3;
            }
...
- (void)drawDirectVisionSunGlare {
    ...
    GLfloat sunGlareAngularSize =
        atan([self radius] / HPdistance([PLAYER viewpointPosition], [self position])) * SUN_GLARE_MULT_FACTOR +
    ...
    if (directVisionSunGlare) {
        ...
        // in default Lave
        GLfloat distanceReductionFactor =
            OOClamp_0_1_f(([self radius] * [self radius] * 182.0) / HPdistance2([PLAYER position], [self position]));
        GLfloat sunGlareFilterMultiplierLocal = [PLAYER sunGlareFilter];
        GLfloat directVisionSunGlareColor[4] = {discColor[0],
                                                discColor[1],
                                                discColor[2],
                                                directVisionSunGlare * atmosphericReductionFactor *
                                                    distanceReductionFactor * (1.0f - sunGlareFilterMultiplierLocal) *
                                                    0.0085f};
...
- (BOOL)changeSunProperty:(NSString *)key withDictionary:(NSDictionary *)dict {
    ...
    if ([key isEqualToString:@ "sun_radius"]) {
        oldRadius =
            [object doubleValue];  // clamp corona_flare in case planetinfo.plist / savegame contains the wrong value
        ...
        OOLogWARN(
            @ "script.warning", @ "Change to property '%@' not applied, will apply only after leaving this system.", key);
        return NO;
    }
    return YES;
}
