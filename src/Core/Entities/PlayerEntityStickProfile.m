...
- (void)stickProfileInputHandler:(GuiDisplayGen *)gui view:(MyOpenGLView *)gameView {
    if ([gameView isDown:gvMouseLeftButton]) {
        NSPoint mouse_position = NSMakePoint([gameView virtualJoystickPosition].x * [gui size].width,
...
- (void)stickProfileGraphAxisProfile:(GLfloat)alpha screenAt:(Vector)screenAt screenSize:(NSSize)screenSize {
    [stickProfileScreen graphProfile:alpha
                                  at:make_vector(screenAt.x - 110.0, screenAt.y - 100, screenAt.z)
                                size:NSMakeSize(220, 220)];
    return;
}
...
- (void)graphProfile:(GLfloat)alpha at:(Vector)at size:(NSSize)size {
    ...
    for (i = 0; i <= size.width - 20; i++) {
        glVertex3f(
            at.x + i + 10, at.y + 10 + (size.height - 20) * [profile rawValue:((float)i) / (size.width - 20)], at.z);
    }
    OOGLEND();
    ...
    if ([profile isKindOfClass:[OOJoystickSplineAxisProfile class]]) {
        ...
        for (i = 0; i < (NSInteger)[control_points count]; i++) {
            ...
            GLDrawFilledOval(at.x + 10 + point.x * (size.width - 20),
                             at.y + 10 + point.y * (size.height - 20),
                             at.z,
                             NSMakeSize(4, 4),
                             20);
        }
    ...
    OODrawStringAligned(
        DESC(@ "oolite-stickprofile-movement"), at.x + size.width - 5, at.y, at.z, NSMakeSize(8, 10), YES);
...
- (void)showScreen {
    ...
    [gui setArray:[NSArray arrayWithObjects:DESC(@ "oolite-stickprofile-axis"), [self currentAxis], nil]
    ...
    [gui setArray:[NSArray arrayWithObjects:DESC(@ "oolite-stickprofile-deadzone"),
                                            [NSString stringWithFormat:@ "%@%@ (%0.4f)",
    ...
    [gui setArray:[NSArray arrayWithObjects:DESC(@ "oolite-stickprofile-profile-type"), [self profileType], nil]
