    ...
    if (quantity > 0) {
        if (units == UNITS_TONS) {
            ...
            if (quantity > 0) {
                ...
                if (quantity % amountToLoadInCargopod == 0)
                    podsRequiredForQuantity--;
...
- (OOScalar)chart_zoom
{
    if (_missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_SHORT || _missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_SHORT_ANA_QUICKEST || _missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_SHORT_ANA_SHORTEST) {
        return 1.0;
    } else if (_missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_LONG || _missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_LONG_ANA_SHORTEST || _missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_LONG_ANA_QUICKEST) {
        return CHART_MAX_ZOOM;
    } else if (_missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_CUSTOM || _missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_CUSTOM_ANA_QUICKEST || _missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_CUSTOM_ANA_SHORTEST) {
...
- (NSPoint)adjusted_chart_centre
{
    ...
    if (_missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_SHORT || _missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_SHORT_ANA_QUICKEST || _missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_SHORT_ANA_SHORTEST) {
        return galaxy_coordinates;
    } else if (_missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_LONG || _missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_LONG_ANA_QUICKEST || _missionBackgroundSpecial == GUI_BACKGROUND_SPECIAL_LONG_ANA_SHORTEST) {
...
#if OOLITE_ESPEAK
    [result setObject:[UNIVERSE voiceName:voice_no]
               forKey:@"speech_voice"];
...
- (BOOL)setCommanderDataFromDictionary:(NSDictionary*)dict
{
    ...
    // Required keys
    if ([dict oo_stringForKey:@"ship_desc"] == nil)
        return NO;
    // galaxy_seed is used is 1.80 or earlier
    if ([dict oo_stringForKey:@"galaxy_seed"] == nil && [dict oo_stringForKey:@"galaxy_id"] == nil)
        return NO;
    // galaxy_coordinates is used is 1.80 or earlier
    if ([dict oo_stringForKey:@"galaxy_coordinates"] == nil && [dict oo_stringForKey:@"system_id"] == nil)
        return NO;
    ...
    if (shipDict == nil)
        return NO;
    if (![self setUpShipFromDictionary:shipDict])
        return NO;
    ...
    if ([dict oo_stringForKey:@"galaxy_id"] != nil) {
        ...
        if (longRangeChartMode == OOLRC_MODE_UNKNOWN)
    ...
    */
    ...
    if (reputation == nil)
        reputation = [[NSMutableDictionary alloc] init];
...
- (BOOL)setUpAndConfirmOK:(BOOL)stopOnError saveGame:(BOOL)saveGame
{
    ...
    if (![gc inFullScreenMode] && stopOnError)
        [gc stopAnimationTimer]; // start of critical section
    ...
    if (EXPECT_NOT(![[OOJavaScriptEngine sharedEngine] reset] && stopOnError)) // always (try to) reset the engine, then find out if we need to stop.
    {
        ...
        */
        ...
        if (![gc inFullScreenMode])
            [gc startAnimationTimer]; // keep the game ticking over.
        return NO;
    }

    // end of critical section
    if (![gc inFullScreenMode] && stopOnError)
        [gc startAnimationTimer];
...
- (BOOL)setUpShipFromDictionary:(NSDictionary*)shipDict
{
    ...
    if (![super setUpFromDictionary:shipDict])
        return NO;
    ...
    if (max_missiles > PLAYER_MAX_MISSILES)
...
- (void)update:(OOTimeDelta)delta_t
{
    ...
    if (EXPECT_NOT(status == STATUS_START_GAME && gui_screen != GUI_SCREEN_INTRO1 && gui_screen != GUI_SCREEN_SHIPLIBRARY && gui_screen != GUI_SCREEN_GAMEOPTIONS && gui_screen != GUI_SCREEN_STICKMAPPER && gui_screen != GUI_SCREEN_STICKPROFILE && gui_screen != GUI_SCREEN_NEWGAME && gui_screen != GUI_SCREEN_OXZMANAGER && gui_screen != GUI_SCREEN_LOAD && gui_screen != GUI_SCREEN_KEYBOARD && gui_screen != GUI_SCREEN_KEYBOARD_CONFIRMCLEAR && gui_screen != GUI_SCREEN_KEYBOARD_CONFIG && gui_screen != GUI_SCREEN_KEYBOARD_ENTRY && gui_screen != GUI_SCREEN_KEYBOARD_LAYOUT)) {
...
- (void)updateAlertConditionForNearbyEntities
{
    ...
    [self setAlertFlag:ALERT_FLAG_MASS_LOCK
                    to:massLocked];
...
- (void)performDockingRequest:(StationEntity*)stationForDocking
{
    if (stationForDocking == nil)
        return;
    if (![stationForDocking isStation] || ![stationForDocking isKindOfClass:[StationEntity class]])
        return;
    if ([self isDocked])
        return;
    ...
    if (velmag > 0) {
        ...
        if (velmag > VELOCITY_CLEANUP_MIN) {
            ...
            if (velmag > VELOCITY_CLEANUP_FULL)
                ...
                rate = (velmag - VELOCITY_CLEANUP_MIN) / (VELOCITY_CLEANUP_FULL - VELOCITY_CLEANUP_MIN) * VELOCITY_CLEANUP_RATE;
            velmag2 -= velmag * rate;
        }
        if (velmag2 < 0.0f)
            velocity = kZeroVector;
...
- (void)performWitchspaceExitUpdates:(OOTimeDelta)delta_t
{
    if ([UNIVERSE breakPatternOver]) {
        ...
        if ([UNIVERSE planet]) {
            ...
            if ([UNIVERSE inInterstellarSpace])
                [UNIVERSE addMessage:DESC(@"witch-engine-malfunction") forCount:3.0]; // if sun gone nova, print nothing
        }
...
- (void)performDockingUpdates:(OOTimeDelta)delta_t
{
    ...
    if ([UNIVERSE ECMVisualFXEnabled])
        [UNIVERSE terminatePostFX:OO_POSTFX_CRTBADSIGNAL];
}
...
- (void)gameOverFadeToBW
{
    ...
    if ([UNIVERSE detailLevel] >= DETAIL_LEVEL_SHADERS && secondsToBWFadeOut > 0.0f) {
        ...
        if (originalColorSaturation == -1.0f)
            originalColorSaturation = [gameView colorSaturation];
...
- (BOOL)isValidTarget:(Entity*)target
{
    ...
    if ([target isShip]) {
        ...
        if (tstatus == STATUS_ENTERING_WITCHSPACE || tstatus == STATUS_IN_HOLD || tstatus == STATUS_DOCKED) { // checks for ships entering wormholes, docking, or been scooped
            return NO;
        }
        return YES;
    }
...
- (void)showShipModelWithKey:(NSString*)shipKey shipData:(NSDictionary*)shipData personality:(uint16_t)personality factorX:(GLfloat)factorX factorY:(GLfloat)factorY factorZ:(GLfloat)factorZ inContext:(NSString*)context
{
    if (shipKey == nil)
        return;
    if (shipData == nil)
        shipData = [[OOShipRegistry sharedRegistry] shipInfoForKey:shipKey];
    if (shipData == nil)
        return;
    ...
    if (personality != ENTITY_PERSONALITY_INVALID)
        [ship setEntityPersonalityInt:personality];
    ...
    if (subEntStatus != nil)
        [ship deserializeShipSubEntitiesFrom:(NSString*)subEntStatus];
...
- (void)updateTargeting
{
    ...
    if ([self primaryTarget] == nil && (ident_engaged || missile_status != MISSILE_STATUS_SAFE) && ([self status] == STATUS_IN_FLIGHT || [self status] == STATUS_WITCHSPACE_COUNTDOWN)) {
...
- (void)resetHud
{
    ...
    if (![self switchHudTo:hud_desc])
        [self switchHudTo:@"hud.plist"]; // ensure we have a HUD to fall back to
}
...
{
    if (target_system_id == system_id && ![UNIVERSE inInterstellarSpace])
        return 0.0f;
        ...
        if (missile_entity[next_missile]) {
            ...
            if (missile_status != MISSILE_STATUS_SAFE) {
                ...
                if ([missile_entity[activeMissile] isMissile]) {
                    if ([self hasEquipmentItemProviding:@"EQ_MULTI_TARGET"] && ([missile_entity[next_missile] primaryTarget] != nil)) {
...
- (void)interpretAIMessage:(NSString*)ms
{
    ...
    if ([ms isEqual:@"ECM"] && ![self isDocked])
...
- (BOOL)mountMissile:(ShipEntity*)missile
{
    ...
    for (i = 0; i < max_missiles; i++) {
        if (missile_entity[i] == nil) {
            ...
            missiles++;
            if (missiles == 1)
                [self setActiveMissile:0]; // auto select the first purchased missile
            return YES;
        }
    }
...
{
    if ([self missileCount] >= [self missileCapacity])
        return NO;
...
- (ShipEntity*)fireMissile
{
    ...
    if ([missile isMine] && (missile_status != MISSILE_STATUS_SAFE)) {
        ...
        if (firedMissile != nil)
            [self playMineLaunched:[self missileLaunchPosition] weaponIdentifier:identifier];
    } else {
...
- (OOEnergyUnitType)installedEnergyUnitType
{
    if ([self hasEquipmentItem:@"EQ_NAVAL_ENERGY_UNIT"])
        return ENERGY_UNIT_NAVAL;
    if ([self hasEquipmentItem:@"EQ_ENERGY_UNIT"])
        return ENERGY_UNIT_NORMAL;
...
- (OOEnergyUnitType)energyUnitType
{
    ...
    if ([self hasEquipmentItem:@"EQ_ENERGY_UNIT"])
        return ENERGY_UNIT_NORMAL;
    if ([self hasEquipmentItem:@"EQ_NAVAL_ENERGY_UNIT_DAMAGED"])
        return ENERGY_UNIT_NAVAL_DAMAGED;
    if ([self hasEquipmentItem:@"EQ_ENERGY_UNIT_DAMAGED"])
        return ENERGY_UNIT_NORMAL_DAMAGED;
    return ENERGY_UNIT_NONE;
}
...
- (void)takeEnergyDamage:(double)amount from:(Entity*)ent becauseOf:(Entity*)other weaponIdentifier:(NSString*)weaponIdentifier
{
    ...
    if ([self status] == STATUS_ESCAPE_SEQUENCE)
        return; // if the player has ejected, don't deal more damage
    if (amount == 0.0)
        return;
    ...
    if ([ent isShip])
        [(ShipEntity*)ent doScriptEvent:OOJSID("shipAttackedOther") withArgument:self];
    ...
    if (cascading)
        energy = 0.0; // explicitly set energy to zero when cascading, in case an oxp raised the energy in noteTakingDamage.
...
{
    if ([self status] == STATUS_DEAD || amount < 0)
        return;
...
- (ShipEntity*)launchEscapeCapsule
{
    ...
    if ([UNIVERSE displayGUI])
        [self switchToMainView]; // Clear the F7 screen!
    [UNIVERSE setViewDirection:VIEW_FORWARD];
...
{
    if ([self status] == STATUS_DEAD)
        return; // no bounty if we died while trying

    if (other == nil || [other isSubEntity])
        return;
...
{
    if ([self isDocked])
        return; // Can't die while docked. (Doing so would cause breakage elsewhere.)
...
- (void)enterDock:(StationEntity*)station
{
    ...
    if ([self status] == STATUS_LAUNCHING)
        return; // a JS script has aborted the docking.
...
- (void)docked
{
    ...
    if (dockedStation == [UNIVERSE station]) {
        ...
        if (being_fined && ![[UNIVERSE sun] willGoNova] && ![dockedStation suppressArrivalReports])
            [self getFined];
    }

    // it's time to check the script - can trigger legacy missions
    if (gui_screen != GUI_SCREEN_MISSION)
        [self checkScript]; // a scripted pilot could have created a mission screen.
...
- (void)setGuiToStatusScreen
{
    ...
    if (concealment >= OO_SYSTEMCONCEALMENT_NONAME)
        targetSystemName = DESC(@"status-unknown-system");
    ...
    if (nextHop != target_system_id) {
        ...
        if (concealment >= OO_SYSTEMCONCEALMENT_NONAME)
            nextHopSystemName = DESC(@"status-unknown-system");
    ...
    if (guiChanged) {
        ...
        if (bgDescriptor == nil)
            bgDescriptor = [UNIVERSE screenTextureDescriptorForKey:@"status"];
...
- (void)setGuiToEquipShipScreen:(int)skipParam selectingFacingFor:(NSString*)eqKeyForSelectFacing
{
    ...
    if (eqKeyForSelectFacing != nil) // Weapons purchase subscreen.
    {
        ...
        for (eqEnum = [OOEquipmentType equipmentEnumeratorOutfitting]; (eqType = [eqEnum nextObject]); i++) {
            ...
            if (minTechLevel != 0 && [self hasEquipmentItem:[eqType damagedIdentifier]])
                minTechLevel--;
            ...
            if (isOK) {
                if (techlevel < minTechLevel)
                    isOK = NO;
                if (![self canAddEquipment:eqKey inContext:@"purchase"])
                    isOK = NO;
                if (available_facings == 0 && [eqType isPrimaryWeapon])
                    isOK = NO;
                if (isOK)
                    [equipmentAllowed addObject:eqKey];
            }
    ...
    // GUI stuff
    {
        ...
        if (count > 0) {
            ...
            for (i = skip; i < count && (row - start_row < (OOGUIRow)n_rows); i++) {
                ...
                if (dispCol == nil)
                    dispCol = [gui colorFromSetting:kGuiEquipmentOptionColor defaultValue:nil];
...
- (void)showInformationForSelectedUpgradeWithFormatString:(NSString*)formatString
{
    ...
    if (eqKey) {
        if (![eqKey hasPrefix:@"More:"]) {
            ...
            if (formatString)
                desc = [NSString stringWithFormat:formatString, desc];
...
- (void)setGuiToIntroFirstGo:(BOOL)justCobra
{
    ...
    if (justCobra) {
        ...
        if (errors != nil) {
            ...
            for (i--; i >= ms_start; i--)
                [gui setColor:[OOColor redColor] forRow:i];
            msgLine++;
        }
...
- (BOOL)tryBuyingItem:(NSString*)eqKey
{
    ...
    if ([eqKey isEqualToString:@"EQ_RENOVATION"]) {
        ...
        if (techLevel == NSNotFound)
            techLevel = [[UNIVERSE currentSystemData] oo_unsignedIntForKey:KEY_TECHLEVEL];
...
- (BOOL)changePassengerBerths:(int)addRemove
{
    ...
    // unsigned 	passenger_space = [[OOEquipmentType equipmentTypeWithIdentifier:@"EQ_PASSENGER_BERTH"] requiredCargoSpace];
    // if (passenger_space == 0) passenger_space = PASSENGER_BERTH_SPACE;
    if ((max_passengers < 1 && addRemove == -1) || ([self maxAvailableCargoSpace] - current_cargo < PASSENGER_BERTH_SPACE && addRemove == 1))
        return NO;
...
{
    OOMassUnit unit = [shipCommodityData massUnitForGood:type];
    if ([self specialCargo] && unit == UNITS_TONS)
        return 0; // don't do anything if we've got a special cargo...
    ...
    // eg: with maxAvailableCargoSpace 2 & gold 1499kg, you can still add 1 ton alloy.
        ...
        if (safeAmount < amount)
            amount = (safeAmount < oldAmount) ? oldAmount : safeAmount;
...
- (void)showMarketScreenHeaders
{
    ...
    [gui setArray:[NSArray arrayWithObjects:DESC(@"commodity-column-title"), OOPadStringToEms(DESC(@"price-column-title"), 3.5),
                      OOPadStringToEms(DESC(@"for-sale-column-title"), 3.75), OOPadStringToEms(DESC(@"in-hold-column-title"), 5.75), DESC(@"oolite-legality-column-title"), DESC(@"oolite-extras-column-title"), nil]
    ...
    [gui setArray:[NSArray arrayWithObjects:DESC(@"commodity-column-title"), DESC(@"oolite-extras-column-title"), OOPadStringToEms(DESC(@"price-column-title"), 3.5),
                      OOPadStringToEms(DESC(@"for-sale-column-title"), 3.75), OOPadStringToEms(DESC(@"in-hold-column-title"), 5.75), DESC(@"oolite-legality-column-title"), nil]
           forRow:GUI_ROW_MARKET_KEY];
}
...
- (NSString*)marketScreenTitle
{
    ...
    NSString* system = nil;
    if ([UNIVERSE sun] != nil)
        system = [UNIVERSE getSystemName:system_id];
...
- (void)setGuiToMarketScreen
{
    ...
    // GUI stuff
    {
        ...
        if (current_cargo > [self maxAvailableCargoSpace])
            current_cargo = [self maxAvailableCargoSpace];
...
{
    if ([equipmentKey isEqualToString:@"EQ_RENOVATION"] && !(ship_trade_in_factor < 85 || [[[self shipSubEntityEnumerator] allObjects] count] < [self maxShipSubEntities]))
        return NO;
    if (![super canAddEquipment:equipmentKey inContext:context])
        return NO;

    NSArray* conditions = [[OOEquipmentType equipmentTypeWithIdentifier:equipmentKey] conditions];
    if (conditions != nil && ![self scriptTestConditions:conditions])
        return NO;
    ...
    for (i = 0; i < [eqScripts count]; i++) {
        if ([[[eqScripts oo_arrayAtIndex:i] oo_stringAtIndex:0] isEqualToString:equipmentKey]) {
            ...
            for (j = 0; j < [customEquipActivation count]; j++) {
                ...
                if ([[item oo_stringForKey:CUSTOMEQUIP_EQUIPKEY] isEqualToString:equipmentKey])
                    return;
            }
...
- (BOOL)hasOneEquipmentItem:(NSString*)itemKey includeMissiles:(BOOL)includeMissiles
{
    ...
    if ([super hasOneEquipmentItem:itemKey includeMissiles:NO whileLoading:NO])
        return YES;
    ...
    if (includeMissiles) {
        unsigned i;
        for (i = 0; i < max_missiles; i++) {
            if ([[self missileForPylon:i] hasPrimaryRole:itemKey])
                return YES;
        }
    }
...
- (BOOL)removeFromPylon:(NSUInteger)pylon
{
    ...
    if (missile_entity[pylon] != nil) {
        ...
        if (pylon <= activeMissile) {
            if (activeMissile == missiles && missiles > 0)
                activeMissile--;
            if (activeMissile > 0)
                activeMissile--;
            else
...
- (void)getFined
{
    ...
    if ([UNIVERSE inInterstellarSpace])
        local_gov = 1; // equivalent to Feudal. I'm assuming any station in interstellar space is military. -- Ahruman 2008-05-29
...
- (BOOL)moveTargetMemoryBy:(NSInteger)delta
{
    ...
    while (i++ < PLAYER_TARGET_MEMORY_SIZE) // limit loops
    {
        ...
        while (idx < 0)
            idx += PLAYER_TARGET_MEMORY_SIZE;
        while (idx >= PLAYER_TARGET_MEMORY_SIZE)
            idx -= PLAYER_TARGET_MEMORY_SIZE;
...
- (BOOL)setExtraGuiScreenKeys:(OOGUIScreenID)gui definition:(OOJSGuiScreenKeyDefinition*)definition
{
    ...
    if (result)
        [extraGuiScreenKeys setObject:[newarray mutableCopy] forKey:[NSString stringWithFormat:@"%d", gui]];
    [newarray release];
    return result;
}
...
{
    return missionChoice && commanderNameString && cdrDetailArray && currentPage && n_key_roll_left && n_key_roll_right && n_key_pitch_forward && n_key_pitch_back && n_key_yaw_left && n_key_yaw_right && n_key_view_forward && n_key_view_aft && n_key_view_port && n_key_view_starboard && n_key_launch_ship && n_key_gui_screen_options && n_key_gui_screen_equipship && n_key_gui_screen_interfaces && n_key_gui_screen_status && n_key_gui_chart_screens && n_key_gui_system_data && n_key_gui_market && n_key_gui_arrow_left && n_key_gui_arrow_right && n_key_gui_arrow_up && n_key_gui_arrow_down && n_key_gui_page_up && n_key_gui_page_down && n_key_gui_select && n_key_increase_speed && n_key_decrease_speed && n_key_inject_fuel && n_key_fire_lasers && n_key_launch_missile && n_key_next_missile && n_key_ecm && n_key_prime_next_equipment && n_key_prime_previous_equipment && n_key_activate_equipment && n_key_mode_equipment && n_key_fastactivate_equipment_a && n_key_fastactivate_equipment_b && n_key_target_missile && n_key_untarget_missile && n_key_target_incoming_missile && n_key_ident_system && n_key_scanner_zoom && n_key_scanner_unzoom && n_key_launch_escapepod && n_key_galactic_hyperspace && n_key_hyperspace && n_key_jumpdrive && n_key_dump_cargo && n_key_rotate_cargo && n_key_autopilot && n_key_autodock && n_key_snapshot && n_key_docking_music && n_key_advanced_nav_array_next && n_key_advanced_nav_array_previous && n_key_info_next_system && n_key_info_previous_system && n_key_map_home && n_key_map_end && n_key_map_next_system && n_key_map_previous_system && n_key_map_info && n_key_map_zoom_in && n_key_map_zoom_out && n_key_system_home && n_key_system_end && n_key_system_next_system && n_key_system_previous_system && n_key_pausebutton && n_key_show_fps && n_key_bloom_toggle && n_key_mouse_control_roll && n_key_mouse_control_yaw && n_key_hud_toggle && n_key_comms_log && n_key_prev_compass_mode && n_key_next_compass_mode && n_key_chart_highlight && n_key_market_filter_cycle && n_key_market_sorter_cycle && n_key_market_buy_one && n_key_market_sell_one && n_key_market_buy_max && n_key_market_sell_max && n_key_next_target && n_key_previous_target && n_key_custom_view && n_key_custom_view_zoom_out && n_key_custom_view_zoom_in && n_key_custom_view_roll_left && n_key_custom_view_pan_left && n_key_custom_view_roll_right && n_key_custom_view_pan_right && n_key_custom_view_rotate_up && n_key_custom_view_pan_up && n_key_custom_view_rotate_down && n_key_custom_view_pan_down && n_key_custom_view_rotate_left && n_key_custom_view_rotate_right && n_key_docking_clearance_request && n_key_weapons_online_toggle && n_key_cycle_next_mfd && n_key_cycle_previous_mfd && n_key_switch_next_mfd && n_key_switch_previous_mfd && n_key_oxzmanager_setfilter && n_key_oxzmanager_showinfo && n_key_oxzmanager_copyurl && n_key_oxzmanager_extract &&
#if OO_FOV_INFLIGHT_CONTROL_ENABLED
        n_key_inc_field_of_view && n_key_dec_field_of_view &&
#endif
        n_key_dump_target_state && n_key_dump_entity_list && n_key_debug_full && n_key_debug_collision && n_key_debug_console_connect && n_key_debug_bounding_boxes && n_key_debug_shaders && n_key_debug_off && _sysInfoLight.x && selFunctionIdx && stickFunctions && keyFunctions && customEquipActivation && customActivatePressed && customModePressed && kbdLayouts && showingLongRangeChart && _missionAllowInterrupt && _missionScreenID && _missionTitle && _missionTextEntry;
}
#endif
