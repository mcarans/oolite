...
- (GLfloat)baseMass {
    if (sBaseMass <= 0.0) {
        // First call with initialised mass (in [UNIVERSE setUpInitialUniverse]) is always to the cobra 3, even when
        // starting with a savegame.
        if ([self mass] > 0.0)  // bootstrap the base mass.
        {
            OOLog(@ "fuelPrices", @ "Setting Cobra3 base mass to: %.2f ", [self mass]);
            sBaseMass = [self mass];
        } else {
            // This happened on startup when [UNIVERSE setUpSpace] was called before player init, inside [UNIVERSE
...
- (void)unloadCargoPodsForType:(OOCommodityType)type amount:(OOCargoQuantity)quantity {
    ...
    for (i = n_cargo - 1; (i >= 0 && cargoToGo > 0); i--) {
        ...
        if (co_type == nil || [co_type isEqualToString:type]) {
            ...
            {
                OOLog(@ "player.badCargoPod",
                      @ "Cargo pod %@ has bad commodity type (COMMODITY_UNDEFINED), rejecting.",
                      cargoItem);
                continue;
            }
        }
...
- (void)createCargoPodWithType:(OOCommodityType)type andAmount:(OOCargoQuantity)amount {
    ...
    if (container) {
        ...
        OOLogERR(
            @ "player.loadCargoPods.noContainer", @ "%@", @ "couldn't create a container in [PlayerEntity loadCargoPods]");
...
- (NSPoint)adjusted_chart_centre {
    ...
    double
        scroll_pos;  // cursor coordinate at which we'd want to scoll chart in the direction we're currently considering
    double
        ecc;  // chart centre coordinate we'd want if the cursor was on the edge of the galaxy in the current direction
    ...
    // When fully zoomed in we want to centre chart on chart_centre_coordinates.  When zoomed out we want the chart
    // centred on (128.0, 128.0) so the galaxy fits the screen width.  For intermediate zoom we interpolate.
    acc.x =
        chart_centre_coordinates.x + (128.0 - chart_centre_coordinates.x) * (chart_zoom - 1.0) / (CHART_MAX_ZOOM - 1.0);
    acc.y =
        chart_centre_coordinates.y + (128.0 - chart_centre_coordinates.y) * (chart_zoom - 1.0) / (CHART_MAX_ZOOM - 1.0);

    // If the cursor is out of the centre non-scrolling part of the screen adjust the chart centre.  If the cursor is
    // just at scroll_pos we want to return the chart centre as it is, but if it's at the edge of the galaxy we want the
    // centre positioned so the cursor is at the edge of the screen
    if (chart_focus_coordinates.x - acc.x <= -CHART_SCROLL_AT_X * chart_zoom) {
        ...
        if (scroll_pos >= 256.0) {
            ...
            acc.x = ((chart_focus_coordinates.x - scroll_pos) * ecc + (256.0 - chart_focus_coordinates.x) * acc.x) /
                    (256.0 - scroll_pos);
        }
    }
    if (chart_focus_coordinates.y - acc.y <= -CHART_SCROLL_AT_Y * chart_zoom) {
        ...
        if (scroll_pos >= 256.0) {
            ...
            acc.y = ((chart_focus_coordinates.y - scroll_pos) * ecc + (256.0 - chart_focus_coordinates.y) * acc.y) /
                    (256.0 - scroll_pos);
        }
    }
    return acc;
...
- (void)setSystemID:(OOSystemID)sid {
    system_id = sid;
    galaxy_coordinates = PointFromString([[UNIVERSE systemManager] getProperty:@ "coordinates"
                                                                     forSystem:sid
                                                                      inGalaxy:galaxy_number]);
...
- (void)setTargetSystemID:(OOSystemID)sid {
    ...
    cursor_coordinates = PointFromString([[UNIVERSE systemManager]
         getProperty:@ "coordinates"
        forSystemKey:[UNIVERSE keyForPlanetOverridesForSystem:sid inGalaxy:galaxy_number]]);
}
...
- (void)setInfoSystemID:(OOSystemID)sid moveChart:(BOOL)moveChart {
    if (sid != info_system_id) {
        ...
        if (gui_screen == GUI_SCREEN_LONG_RANGE_CHART || gui_screen == GUI_SCREEN_SHORT_RANGE_CHART) {
            if (moveChart) {
                target_chart_focus = [[UNIVERSE systemManager] getCoordinatesForSystem:info_system_id
                                                                              inGalaxy:galaxy_number];
            }
        } else {
            ...
            if (moveChart) {
                chart_centre_coordinates = [[UNIVERSE systemManager] getCoordinatesForSystem:info_system_id
                                                                                    inGalaxy:galaxy_number];
                target_chart_centre = chart_centre_coordinates;
...
- (void)nextInfoSystem {
    ...
    NSArray *route = [[[UNIVERSE routeFromSystem:system_id toSystem:target_system_id
                                     optimizedBy:ANA_mode] oo_arrayForKey:@ "route"] retain];
    NSUInteger i;
    if (route == nil) {
        [self setInfoSystemID:target_system_id moveChart:YES];
        return;
...
- (BOOL)infoSystemOnRoute {
    NSArray *route = [[UNIVERSE routeFromSystem:system_id toSystem:target_system_id
                                    optimizedBy:ANA_mode] oo_arrayForKey:@ "route"];
    NSUInteger i;
    if (route == nil) {
        return NO;
    }
...
- (NSDictionary *)commanderDataDictionary {
    ...
    [result setObject:[[[NSBundle mainBundle] infoDictionary] objectForKey:@ "CFBundleVersion"]
               forKey:@ "written_by_version"];
    ...
    // Variable requiredCargoSpace not suitable for Oolite as it currently stands: it retroactively changes a savegame
    // cargo space.
    // unsigned 	passenger_space = [[OOEquipmentType equipmentTypeWithIdentifier:@"EQ_PASSENGER_BERTH"]
    // requiredCargoSpace]; if (passenger_space == 0) passenger_space = PASSENGER_BERTH_SPACE;
    ...
    */
    ...
    // docking clearance
    [result setObject:[NSNumber numberWithBool:[UNIVERSE dockingClearanceProtocolActive]]
               forKey:@ "docking_clearance_protocol"];
    ...
    [result setObject:dockedStation != nil ? [dockedStation primaryRole] : (NSString *)@ ""
               forKey:@ "docked_station_role"];
    if (dockedStation) {
    ...
    munge_checksum(galaxy_coordinates.x);
    munge_checksum(galaxy_coordinates.y);
    munge_checksum(credits);
    munge_checksum(fuel);
    munge_checksum(max_cargo);
    munge_checksum(missiles);
    munge_checksum(legalStatus);
    munge_checksum(market_rnd);
...
- (BOOL)setCommanderDataFromDictionary:(NSDictionary *)dict {
    ...
    if ([dict oo_stringForKey:@ "galaxy_id"] != nil) {
        ...
        NSArray *coord_vals = ScanTokensFromString([[UNIVERSE systemManager] getProperty:@ "coordinates"
                                                                               forSystem:system_id
                                                                                inGalaxy:galaxy_number]);
        ...
        coord_vals = ScanTokensFromString([[UNIVERSE systemManager] getProperty:@ "coordinates"
                                                                      forSystem:target_system_id
                                                                       inGalaxy:galaxy_number]);
    ...
    // compatibility for loading 1.80 savegames
    {
        ...
        if ([dict objectForKey:@ "current_system_name"]) {
            ...
            if (system_id == -1)
                system_id = [UNIVERSE findSystemNumberAtCoords:galaxy_coordinates
                                                    withGalaxy:galaxy_number
                                               includingHidden:YES];
        } else {
        ...
        if ([dict objectForKey:@ "target_system_name"]) {
            ...
            if (target_system_id == -1)
                target_system_id = [UNIVERSE findSystemNumberAtCoords:cursor_coordinates
                                                           withGalaxy:galaxy_number
                                                      includingHidden:YES];
        } else {
            target_system_id = [UNIVERSE findSystemNumberAtCoords:cursor_coordinates
    ...
    NSMutableDictionary *equipment =
        [NSMutableDictionary dictionaryWithDictionary:[dict oo_dictionaryForKey:@ "extra_equipment"]];
    ...
    */
    ...
    if ([equipment oo_boolForKey:@ "EQ_ENERGY_BOMB"] &&
        [OOEquipmentType equipmentTypeWithIdentifier:@ "EQ_ENERGY_BOMB"] == nil) {
    ...
    primedEquipment =
        [self eqScriptIndexForKey:[dict oo_stringForKey:@ "primed_equipment"]];  // if key not found primedEquipment is
                                                                                // set to primed-none

    [self setFastEquipmentA:[dict oo_stringForKey:@ "primed_equipment_a" defaultValue:@ "EQ_CLOAKING_DEVICE"]];
    [self
        setFastEquipmentB:[dict oo_stringForKey:@ "primed_equipment_b"
                                   defaultValue:@ "EQ_ENERGY_BOMB"]];  // even though there isn't one, for compatibility.
    ...
    voice_no = [UNIVERSE setVoice:[UNIVERSE voiceNumber:[dict oo_stringForKey:@ "speech_voice" defaultValue:nil]]
                      withGenderM:voice_gender_m];
#endif
        ...
        if ([[contractInfo objectForKey:CARGO_KEY_TYPE] isKindOfClass:[NSNumber class]]) {
            ...
            if (![[UNIVERSE commodities] goodDefined:new_type]) {
                OOLog(
                    ...
                    @ "Cargo contract to deliver %@ could not be loaded from the saved game, as the commodity is no longer defined",
                    new_type);
    ...
    if (max_cargo < max_passengers * PASSENGER_BERTH_SPACE) {
        // Something went wrong. Possibly the save file was hacked to contain more passenger cabins than the available
        // cargo space would allow - Nikos 20110731
    ...
    if (passengers && ([passengers count] > max_passengers)) {
        OOLogWARN(@ "setCommanderDataFromDictionary.inconsistency.passengers",
        ...
        for (NSInteger i = (NSInteger)[passengers count] - 1; i >= max_passengers; i--) {
            [passenger_record
                removeObjectForKey:[[passengers oo_dictionaryAtIndex:i] oo_stringForKey:PASSENGER_KEY_NAME]];
    ...
    if (excessCargo > 0) {
        OOLogWARN(@ "setCommanderDataFromDictionary.inconsistency.cargo",
        ...
        foreach (type, [shipCommodityData goods]) {
            ...
            BOOL roundedTon =
                (units != UNITS_TONS) && ((units == UNITS_KILOGRAMS && oldAmount > MAX_KILOGRAMS_IN_SAFE) ||
                                          (units == UNITS_GRAMS && oldAmount > MAX_GRAMS_IN_SAFE));
            if (roundedTon || (units == UNITS_TONS && oldAmount > 0)) {
                // let's remove stuff
                OOCargoQuantity partAmount = oldAmount;
                toRemove = 0;
                while (remainingExcess > 0 && partAmount > 0) {
                    if (EXPECT_NOT(roundedTon && ((units == UNITS_KILOGRAMS && partAmount > MAX_KILOGRAMS_IN_SAFE) ||
                                                  (units == UNITS_GRAMS && partAmount > MAX_GRAMS_IN_SAFE)))) {
                        toRemove +=
                            (units == UNITS_KILOGRAMS)
                                ? (partAmount > (KILOGRAMS_PER_POD + MAX_KILOGRAMS_IN_SAFE)
                                       ? KILOGRAMS_PER_POD
                                       : partAmount - MAX_KILOGRAMS_IN_SAFE)
                                : (partAmount > (GRAMS_PER_POD + MAX_GRAMS_IN_SAFE) ? GRAMS_PER_POD
                                                                                    : partAmount - MAX_GRAMS_IN_SAFE);
    ...
    OOWeaponFacingSet available_facings = [shipyard_info oo_unsignedIntForKey:KEY_WEAPON_FACINGS
                                                                 defaultValue:[self weaponFacings]];
    ...
    */
            ...
            if (missile_desc != nil && ![missile_desc isEqualToString:@ "NONE"]) {
                ...
                if (amiss) {
                    ...
                    OOLogWARN(
                        ...
                        @ "couldn't find missile with role '%@' in [PlayerEntity setCommanderDataFromDictionary:], missile entry discarded.",
                        missile_desc);
                }
            }
    ...
    } else  // no missile_roles
    {
        for (NSUInteger i = 0; i < missiles; i++) {
            ...
            missile_entity[i] =
                [UNIVERSE newShipWithRole:@ "EQ_MISSILE"];  // retain count = 1 - should be okay as long as we keep a
                                                           // missile with this role in the base package.
        }
    }
    ...
    // custom view no.
    if (_customViews != nil) _customViewIndex = [dict oo_unsignedIntForKey:@ "custom_view_index"] % [_customViews count];
...
- (void)deferredInit {
    ...
    NSAssert([super initWithKey:PLAYER_SHIP_DESC definition:[NSDictionary dictionary]] == self,
    ...
    [hud resetGuis:[NSDictionary dictionaryWithObjectsAndKeys:[NSDictionary dictionary],
                                                              @ "message_gui",
...
- (BOOL)setUpAndConfirmOK:(BOOL)stopOnError saveGame:(BOOL)saveGame {
    ...
    if (EXPECT_NOT(![[OOJavaScriptEngine sharedEngine] reset] &&
                   stopOnError))  // always (try to) reset the engine, then find out if we need to stop.
    {
        /*
    ...
    [OOJSScript jsScriptFromFileNamed:@ "oolite-locale-functions.js" properties:nil];
    ...
    [self setSystemID:[UNIVERSE findSystemNumberAtCoords:[self galaxy_coordinates]
                                              withGalaxy:galaxy_number
                                         includingHidden:YES]];
    ...
    [self setGalacticHyperspaceBehaviourTo:[[UNIVERSE globalSettings] oo_stringForKey:@ "galactic_hyperspace_behaviour"
                                                                         defaultValue:@ "BEHAVIOUR_STANDARD"]];
    [self setGalacticHyperspaceFixedCoordsTo:[[UNIVERSE globalSettings]
                                                 oo_stringForKey:@ "galactic_hyperspace_fixed_coords"
                                                    defaultValue:@ "96 96"]];
...
- (void)completeSetUpAndSetTarget:(BOOL)setTarget {
    ...
    JSContext *context = OOJSAcquireContext();
    [self doWorldScriptEvent:OOJSID("startUp")
                   inContext:context
               withArguments:NULL
                       count:0
                   timeLimit:MAX(0.0,
                                 [[NSUserDefaults standardUserDefaults] oo_floatForKey:@ "start-script-limit-value"
                                                                          defaultValue:kOOJSLongTimeLimit])];
    OOJSRelinquishContext(context);
}

- (void)startUpComplete {
    JSContext *context = OOJSAcquireContext();
    [self doWorldScriptEvent:OOJSID("startUpComplete")
                   inContext:context
               withArguments:NULL
                       count:0
                   timeLimit:kOOJSLongTimeLimit];
    OOJSRelinquishContext(context);
}
...
- (BOOL)setUpShipFromDictionary:(NSDictionary *)shipDict {
    ...
    if (EXPECT(_scaleFactor == 1.0f)) {
        ...
        forwardViewOffset = vector_multiply_scalar(
            [shipDict oo_vectorForKey:@ "view_position_forward" defaultValue:forwardViewOffset], _scaleFactor);
        aftViewOffset = vector_multiply_scalar(
            [shipDict oo_vectorForKey:@ "view_position_aft" defaultValue:aftViewOffset], _scaleFactor);
        portViewOffset = vector_multiply_scalar(
            [shipDict oo_vectorForKey:@ "view_position_port" defaultValue:portViewOffset], _scaleFactor);
        starboardViewOffset = vector_multiply_scalar(
            [shipDict oo_vectorForKey:@ "view_position_starboard" defaultValue:starboardViewOffset], _scaleFactor);
    }
    ...
    script = [OOScript jsScriptFromFileNamed:[shipDict oo_stringForKey:@ "script"] properties:scriptProperties];
    if (script == nil) {
        // Do not switch to using a default value above; we want to use the default script if loading fails.
        script = [OOScript jsScriptFromFileNamed:@ "oolite-default-player-script.js" properties:scriptProperties];
    }
    [script retain];
...
- (GLfloat)lookingAtSunWithThresholdAngleCos:(GLfloat)thresholdAngleCos {
    ...
    for (i = 0; i < ent_count; i++) {
        if (uni_entities[i]->isSunlit) {
            if ([uni_entities[i] isPlanet] || ([uni_entities[i] isShip] && [uni_entities[i] isVisible])) {
                ...
                if (EXPECT(vdir > VIEW_STARBOARD || ![uni_entities[i] isPlayer])) {
                    ...
                    /* BUG: if the shadowing entity is not spherical, this gives over-shadowing. True elsewhere as well,
                     * but not so obvious there. */
                    if (shadow < 1) {
                        return 0.0f;
                    }
                }
     ...
     */
    if (thresholdAngleCos <= measuredCosAbs &&
        measuredCosAbs <= 1.1f)  // angle from viewpoint to sun <= desired threshold
    {
...
- (GLfloat)insideAtmosphereFraction {
    ...
    {
        insideAtmoFrac =
            1.0f - ([self dialAltitude] * (GLfloat)PLAYER_DIAL_MAX_ALTITUDE / (10.0f * (GLfloat)ATMOSPHERE_DEPTH));
    }
...
- (void)update:(OOTimeDelta)delta_t {
    ...
    if (EXPECT_NOT(status == STATUS_START_GAME && gui_screen != GUI_SCREEN_INTRO1 &&
                   gui_screen != GUI_SCREEN_SHIPLIBRARY && gui_screen != GUI_SCREEN_GAMEOPTIONS &&
                   gui_screen != GUI_SCREEN_STICKMAPPER && gui_screen != GUI_SCREEN_STICKPROFILE &&
                   gui_screen != GUI_SCREEN_NEWGAME && gui_screen != GUI_SCREEN_OXZMANAGER &&
                   gui_screen != GUI_SCREEN_LOAD && gui_screen != GUI_SCREEN_KEYBOARD &&
                   gui_screen != GUI_SCREEN_KEYBOARD_CONFIRMCLEAR && gui_screen != GUI_SCREEN_KEYBOARD_CONFIG &&
                   gui_screen != GUI_SCREEN_KEYBOARD_ENTRY && gui_screen != GUI_SCREEN_KEYBOARD_LAYOUT)) {
...
- (void)doBookkeeping:(double)delta_t {
    ...
    if ([self hasCloakingDevice] && cloaking_device_active) {
        ...
        if (energy < CLOAKING_DEVICE_MIN_ENERGY) [self deactivateCloakingDevice];
    }
    ...
    if ([self hasMilitaryJammer]) {
        ...
        if (military_jammer_active) {
            ...
            if (energy < MILITARY_JAMMER_MIN_ENERGY) military_jammer_active = NO;
        } else {
            if (energy > 1.5 * MILITARY_JAMMER_MIN_ENERGY) military_jammer_active = YES;
        }
    }
    ...
    */
    ...
    if ((forward_shield > fwdMax * 0.25) && (aft_shield > aftMax * 0.25)) {
        ...
        float minEnergyBankLevel = [[UNIVERSE globalSettings] oo_floatForKey:@ "shield_charge_energybank_threshold"
                                                                defaultValue:0.25];
        energyForShields =
            MAX(0.0,
                energy - 0.1 -
                    (maxEnergy *
                     minEnergyBankLevel));  // NB: The - 0.1 ensures the energy value does not 'bounce' across the
                                            // critical energy message and causes spurious energy-low warnings
    }
    ...
    if (sun) {
        ...
        if ([sun willGoNova]) external_temp *= 3;
    ...
    if ((status != STATUS_ESCAPE_SEQUENCE) && (status != STATUS_ENTERING_WITCHSPACE)) {
        ...
        if (external_temp > heatThreshold && external_temp > ship_temperature)
            ...
            if (ship_temperature > SHIP_MIN_CABIN_TEMP)
                ship_temperature +=
                    (float)((external_temp - heatThreshold - ship_temperature) * SHIP_COOLING_FACTOR * deltaInsulation);
        }

        if (ship_temperature > SHIP_MAX_CABIN_TEMP) [self takeHeatDamage:delta_t * ship_temperature];
    }
    ...
    if ((status == STATUS_ESCAPE_SEQUENCE) && (shot_time > ESCAPE_SEQUENCE_TIME)) {
        ...
        // reset legal status again! Could have changed if a previously launched missile hit a clean NPC while in the
        // escape pod.
        ...
        Entity *dockTargetEntity =
            [UNIVERSE entityForUniversalID:_dockTarget];  // main station in the original system, unless overridden.
    ...
    if (hyperspeed_engaged) {
        ...
        if (flightSpeed > maxFlightSpeed * HYPERSPEED_FACTOR) flightSpeed = (float)(maxFlightSpeed * HYPERSPEED_FACTOR);

        // check for mass lock
        hyperspeed_locked = [self massLocked];
        // check for mass lock & external temperature?
        // hyperspeed_locked = flightSpeed * air_friction > 40.0f+(ship_temperature - external_temp ) *
        // SHIP_COOLING_FACTOR || [self massLocked];
        ...
        if (afterburner_engaged) {
            ...
            if (flightSpeed > maxInjectionSpeed) {
                ...
                if (flightSpeed < maxInjectionSpeed) flightSpeed += (float)(speed_delta * delta_t * abFactor);
                if (flightSpeed > maxInjectionSpeed) flightSpeed = maxInjectionSpeed;
            }
            fuel_accumulator -= (float)(delta_t * afterburner_rate);
            while ((fuel_accumulator < 0) && (fuel > 0)) {
                fuel_accumulator += 1.0f;
                if (--fuel <= MIN_FUEL) afterburner_engaged = NO;
            }
        } else {
            ...
            if (travelling_at_hyperspeed) {
                ...
                if (flightSpeed < maxFlightSpeed) flightSpeed = maxFlightSpeed;
            }
        }
    }
    ...
    if (primeTarget && HPdistance2([primeTarget position], [self position]) > SCANNER_MAX_RANGE2 &&
        !autopilot_engaged) {
    ...
    if (mouseWheelDelta > 0.0f) {
        if (mouseWheelDelta < delta_t)
            [gView setMouseWheelDelta:0.0f];
        else
            [gView setMouseWheelDelta:mouseWheelDelta - delta_t];
    ...
    for (i = 0; i < ent_count; i++)  // scanner lollypops
    {
        ...
        if (EXPECT_NOT(needHyperspeedNearest)) {
            ...
            if (scannedEntity != self && [scannedEntity canCollide] &&
                (![scannedEntity isShip] || ![self collisionExceptedFor:(ShipEntity *)scannedEntity])) {
        ...
        if (scannedEntity->zero_distance < SCANNER_MAX_RANGE2 || !scannedEntity->isShip) {
            ...
            if ([self massLockable] /*|| alertFlags > ALERT_FLAG_YELLOW_LIMIT*/) {
                massLocked |= [self checkEntityForMassLock:scannedEntity
                                             withScanClass:theirClass];  // we just need one masslocker..
            }
...
- (void)updateAlertCondition {
    ...
    if (cond == ALERT_CONDITION_DOCKED || cond == ALERT_CONDITION_GREEN ||
        (cond == ALERT_CONDITION_YELLOW && energy == maxEnergy)) {
        ...
        fleeing_status = PLAYER_FLEEING_NONE;
    } else if ((fleeing_status == PLAYER_FLEEING_MAYBE || fleeing_status == PLAYER_FLEEING_UNLIKELY) &&
               cargo_dump_time > last_shot_time) {
    ...
    } else if (fleeing_status == PLAYER_FLEEING_NONE && cond == ALERT_CONDITION_RED && last_shot_time + 10 < t &&
               flightSpeed > 0.75 * maxFlightSpeed) {
        fleeing_status = PLAYER_FLEEING_MAYBE;
    } else if ((fleeing_status == PLAYER_FLEEING_MAYBE || fleeing_status == PLAYER_FLEEING_CARGO) &&
               cond == ALERT_CONDITION_RED && last_shot_time + 10 < t && flightSpeed > 0.75 * maxFlightSpeed &&
               energy < maxEnergy * 0.5 &&
               (forward_shield < [self maxForwardShieldLevel] * 0.25 || aft_shield < [self maxAftShieldLevel] * 0.25)) {
...
- (void)updateClocks:(OOTimeDelta)delta_t {
    ...
    if (ship_clock_adjust > 0.0)  // adjust for coming out of warp (add LY * LY hrs)
    {
        ...
        if (ship_clock_adjust > 0) {
            if (fine_adjust > ship_clock_adjust) fine_adjust = ship_clock_adjust;
    ...
    if (ship_clock > fps_check_time) {
        if (![self clockAdjusting]) {
            fps_counter = (int)([UNIVERSE timeAccelerationFactor] *
                                floor([UNIVERSE framesDoneThisUpdate] / (fps_check_time - last_fps_check_time)));
...
- (void)requestDockingClearance:(StationEntity *)stationForDocking {
    if (dockingClearanceStatus != DOCKING_CLEARANCE_STATUS_REQUESTED &&
        dockingClearanceStatus != DOCKING_CLEARANCE_STATUS_GRANTED) {
...
- (void)cancelDockingRequest:(StationEntity *)stationForDocking {
    ...
    if (dockingClearanceStatus == DOCKING_CLEARANCE_STATUS_GRANTED ||
        dockingClearanceStatus == DOCKING_CLEARANCE_STATUS_REQUESTED) {
        NSString *stationDockingClearanceStatus = [stationForDocking acceptDockingClearanceRequestFrom:self];
        if (stationDockingClearanceStatus != nil &&
            [stationDockingClearanceStatus isEqualToString:@ "DOCKING_CLEARANCE_CANCELLED"]) {
...
- (BOOL)engageAutopilotToStation:(StationEntity *)stationForDocking {
    ...
    if ([self status] == STATUS_WITCHSPACE_COUNTDOWN)
        [self cancelWitchspaceCountdown];  // cancel witchspace countdown properly
...
- (void)performInFlightUpdates:(OOTimeDelta)delta_t {
    ...
    if (velmag > 0) {
        ...
        if (velmag > VELOCITY_CLEANUP_MIN) {
            ...
            if (velmag > VELOCITY_CLEANUP_FULL)
                rate = VELOCITY_CLEANUP_RATE;
            else
                rate = (velmag - VELOCITY_CLEANUP_MIN) / (VELOCITY_CLEANUP_FULL - VELOCITY_CLEANUP_MIN) *
        ...
        if (velmag2 < 0.0f)
            ...
            velocity = vector_multiply_scalar(velocity, velmag2 / velmag);
    }
...
- (void)performWitchspaceExitUpdates:(OOTimeDelta)delta_t {
    if ([UNIVERSE breakPatternOver]) {
        ...
        if ([UNIVERSE planet]) {
            ...
            if ([UNIVERSE inInterstellarSpace])
                [UNIVERSE addMessage:DESC(@ "witch-engine-malfunction")
                            forCount:3.0];  // if sun gone nova, print nothing
        }
...
- (void)performLaunchingUpdates:(OOTimeDelta)delta_t {
    ...
    if ([UNIVERSE breakPatternOver]) {
        ...
        [self setDockingClearanceStatus:DOCKING_CLEARANCE_STATUS_NONE];
        StationEntity *stationLaunchedFrom = [UNIVERSE nearestEntityMatchingPredicate:IsStationPredicate
                                                                            parameter:NULL
...
- (void)gameOverFadeToBW {
    float secondsToBWFadeOut = [[NSUserDefaults standardUserDefaults] oo_floatForKey:@ "gameover-seconds-to-bw-fadeout"
                                                                        defaultValue:5.0f];
    if ([UNIVERSE detailLevel] >= DETAIL_LEVEL_SHADERS && secondsToBWFadeOut > 0.0f) {
        ...
        if ([self shotTime] < secondsToBWFadeOut) {
            ...
            if (fps_counter != 0) {
                [gameView adjustColorSaturation:-(originalColorSaturation * (1.0f / secondsToBWFadeOut) *
                                                  [UNIVERSE timeAccelerationFactor] / fps_counter)];
            }
        }
...
- (BOOL)isValidTarget:(Entity *)target {
    ...
    if (target->zero_distance > SCANNER_MAX_RANGE2) return NO;
    ...
    if ([target isShip]) {
        ...
        if (tstatus == STATUS_ENTERING_WITCHSPACE || tstatus == STATUS_IN_HOLD ||
            tstatus == STATUS_DOCKED) {  // checks for ships entering wormholes, docking, or been scooped
            return NO;
        }
        return YES;
    }
...
- (void)showGameOver {
    ...
    NSString *scoreMS =
        [NSString stringWithFormat:OOExpandKey(@ "gameoverscreen-score-@"), KillCountToRatingAndKillString(ship_kills)];
...
- (void)showShipModelWithKey:(NSString *)shipKey
    ...
    if (context)
        OOLog(@ "script.debug.note.showShipModel", @ "::::: showShipModel:'%@' in context: %@.", [ship name], context);
...
- (void)updateTargeting {
    ...
    if (missile_status != MISSILE_STATUS_SAFE) {
        ...
        for (i = 0; i < max_missiles; i++) {
            if ([missile_entity[i] primaryTarget] != nil && ![self isValidTarget:[missile_entity[i] primaryTarget]]) {
                [UNIVERSE addMessage:DESC(@ "target-lost") forCount:3.0];
                [self playTargetLost];
    ...
    if ([self primaryTarget] == nil && (ident_engaged || missile_status != MISSILE_STATUS_SAFE) &&
            ...
            case WH_SCANINFO_COLLAPSE_TIME:
                if ([self clockTimeAdjusted] > [wh scanTime] + 4) {
                    ...
                    [UNIVERSE addCommsMessage:[NSString stringWithFormat:DESC(@ "wormhole-arrival-time-computed-@"),
                                                                         ClockToString([wh estimatedArrivalTime], NO)]
                                     forCount:5.0];
                }
                break;
...
- (void)applyRoll:(GLfloat)roll1 andClimb:(GLfloat)climb1 {
    if (roll1 == 0.0 && climb1 == 0.0 && hasRotated == NO) return;

    if (roll1) quaternion_rotate_about_z(&orientation, -roll1);
    if (climb1) quaternion_rotate_about_x(&orientation, -climb1);
...
- (HPVector)breakPatternPosition {
    return HPvector_add(
        position, vectorToHPVector(quaternion_rotate_vector(quaternion_conjugate(orientation), forwardViewOffset)));
}
...
- (HPVector)viewpointPosition {
    ...
    viewpoint.x += offset.x * r.m[0][0];
    viewpoint.y += offset.x * r.m[1][0];
    viewpoint.z += offset.x * r.m[2][0];
    viewpoint.x += offset.y * r.m[0][1];
    viewpoint.y += offset.y * r.m[1][1];
    viewpoint.z += offset.y * r.m[2][1];
    viewpoint.x += offset.z * r.m[0][2];
    viewpoint.y += offset.z * r.m[1][2];
    viewpoint.z += offset.z * r.m[2][2];
...
- (BOOL)switchHudTo:(NSString *)hudFileName {
    ...
    if (hudDict == nil) {
        OOLog(@ "PlayerEntity.switchHudTo.failed",
              @ "HUD dictionary file %@ to switch to not found or invalid.",
              hudFileName);
        return NO;
    }
...
- (GLfloat)dialRoll {
    ...
    if ((result < 1.0f) && (result > -1.0f)) return result;
...
- (GLfloat)dialFuel {
    ...
    if (fuel > [self fuelCapacity]) return 1.0f;
...
#ifndef NDEBUG
    result = [NSString stringWithFormat:@ "%@ (%d, %zu KiB, avg %zu bytes)",
                                        result,
                                        gLiveEntityCount,
                                        gTotalEntityMemory >> 10,
                                        gTotalEntityMemory / gLiveEntityCount];
#endif
...
- (unsigned)countMissiles {
    ...
    unsigned i;
    for (i = 0; i < max_missiles; i++) {
        if (missile_entity[i]) n_missiles++;
    }
    return n_missiles;
}
...
- (OOMissileStatus)dialMissileStatus {
    if ([self weaponsOnline]) {
        ...
        if (missile_status != MISSILE_STATUS_SAFE) {
            OOLogERR(@ "player.missilesUnsafe",
                     @ "%@",
...
- (OOFuelScoopStatus)dialFuelScoopStatus {
    ...
    if ([self hasScoop]) {
        if (scoopsActive) return SCOOP_STATUS_ACTIVE;
        if (cargoOnBoard >= [self maxAvailableCargoSpace] || specialCargo) return SCOOP_STATUS_FULL_HOLD;
...
- (void)addRoleForAggression:(ShipEntity *)victim {
    if ([victim isExplicitlyUnpiloted] || [victim isHulk] || [victim hasHostileTarget] ||
        [[victim primaryAggressor] isPlayer]) {
        return;
    }
    NSString *role = nil;
...
- (void)validateCompassTarget {
        ...
        && ![the_sun goneNova])                    // and the system has not been novabombed
    {
        ...
        if (EXPECT_NOT(new_target != [self compassTarget])) {
            ...
            [self
                doScriptEvent:OOJSID("compassTargetChanged")
                withArguments:[NSArray arrayWithObjects:new_target, OOStringFromCompassMode([self compassMode]), nil]];
        }
    }
}
...
- (void)safeAllMissiles {
    ...
    unsigned i;
    for (i = 0; i < max_missiles; i++) {
        if (missile_entity[i] && [missile_entity[i] primaryTarget] != nil) [missile_entity[i] removeTarget:nil];
    }
            ...
            if (missile_status != MISSILE_STATUS_SAFE) {
                ...
                if ([missile_entity[activeMissile] isMissile]) {
                    if ([self hasEquipmentItemProviding:@ "EQ_MULTI_TARGET"] &&
                        ...
                        // never inherit target if we have EQ_MULTI_TARGET installed! [ Bug #16221 : Targeting
...
- (ShipEntity *)launchMine:(ShipEntity *)mine {
    ...
    [self dumpItem:mine];  // includes UNIVERSE addEntity: CLASS_CARGO, STATUS_IN_FLIGHT, AI state GLOBAL ( the last one
                           // starts the timer !)
...
- (void)currentWeaponStats {
    ...
    // Did find & correct a minor mismatch between player and NPC weapon stats. This is the resulting code - Kaks
    // 20101027
...
- (GLfloat)doesHitLine:(HPVector)v0:(HPVector)v1:(ShipEntity **)hitEntity {
    if (hitEntity) hitEntity[0] = (ShipEntity *)nil;
    ...
    Vector w0 =
        make_vector(dot_product(u0, v_right), dot_product(u0, v_up), dot_product(u0, v_forward));  // in ijk vectors
...
- (void)takeEnergyDamage:(double)amount
    ...
    if (cascading)
        energy =
            0.0;  // explicitly set energy to zero when cascading, in case an oxp raised the energy in noteTakingDamage.
    ...
    if (energy <= 0.0)  // use normal ship temperature calculations for heat damage
    {
        ...
        while (amount > 0.0) {
            internal_damage =
                ((ranrot_rand() & PLAYER_INTERNAL_DAMAGE_FACTOR) < amount);  // base chance of damage to systems
            if (internal_damage) {
...
- (ProxyPlayerEntity *)createDoppelganger {
    ProxyPlayerEntity *result = (ProxyPlayerEntity *)[[UNIVERSE newShipWithName:[self shipDataKey]
                                                                 usePlayerProxy:YES] autorelease];
...
- (ShipEntity *)launchEscapeCapsule {
    ...
    */
    Vector launchVector =
        vector_add([doppelganger velocity],
                   vector_add(vector_multiply_scalar(v_up, 15.0f), vector_multiply_scalar(v_forward, -90.0f)));
    ...
    [self doScriptEvent:OOJSID("shipLaunchedEscapePod")
           withArgument:escapePod];  // no player.ship properties should be available to script
...
- (BOOL)takeInternalDamage {
    ...
    unsigned n_considered = (n_cargo + n_mass) * ship_trade_in_factor /
                            100;  // a lower value of n_considered means more vulnerable to damage.
...
- (void)loseTargetStatus {
    ...
    Entity *my_entities[ent_count];
    int i;
    for (i = 0; i < ent_count; i++) my_entities[i] = [uni_entities[i] retain];  //	retained
    for (i = 0; i < ent_count; i++) {
...
- (void)docked {
    ...
    if ([dockedStation marketMonitored]) {
        ...
        [self markAsOffender:[dockedStation legalStatusOfManifest:shipCommodityData export:NO]
                  withReason:kOOLegalStatusReasonIllegalImports];
    ...
    // check contracts
    NSString *passengerAndCargoReport =
        [self checkPassengerContracts];  // Is also processing cargo and parcel contracts.
    ...
    if ([dockedStation requiresDockingClearance] && ![self clearedToDock] && ![[UNIVERSE sun] willGoNova]) {
...
- (void)leaveDock:(StationEntity *)station {
    ...
    if ([station marketMonitored]) {
        ...
        OOCreditsQuantity oldbounty = [self bounty];
        [self markAsOffender:[station legalStatusOfManifest:shipCommodityData export:YES]
                  withReason:kOOLegalStatusReasonIllegalExports];
        if ([self bounty] > oldbounty) {
            [self addRoleToPlayer:@ "trader-smuggler"];
        }
    }
...
- (void)witchStart {
    // chances of entering witchspace with autopilot on are very low, but as Berlios bug #18307 has shown us, entirely
    // possible so in such cases we need to ensure that at least the docking music stops playing
...
- (double)hyperspaceJumpDistance {
    NSPoint targetCoordinates = PointFromString([[UNIVERSE systemManager] getProperty:@ "coordinates"
                                                                            forSystem:[self nextHopTargetSystemID]
                                                                             inGalaxy:galaxy_number]);
    return distanceBetweenPlanetPositions(
        targetCoordinates.x, targetCoordinates.y, galaxy_coordinates.x, galaxy_coordinates.y);
}
...
- (void)enterGalacticWitchspace {
    if (![self witchJumpChecklist:true]) return;
    ...
    ShipScriptEvent(context,
                    self,
                    "shipWillEnterWitchspace",
                    STRING_TO_JSVAL(JS_InternString(context, [[self jumpCause] UTF8String])),
                    INT_TO_JSVAL(destGalaxy));
    OOJSRelinquishContext(context);
    ...
    if (contracts) [contracts removeAllObjects];

    if (parcels) [parcels removeAllObjects];

    // remove any mission destinations for the old galaxy
    if (missionDestinations) [missionDestinations removeAllObjects];
    ...
    if (passengers) {
        ...
        for (i = 0; i < [passengers count]; i++) {
            ...
            NSMutableDictionary *passenger_info =
    ...
    switch (galacticHyperspaceBehaviour) {
        case GALACTIC_HYPERSPACE_BEHAVIOUR_FIXED_COORDINATES:
            system_id = [UNIVERSE findSystemNumberAtCoords:galacticHyperspaceFixedCoords
                                                withGalaxy:galaxy_number
                                           includingHidden:YES];
            break;
    ...
    cursor_coordinates = PointFromString([[UNIVERSE systemManager] getProperty:@ "coordinates"
                                                                     forSystem:system_id
                                                                      inGalaxy:galaxy_number]);
...
- (void)enterWormhole:(WormholeEntity *)w_hole {
    if ([self status] == STATUS_ENTERING_WITCHSPACE || [self status] == STATUS_EXITING_WITCHSPACE) {
    ...
    ShipScriptEvent(context,
                    ...
                    "shipWillEnterWitchspace",
                    STRING_TO_JSVAL(JS_InternString(context, [[self jumpCause] UTF8String])),
                    INT_TO_JSVAL([w_hole destination]));
    OOJSRelinquishContext(context);
    ...
    if (misjump && [self scriptedMisjumpRange] != 0.5) {
        [w_hole setMisjumpWithRange:[self scriptedMisjumpRange]];  // overrides wormholes, if player also had
                                                                   // non-default scriptedMisjumpRange
    }
...
- (void)enterWitchspace {
    ...
    if (ship_trade_in_factor < 80) {
        malfunc_chance -=
            (1 + ranrot_rand() % (81 - ship_trade_in_factor)) / 2;  // increase chance of misjump in worn-out craft
    ...
    if (malfunc && !misjump) {
        ...
        if ([self takeInternalDamage])  // Depending on ship type and loaded cargo, this will be true for 20 - 50% of
                                        // the time.
        {
    ...
    [UNIVERSE addEntity:wormhole];  // Add new wormhole to Universe to let other ships target it. Required for ships
...
- (void)witchJumpTo:(OOSystemID)sTo misjump:(BOOL)misjump {
    ...
    // set clock after "playerWillEnterWitchspace" and before  removeAllEntitiesExceptPlayer, to allow escorts time to
    // follow their mother.
    NSPoint destCoords = PointFromString([[UNIVERSE systemManager] getProperty:@ "coordinates"
                                                                     forSystem:sTo
                                                                      inGalaxy:galaxy_number]);
    double distance =
        distanceBetweenPlanetPositions(destCoords.x, destCoords.y, galaxy_coordinates.x, galaxy_coordinates.y);
    ...
    if (!misjump) {
        ...
        if (randf() < 0.1)
            [self erodeReputation];  // once every 10 misjumps - should be much rarer than successful jumps!
...
- (void)leaveWitchspace {
    ...
    if (galactic_witchjump) {
        [self doScriptEvent:OOJSID("playerEnteredNewGalaxy")
               withArgument:[NSNumber numberWithUnsignedInt:galaxy_number]];
    }
...
- (void)setGuiToStatusScreen {
    ...
    NSDictionary *systemInfo = [[UNIVERSE systemManager] getPropertiesForSystem:target_system_id
                                                                       inGalaxy:galaxy_number];
    ...
    // GUI stuff
    {
        ...
        NSString *legal_desc = nil, *rating_desc = nil, *alert_desc = nil, *fuel_desc = nil, *credits_desc = nil;
    ...
    if (EXPECT_NOT([[NSUserDefaults standardUserDefaults] boolForKey:@ "show-ship-model-in-status-screen"])) {
        [UNIVERSE removeDemoShips];
        [self showShipModelWithKey:[self shipDataKey]
                          shipData:nil
                       personality:[self entityPersonalityInt]
                           factorX:2.5
                           factorY:1.7
                           factorZ:8.0
                         inContext:@ "GUI_SCREEN_STATUS"];
    ...
    if (guiChanged) {
        ...
        if ([self status] == STATUS_DOCKED) {
            ...
            if (alertCondition == ALERT_CONDITION_RED)
                bgDescriptor = [UNIVERSE screenTextureDescriptorForKey:@ "status_red_alert"];
            else
                bgDescriptor = [UNIVERSE screenTextureDescriptorForKey:@ "status_in_flight"];
        }
...
- (NSArray *)equipmentList {
        ...
        if ([eqType isVisible]) {
            if ([eqType canCarryMultiple] && ![eqType isMissileOrMine]) {
                ...
                // all items okay
                    ...
                    if (count == 1) {
                        [quip2 addObject:[NSArray arrayWithObjects:[eqType name],
                    ...
                    // display plural form
                        ...
                        [quip2 addObject:[NSArray arrayWithObjects:alldesc,
                                                                   [NSNumber numberWithBool:YES],
                                                                   [eqType displayColor],
                                                                   nil]];
                    }
                ...
                // all broken, only one installed
                    ...
                    if (prioritiseDamaged) {
                        [quip1 addObject:[NSArray arrayWithObjects:desc,
                                                                   [NSNumber numberWithBool:NO],
                                                                   [eqType displayColor],
                                                                   nil]];
                    } else {
                        [quip2 addObject:[NSArray arrayWithObjects:desc,
                                                                   [NSNumber numberWithBool:NO],
                                                                   [eqType displayColor],
                                                                   nil]];
                    }
                ...
                // some broken, multiple installed
                    ...
                    if (prioritiseDamaged) {
                        [quip1 addObject:[NSArray arrayWithObjects:alldesc,
                                                                   [NSNumber numberWithBool:NO],
                                                                   [eqType displayColor],
                                                                   nil]];
                    } else {
    ...
    if (max_passengers > 0) {
        ...
        [quip2 addObject:[NSArray arrayWithObjects:desc,
                                                   [NSNumber numberWithBool:YES],
                                                   [[OOEquipmentType equipmentTypeWithIdentifier:@ "EQ_PASSENGER_BERTH"]
                                                       displayColor],
                                                   nil]];
    }
    ...
    if (!isWeaponNone(forward_weapon_type)) {
        ...
        [quip2 addObject:[NSArray arrayWithObjects:desc,
                                                   [NSNumber numberWithBool:YES],
                                                   [forward_weapon_type displayColor],
                                                   nil]];
    }
...
- (NSString *)primedEquipmentName:(NSInteger)offset {
    ...
    if (idx == c) {
        ...
        return
            [[OOEquipmentType equipmentTypeWithIdentifier:[[eqScripts oo_arrayAtIndex:idx] oo_stringAtIndex:0]] name];
    }
}
...
- (BOOL)setPrimedEquipment:(NSString *)eqKey showMessage:(BOOL)showMsg {
    ...
    if (primedEquipment == c && !unprimeEq) {
        ...
        if (primedEquipment != current && showMsg == YES) {
            NSString *equipmentName = [[OOEquipmentType
                equipmentTypeWithIdentifier:[[eqScripts oo_arrayAtIndex:primedEquipment] oo_stringAtIndex:0]] name];
            [UNIVERSE addMessage:unprimeEq ? OOExpandKey(@ "equipment-primed-none")
                                           : OOExpandKey(@ "equipment-primed", equipmentName)
                        forCount:2.0];
        }
    }
    return result;
...
- (NSArray *)cargoList {
    ...
    for (cargoEnum = [list objectEnumerator]; (commodity = [cargoEnum nextObject]);) {
        ...
        if (extended) {
            [manifest
                addObject:OOExpandKey(@ "manifest-cargo-quantity-extended", quantity, units, commodityName, containers)];
        } else {
...
- (NSArray *)contractsListForScriptingFromArray:(NSArray *)contracts_array forCargo:(BOOL)forCargo {
    ...
    for (i = 0; i < [contracts_array count]; i++) {
        ...
        if (forCargo) {
            ...
            [contract setObject:[NSNumber numberWithUnsignedInt:[dict oo_intForKey:CARGO_KEY_AMOUNT]]
                         forKey:@ "quantity"];
            ...
            [contract setObject:[NSNumber numberWithUnsignedInt:[dict oo_unsignedIntForKey:CONTRACT_KEY_RISK]]
                         forKey:CONTRACT_KEY_RISK];
        }
        ...
        [contract setObject:[NSNumber numberWithInt:[dict oo_intForKey:CONTRACT_KEY_PREMIUM]]
                     forKey:CONTRACT_KEY_PREMIUM];
...
- (void)setGuiToSystemDataScreenRefreshBackground:(BOOL)refreshBackground {
    ...
    // GUI stuff
    {
        ...
        NSString *government_desc = [infoSystemData
            oo_stringForKey:KEY_GOVERNMENT_DESC
               defaultValue:OODisplayStringFromGovernmentID([infoSystemData oo_intForKey:KEY_GOVERNMENT])];
        NSString *economy_desc =
        ...
        if (concealment >= OO_SYSTEMCONCEALMENT_NODATA) {
            OOGUIRow i = [gui addLongText:OOExpandKey(@ "sysdata-data-on-system-no-data")
                            startingAtRow:15
                                    align:GUI_ALIGN_LEFT];
            missionTextRow = i;
            for (i--; i > 14; --i) {
                [gui setColor:[gui colorFromSetting:kGuiSystemdataDescriptionColor defaultValue:[OOColor greenColor]]
                       forRow:i];
            }
        } else {
            NSPoint infoSystemCoordinates = [[UNIVERSE systemManager] getCoordinatesForSystem:info_system_id
                                                                                     inGalaxy:galaxy_number];
            double distance = distanceBetweenPlanetPositions(
                infoSystemCoordinates.x, infoSystemCoordinates.y, galaxy_coordinates.x, galaxy_coordinates.y);
            ...
            if (ANA_mode != OPTIMIZED_BY_NONE) {
                ...
                if (routeInfo != nil) {
                    ...
                    distanceInfo = [NSString
                        stringWithFormat:@ "%.1f ly / %.1f %@ / %d %@",
                                         routeDistance,
                                         routeTime,
                                         // don't rely on DESC_PLURAL for routeTime since it is of type double
                                         routeTime > 1.05 || routeTime < 0.95 ? DESC(@ "sysdata-route-hours%1")
                                                                              : DESC(@ "sysdata-route-hours%0"),
                                         routeJumps,
            ...
            for (i = 1; i <= 16; i++) {
                ...
                NSString *line = OOExpandKeyWithSeed(infoSystemRandomSeed,
                                                     ln,
                                                     economy_desc,
                                                     government_desc,
                                                     techLevel,
                ...
                if (![line isEqualToString:@ ""]) {
                    ...
                    if ([lines count] == 1) {
                        [gui setArray:[NSArray arrayWithObjects:[lines objectAtIndex:0], nil] forRow:i];
                    }
                    if ([lines count] == 2) {
                        [gui setArray:[NSArray arrayWithObjects:[lines objectAtIndex:0], [lines objectAtIndex:1], nil]
                               forRow:i];
                    }
                    if ([lines count] == 3) {
                        if ([[lines objectAtIndex:2] isEqualToString:@ ""]) {
                            [gui setArray:[NSArray
                                              arrayWithObjects:[lines objectAtIndex:0], [lines objectAtIndex:1], nil]
                                   forRow:i];
                        } else {
                    ...
                    [gui setArray:[NSArray arrayWithObjects:@ "", nil] forRow:i];
                }
            }
...
- (void)setGuiToChartScreenFrom:(OOGUIScreenID)oldScreen {
    ...
    // GUI stuff
    {
        ...
        // refresh the short range chart cache, in case we've just loaded a save game with different local overrides,
        // etc.
        [gui refreshStarChart];
        //[gui setText:targetSystemName forRow:19];
        // distance-f & est-travel-time-f are identical between short & long range charts in standard Oolite, however
        // can be alterered separately via OXPs
        ...
        if (gui_screen == GUI_SCREEN_LONG_RANGE_CHART) {
            NSString *displaySearchString =
                planetSearchString ? [planetSearchString capitalizedString] : (NSString *)@ "";
            [gui setText:[NSString stringWithFormat:DESC(@ "long-range-chart-find-planet-@"), displaySearchString]
    ...
    if (guiChanged) {
        ...
        [gui setBackgroundTextureKey:@ "short_range_chart"];
        if (found_system_id >= 0) {
            [UNIVERSE findSystemCoordinatesWithPrefix:[[UNIVERSE getSystemName:found_system_id] lowercaseString]
                                           exactMatch:YES];
        }
        [self noteGUIDidChangeFrom:oldScreen to:gui_screen];
    }
...
#define OO_SETACCESSCONDITIONFORROW(condition, row)          \
    do {                                                     \
        ...
        [gui clear];
        [gui setTitle:[NSString stringWithFormat:DESC(@ "status-commander-@"),
                                                 [self commanderName]]];  // Same title as status screen.
        ...
        if (displayModeIndex == NSNotFound) {
            OOLogWARN(@ "display.currentMode.notFound",
                      @ "%@",
        ...
        NSString *displayModeString = [self screenModeStringForWidth:modeWidth
                                                              height:modeHeight
        ...
        if ([gameView hdrOutput]) {
            ...
            int brightnessIdx =
                [brightnesses indexOfObject:[NSString stringWithFormat:@ "%d", (int)[gameView hdrMaxBrightness]]];

            if (brightnessIdx == NSNotFound) {
                OOLogWARN(@ "hdr.maxBrightness.notFound",
                          @ "%@",
        ...
        if ([OOSound respondsToSelector:@selector(masterVolume)] && [OOSound isSoundOK]) {
            ...
            if (vol > 0)
                [gui setText:[NSString stringWithFormat:@ "%@%@ ", soundVolumeWordDesc, SliderString(vol)]
        ...
        [gui setText:[NSString stringWithFormat:@ "%@%@ (%.1f) ", gammaWordDesc, SliderString(gamma5), gamma]
        ...
        [gui setText:[NSString stringWithFormat:@ "%@%@ (%d%c) ",
                                                fovWordDesc,
                                                SliderString(fovTicks),
                                                (int)fov,
                                                176 /*176 is the degrees symbol Unicode code point*/]
        ...
        NSString *colorblindModeDesc = [[[UNIVERSE descriptions] oo_arrayForKey:@ "colorblind_mode"]
            oo_stringAtIndex:[UNIVERSE useShaders] ? colorblindMode : 0];
...
#if OOLITE_ESPEAK
        {
            ...
            message = [NSString
                stringWithFormat:@ "%@", DESC(voice_gender_m ? @ "gameoptions-voice-M" : @ "gameoptions-voice-F")];
        ...
        NSString *musicMode = [UNIVERSE descriptionForArrayKey:@ "music-mode"
                                                         index:[[OOMusicController sharedController] mode]];
        ...
        if (![gameView hdrOutput]) {
            ...
            int paperWhiteTicks =
                (int)((paperWhite - MIN_HDR_PAPERWHITE) * 20 / (MAX_HDR_PAPERWHITE - MIN_HDR_PAPERWHITE));
            NSString *paperWhiteWordDesc = DESC(@ "gameoptions-hdr-paperwhite");
            [gui setText:[NSString stringWithFormat:@ "%@%@ (%d) ",
                                                    paperWhiteWordDesc,
                                                    SliderString(paperWhiteTicks),
                                                    (int)paperWhite]
...
- (void)setGuiToLoadSaveScreen {
    ...
    if ([self status] == STATUS_DOCKED) {
        ...
        canLoadOrSave = (([self dockedStation] == [UNIVERSE station] || [[self dockedStation] allowsSaving]) &&
                         !([[UNIVERSE sun] goneNova] || [[UNIVERSE sun] willGoNova]));
    }
    ...
    // GUI stuff
    {
        ...
        if (canQuickSave) first_sel_row = GUI_ROW(, QUICKSAVE);
        ...
        if ([gui setForegroundTextureKey:[self status] == STATUS_DOCKED ? @ "docked_overlay" : @ "paused_overlay"] &&
...
- (OOWeaponFacingSet)availableFacings {
    ...
    unsigned available_facings = [shipyardInfo oo_unsignedIntForKey:KEY_WEAPON_FACINGS
                                                       defaultValue:[self weaponFacings]];  // use defaults  explicitly
...
- (void)setGuiToEquipShipScreen:(int)skipParam selectingFacingFor:(NSString *)eqKeyForSelectFacing {
    ...
    if (dockedStation) {
        ...
        if ([dockedStation equivalentTechLevel] != NSNotFound) techlevel = [dockedStation equivalentTechLevel];
    }
    ...
    [options addObjectsFromArray:[[shipyardInfo oo_dictionaryForKey:KEY_STANDARD_EQUIPMENT]
                                     oo_arrayForKey:KEY_EQUIPMENT_EXTRAS]];
    ...
    if (eqKeyForSelectFacing != nil)  // Weapons purchase subscreen.
    {
        ...
        for (eqEnum = [OOEquipmentType equipmentEnumeratorOutfitting]; (eqType = [eqEnum nextObject]); i++) {
            ...
            // if you have a damaged system you can get it repaired at a tech level one less than that required to buy
            // it
            ...
            if (techlevel < minTechLevel && techlevel + 3 > minTechLevel) {
                ...
                while (minTechLevel > 0 && minTechLevel > originalMinTechLevel - 3 &&
                       !(dayRnd & 7))  // bargain tech days every 1/8 days
                {
                    dayRnd = dayRnd >> 2;
            ...
            if (searchStatus == STATUS_TEST) {
                ...
                if ([eqKey isEqualToString:last_outfitting_key]) searchStatus = isOK ? STATUS_ACTIVE : STATUS_DEAD;
            }
        }
        ...
        if (count > 0) {
            if (skip > 0)  // lose the first row to Back <--
            {
                ...
                if (count <= n_rows || skip < n_rows)
                    ...
                    if (previous < 2) previous = 0;  // if only one previous item, just show it
                }
                ...
                [gui setColor:[gui colorFromSetting:kGuiEquipmentScrollColor defaultValue:[OOColor greenColor]]
                       forRow:row];
            ...
            for (i = skip; i < count && (row - start_row < (OOGUIRow)n_rows); i++) {
                ...
                if ([eqKey isEqual:@ "EQ_FUEL"]) {
                    ...
                    [gui setColor:[gui colorFromSetting:kGuiEquipmentRepairColor defaultValue:[OOColor orangeColor]]
                           forRow:row];
                } else {
                    price = pricePerUnit;
                }
                ...
                if ([eqKeyForSelectFacing isEqualToString:eqKey]) {
                    ...
                    while (facing_count < 5) {
                        ...
                        if (weaponMounted) {
                            [gui setColor:[gui colorFromSetting:kGuiEquipmentLaserFittedColor
                                                   defaultValue:[OOColor colorWithRed:0.0f
                                                                                green:0.6f
                                                                                 blue:0.0f
                                                                                alpha:1.0f]]
                                   forRow:row];
                        } else {
                            [gui setColor:[gui colorFromSetting:kGuiEquipmentLaserColor
                                                   defaultValue:[OOColor greenColor]]
                                   forRow:row];
                        }
                        if (displayRow)  // Always true for the first pass. The first pass is used to display the name
                                         // of the weapon being purchased.
                        {
                            priceString = [NSString stringWithFormat:@ " %@ ", OOCredits(price * multiplier)];

                            [gui setKey:eqKey forRow:row];
                            [gui setArray:[NSArray arrayWithObjects:desc,
                                                                    (facing_count > 0 ? priceString : (NSString *)@ ""),
                                                                    timeString,
                                                                    nil]
                                   forRow:row];
                            row++;
            ...
            if ([gui selectedRow] != start_row) [gui setSelectedRow:start_row];
            ...
            [gui setText:DESC(@ "equip-no-equipment-available-for-purchase")
                  forRow:GUI_ROW_NO_SHIPS
                   align:GUI_ALIGN_CENTER];
            [gui setColor:[gui colorFromSetting:kGuiEquipmentUnavailableColor defaultValue:[OOColor greenColor]]
                   forRow:GUI_ROW_NO_SHIPS];
    ...
    if (eqKey) {
        if (![eqKey hasPrefix:@ "More:"]) {
            ...
            if ([self hasEquipmentItem:eq_key_damaged]) {
                ...
                if ([eqKey hasSuffix:@ "ENERGY_UNIT"] &&
                    ([self hasEquipmentItem:@ "EQ_ENERGY_UNIT_DAMAGED"] || [self hasEquipmentItem:@ "EQ_ENERGY_UNIT"] ||
                     [self hasEquipmentItem:@ "EQ_NAVAL_ENERGY_UNIT_DAMAGED"]))
                    desc = [NSString stringWithFormat:DESC(@ "@-will-replace-other-energy"), desc];
                if (weight > 0)
                    desc = [NSString stringWithFormat:DESC(@ "upgradeinfo-@-weight-d-of-equipment"), desc, weight];
            }
...
- (void)setGuiToInterfacesScreen:(int)skip {
    ...
    NSArray *interfaceKeys =
        [interfaces keysSortedByValueUsingSelector:@selector(interfaceCompare:)];  // sorts by category, then title
    int i;
    ...
    // GUI stuff
    {
        ...
        if (count > 0) {
            if (skip > 0)  // lose the first row to Back <--
            {
                ...
                [gui setKey:[NSString stringWithFormat:@ "More:%d", previous] forRow:row];
                [gui setColor:[gui colorFromSetting:kGuiInterfaceScrollColor defaultValue:[OOColor greenColor]]
                       forRow:row];
                [gui setArray:[NSArray arrayWithObjects:DESC(@ "gui-back"), @ " <-- ", nil] forRow:row];
                row++;
            }
            ...
            [gui setText:DESC(@ "interfaces-no-interfaces-available-for-use")
                  forRow:GUI_ROW_NO_INTERFACES
                   align:GUI_ALIGN_LEFT];
            [gui setColor:[gui colorFromSetting:kGuiInterfaceNoneColor defaultValue:[OOColor greenColor]]
                   forRow:GUI_ROW_NO_INTERFACES];
        ...
        NSString *desc = [NSString stringWithFormat:DESC(@ "interfaces-for-ship-@-and-station-@"),
                                                    [self displayName],
                                                    [[self dockedStation] displayName]];
        [gui setColor:[gui colorFromSetting:kGuiInterfaceHeadingColor defaultValue:nil]
               forRow:GUI_ROW_INTERFACES_HEADING];
...
- (void)activateSelectedInterface {
    ...
    if ([key hasPrefix:@ "More:"]) {
        ...
        if ([gui selectedRow] < 0) [gui setSelectedRow:GUI_ROW_INTERFACES_START];
        if (from_item == 0) [gui setSelectedRow:GUI_ROW_INTERFACES_START + GUI_MAX_ROWS_INTERFACES - 1];
        [self showInformationForSelectedInterface];
...
- (void)noteGUIWillChangeTo:(OOGUIScreenID)toScreen {
    ...
    ShipScriptEvent(context,
                    self,
                    "guiScreenWillChange",
                    OOJSValueFromGUIScreenID(context, toScreen),
                    OOJSValueFromGUIScreenID(context, gui_screen));
    OOJSRelinquishContext(context);
}
...
- (void)noteGUIDidChangeFrom:(OOGUIScreenID)fromScreen to:(OOGUIScreenID)toScreen refresh:(BOOL)refresh {
    ...
    if (fromScreen != toScreen || refresh) {
        ...
        if (toScreen == GUI_SCREEN_SYSTEM_DATA) {
            ...
            [[UNIVERSE sun]
                setSunColor:[OOColor colorWithDescription:[[UNIVERSE systemManager] getProperty:@ "sun_color"
                                                                                      forSystem:info_system_id
                                                                                       inGalaxy:[self galaxyNumber]]]];
        } else {
        ...
        if (![[UNIVERSE gameController] isGamePaused]) {
            ...
            ShipScriptEvent(context,
                            ...
                            "guiScreenChanged",
                            OOJSValueFromGUIScreenID(context, toScreen),
                            OOJSValueFromGUIScreenID(context, fromScreen));
            OOJSRelinquishContext(context);
        }
    }
}
...
- (void)buySelectedItem {
    ...
    if ([key hasPrefix:@ "More:"]) {
        ...
        if (weaponKey != nil) {
            ...
            if ([gui selectedRow] < 0) [gui setSelectedRow:GUI_ROW_EQUIPMENT_START];
            if (from_item == 0) [gui setSelectedRow:GUI_ROW_EQUIPMENT_START + GUI_MAX_ROWS_EQUIPMENT - 1];
            [self showInformationForSelectedUpgrade];
        }
    ...
    if ([itemText isEqual:FORWARD_FACING_STRING]) chosen_weapon_facing = WEAPON_FACING_FORWARD;
    if ([itemText isEqual:AFT_FACING_STRING]) chosen_weapon_facing = WEAPON_FACING_AFT;
    if ([itemText isEqual:PORT_FACING_STRING]) chosen_weapon_facing = WEAPON_FACING_PORT;
    if ([itemText isEqual:STARBOARD_FACING_STRING]) chosen_weapon_facing = WEAPON_FACING_STARBOARD;
    ...
    if ([self tryBuyingItem:key]) {
        ...
        if (credits != old_credits || ![key hasPrefix:@ "EQ_WEAPON_"]) {
            ...
            [self doScriptEvent:OOJSID("playerBoughtEquipment")
                  withArguments:[NSArray
                                    arrayWithObjects:key, [NSNumber numberWithLongLong:(old_credits - credits)], nil]];
...
- (BOOL)tryBuyingItem:(NSString *)eqKey {
    ...
    if ([eqType isPrimaryWeapon]) {
        ...
        if (current_weapon != nil) {
            tradeIn =
                [UNIVERSE getEquipmentPriceForKey:OOEquipmentIdentifierFromWeaponType(current_weapon)] * multiplier;
        }
    ...
    if ([eqKey hasSuffix:@ "MISSILE"] || [eqKey hasSuffix:@ "MINE"]) {
        ...
        if (weapon)
            OOLog(kOOLogBuyMountedOK, @ "Got ship for mounted weapon role %@", eqKey);
        else
            OOLog(kOOLogBuyMountedFailed, @ "Could not find ship for mounted weapon role %@", eqKey);
...
- (BOOL)changePassengerBerths:(int)addRemove {
    ...
    // unsigned 	passenger_space = [[OOEquipmentType equipmentTypeWithIdentifier:@"EQ_PASSENGER_BERTH"]
    // requiredCargoSpace]; if (passenger_space == 0) passenger_space = PASSENGER_BERTH_SPACE;
    if ((max_passengers < 1 && addRemove == -1) ||
        ([self maxAvailableCargoSpace] - current_cargo < PASSENGER_BERTH_SPACE && addRemove == 1))
        return NO;
...
- (OOCreditsQuantity)removeMissiles {
    ...
    for (i = 0; i < missiles; i++) {
        ...
        if (weapon_key != nil) tradeIn += (int)[UNIVERSE getEquipmentPriceForKey:weapon_key];
    }
    ...
    */
    ...
    foreach (good, [shipCommodityData goods]) {
        ...
        if (commodityUnits != UNITS_TONS) {
            ...
            assert(KILOGRAMS_PER_POD > MAX_KILOGRAMS_IN_SAFE &&
                   GRAMS_PER_POD > MAX_GRAMS_IN_SAFE);  // otherwise we're in trouble!

            if (commodityUnits == UNITS_KILOGRAMS)
                quantity = ((KILOGRAMS_PER_POD - MAX_KILOGRAMS_IN_SAFE - 1) + quantity) / KILOGRAMS_PER_POD;
            else
                quantity = ((GRAMS_PER_POD - MAX_GRAMS_IN_SAFE - 1) + quantity) / GRAMS_PER_POD;
        }
...
- (void)showMarketScreenHeaders {
    ...
    [gui setColor:[gui colorFromSetting:kGuiMarketHeadingColor defaultValue:[OOColor greenColor]]
           forRow:GUI_ROW_MARKET_KEY];
    [gui setArray:[NSArray arrayWithObjects:DESC(@ "commodity-column-title"),
                                            OOPadStringToEms(DESC(@ "price-column-title"), 3.5),
                                            OOPadStringToEms(DESC(@ "for-sale-column-title"), 3.75),
                                            OOPadStringToEms(DESC(@ "in-hold-column-title"), 5.75),
                                            DESC(@ "oolite-legality-column-title"),
                                            DESC(@ "oolite-extras-column-title"),
                                            nil]
           forRow:GUI_ROW_MARKET_KEY];
    [gui setArray:[NSArray arrayWithObjects:DESC(@ "commodity-column-title"),
                                            DESC(@ "oolite-extras-column-title"),
...
- (void)showMarketScreenDataLine:(OOGUIRow)row
    ...
    NSString *available =
        OOPadStringToEms(((available_units > 0) ? (NSString *)[NSString stringWithFormat:@ "%d", available_units]
                                                : DESC(@ "commodity-quantity-none")),
                         2.5);

    NSUInteger priceDecimal = pricePerUnit % 10;
    NSString *price = [NSString
        stringWithFormat:@ " %@.%llu ",
                         OOPadStringToEms([NSString stringWithFormat:@ "%lu", (unsigned long)(pricePerUnit / 10)], 2.5),
                         priceDecimal];

    // this works with up to 9999 tons of gemstones. Any more than that, they deserve the formatting they get! :)

    NSString *owned =
        OOPadStringToEms((units_in_hold > 0) ? (NSString *)[NSString stringWithFormat:@ "%d", units_in_hold]
                                             : DESC(@ "commodity-quantity-none"),
                         4.5);
    ...
    [gui setArray:[NSArray arrayWithObjects:desc, extradesc, price, units_available, units_owned, legaldesc, nil]
           forRow:row++];
}
...
- (void)setGuiToMarketScreen {
    ...
    // following changed to work whether docked or not
    NSArray *goods = [self applyMarketSorter:[self applyMarketFilter:[localMarket goods] onMarket:localMarket]
                                    onMarket:localMarket];
    ...
    if (marketSelectedCommodity != nil &&
        ([marketSelectedCommodity isEqualToString:@ "<<<"] || [marketSelectedCommodity isEqualToString:@ ">>>"])) {
        // nothing?
    } else {
    ...
    // GUI stuff
    {
        ...
        if (dockedStation == nil && [[self primaryTarget] isStation] &&
            [(StationEntity *)[self primaryTarget] marketBroadcast]) {
            dockedStation = [self primaryTarget];
        }
        ...
        if ([goods count] > 0) {
            ...
            if (marketOffset < maxOffset) {
                ...
                [gui setColor:[gui colorFromSetting:kGuiMarketScrollColor defaultValue:[OOColor greenColor]]
            ...
            [gui setColor:[gui colorFromSetting:kGuiMarketFilteredAllColor defaultValue:[OOColor yellowColor]]
                   forRow:GUI_ROW_MARKET_START];
        ...
        [gui setColor:[gui colorFromSetting:kGuiMarketFilterInfoColor defaultValue:[OOColor greenColor]]
...
- (void)setGuiToMarketInfoScreen {
    ...
    // GUI stuff
    {
        ...
        [gui setTitle:[NSString stringWithFormat:DESC(@ "oolite-commodity-information-@"),
                                                 [shipCommodityData nameForGood:marketSelectedCommodity]]];

        [self showMarketScreenHeaders];
        [self showMarketScreenDataLine:GUI_ROW_MARKET_START
                               forGood:marketSelectedCommodity
                              inMarket:localMarket
                          holdQuantity:quantityInHold[j]];
        ...
        if (contracted > 0) {
            ...
            [gui setColor:[gui colorFromSetting:kGuiMarketContractedColor defaultValue:nil]
                   forRow:GUI_ROW_MARKET_START + 1];
            [gui setText:[NSString stringWithFormat:DESC(@ "oolite-commodity-contracted-d-@"),
                                                    contracted,
                                                    DisplayStringForMassUnit(unit)]
                  forRow:GUI_ROW_MARKET_START + 1];
        }
        ...
        if (info == nil || [info length] == 0) {
            i = [gui addLongText:DESC(@ "oolite-commodity-no-comment")
                   startingAtRow:GUI_ROW_MARKET_START + 2
                           align:GUI_ALIGN_LEFT];
        } else {
...
- (void)showMarketCashAndLoadLine {
    ...
    [gui setColor:[gui colorFromSetting:kGuiMarketCashColor defaultValue:[OOColor yellowColor]]
...
- (BOOL)tryBuyingCommodity:(OOCommodityType)index all:(BOOL)all {
    ...
    if (unit == UNITS_TONS && purchase + current_cargo > [self maxAvailableCargoSpace]) {
        ...
        if (current_cargo == [self maxAvailableCargoSpace]) {
            ...
            if (unit == UNITS_KILOGRAMS) {
                if (manifest_quantity % KILOGRAMS_PER_POD <= MAX_KILOGRAMS_IN_SAFE &&
                    (manifest_quantity + purchase) % KILOGRAMS_PER_POD > MAX_KILOGRAMS_IN_SAFE) {
                    // going from < n500 to >= n500 would increase pods needed by 1
                    purchase = MAX_KILOGRAMS_IN_SAFE - manifest_quantity;  // max possible
                }
            } else  // UNITS_GRAMS
            {
                if (manifest_quantity % GRAMS_PER_POD <= MAX_GRAMS_IN_SAFE &&
    ...
    [self doScriptEvent:OOJSID("playerBoughtCargo")
          withArguments:[NSArray arrayWithObjects:index,
                                                  [NSNumber numberWithInt:purchase],
                                                  [NSNumber numberWithUnsignedLongLong:pricePerUnit],
                                                  nil]];
...
- (BOOL)trySellingCommodity:(OOCommodityType)index all:(BOOL)all {
    ...
    if (sell > available_units) sell = available_units;                        // limit to what's in the hold
    if (sell + market_quantity > capacity) sell = capacity - market_quantity;  // avoid flooding the market
    if (sell <= 0) return NO;  // stop if that results in nothing to be sold
    ...
    [self doScriptEvent:OOJSID("playerSoldCargo")
          withArguments:[NSArray arrayWithObjects:index,
                                                  [NSNumber numberWithInt:sell],
                                                  [NSNumber numberWithUnsignedLongLong:pricePerUnit],
                                                  nil]];
...
- (BOOL)canAddEquipment:(NSString *)equipmentKey inContext:(NSString *)context {
    if ([equipmentKey isEqualToString:@ "EQ_RENOVATION"] &&
        !(ship_trade_in_factor < 85 || [[[self shipSubEntityEnumerator] allObjects] count] < [self maxShipSubEntities]))
        return NO;
...
- (void)addEquipmentWithScriptToCustomKeyArray:(NSString *)equipmentKey {
    ...
    for (i = 0; i < [eqScripts count]; i++) {
        if ([[[eqScripts oo_arrayAtIndex:i] oo_stringAtIndex:0] isEqualToString:equipmentKey]) {
            ...
            NSMutableDictionary *customKey = [[NSMutableDictionary alloc]
                initWithObjectsAndKeys:equipmentKey, CUSTOMEQUIP_EQUIPKEY, [eq name], CUSTOMEQUIP_EQUIPNAME, nil];

            // grab any default keys from the equipment item
            // default activate
            object = [eq defaultActivateKey];
            if ((object != nil && [object count] > 0)) [customKey setObject:object forKey:CUSTOMEQUIP_KEYACTIVATE];
            // default mode
            object = [eq defaultModeKey];
            if ((object != nil && [object count] > 0)) [customKey setObject:object forKey:CUSTOMEQUIP_KEYMODE];
...
- (void)getFined {
    ...
    if ([UNIVERSE inInterstellarSpace])
        local_gov = 1;  // equivalent to Feudal. I'm assuming any station in interstellar space is military. -- Ahruman
                        // 2008-05-29
...
- (void)setDefaultCustomViews {
    NSArray *customViews =
        [[[OOShipRegistry sharedRegistry] shipInfoForKey:PLAYER_SHIP_DESC] oo_arrayForKey:@ "custom_views"];
...
- (void)removeTrumble:(OOTrumble *)deadTrumble {
    ...
    for (i = 0; (trumble_index == NSNotFound) && (i < trumbleCount); i++) {
        if (trumble[i] == deadTrumble) trumble_index = i;
    }
...
- (id)trumbleValue {
    ...
    return [NSArray arrayWithObjects:[NSNumber numberWithUnsignedInteger:trumbleCount],
                                     [NSNumber numberWithInt:trumbleHash],
                                     trumbleArray,
                                     nil];
}
...
- (void)setTrumbleValueFrom:(NSObject *)trumbleValue {
    ...
    if (trumbleValue) {
        ...
        if (![trumbleValue isKindOfClass:[NSArray class]])
            ...
            if ([values count] >= 1) putativeNTrumbles = [values oo_intAtIndex:0];
            if ([values count] >= 2) putativeHash = [values oo_intAtIndex:1];
            if ([values count] >= 3) putativeTrumbleArray = [values oo_arrayAtIndex:2];
        }
        ...
        if (putativeHash != trumbleHash) info_failed = YES;
...
- (void)customViewPanUp:(OOScalar)angle {
    quaternion_rotate_about_axis(&customViewQuaternion, customViewRightVector, angle);
    [self setCustomViewData];
    customViewRotationCenter = vector_subtract(
        customViewOffset,
        vector_multiply_scalar(customViewForwardVector, dot_product(customViewOffset, customViewForwardVector)));
}
...
- (BOOL)doWorldEventUntilMissionScreen:(jsid)message {
    ...
    if (gui_screen != GUI_SCREEN_MISSION && [dockingReport length] > 0 && [self isDocked] &&
...
- (void)setGalacticHyperspaceFixedCoords:(NSPoint)point {
    return [self setGalacticHyperspaceFixedCoordsX:OOClamp_0_max_f(round(point.x), 255.0f)
                                                 y:OOClamp_0_max_f(round(point.y), 255.0f)];
}
...
- (GLfloat)fuelChargeRate {
    ...
    if (EXPECT(ship_trade_in_factor <= 90 && ship_trade_in_factor >= 75)) {
        ...
        // OOLog(@"fuelPrices", @"\"%@\" - repair status: %d%%, adjusted rate to:%.2f)", [self shipDataKey],
        // ship_trade_in_factor, rate);
    }
...
- (void)setDockTarget:(ShipEntity *)entity {
    if ([entity isStation])
        _dockTarget = [entity universalID];
    else
        _dockTarget = NO_TARGET;
...
- (BOOL)isDocked {
    ...
    if (isDockedStatus) {
        if ([self dockedStation] == nil) {
            ...
            OOLogERR(kOOLogInconsistentState,
                     @ "status is %@, but dockedStation is nil; treating as not docked. %@",
                     OOStringFromEntityStatus([self status]),
        ...
        if ([self dockedStation] != nil && [self status] != STATUS_LAUNCHING) {
            OOLogERR(kOOLogInconsistentState,
                     @ "status is %@, but dockedStation is not nil; treating as docked. %@",
                     OOStringFromEntityStatus([self status]),
                     @ "This is an internal error, please report it.");
            [self setStatus:STATUS_DOCKED];
            isDockedStatus = YES;
...
- (void)penaltyForUnauthorizedDocking {
    ...
    credits -= amountToPay;
    [self addMessageToReport:[NSString stringWithFormat:DESC(@ "station-docking-clearance-fined-@-cr"),
                                                        OOCredits(amountToPay)]];
}
...
- (void)updateWormholes {
    ...
    if ([scannedWormholes count] == 0) return;
        ...
        while (i--) {
            ...
            if (def_existing && [[def_existing name] isEqualToString:[definition name]]) {
                ...
                foreach (key, [keydefs allKeys]) {
                    j = [checklist count];
                    while (j--) {
                        if ([[NSString stringWithFormat:@ "%@", [keydefs objectForKey:key]]
                                isEqualToString:[NSString stringWithFormat:@ "%@", [checklist objectAtIndex:j]]]) {
                            result = NO;
                            OOLog(kOOLogException,
...
- (BOOL)suppressClangStuff {
    return missionChoice && commanderNameString && cdrDetailArray && currentPage && n_key_roll_left &&
           n_key_roll_right && n_key_pitch_forward && n_key_pitch_back && n_key_yaw_left && n_key_yaw_right &&
           n_key_view_forward && n_key_view_aft && n_key_view_port && n_key_view_starboard && n_key_launch_ship &&
           n_key_gui_screen_options && n_key_gui_screen_equipship && n_key_gui_screen_interfaces &&
           n_key_gui_screen_status && n_key_gui_chart_screens && n_key_gui_system_data && n_key_gui_market &&
           n_key_gui_arrow_left && n_key_gui_arrow_right && n_key_gui_arrow_up && n_key_gui_arrow_down &&
           n_key_gui_page_up && n_key_gui_page_down && n_key_gui_select && n_key_increase_speed &&
           n_key_decrease_speed && n_key_inject_fuel && n_key_fire_lasers && n_key_launch_missile &&
           n_key_next_missile && n_key_ecm && n_key_prime_next_equipment && n_key_prime_previous_equipment &&
           n_key_activate_equipment && n_key_mode_equipment && n_key_fastactivate_equipment_a &&
           n_key_fastactivate_equipment_b && n_key_target_missile && n_key_untarget_missile &&
           n_key_target_incoming_missile && n_key_ident_system && n_key_scanner_zoom && n_key_scanner_unzoom &&
           n_key_launch_escapepod && n_key_galactic_hyperspace && n_key_hyperspace && n_key_jumpdrive &&
           n_key_dump_cargo && n_key_rotate_cargo && n_key_autopilot && n_key_autodock && n_key_snapshot &&
           n_key_docking_music && n_key_advanced_nav_array_next && n_key_advanced_nav_array_previous &&
           n_key_info_next_system && n_key_info_previous_system && n_key_map_home && n_key_map_end &&
           n_key_map_next_system && n_key_map_previous_system && n_key_map_info && n_key_map_zoom_in &&
           n_key_map_zoom_out && n_key_system_home && n_key_system_end && n_key_system_next_system &&
           n_key_system_previous_system && n_key_pausebutton && n_key_show_fps && n_key_bloom_toggle &&
           n_key_mouse_control_roll && n_key_mouse_control_yaw && n_key_hud_toggle && n_key_comms_log &&
           n_key_prev_compass_mode && n_key_next_compass_mode && n_key_chart_highlight && n_key_market_filter_cycle &&
           n_key_market_sorter_cycle && n_key_market_buy_one && n_key_market_sell_one && n_key_market_buy_max &&
           n_key_market_sell_max && n_key_next_target && n_key_previous_target && n_key_custom_view &&
           n_key_custom_view_zoom_out && n_key_custom_view_zoom_in && n_key_custom_view_roll_left &&
           n_key_custom_view_pan_left && n_key_custom_view_roll_right && n_key_custom_view_pan_right &&
           n_key_custom_view_rotate_up && n_key_custom_view_pan_up && n_key_custom_view_rotate_down &&
           n_key_custom_view_pan_down && n_key_custom_view_rotate_left && n_key_custom_view_rotate_right &&
           n_key_docking_clearance_request && n_key_weapons_online_toggle && n_key_cycle_next_mfd &&
           n_key_cycle_previous_mfd && n_key_switch_next_mfd && n_key_switch_previous_mfd &&
           n_key_oxzmanager_setfilter && n_key_oxzmanager_showinfo && n_key_oxzmanager_extract &&
#if OO_FOV_INFLIGHT_CONTROL_ENABLED
           n_key_inc_field_of_view && n_key_dec_field_of_view &&
#endif
           n_key_dump_target_state && n_key_dump_entity_list && n_key_debug_full && n_key_debug_collision &&
           n_key_debug_console_connect && n_key_debug_bounding_boxes && n_key_debug_shaders && n_key_debug_off &&
           _sysInfoLight.x && selFunctionIdx && stickFunctions && keyFunctions && customEquipActivation &&
           customActivatePressed && customModePressed && kbdLayouts && showingLongRangeChart &&
           _missionAllowInterrupt && _missionScreenID && _missionTitle && _missionTextEntry;
}
#endif
