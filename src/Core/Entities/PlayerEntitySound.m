...
enum { kBuySellSourcePoolSize = 4, kWarningPoolSize = 6, kWeaponPoolSize = 3, kDamagePoolSize = 4, kMiscPoolSize = 2 };
...
- (void)setUpSound {
    ...
    sDamageSoundPool = [[OOSoundSourcePool alloc]
        initWithCount:kDamagePoolSize
        minRepeatTime:0.1];  // Repeat time limit is to avoid playing a scrape sound every frame on glancing scrapes.
                             // This does limit the number of laser hits that can be played in a furrball, though; maybe
                             // lasers and scrapes should use different pools.
...
- (void)setUpWeaponSounds {
    ...
    // special case: turrets aren't defined with a "EQ_WEAPON" prefix, and plasma shots don't have a matching equipment
    // item, so add a unique entry here. this could be overridden if an OXP creates an equipment item with this key.
...
// grab a local copy of the sound identifiers for weapons to make the process of looking up a sound ref as fast as
// possible but we must ensure that no nil values are used for setObject
