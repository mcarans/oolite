...
- (id)initAsMainPlanetForSystem:(OOSystemID)s {
    ...
    return [self initFromDictionary:planetInfo
                     withAtmosphere:[planetInfo oo_boolForKey:@ "has_atmosphere" defaultValue:YES]
                            andSeed:[[UNIVERSE systemManager] getRandomSeedForSystem:s inGalaxy:[PLAYER galaxyNumber]]
                          forSystem:s];
}
...
- (id)initFromDictionary:(NSDictionary *)dict
          withAtmosphere:(BOOL)atmosphere
                 andSeed:(Random_Seed)seed
               forSystem:(OOSystemID)systemID {
    ...
    _name = nil;
    [self setName:OOExpand([dict oo_stringForKey:KEY_PLANETNAME
                                    defaultValue:[planetInfo oo_stringForKey:KEY_PLANETNAME defaultValue:@ "%H"]])];
    ...
    _lastLaunchTime =
        [UNIVERSE getTime] + 30.0 - _shuttleLaunchInterval;  // launch 30s after player enters universe.
    ...
    _terminatorThresholdVector = [planetInfo oo_vectorForKey:@ "terminator_threshold_vector"
                                                defaultValue:OO_TERMINATOR_THRESHOLD_VECTOR_DEFAULT];

    // Load material parameters, including atmosphere.
    RANROTSeed planetNoiseSeed = RANROTGetFullSeed();
    [planetInfo setObject:[NSValue valueWithBytes:&planetNoiseSeed objCType:@encode(RANROTSeed)]
                   forKey:@ "noise_map_seed"];
...
#if NEW_ATMOSPHERE
    if (atmosphere) {
        // shader atmosphere always has a radius of collision_radius + ATMOSPHERE_DEPTH. For texture atmosphere, we need
        // to check if a shader atmosphere is also used. If yes, set its radius to just cover the planet so that it
        // doesn't conflict with the shader atmosphere at the planet edges. If no shader atmosphere is used, then set it
        // to the standard radius
        double atmosphereRadius =
            [UNIVERSE detailLevel] >= DETAIL_LEVEL_SHADERS ? collision_radius : collision_radius + ATMOSPHERE_DEPTH;
        _atmosphereDrawable = [[OOPlanetDrawable atmosphereWithRadius:atmosphereRadius] retain];
        _atmosphereShaderDrawable =
            [[OOPlanetDrawable atmosphereWithRadius:collision_radius + ATMOSPHERE_DEPTH] retain];

        // convert the atmosphere settings to generic 'material parameters'
        percent_land = 100 - [dict oo_intForKey:@ "percent_cloud"
                                   defaultValue:100 - (3 + (gen_rnd_number() & 31) + (gen_rnd_number() & 31))];
        ...
        // OOLog (@"planet.debug",@" translated air colour:%@ cloud colour:%@ polar cloud color:%@", [_airColor
        // rgbaDescription],[(OOColor *)[planetInfo objectForKey:@"cloud_color"] rgbaDescription],[(OOColor
        // *)[planetInfo objectForKey:@"polar_cloud_color"] rgbaDescription]);

        _materialParameters =
            [planetInfo dictionaryWithValuesForKeys:[NSArray arrayWithObjects:@ "cloud_fraction",
                                                                              @ "air_color",
                                                                              @ "air_color_mix_ratio",
                                                                              @ "air_density",
                                                                              @ "cloud_color",
    ...
    {
        _materialParameters =
            [planetInfo dictionaryWithValuesForKeys:[NSArray arrayWithObjects:@ "land_fraction",
                                                                              @ "land_color",
                                                                              @ "sea_color",
                                                                              @ "polar_land_color",
                                                                              @ "polar_sea_color",
    ...
    if ([dict objectForKey:@ "rotational_velocity"]) {
        _rotationalVelocity = [dict oo_floatForKey:@ "rotational_velocity"
                                      defaultValue:0.01f * randf()];  // 0.0 .. 0.01 avr 0.005
    } else {
        _rotationalVelocity = [planetInfo oo_floatForKey:@ "rotation_speed"
                                            defaultValue:0.005f * randf()];  // 0.0 .. 0.005 avr 0.0025
        _rotationalVelocity *= [planetInfo oo_floatForKey:@ "rotation_speed_factor" defaultValue:1.0f];
    }

    _atmosphereRotationalVelocity = [dict oo_floatForKey:@ "atmosphere_rotational_velocity"
                                            defaultValue:0.01f * randf()];
...
#define CPROP(PROP) \
    OOLog(@ "planetinfo.record", @ #PROP " = %@;", [(OOColor *)[planetInfo objectForKey:@ #PROP] descriptionComponents]);
...
static Vector RandomHSBColor(void) {
    return (Vector){gen_rnd_number() / 256.0, gen_rnd_number() / 256.0, 0.5 + gen_rnd_number() / 512.0};
}

static Vector LighterHSBColor(Vector c) { return (Vector){c.x, c.y * 0.25f, 1.0f - (c.z * 0.1f)}; }
...
- (void)setUpTerrainParametersWithSourceInfo:(NSDictionary *)sourceInfo targetInfo:(NSMutableDictionary *)targetInfo {
    NSArray *keys = [NSArray arrayWithObjects:@ "atmosphere_rotational_velocity",
                                              @ "rotational_velocity",
                                              @ "cloud_alpha",
                                              @ "has_atmosphere",
                                              @ "percent_cloud",
...
- (void)setUpColorParametersWithSourceInfo:(NSDictionary *)sourceInfo
    ...
    if (!isAtmosphere) {
        ...
        if (color != nil)
            landHSB = HSBColorWithColor(color);
        else
        ...
        if (color != nil)
            seaHSB = HSBColorWithColor(color);
        else
        ...
        if (color != nil)
            illumHSB = HSBColorWithColor(color);
        else {
            NSString *illumHSBColorString = [sourceInfo oo_stringForKey:@ "illumination_hsb_color"];
            if (illumHSBColorString)
                ScanVectorFromString(illumHSBColorString, &illumHSB);
            else
                illumHSB = HSBColorWithColor([OOColor colorWithRed:0.8f green:0.8f blue:0.4f alpha:1.0f]);
        }
        ...
        [targetInfo setObject:[NSNumber numberWithFloat:[sourceInfo oo_floatForKey:@ "air_color_mix_ratio"]]
                       forKey:@ "air_color_mix_ratio"];
    }
    terminatorThreshold = [sourceInfo oo_vectorForKey:@ "terminator_threshold_vector"
                                         defaultValue:OO_TERMINATOR_THRESHOLD_VECTOR_DEFAULT];
    [targetInfo
        setObject:[NSString
                      stringWithFormat:@ "%f %f %f", terminatorThreshold.x, terminatorThreshold.y, terminatorThreshold.z]
           forKey:@ "terminator_threshold_vector"];
}
...
- (id)initAsMiniatureVersionOfPlanet:(OOPlanetEntity *)planet {
    ...
    [_atmosphereDrawable setRadius:collision_radius + ATMOSPHERE_DEPTH * PLANET_MINIATURE_FACTOR *
                                                          2.0];  // not to scale: invisible otherwise
    if (_atmosphereShaderDrawable)
        [_atmosphereShaderDrawable setRadius:collision_radius + ATMOSPHERE_DEPTH * PLANET_MINIATURE_FACTOR * 2.0];
...
- (NSString *)descriptionComponents {
    return
        [NSString stringWithFormat:@ "position: %@ radius: %g m", HPVectorDescription([self position]), [self radius]];
}
        ...
        if (EXPECT_NOT(_atmosphereDrawable && cam_zero_distance < _mesopause2)) {
            ...
            if (EXPECT_NOT(alt > 0 && alt <= 1.0))  // ensure aleph is clamped between 0 and 1
            {
                ...
                [UNIVERSE setSkyColorRed:[mixColor redComponent] * aleph2
                                   ...
                                   alpha:aleph];
                double atmosphereRadius =
                    canDrawShaderAtmosphere ? collision_radius : collision_radius + (ATMOSPHERE_DEPTH * alt);
                [_atmosphereDrawable setRadius:atmosphereRadius];
                if (_atmosphereShaderDrawable)
                    [_atmosphereShaderDrawable setRadius:collision_radius + (ATMOSPHERE_DEPTH * alt)];
            ...
            if (EXPECT_NOT([_atmosphereDrawable radius] < collision_radius + ATMOSPHERE_DEPTH)) {
                ...
                if (_atmosphereShaderDrawable)
                    [_atmosphereShaderDrawable setRadius:collision_radius + ATMOSPHERE_DEPTH];
            }
...
- (void)drawImmediate:(bool)immediate translucent:(bool)translucent {
    ...
    if (![UNIVERSE viewFrustumIntersectsSphereAt:cameraRelativePosition
                                      withRadius:([self radius] + ATMOSPHERE_DEPTH)]) {
        // Don't draw
        return;
    }
...
- (BOOL)planetHasStation {
    ...
    station = [UNIVERSE nearestShipMatchingPredicate:IsStationPredicate parameter:nil relativeToEntity:self];
...
- (void)launchShuttle {
    ...
    HPVector launch_pos = HPvector_add(
        position, vectorToHPVector(vector_multiply_scalar(vector_forward_from_quaternion(q1), start_distance)));
...
- (void)setTextureFileName:(NSString *)textureName {
    ...
    if (textureName != nil) {
        NSDictionary *spec = [NSDictionary dictionaryWithObjectsAndKeys:textureName,
                                                                        @ "name",
                                                                        @ "yes",
                                                                        @ "repeat_s",
                                                                        @ "linear",
        ...
        if (shadersOn) {
            ...
            if ([diffuseMap isCubeMap]) {
                macros = [materialDefaults oo_dictionaryForKey:isMoon ? @ "moon-customized-cubemap-macros"
                                                                      : @ "planet-customized-cubemap-macros"];
            } else {
                macros = [materialDefaults
                    oo_dictionaryForKey:isMoon ? @ "moon-customized-macros" : @ "planet-customized-macros"];
            }
        } else
            textureName = @ "dynamic";
        ...
        if (_normSpecMapName) {
            NSDictionary *nspec = [NSDictionary dictionaryWithObjectsAndKeys:_normSpecMapName,
                                                                             @ "name",
                                                                             @ "yes",
                                                                             @ "repeat_s",
                                                                             @ "linear",
            ...
            if (normalMap != nil)  // OOTexture will have logged a file-not-found warning.
            {
                if (shadersOn) {
                    ...
                    if ([normalMap isCubeMap]) {
                        macros = [materialDefaults
                            oo_dictionaryForKey:isMoon ? @ "moon-customized-cubemap-normspec-macros"
                                                       : @ "planet-customized-cubemap-normspec-macros"];
                    } else {
                        macros = [materialDefaults oo_dictionaryForKey:isMoon ? @ "moon-customized-normspec-macros"
    ...
    if (!isMoon) {
        if (shadersOn) {
            NSMutableDictionary *aConfig =
                [[[materialDefaults oo_dictionaryForKey:@ "atmosphere-material"] mutableCopy] autorelease];
        ...
        [OOStandaloneAtmosphereGenerator generateAtmosphereTexture:&atmosphere withInfo:_materialParameters];
    ...
    if (shadersOn) {
        NSMutableDictionary *config =
            [[[materialDefaults oo_dictionaryForKey:@ "planet-material"] mutableCopy] autorelease];
