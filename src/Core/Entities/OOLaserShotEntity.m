...
- (instancetype)initLaserFromShip:(ShipEntity *)srcEntity direction:(OOWeaponFacing)direction offset:(Vector)offset {
    ...
    if (ship == srcEntity) {
        // main laser offset
        [self setPosition:HPvector_add([ship position],
                                       vectorToHPVector(OOVectorMultiplyMatrix(offset, [ship drawRotationMatrix])))];
    } else {
        // subentity laser
...
- (NSString *)descriptionComponents {
    return [NSString stringWithFormat:@ "ttl: %.3fs - %@ orientation %@",
                                      _lifetime,
                                      [super descriptionComponents],
                                      QuaternionDescription([self orientation])];
}
...
- (void)update:(OOTimeDelta)delta_t {
    ...
    if ([ship isPlayer]) {
        ...
        */
        [self setPosition:HPvector_add([ship position],
                                       vectorToHPVector(OOVectorMultiplyMatrix(_offset, [ship drawRotationMatrix])))];
        ...
        sShotTexture = [[OOTexture textureWithName:@ "oolite-laser-blur.png"
                                          inFolder:@ "Textures"
                                           options:kOOTextureMinFilterMipMap | kOOTextureMagFilterLinear |
                                                   kOOTextureAlphaMask | kOOTextureRepeatT
