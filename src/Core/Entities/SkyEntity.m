...
- (id)initWithColors:(OOColor *)col1:(OOColor *)col2 andSystemInfo:(NSDictionary *)systemInfo {
    ...
    float clusterChance, alpha, scale, starCountMultiplier, nebulaCountMultiplier;
    ...
    // Load colours
    BOOL nebulaColorSet = [self readColor1:&col1
                                 andColor2:&col2
                                 andColor3:&col3
                                 andColor4:&col4
                            fromDictionary:systemInfo];
    ...
    skyDrawable = [[OOSkyDrawable alloc] initWithColor1:col1
                                                 Color2:col2
                                                 Color3:col3
                                                 Color4:col4
                                              starCount:starCount
...
- (BOOL)changeProperty:(NSString *)key withDictionary:(NSDictionary *)dict {
    ...
    if ([key isEqualToString:@ "sun_color"]) {
        ...
        OOLogWARN(@ "script.warning",
                  @ "Change to property '%@' not applied, will apply only on leaving and re-entering this system.",
                  key);
        return NO;
    }
    return YES;
    ...
    if (colorDesc != nil) {
        ...
        if (color != nil)
            *ioColor1 = color;
            ...
            OOLogWARN(@ "sky.fromDict", @ "could not interpret \"%@\" as a colour.", colorDesc);
    }
