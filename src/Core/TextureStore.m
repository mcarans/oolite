...
static FloatRGB FloatRGBFromDictColor(NSDictionary *dictionary, NSString *key) {
    ...
    if (color == nil) {
        ...
        OOLog(@"textureStore.FloatRGBFromDictColor.nilColor",
              @"Expected color for key \"%@\" in dictionary %@, got nil. Setting color to %@",
              key,
              dictionary,
              [color rgbaDescription]);
    }
...
static FloatRGB Blend(float fraction, FloatRGB a, FloatRGB b) {
    return (FloatRGB){OOLerp(a.r, b.r, fraction), OOLerp(a.g, b.g, fraction), OOLerp(a.b, b.b, fraction)};
}
...
static void fillSquareImageDataWithCloudTexture(
    unsigned char *imageBuffer, int width, OOColor *cloudcolor, float impress, float bias);
...
+ (BOOL)getPlanetTextureNameFor:(NSDictionary *)planetInfo
    ...
    *textureHeight = texture_h;
    ...
    fillSquareImageWithPlanetTex(
        imageBuffer, texture_w, 1.0, sea_bias, sea_color, pale_sea_color, land_color, polar_land_color);
...
void fillRanNoiseBuffer() {
    ...
    int i;
    for (i = 0; i < 16384; i++) ranNoiseBuffer[i] = randf();
}
...
static float q_factor(float *accbuffer,
    ...
    if (polar_x_smooth) q = q * (1.0 - polar_x) + polar_x * polar_x_value;
    if (polar_y_smooth) q = q * (1.0 - polar_y) + polar_y * polar_y_value;
...
static void fillSquareImageDataWithCloudTexture(
    ...
    [[UNIVERSE gameView] dumpRGBAToFileNamed:name bytes:imageBuffer width:width height:width rowBytes:width * 4];
#endif
}
...
#if DEBUG_DUMP
    OOLog(@"planetTex.dump",
          [NSString
              stringWithFormat:@"Saving generated texture to file planet-%u-%u-old.", sNoiseSeed.high, sNoiseSeed.low]);

    [[UNIVERSE gameView]
        dumpRGBAToFileNamed:[NSString stringWithFormat:@"planet-%u-%u-old", sNoiseSeed.high, sNoiseSeed.low]
                      bytes:imageBuffer
                      width:width
                     height:width
                   rowBytes:width * 4];
