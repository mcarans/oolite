...
static NSData *ChangeDTDIfApplicable(NSData *data) {
    ...
    size_t length, newLength, offset = 0, newOffset = 0;
    ...
    // Check if it starts with an XML declaration. Bogus: there are valid XML declarations which don't match
    // xmlDeclLine.
    ...
    for (i = 0;; i++) {
        ...
        if (srcDTDLineSize <= length - offset && memcmp(bytes + offset, srcDTDLine, srcDTDLineSize) == 0) {
            // Match
            break;
        }
    }
