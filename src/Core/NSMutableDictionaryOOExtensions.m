...
- (void)mergeEntriesFromDictionary:(NSDictionary*)otherDictionary {
    ...
    for (otherKeysEnum = [otherDictionary keyEnumerator]; (key = [otherKeysEnum nextObject]);) {
        if (![self objectForKey:key])
            ...
            if ([thisObject isKindOfClass:[NSDictionary class]] && [otherObject isKindOfClass:[NSDictionary class]] &&
                (![thisObject isEqual:otherObject])) {
                NSMutableDictionary* mergeObject =
            ...
            if ([thisObject isKindOfClass:[NSArray class]] && [otherObject isKindOfClass:[NSArray class]] &&
                (![thisObject isEqual:otherObject])) {
            ...
            if (!merged) [self setObject:otherObject forKey:key];
        }
    }
}
