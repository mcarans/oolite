...
- (void)setupForPlayer:(PlayerEntity*)p1 digram:(NSString*)digramString {
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (pointscheme[i] == 0) pointColor[i] = colorPoint1;
        if (pointscheme[i] == 1) pointColor[i] = colorPoint2;
    }
    //
    for (i = 0; i < 4; i++) {
...
- (void)spawnFrom:(OOTrumble*)parentTrumble {
    if (parentTrumble) {
        // mutate..
        unichar mutation1 =
            ranrot_rand() & ranrot_rand() & ranrot_rand() & 0xff;  // each bit has a 1/8 chance of being set
        unichar mutation2 =
            ranrot_rand() & ranrot_rand() & ranrot_rand() & 0xff;  // each bit has a 1/8 chance of being set
        ...
        if (size < 0.5) size = 0.5;  // minimum size
    ...
    */
    ...
    switch (eyeFrame) {
        ...
        case TRUMBLE_EYES_OPEN:
            eyeTextureOffset = 0.0;
            break;
        case TRUMBLE_EYES_SHUT:
            eyeTextureOffset = 0.25;
            break;
        case TRUMBLE_EYES_WIDE:
            eyeTextureOffset = 0.5;
            break;
    }
    ...
    switch (mouthFrame) {
        case TRUMBLE_MOUTH_POUT:
            mouthTextureOffset = 0.500;
            break;
        case TRUMBLE_MOUTH_NONE:
        case TRUMBLE_MOUTH_NORMAL:
            mouthTextureOffset = 0.625;
            break;
        case TRUMBLE_MOUTH_GROWL:
            mouthTextureOffset = 0.750;
            break;
        case TRUMBLE_MOUTH_SNARL:
            mouthTextureOffset = 0.875;
            break;
    }
...
- (void)updateTrumble:(double)delta_t {
    // player movement
    NSPoint p_mov = NSMakePoint(TRUMBLE_MAX_ROTATIONAL_VELOCITY * [player dialPitch],
                                TRUMBLE_MAX_ROTATIONAL_VELOCITY * [player dialRoll]);
    ...
    [self calcGrowthRate];
    if (hunger > 0.75) growth_rate = 0.0;
    if (hunger > 1.0) hunger = 1.0;  // clamp

    // feelings
    //
    GLfloat temp = [player hullHeatLevel];
    discomfort += delta_t * hunger * 0.02 * (1.0 - hunger);
    if (temp > 0.33) discomfort += delta_t * (temp - 0.33) * (temp - 0.33) * 0.05;
    if (discomfort > 1.0) discomfort = 1.0;  // clamp
    ...
    if (hunger > 0.50) {
        ...
        if (selectedCargopod) {
            ...
            if (discomfort < 0.0) discomfort = 0.0;
            if (trumbleAppetiteAccumulator > 10.0) {
                // eaten all of this cargo!
                NSString* ms =
                    [NSString stringWithFormat:DESC(@ "trumbles-eat-@"),
...
- (void)updateIdle:(double)delta_t {
    ...
    if (animationTime > animationDuration) {
        ...
        [self actionIdle];
        if (randf() < 0.25) [self actionBlink];
        if (randf() < 0.10) [self randomizeMotionX];
        if (randf() < 0.10) [self randomizeMotionY];
        if (randf() < 0.05) [self actionProot];
        if (randf() < 0.01) [self actionSleep];
        if (randf() < 0.01) [self actionSnarl];
        //
        if (readyToSpawn) {
            [self actionSpawn];
            readyToSpawn = NO;
...
- (void)updateBlink:(double)delta_t {
    ...
    if (animationTime > animationDuration) {
        ...
        [self actionIdle];
        if (randf() < 0.05) [self actionBlink];
        if (randf() < 0.1) [self actionProot];
        animation = nextAnimation;
        animationTime = 0.0;
        eyeFrame = TRUMBLE_EYES_OPEN;
    }
...
- (void)updateSleep:(double)delta_t {
    ...
    if (animationTime > animationDuration) {
        ...
        mouth_position.y = saved_float2;
        [self actionIdle];
        if (randf() < 0.25) [self actionProot];
        animation = nextAnimation;
        animationTime = 0.0;
        eyeFrame = TRUMBLE_EYES_OPEN;
    }
...
- (NSDictionary*)dictionary {
    return [NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithCharacters:digram length:2],
                                                      @ "digram",
                                                      [NSNumber numberWithFloat:hunger],
                                                      @ "hunger",
                                                      [NSNumber numberWithFloat:discomfort],
                                                      @ "discomfort",
                                                      [NSNumber numberWithFloat:size],
                                                      @ "size",
                                                      [NSNumber numberWithFloat:growth_rate],
                                                      @ "growth_rate",
                                                      [NSNumber numberWithFloat:rotation],
                                                      @ "rotation",
                                                      [NSNumber numberWithFloat:rotational_velocity],
                                                      @ "rotational_velocity",
                                                      StringFromPoint(position),
                                                      @ "position",
                                                      StringFromPoint(movement),
                                                      @ "movement",
                                                      nil];
}
