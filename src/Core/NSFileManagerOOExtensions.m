...
- (NSArray *)commanderContentsOfPath:(NSString *)savePath {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:savePath isDirectory:&pathIsDirectory] && pathIsDirectory) {
        ...
        for (i = 0; i < [contents count]; i++) {
            ...
            if (![[NSFileManager defaultManager] fileExistsAtPath:path isDirectory:&pathIsDirectory] &&
                pathIsDirectory) {
...
- (NSString *)defaultCommanderPath {
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:savedir isDirectory:&pathIsDirectory]) {
        ...
        if ([self oo_createDirectoryAtPath:savedir attributes:nil]) {
            ...
            OOLogERR(@ "savedGame.defaultPath.create.failed",
                     @ "Unable to create '%@'. Saved games will go to the home directory.",
                     savedir);
    ...
    // is it a directory?
    if (!pathIsDirectory) {
        OOLogERR(@ "savedGame.defaultPath.notDirectory",
                 @ "'%@' is not a directory, saved games will go to the home directory.",
                 savedir);
        return NSHomeDirectory();
    }
...
- (BOOL)chdirToSnapshotPath {
    ...
    if (![self changeCurrentDirectoryPath:savedir]) {
        ...
        if (![self changeCurrentDirectoryPath:savedir]) {
            OOLog(@ "savedSnapshot.defaultPath.chdir.failed",
                  @ "Created %@ but couldn't make it the current directory.",
                  savedir);
            return NO;
        }
    }
