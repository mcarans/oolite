...
- (void)setColor:(OOColor *)color forRow:(OOGUIRow)row {
    if (RowInRange(row, rowRange)) [rowColor replaceObjectAtIndex:row withObject:color];
}
...
- (void)reportSelectedRow:(int)row {
    [PLAYER doScriptEvent:OOJSID("guiSelectedRowChanged")
            withArguments:[NSArray arrayWithObjects:[self keyForRow:row],
                                                    [NSNumber numberWithInt:row],
                                                    [self selectedRowText],
                                                    nil]];
}
...
- (void)setKey:(NSString *)str forRow:(OOGUIRow)row {
    if (RowInRange(row, rowRange)) [rowKey replaceObjectAtIndex:row withObject:str];
}
...
- (OOGUIRow)addLongText:(NSString *)str startingAtRow:(OOGUIRow)row align:(OOGUIAlignment)alignment {
    ...
    foreach (line, lines) {
        ...
        if ([words count] > 0) {
            while ([words count] > 1) {
                ...
                [accum appendString:@ " "];
                if (OORectFromString(accum, 0.0f, 0.0f, chSize).size.width +
                        OORectFromString([words oo_stringAtIndex:1], 0.0f, 0.0f, chSize).size.width >
                    limit) {
                    // can't fit next word on this line
                    [output
                        appendString:[accum stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]];
                    [output appendString:@ "\n"];
                    [accum setString:@ ""];
                }
                [words removeObjectAtIndex:0];
...
- (NSArray *)getLastLines  // text, colour, fade time - text, colour, fade time
{
    ...
    if (i > 0) {
        ...
        return [NSArray
            arrayWithObjects:[rowText oo_stringAtIndex:i - 1],
                             ...
                             [NSNumber numberWithFloat:rowFadeTime[i]],
                             nil];
    } else {
...
- (void)printLongText:(NSString *)str
    ...
    if ([str rangeOfString:@ "\n"].location != NSNotFound) {
        ...
        unsigned i;
        for (i = 0; i < [lines count]; i++)
            [self printLongText:[lines oo_stringAtIndex:i]
                          align:alignment
                          color:text_color
                       fadeTime:text_fade
                            key:text_key
                     addToArray:text_array];
        return;
    }

    OOGUIRow row = currentRow;
    if (row == (OOGUIRow)n_rows - 1) [self scrollUp:1];
    NSSize chSize = pixel_text_size;
    NSSize strsize = OORectFromString(str, 0.0f, 0.0f, chSize).size;
    if (strsize.width < size_in_pixels.width) {
        [self setText:str forRow:row align:alignment];
        if (text_color) [self setColor:text_color forRow:row];
        if (text_key) [self setKey:text_key forRow:row];
        rowFadeTime[row] = text_fade;
        if (currentRow < (OOGUIRow)n_rows - 1) currentRow++;
        if (text_array) [text_array addObject:str];
    } else {
        NSMutableArray *words = ScanTokensFromString(str);
        NSMutableString *string1 = [NSMutableString stringWithCapacity:256];
        NSMutableString *string2 = [NSMutableString stringWithCapacity:256];
        ...
        if (text_color) [self setColor:text_color forRow:row];
        if (text_key) [self setKey:text_key forRow:row];
        if (text_array) [text_array addObject:string1];
        rowFadeTime[row] = text_fade;
        [self printLongText:string2
                      align:alignment
                      color:text_color
                   fadeTime:text_fade
                        key:text_key
...
- (void)printLineNoScroll:(NSString *)str
    ...
    if (text_color) [self setColor:text_color forRow:currentRow];
    if (text_key) [self setKey:text_key forRow:currentRow];
    if (text_array) [text_array addObject:str];
    rowFadeTime[currentRow] = text_fade;
}

- (void)setArray:(NSArray *)arr forRow:(OOGUIRow)row {
    if (RowInRange(row, rowRange)) [rowText replaceObjectAtIndex:row withObject:arr];
}
...
- (void)insertItemsFromArray:(NSArray *)items
    ...
    if ((item_keys) && ([item_keys count] != n_items)) {
        // throw exception
        [NSException
             raise:@ "ArrayLengthMismatchException"
            format:
                @ "The NSArray sent as 'item_keys' to insertItemsFromArray::: must contain the same number of objects as the NSArray 'items'"];
    }
    ...
    for (i = n_rows; i >= row + n_items; i--) {
        ...
        if ([old_row_info isKindOfClass:[NSArray class]]) [self setArray:old_row_info forRow:i];
        if ([old_row_info isKindOfClass:[NSString class]]) [self setText:(NSString *)old_row_info forRow:i];
    }
    for (i = 0; i < n_items; i++) {
        ...
        if ([new_row_info isKindOfClass:[NSArray class]]) [self setArray:new_row_info forRow:row + i];
        if ([new_row_info isKindOfClass:[NSString class]]) [self setText:(NSString *)new_row_info forRow:row + i];
        if (item_keys)
...
static OOTexture *TextureForGUITexture(NSDictionary *descriptor, uint32_t srgbaOption) {
    /*
            GUI textures like backgrounds, foregrounds etc. are not processed in any way after loading. However, they
       are subject to tone nmapping and gamma correction at the end of the render pass. So we need to declare them as
            ...
            Also, remember that if no shaders are in use (as in lower detail levels), then we don't need to declare
       anything.
    */
    ...
    if (withBackground) {
        ...
        switch (spec) {
            ...
            case GUI_BACKGROUND_SPECIAL_LONG_ANA_QUICKEST:
                bgDescriptor =
                    [UNIVERSE screenTextureDescriptorForKey:[NSString stringWithFormat:@ "long_range_chart%d_mission",
                                                                                       galaxy_number + 1]];
                if (bgDescriptor == nil) {
                    bgDescriptor = [UNIVERSE screenTextureDescriptorForKey:@ "long_range_chart_mission"];
                    if (bgDescriptor == nil) {
                        bgDescriptor =
                            [UNIVERSE screenTextureDescriptorForKey:[NSString stringWithFormat:@ "long_range_chart%d",
                                                                                               galaxy_number + 1]];
                        if (bgDescriptor == nil) {
...
- (NSDictionary *)textureDescriptorFromJSValue:(jsval)value
    ...
    if (result != nil && ![self preloadGUITexture:result]) {
        OOJSReportWarning(
            context, @ "%@: texture \"%@\" could not be found.", callerDescription, [result oo_stringForKey:@ "name"]);
        result = nil;
    }
...
- (void)drawEquipmentList:(NSArray *)eqptList z:(GLfloat)z {
    ...
    if (statusPage > 1) {
        [self setColor:[self colorFromSetting:kGuiStatusEquipmentScrollColor defaultValue:[OOColor greenColor]]
                forRow:firstRow];
...
- (int)drawGUI:(GLfloat)alpha drawCursor:(BOOL)drawCursor {
    ...
    if (drawCursor) {
        ...
        GLfloat h3 = 9.0f;
        OOGL(
            glColor4f(0.6f, 0.6f, 1.0f, 1.0f));  // original value of (0.2f, 0.2f, 1.0f, 0.5f) too dark - Nikos 20130616
        ...
        OOGLBEGIN(GL_LINES);
        glVertex3f((float)cursor_x - h1, (float)cursor_y, z);
        glVertex3f((float)cursor_x - h3, (float)cursor_y, z);
        glVertex3f((float)cursor_x + h1, (float)cursor_y, z);
        glVertex3f((float)cursor_x + h3, (float)cursor_y, z);
        glVertex3f((float)cursor_x, (float)cursor_y - h1, z);
        glVertex3f((float)cursor_x, (float)cursor_y - h3, z);
        glVertex3f((float)cursor_x, (float)cursor_y + h1, z);
        glVertex3f((float)cursor_x, (float)cursor_y + h3, z);
        OOGLEND();
...
- (void)drawGLDisplay:(GLfloat)x:(GLfloat)y:(GLfloat)z:(GLfloat)alpha {
    ...
    if (backgroundColor) {
        ...
        if (playerStatus != STATUS_START_GAME && playerStatus != STATUS_DEAD) {
            OOGL(glColor4f([backgroundColor redComponent],
    ...
    if (foregroundSprite != nil) {
        [foregroundSprite blitCentredToX:x + 0.5f * size_in_pixels.width
                                       Y:y + 0.5f * size_in_pixels.height
                                       Z:z
                                   alpha:alpha];
    }

    if (!RowInRange(selectedRow, selectableRange)) selectedRow = -1;  // out of Range;
    ...
    if (title != nil) {
        ...
        OODrawString(title,
                     ...
                     y + size_in_pixels.height - pixel_title_size.height,
                     z,
                     titleCharacterSize);

        // draw a horizontal divider
        //
        [self setGLColorFromSetting:kGuiScreenDividerColor
                       defaultValue:[OOColor colorWithWhite:0.75 alpha:1.0]
                              alpha:alpha];
        ...
        if ([[rowText objectAtIndex:i] isKindOfClass:[NSString class]]) {
            ...
            if (![text isEqual:@ ""]) {
                ...
                if (i == (unsigned)selectedRow) {
                    NSRect block =
                        OORectFromString(text, x + rowPosition[i].x + 2, y + rowPosition[i].y + 2, characterSize);
                    OOStopDrawingStrings();
                    [self setGLColorFromSetting:kGuiSelectedRowBackgroundColor
                                   defaultValue:[OOColor redColor]
                                          alpha:alpha];
                    OOGLBEGIN(GL_QUADS);
                ...
                if ((showTextCursor) && (i == (unsigned)currentRow)) {
                    ...
                    NSPoint cu = NSMakePoint(x + rowPosition[i].x + tr.size.width + 0.2f * characterSize.width,
                                             y + rowPosition[i].y);
                    tr.origin = cu;
                    tr.size.width = 0.5f * characterSize.width;
                    GLfloat g_alpha = 0.5f * (1.0f + (float)sin(6 * [UNIVERSE getTime]));
                    OOStopDrawingStrings();
                    [self setGLColorFromSetting:kGuiTextInputCursorColor
                                   defaultValue:[OOColor redColor]
                                          alpha:row_alpha[i] * g_alpha];
                    OOGLBEGIN(GL_QUADS);
        ...
        if ([[rowText objectAtIndex:i] isKindOfClass:[NSArray class]]) {
            ...
            for (j = 0; j < max_columns; j++) {
                ...
                if ([text length] != 0) {
                    ...
                    NSString *hilitedText = [text
                        stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@ " \037"]];
                    ...
                    if (EXPECT_NOT(txtRange.location == NSNotFound)) {
                        ...
                        /*						// if we're displaying commodity-quantity-none,
                           let's try and be pixel perfect! NSString *qtyNone = DESC(@"commodity-quantity-none");
                                                                        txtRange = [hilitedText rangeOfString:qtyNone];

                                                                        if (txtRange.location == 0) // bingo!
                                                                        {
                                                                                rowPosition[i].x +=
                           OORectFromString(@"0", 0, 0, characterSize).size.width - OORectFromString(qtyNone, 0, 0,
                           characterSize).size.width;
                                                                                } */
                    }
                    ...
                    block = OORectFromString(
                        hilitedText, x + rowPosition[i].x + 1 + leadingSpaces, y + rowPosition[i].y + 2, characterSize);
                    block.size.width += 3;
...
- (void)setStarChartTitle {
    PlayerEntity *player = PLAYER;
    OOGalaxyID galaxy_number = [player galaxyNumber];
    NSInteger system_id = [UNIVERSE findSystemNumberAtCoords:[player cursor_coordinates]
                                                  withGalaxy:[player galaxyNumber]
                                             includingHidden:NO];

    NSString *location_key =
        [NSString stringWithFormat:@ "long-range-chart-title-%d-%ld", galaxy_number, (long)system_id];
...
- (void)drawStarChart:(GLfloat)x:(GLfloat)y:(GLfloat)z:(GLfloat)alpha:(BOOL)compact {
    ...
    NSPoint info_system_coordinates = [[UNIVERSE systemManager] getCoordinatesForSystem:[player infoSystemID]
    ...
    clipRect = NSMakeRect((viewSize.width - size_in_pixels.width * pixelRatio) / 2.0,
                          (viewSize.height + size_in_pixels.height * pixelRatio) / 2.0 -
                              (pixel_title_size.height + 15 + (textRow - 2) * MAIN_GUI_ROW_HEIGHT) * pixelRatio,
    ...
    cu =
        NSMakePoint((float)(hscale * galaxy_coordinates.x + hoffset), (float)(vscale * galaxy_coordinates.y + voffset));
    ...
    if (_refreshStarChart || galaxy_id != saved_galaxy_id) {
        ...
        for (i = 0; i < num_nearby_systems; i++) [nearby_systems[i].p_name release];
    ...
    if (advancedNavArrayMode != OPTIMIZED_BY_NONE &&
        [player hasEquipmentItemProviding:@ "EQ_ADVANCED_NAVIGATIONAL_ARRAY"]) {
        ...
        if (routeInfo == nil || planetNumber != savedPlanetNumber || destNumber != savedDestNumber ||
            advancedNavArrayMode != savedArrayMode) {
            [routeInfo release];
            routeInfo = [[UNIVERSE routeFromSystem:planetNumber toSystem:destNumber
                                       optimizedBy:advancedNavArrayMode] retain];
        ...
        [self drawAdvancedNavArrayAtX:x + hoffset
                                    y:y + voffset
                                    z:z
                                alpha:alpha
                           usingRoute:(planetNumber != destNumber ? (id)routeInfo : nil)
                          optimizedBy:advancedNavArrayMode
                                 zoom:zoom];
    ...
    cu =
        NSMakePoint((float)(hscale * cursor_coordinates.x + hoffset), (float)(vscale * cursor_coordinates.y + voffset));
    ...
    for (i = 0; i < num_nearby_systems; i++) {
        ...
        if ((dx > zoom * (CHART_WIDTH_AT_MAX_ZOOM / 2.0 + CHART_CLIP_BORDER)) ||
            (dy > zoom * (CHART_HEIGHT_AT_MAX_ZOOM + CHART_CLIP_BORDER)))
            continue;
            ...
            switch (chart_mode) {
                case OOLRC_MODE_ECONOMY:
                    if (EXPECT(noNova)) {
                        ...
                        [self setGLColorFromSetting:[NSString stringWithFormat:kGuiChartEconomyUColor,
                                                                               (unsigned long)systemParameter]
                ...
                case OOLRC_MODE_GOVERNMENT:
                    if (EXPECT(noNova)) {
                        ...
                        [self setGLColorFromSetting:[NSString stringWithFormat:kGuiChartGovernmentUColor,
                                                                               (unsigned long)systemParameter]
                                       defaultValue:[OOColor colorWithRed:govcol[systemParameter * 3]
                                                                    green:govcol[1 + (systemParameter * 3)]
                                                                     blue:govcol[2 + (systemParameter * 3)]
                                                                    alpha:1.0f]
                                              alpha:1.0];
                    } else {
                        r = g = b = 0.3;
                ...
                case OOLRC_MODE_SUNCOLOR:
                    if (EXPECT(noNova)) {
                        r = g = b = 1.0;
                        OOColor *sunColor =
                            [OOColor colorWithDescription:[[UNIVERSE systemManager] getProperty:@ "sun_color"
                                                                                      forSystem:i
                                                                                       inGalaxy:galaxy_id]];
                        if (sunColor != nil) {
    ...
    if (n_matches == 0) {
        ...
        for (i = 0; i < 256; i++) {
            ...
            if (mark && (dx <= zoom * (CHART_WIDTH_AT_MAX_ZOOM / 2.0 + CHART_CLIP_BORDER)) &&
                (dy <= zoom * (CHART_HEIGHT_AT_MAX_ZOOM + CHART_CLIP_BORDER))) {
                star.x = (float)(sys_coordinates.x * hscale + hoffset);
                star.y = (float)(sys_coordinates.y * vscale + voffset);
                ...
                if (i == foundIndex || n_matches == 1) {
                    ...
                    if (zoom > CHART_ZOOM_SHOW_LABELS && advancedNavArrayMode == OPTIMIZED_BY_NONE) {
                        [self setGLColorFromSetting:kGuiChartMatchLabelColor
                                       defaultValue:[OOColor cyanColor]
                                              alpha:alpha];
                        OODrawString([UNIVERSE systemNameIndex:i],
                                     x + star.x + 2.0,
                                     y + star.y - 10.0f,
                                     z,
                                     NSMakeSize(10 * systemNameScale, 10 * systemNameScale));
                        [self setGLColorFromSetting:kGuiChartMatchBoxColor
    ...
    for (i = 0; i < num_nearby_systems; i++) {
        ...
        if (zoom < CHART_ZOOM_SHOW_LABELS) {
            ...
            {
                d = distanceBetweenPlanetPositions(
                    galaxy_coordinates.x, galaxy_coordinates.y, sys_coordinates.x, sys_coordinates.y);
                if (d <= jumpRange) {
                    [self setGLColorFromSetting:kGuiChartLabelReachableColor
                                   defaultValue:[OOColor yellowColor]
                                          alpha:alpha];
                } else {
            ...
            } else if (sys->gov >= 0)  // Not a nova? Show the info.
            {
                if (concealment[targetIdx] >= OO_SYSTEMCONCEALMENT_NODATA) {
                    ...
                    OODrawHilightedPlanetInfo(
                        sys->gov, sys->eco, sys->tec, x + star.x + 2.0, y + star.y + 2.0, z, chSize);
                }
            }
        }
    }
    ...
    // distance-f & est-travel-time-f are identical between short & long range charts in standard Oolite, however can be
    // alterered separately via OXPs
    ...
    if ([PLAYER guiScreen] == GUI_SCREEN_SHORT_RANGE_CHART) {
        if (jumps > 0) {
            [self setArray:[NSArray arrayWithObjects:@ "", OOExpandKey(@ "short-range-chart-jumps", jumps), nil]
                    forRow:textRow + 1];
        } else {
    ...
    cu = NSMakePoint((float)(hscale * info_system_coordinates.x + hoffset),
    ...
    OOGLBEGIN(GL_QUADS);
    glVertex3f(
        x + 0, (float)(y + size_in_pixels.height - (textRow - 1) * MAIN_GUI_ROW_HEIGHT - pixel_title_size.height), z);
    glVertex3f(x + size_in_pixels.width,
               (GLfloat)(y + size_in_pixels.height - (textRow - 1) * MAIN_GUI_ROW_HEIGHT - pixel_title_size.height),
               z);
    glVertex3f(x + size_in_pixels.width,
               (GLfloat)(y + size_in_pixels.height - (textRow - 1) * MAIN_GUI_ROW_HEIGHT - pixel_title_size.height - 2),
               z);
    glVertex3f(x + 0,
               (GLfloat)(y + size_in_pixels.height - (textRow - 1) * MAIN_GUI_ROW_HEIGHT - pixel_title_size.height - 2),
               z);
...
- (void)drawAdvancedNavArrayAtX:(float)x
    ...
    NSPoint star = NSZeroPoint, star2 = NSZeroPoint, starabs = NSZeroPoint, star2abs = NSZeroPoint;
    OOSystemDescriptionManager *systemManager = [UNIVERSE systemManager];
    OOGalaxyID g = [PLAYER galaxyNumber];
    OOSystemID planetNumber = [PLAYER systemID];

    OOColor *defaultConnectionColor = [self colorFromSetting:kGuiChartConnectionColor
                                                defaultValue:[OOColor colorWithWhite:0.25 alpha:1.0]];
    OOColor *currentJumpColorStart = [self colorFromSetting:kGuiChartCurrentJumpStartColor
                                               defaultValue:[OOColor colorWithWhite:0.25 alpha:0.0]];
    OOColor *currentJumpColorEnd = [self colorFromSetting:kGuiChartCurrentJumpEndColor
                                             defaultValue:[OOColor colorWithWhite:0.25 alpha:0.0]];
        ...
        for (OOSystemID j = loopstart; j < 256; j++) {
            ...
            if (d <= jumpRange)  // another_commander - Default to 7.0 LY.
            {
                ...
                if (optimizeBy == OPTIMIZED_BY_NONE) {
                    ...
                    OOGL(glColor4f(
                        OOLerp(lr, lr2, frac), OOLerp(lg, lg2, frac), OOLerp(lb, lb2, frac), OOLerp(la, la2, frac)));
                    glVertex3f(x + star2.x, y + star2.y, z);

                } else {
                    thisConnectionColor = [OOColor
                        colorWithDescription:[systemManager
                                                  getProperty:@ "link_color"
                                                 forSystemKey:[NSString stringWithFormat:@ "interstellar: %d %ld %ld",
                                                                                         g,
                                                                                         (long)i,
                                                                                         (long)j]]];
    ...
    if (routeInfo) {
        ...
        for (i = 0; i < route_hops; i++) {
            ...
            star2abs = [systemManager getCoordinatesForSystem:[[routeInfo objectForKey:@ "route"] oo_intAtIndex:i + 1]
                                                     inGalaxy:g];
