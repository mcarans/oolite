...
HPVector OOHPVectorRandomSpatial(OOHPScalar maxLength);  // Random vector uniformly distributed in radius-maxLength
                                                         // sphere. (Longer vectors are more common.)
HPVector OOHPVectorRandomRadial(
    OOHPScalar maxLength);  // Random vector with uniform distribution of direction and radius in radius-maxLength
                            // sphere. (Causes clustering at centre.)
HPVector OORandomPositionInCylinder(
    HPVector centre1, OOHPScalar exclusion1, HPVector centre2, OOHPScalar exclusion2, OOHPScalar radius);
...
#define GLVertexOOHPVector(v)         \
    do {                              \
        HPVector v_ = v;              \
        glVertex3f(v_.x, v_.y, v_.z); \
    } while (0)
#define GLTranslateOOHPVector(v)              \
    do {                                      \
        HPVector v_ = v;                      \
...
OOINLINE HPVector OOHPVectorInterpolate(HPVector a, HPVector b, OOHPScalar where) {
    return make_HPvector(OOLerpd(a.x, b.x, where), OOLerpd(a.y, b.y, where), OOLerpd(a.z, b.z, where));
}

OOINLINE HPVector OOHPVectorTowards(HPVector a, HPVector b, OOHPScalar where) {
    return make_HPvector(a.x + b.x * where, a.y + b.y * where, a.z + b.z * where);
}
...
OOINLINE HPVector HPvector_flip(HPVector v) { return HPvector_subtract(kZeroHPVector, v); }

OOINLINE bool HPvector_equal(HPVector a, HPVector b) { return a.x == b.x && a.y == b.y && a.z == b.z; }

OOINLINE OOHPScalar HPmagnitude2(HPVector vec) { return vec.x * vec.x + vec.y * vec.y + vec.z * vec.z; }

OOINLINE OOHPScalar HPmagnitude(HPVector vec) { return sqrt(HPmagnitude2(vec)); }
...
OOINLINE HPVector HPvector_normal_or_xbasis(HPVector vec) { return HPvector_normal_or_fallback(vec, kBasisXHPVector); }

OOINLINE HPVector HPvector_normal_or_ybasis(HPVector vec) { return HPvector_normal_or_fallback(vec, kBasisYHPVector); }

OOINLINE HPVector HPvector_normal_or_zbasis(HPVector vec) { return HPvector_normal_or_fallback(vec, kBasisZHPVector); }

OOINLINE HPVector HPvector_normal(HPVector vec) { return HPvector_normal_or_fallback(vec, kZeroHPVector); }

OOINLINE OOHPScalar HPdistance2(HPVector v1, HPVector v2) { return HPmagnitude2(HPvector_subtract(v1, v2)); }

OOINLINE OOHPScalar HPdistance(HPVector v1, HPVector v2) { return HPmagnitude(HPvector_subtract(v1, v2)); }
...
OOINLINE OOHPScalar HPdot_product(HPVector a, HPVector b) { return (a.x * b.x) + (a.y * b.y) + (a.z * b.z); }
