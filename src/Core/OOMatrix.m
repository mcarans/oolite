...
OOMatrix OOMatrixForRotation(Vector axis, OOScalar angle) {
    ...
    return OOMatrixConstruct(t * x * x + c,
                             t * x * y + s * z,
                             t * x * z - s * y,
                             0.0f,
                             t * x * y - s * z,
...
OOMatrix OOMatrixForQuaternionRotation(Quaternion orientation) {
    ...
    return OOMatrixConstruct(1.0f - yy - zz,
                             xy - wz,
                             xz + wy,
                             0.0f,
                             xy + wz,
...
OOMatrix OOMatrixForBillboard(HPVector bbPos, HPVector eyePos) {
    ...
    if (EXPECT_NOT(v0.x == 0.0 && v0.y == 0.0))
        arbv = kBasisXVector;
    else
        arbv = kBasisZVector;
...
OOMatrix OOMatrixInverse(OOMatrix M) { return OOMatrixLeftTransform(M, kIdentityMatrix); }
