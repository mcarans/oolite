...
BOOL ScanVectorAndQuaternionFromString(NSString *xyzwxyzString, Vector *outVector, Quaternion *outQuaternion) {
    ...
    if (error) {
        OOLogERR(
            kOOLogStringQuaternionConversion, @ "cannot make vector and quaternion from '%@': %@", xyzwxyzString, error);
        return NO;
    }
...
NSString *StringFromPoint(NSPoint point) { return [NSString stringWithFormat:@ "%f %f", point.x, point.y]; }
...
NSString *OOPadStringToEms(NSString *string, float padEms) {
    ...
    if (numEms > 0) {
        ...
        result = [[@ "" stringByPaddingToLength:(NSUInteger)numEms withString:@ " "
                               startingAtIndex:0] stringByAppendingString:result];
    }
    ...
    if (numEms > 0) {
        ...
        result = [[@ "" stringByPaddingToLength:(NSUInteger)numEms withString:@ "\037"
                               startingAtIndex:0] stringByAppendingString:result];
    }
    return result;
}
...
NSArray *ComponentsFromVersionString(NSString *string) {
    ...
    for (i = 0; i != count; ++i) {
        ...
        if ([component respondsToSelector:@selector(intValue)])
            value = MAX([component intValue], 0);
        else
            value = 0;
...
NSComparisonResult CompareVersions(NSArray *version1, NSArray *version2) {
    NSEnumerator *leftEnum = nil, *rightEnum = nil;
    NSNumber *leftComponent = nil, *rightComponent = nil;
...
NSString *ClockToString(double clock, BOOL adjusting) {
    ...
    if (adjusting)
        format = DESC(@ "clock-format-adjusting");
    else
        format = DESC(@ "clock-format");
...
NSString *EscapedGraphVizString(NSString *string) {
    NSString *const srcStrings[] = {// Note: backslash must be first.
                                    @ "\\",
                                    @ "\"",
                                    @ "\'",
                                    @ "\r",
                                    ...
                                    @ "\t",
                                    nil};
    NSString *const subStrings[] = {// Note: must be same order.
                                    @ "\\\\",
                                    @ "\\\"",
                                    @ "\\\'",
                                    @ "\\r",
    ...
    while (*src != nil) {
        [mutable replaceOccurrencesOfString:*src++ withString:*sub++ options:0 range:(NSRange){0, [mutable length]}];
    }
...
static BOOL NameIsTaken(NSString *name, NSSet *uniqueSet) {
    ...
    if (keywords == nil)
        keywords = [[NSSet alloc] initWithObjects:@ "node", @ "edge", @ "graph", @ "digraph", @ "subgraph", @ "strict", nil];
