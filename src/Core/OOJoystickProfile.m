...
// Quadratic spline from left point to right point, with gradient specified at left.  returns nil if right.x - left.x <=
// 0.0.
- (id)initWithData:(NSPoint)left right:(NSPoint)right gradientleft:(double)gradientleft;

// Quadratic spline from left point to right point, with gradient specified at right.  returns nil if right.x - left.x
// <= 0.0.
- (id)initWithData:(NSPoint)left right:(NSPoint)right gradientright:(double)gradientright;

// Cubic spline from left point to right point, with gradients specified at end points.  returns nil if right.x - left.x
// <= 0.0.
...
- (double)value:(double)x {
    ...
    return x < 0 ? -[self rawValue:(-x - deadzone) / (1.0 - deadzone)]
                 : [self rawValue:(x - deadzone) / (1.0 - deadzone)];
}
...
- (id)initWithData:(NSPoint)left right:(NSPoint)right gradientleft:(double)gradientleft {
    ...
    if ((self = [super init])) {
        ...
        a[0] = left.y * right.x * (right.x - 2 * left.x) / (dx * dx) + right.y * left.x * left.x / (dx * dx) -
               gradientleft * left.x * right.x / dx;
...
- (id)initWithData:(NSPoint)left right:(NSPoint)right gradientright:(double)gradientright {
    ...
    if ((self = [super init])) {
        ...
        a[0] = (left.y * right.x * right.x + right.y * left.x * (left.x - 2 * right.x)) / (dx * dx) +
               gradientright * left.x * right.x / dx;
...
- (id)initWithData:(NSPoint)left
    ...
    if ((self = [super init])) {
        ...
        a[0] =
            (left.y * right.x * right.x * (right.x - 3 * left.x) - right.y * left.x * left.x * (left.x - 3 * right.x)) /
                (dx * dx * dx) -
            (gradientleft * right.x + gradientright * left.x) * left.x * right.x / (dx * dx);
        a[1] = 6 * left.x * right.x * (left.y - right.y) / (dx * dx * dx) +
               (gradientleft * right.x * (right.x + 2 * left.x) + gradientright * left.x * (left.x + 2 * right.x)) /
                   (dx * dx);
        a[2] = 3 * (left.x + right.x) * (right.y - left.y) / (dx * dx * dx) -
               (gradientleft * (2 * right.x + left.x) + gradientright * (2 * left.x + right.x)) / (dx * dx);
...
+ (id)segmentWithData:(NSPoint)left right:(NSPoint)right gradientleft:(double)gradientleft {
    OOJoystickSplineSegment *segment = [[OOJoystickSplineSegment alloc] initWithData:left
                                                                               right:right
    ...
    if ([controlPoints count] == 0) {
        ...
        for (i = 0; i < [controlPoints count]; i++) {
            ...
            if (next.x - left.x > 0.0) {
                // we make the gradient at right equal to the gradient of a straight line between the neighcouring
                // points
                ...
                if (first_segment) {
                    ...
                    segment = [OOJoystickSplineSegment segmentWithData:left
                                                                 right:right
                                                          gradientleft:gradientleft
                                                         gradientright:gradientright];
                }
...
- (void)moveControl:(NSInteger)index point:(NSPoint)point {
    ...
    // either of its neighbours, move it back inside.  Also keep neighbours a distance of at least
    // SPLINE_POINT_MIN_SPACING apart
    if (point.x - left.x < SPLINE_POINT_MIN_SPACING) {
