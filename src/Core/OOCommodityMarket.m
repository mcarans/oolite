...
- (NSString *)shortCommentForGood:(OOCommodityType)good {
    ...
    return OOExpand([definition oo_stringForKey:kOOCommodityShortComment
                                   defaultValue:@ "[oolite-commodity-no-short-comment]"]);
}
...
- (NSArray *)savePlayerAmounts {
    ...
    foreach (good, [self goods]) {
        [amounts
            addObject:[NSArray
                          arrayWithObjects:good, [NSNumber numberWithUnsignedInt:[self quantityForGood:good]], nil]];
    }
...
- (void)loadPlayerAmounts:(NSArray *)amounts {
    ...
    foreach (loaded, amounts) {
        ...
        if (!loadedOK) {
            OOLog(@ "setCommanderDataFromDictionary.warning.cargo",
                  @ "Cargo %@ (%u units) could not be loaded from the saved game, as it is no longer defined",
                  good,
                  q);
        }
    }
...
- (NSArray *)saveStationAmounts {
    ...
    OOCommodityType good = nil;
    foreach (good, [self goods]) {
        [amounts addObject:[NSArray arrayWithObjects:good,
                                                     [NSNumber numberWithUnsignedInt:[self quantityForGood:good]],
                                                     [NSNumber numberWithUnsignedInteger:[self priceForGood:good]],
                                                     nil]];
    }
    return [NSArray arrayWithArray:amounts];
}
...
- (void)loadStationAmounts:(NSArray *)amounts {
    ...
    foreach (loaded, amounts) {
        ...
        if (!loadedOK) {
            OOLog(
                @ "load.warning.cargo",
                @ "Station market good %@ (%u units) could not be loaded from the saved game, as it is no longer defined",
                good,
                q);
        }
    }
