...
static JSBool MissionVariablesEnumerate(
    JSContext *context, JSObject *object, JSIterateOp enumOp, jsval *state, jsid *idp);

#ifndef NDEBUG
static id MissionVariablesConverter(JSContext *context, JSObject *object);
#endif

static JSClass sMissionVariablesClass = {"MissionVariables",
...
#ifndef NDEBUG
static id MissionVariablesConverter(JSContext *context, JSObject *object) { return [PLAYER missionVariables]; }
#endif
...
static JSBool MissionVariablesGetProperty(JSContext *context, JSObject *this, jsid propID, jsval *value) {
    ...
    if (JSID_IS_STRING(propID)) {
        ...
        if ([mvar isKindOfClass:[NSString
                                    class]])  // Currently there should only be strings, but we may want to change this.
        {
...
static JSBool MissionVariablesSetProperty(
    JSContext *context, JSObject *this, jsid propID, JSBool strict, jsval *value) {
    OOJS_NATIVE_ENTER(context)
    ...
    if (JSID_IS_STRING(propID)) {
        ...
        if (key == nil) {
            OOJSReportError(context,
                            @ "Invalid mission variable name \"%@\".",
                            [OOStringFromJSID(propID) escapedForJavaScriptLiteral]);
            return NO;
        }
    ...
    switch (enumOp) {
        ...
        case JSENUMERATE_NEXT: {
            ...
            for (;;) {
                ...
                if (next == nil) break;
                if (![next hasPrefix:@ "mission_"])
                    continue;  // Skip mission instructions, which aren't visible through missionVariables.
