...
static JSClass sStationClass = {"Station",
                                JSCLASS_HAS_PRIVATE,
...
void InitOOJSStation(JSContext *context, JSObject *global) {
    sStationPrototype = JS_InitClass(context,
                                     global,
                                     JSShipPrototype(),
                                     &sStationClass,
        ...
        case kStation_roll:
            if (JS_ValueToNumber(context, *value, &fValue)) {
                /*				if (fValue < -2.0)  fValue = -2.0;
                                                if (fValue > 2.0)  fValue = 2.0;	// clamping to -2.0...2.0 gives
                   us ±M_PI actual maximum rotation [entity setRoll:fValue]; */
...
static JSBool StationAbortDockingForShip(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc == 0) {
        OOJSReportBadArguments(
            context, @ "Station", @ "abortDockingForShip", MIN(argc, 1U), OOJS_ARGV, nil, @ "ship in docking queue");
        return NO;
    }
...
static JSBool StationCanDockShip(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc > 0) {
        if (!JSVAL_IS_OBJECT(OOJS_ARGV[0]) ||
            !JSStationGetShipEntity(context, JSVAL_TO_OBJECT(OOJS_ARGV[0]), &shipToCheck)) {
            return NO;
        }
    }
...
static JSBool StationLaunchShipWithRole(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(shipRole == nil)) {
        OOJSReportBadArguments(
            context, @ "Station", @ "launchShipWithRole", MIN(argc, 1U), OOJS_ARGV, nil, @ "string (role)");
        return NO;
    }
...
static JSBool StationSetInterface(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 1) {
        OOJSReportBadArguments(
            context, @ "Station", @ "setInterface", MIN(argc, 1U), OOJS_ARGV, NULL, @ "key [, definition]");
        return NO;
    }
...
static JSBool StationSetMarketPrice(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 2) {
        OOJSReportBadArguments(
            context, @ "Station", @ "setMarketPrice", MIN(argc, 2U), OOJS_ARGV, NULL, @ "commodity, credits");
        return NO;
    }
    ...
    if (EXPECT_NOT(![[UNIVERSE commodities] goodDefined:commodity])) {
        OOJSReportBadArguments(
            context, @ "Station", @ "setMarketPrice", MIN(argc, 2U), OOJS_ARGV, NULL, @ "Unrecognised commodity type");
        return NO;
    }
    ...
    if (EXPECT_NOT(!gotPrice || price < 0)) {
        OOJSReportBadArguments(context,
                               ...
                               OOJS_ARGV,
                               NULL,
                               @ "Price must be at least 0 decicredits");
        return NO;
    }
...
static JSBool StationSetMarketQuantity(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 2) {
        OOJSReportBadArguments(
            context, @ "Station", @ "setMarketQuantity", MIN(argc, 2U), OOJS_ARGV, NULL, @ "commodity, units");
        return NO;
    }
    ...
    if (EXPECT_NOT(![[UNIVERSE commodities] goodDefined:commodity])) {
        OOJSReportBadArguments(
            context, @ "Station", @ "setMarketQuantity", MIN(argc, 2U), OOJS_ARGV, NULL, @ "Unrecognised commodity type");
        return NO;
    }

    int32 quantity;
    BOOL gotQuantity = JS_ValueToInt32(context, OOJS_ARGV[1], &quantity);
    if (EXPECT_NOT(!gotQuantity || quantity < 0 ||
        ...
        OOJSReportBadArguments(context,
                               ...
                               OOJS_ARGV,
                               NULL,
                               @ "Quantity must be between 0 and the station market capacity");
        return NO;
    }
...
static JSBool StationAddShipToShipyard(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc != 1 || (!JSVAL_IS_NULL(OOJS_ARGV[0]) && !JS_ValueToObject(context, OOJS_ARGV[0], &params))) {
        OOJSReportBadArguments(
            context, @ "Station", @ "addShipToShipyard", MIN(argc, 1U), OOJS_ARGV, NULL, @ "shipyard item definition");
        return NO;
    }
    ...
    if (!shipyardDefinition) {
        OOJSReportBadArguments(
            context, @ "Station", @ "addShipToShipyard", MIN(argc, 1U), OOJS_ARGV, nil, @ "valid dictionary object");
        return NO;
    }
    if (![shipyardDefinition objectForKey:KEY_SHORT_DESCRIPTION]) {
        OOJSReportBadArguments(context,
                               ...
                               OOJS_ARGV,
                               nil,
                               @ "'short_description' in dictionary");
        return NO;
    }
    ...
    if (![shipyardDefinition objectForKey:SHIPYARD_KEY_SHIPDATA_KEY]) {
        OOJSReportBadArguments(
            context, @ "Station", @ "addShipToShipyard", MIN(argc, 1U), OOJS_ARGV, nil, @ "'shipdata_key' in dictionary");
        return NO;
    }
    ...
    if ([[shipInfo oo_stringForKey:@ "roles"] rangeOfString:@ "player"].location == NSNotFound) {
        OOJSReportWarningForCaller(
            context, @ "Station", @ "addShipToShipyard", @ "shipdata_key not suitable for player role.");
        return NO;
    }
    if (!shipyardInfo) {
        OOJSReportWarningForCaller(
            context, @ "Station", @ "addShipToShipyard", @ "No shipyard information found for shipdata_key.");
        return NO;
    }
    ...
    if (![shipyardDefinition objectForKey:SHIPYARD_KEY_PRICE]) {
        ...
        if (price > 0) {
            ...
            OOJSReportBadArguments(
                context, @ "Station", @ "addShipToShipyard", MIN(argc, 1U), OOJS_ARGV, nil, @ "'price' in dictionary");
            return NO;
        }
    }
    ...
    if (![shipyardDefinition objectForKey:SHIPYARD_KEY_PERSONALITY]) {
        ...
        [result setObject:[NSNumber numberWithUnsignedLongLong:[shipyardDefinition
                                                                   oo_unsignedIntForKey:SHIPYARD_KEY_PERSONALITY]]
                   forKey:SHIPYARD_KEY_PERSONALITY];
    }

    NSArray *extras = [shipyardDefinition oo_arrayForKey:KEY_EQUIPMENT_EXTRAS];
    if (!extras) {
        // pick up defaults if extras not supplied
        extras = [NSArray arrayWithArray:[[shipyardInfo oo_dictionaryForKey:KEY_STANDARD_EQUIPMENT]
                                             oo_arrayForKey:KEY_EQUIPMENT_EXTRAS]];
    }
    if ([extras count] > 0) {
        // go looking for lasers and add them directly to our shipInfo
        NSString *fwdWeaponString =
            [[shipyardInfo oo_dictionaryForKey:KEY_STANDARD_EQUIPMENT] oo_stringForKey:KEY_EQUIPMENT_FORWARD_WEAPON];
        NSString *aftWeaponString =
            [[shipyardInfo oo_dictionaryForKey:KEY_STANDARD_EQUIPMENT] oo_stringForKey:KEY_EQUIPMENT_AFT_WEAPON];
        OOWeaponFacingSet availableFacings = [shipyardInfo oo_unsignedIntForKey:KEY_WEAPON_FACINGS
...
static JSBool StationRemoveShipFromShipyard(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc != 1 || (!JSVAL_IS_NULL(OOJS_ARGV[0]) && !gotIndex) || shipIndex < 0 ||
        (shipIndex + 1) > [shipyard count]) {
        OOJSReportBadArguments(
            context, @ "Station", @ "removeShipFromShipyard", MIN(argc, 1U), OOJS_ARGV, NULL, @ "valid ship index");
        return NO;
    }
