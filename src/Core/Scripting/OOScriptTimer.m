...
- (id)initWithNextTime:(OOTimeAbsolute)nextTime interval:(OOTimeDelta)interval {
    OOTimeAbsolute now;
...
- (NSString *)descriptionComponents {
    ...
    if (_interval <= 0.0)
        intervalDesc = @ "one-shot";
    else
        intervalDesc = [NSString stringWithFormat:@ "interval: %g", _interval];
...
- (BOOL)scheduleTimer {
    ...
    if (EXPECT(!sUpdating)) {
        if (EXPECT_NOT(sTimers == nil))
            sTimers = [[OOPriorityQueue alloc] initWithComparator:@selector(compareByNextFireTime:)];
...
+ (void)noteGameReset {
    ...
    // Intermediate array is required so we don't get stuck in an endless loop over reinserted timers. Note that
    // -sortedObjects also clears the queue!
...
- (NSComparisonResult)compareByNextFireTime:(OOScriptTimer *)other {
    ...
    if (_nextTime < otherTime)
        return NSOrderedAscending;
    else if (_nextTime > otherTime)
