...
static JSBool VisualEffectSetMaterialsInternal(
    JSContext *context, uintN argc, jsval *vp, OOVisualEffectEntity *thisEnt, BOOL fromShaders);

static JSClass sVisualEffectClass = {"VisualEffect",
                                     JSCLASS_HAS_PRIVATE,
...
void InitOOJSVisualEffect(JSContext *context, JSObject *global) {
    sVisualEffectPrototype = JS_InitClass(context,
                                          global,
                                          JSEntityPrototype(),
                                          &sVisualEffectClass,
...
static JSBool VisualEffectSetShaders(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (JSVAL_IS_NULL(OOJS_ARGV[0]) || (!JSVAL_IS_NULL(OOJS_ARGV[0]) && !JSVAL_IS_OBJECT(OOJS_ARGV[0]))) {
        ...
        OOJSReportWarning(context,
                          @ "VisualEffect.%@: expected %@ instead of '%@'.",
                          @ "setShaders",
                          @ "object",
...
static JSBool VisualEffectScale(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(scale <= 0.0 || !gotScale)) {
        OOJSReportBadArguments(
            context, @ "VisualEffect", @ "scale", argc, OOJS_ARGV, nil, @ "scale factor must be positive");
        return NO;
    }
