...
static JSClass sMissionClass = {"Mission",
                                0,
...
void InitOOJSMission(JSContext *context, JSObject *global) {
    ...
    JSObject *missionPrototype = JS_InitClass(context,
                                              global,
                                              NULL,
                                              &sMissionClass,
...
void MissionRunCallback() {
    ...
    */
    ...
    @try {
        ...
        [engine callJSFunction:cbFunction forObject:cbThis argc:2 argv:args result:&rval];
    } @catch (NSException *exception) {
        // Squash any exception, allow cleanup to happen and so forth.
        OOLog(kOOLogException,
              @ "Ignoring exception %@:%@ during handling of mission screen completion callback.",
              [exception name],
              [exception reason]);
    }
        ...
        if (!JS_ValueToInt32(context, OOJS_ARGV[i], &dest)) {
            ...
            if (!JSVAL_IS_OBJECT(OOJS_ARGV[i])) {
                OOJSReportBadArguments(
                    context, @ "Mission", @ "markSystem", MIN(argc, 1U), OOJS_ARGV, nil, @ "numbers or objects");
                return NO;
            }
        }
    }
...
static JSBool MissionUnmarkSystem(JSContext *context, uintN argc, jsval *vp) {
    ...
    for (i = 0; i < argc; i++) {
        if (!JS_ValueToInt32(context, OOJS_ARGV[i], &dest)) {
            ...
            if (!JSVAL_IS_OBJECT(OOJS_ARGV[i])) {
                OOJSReportBadArguments(
                    context, @ "Mission", @ "unmarkSystem", MIN(argc, 1U), OOJS_ARGV, nil, @ "numbers or objects");
                return NO;
            }
        }
    }
...
static JSBool MissionSetInstructionsInternal(JSContext *context, uintN argc, jsval *vp, BOOL isKey) {
    ...
    if (EXPECT_NOT(argc == 0)) {
        OOJSReportWarning(
            context,
            @ "Usage error: mission.%@() called with no arguments. Treating as Mission.%@(null). This call may fail in a future version of Oolite.",
            isKey ? @ "setInstructionsKey" : @ "setInstructions",
            isKey ? @ "setInstructionsKey" : @ "setInstructions");
...
static NSDictionary *GetParameterImageDescriptor(JSContext *context, JSObject *object, const char *key) {
    jsval value = JSVAL_NULL;
    if (JS_GetProperty(context, object, key, &value)) {
        return [[UNIVERSE gui] textureDescriptorFromJSValue:value
                                                  inContext:context
                                          callerDescription:@ "mission.runScreen()"];
    } else {
        return nil;
    }
}
...
static JSBool MissionRunScreen(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 1 || !JS_ValueToObject(context, OOJS_ARGV[0], &params)) {
        OOJSReportBadArguments(
            context, @ "mission", @ "runScreen", MIN(argc, 1U), &OOJS_ARGV[0], nil, @ "parameter object");
        return NO;
    }
    ...
    if (JS_GetProperty(context, params, "title", &value) && !JSVAL_IS_VOID(value)) {
        ...
        if (titleKey != nil) {
            ...
            if (message != nil) {
                ...
                OOJSReportWarning(
                    context, @ "Mission.runScreen: titleKey '%@' has no entry in missiontext.plist.", titleKey);
            }
        }
    }
