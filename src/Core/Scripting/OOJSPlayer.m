...
static JSClass sPlayerClass = {"Player",
                               JSCLASS_HAS_PRIVATE,
...
void InitOOJSPlayer(JSContext *context, JSObject *global) {
    sPlayerPrototype = JS_InitClass(context,
                                    global,
                                    NULL,
                                    &sPlayerClass,
...
JSClass *JSPlayerClass(void) { return &sPlayerClass; }

JSObject *JSPlayerPrototype(void) { return sPlayerPrototype; }

JSObject *JSPlayerObject(void) { return sPlayerObject; }
...
static JSBool PlayerGetProperty(JSContext *context, JSObject *this, jsid propID, jsval *value) {
    ...
    switch (JSID_TO_INT(propID)) {
        ...
        case kPlayer_dockingClearanceStatus:
            ...
            *value =
                OOJSValueFromNativeObject(context, DockingClearanceStatusToString([player getDockingClearanceStatus]));
            return YES;
...
static JSBool PlayerCommsMessage(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (message == nil || !gotTime) {
        OOJSReportBadArguments(
            context, @ "Player", @ "commsMessage", argc, OOJS_ARGV, nil, @ "message and optional duration");
        return NO;
    }
...
static JSBool PlayerConsoleMessage(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (message == nil || !gotTime) {
        OOJSReportBadArguments(
            context, @ "Player", @ "consoleMessage", argc, OOJS_ARGV, nil, @ "message and optional duration");
        return NO;
    }
...
static JSBool PlayerReplaceShip(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (shipKey == nil) {
        OOJSReportBadArguments(
            context, @ "Player", @ "replaceShip", MIN(argc, 1U), OOJS_ARGV, nil, @ "string (shipyard key)");
        return NO;
    }
...
static JSBool PlayerSetEscapePodDestination(JSContext *context, uintN argc, jsval *vp) {
        ...
        if (destValue == nil) {
            ...
            if ([destValue isEqualToString:@ "NEARBY_SYSTEM"]) {
                ...
                if (nDests > 0)  // we have a system with a main station!
                {
                    ...
                    // add more time until rescue, with overheads for entering witchspace in case of overlapping
                    // systems.
                    double dist = [dest oo_doubleForKey:@ "distance"];
                    [player addToAdjustTime:(.2 + dist * dist) * 3600.0 + 5400.0 * (ranrot_rand() & 127)];

                    // at the end of the docking sequence we'll check if the target system is the same as the system
                    // we're in...
    ...
    if (OK == NO) {
        OOJSReportBadArguments(context,
                               ...
                               OOJS_ARGV,
                               nil,
                               @ "a valid station, null, or 'NEARBY_SYSTEM'");
    }
    return OK;
...
static JSBool PlayerSetPlayerRole(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (role == nil) {
        OOJSReportBadArguments(
            context, @ "Player", @ "setPlayerRole", MIN(argc, 1U), OOJS_ARGV, nil, @ "string (role) [, number (index)]");
        return NO;
    }
