...
static BOOL GetRelativeToAndRange(
    JSContext *context, NSString *methodName, uintN *ioArgc, jsval **ioArgv, Entity **outRelativeTo, double *outRange);
...
static JSClass sSystemClass = {"System",
                               0,
...
static JSFunctionSpec sSystemStaticMethods[] = {{"infoForSystem", SystemStaticInfoForSystem, 2},
...
void InitOOJSSystem(JSContext *context, JSObject *global) {
    sSystemPrototype = JS_InitClass(context,
                                    global,
                                    NULL,
                                    &sSystemClass,
...
static JSBool SystemGetProperty(JSContext *context, JSObject *this, jsid propID, jsval *value) {
    ...
    switch (JSID_TO_INT(propID)) {
        ...
        case kSystem_planets:
            result =
                [[[UNIVERSE planets] objectEnumeratorFilteredWithSelector:@selector(isVisibleToScripts)] allObjects];
            handled = YES;
            break;
        ...
        case kSystem_allShips:
            ...
            result = [UNIVERSE findShipsMatchingPredicate:JSEntityIsJavaScriptSearchablePredicate
                                                parameter:NULL
                                                  inRange:-1
                                                 ofEntity:nil];
        ...
        case kSystem_allDemoShips:
            OOJS_BEGIN_FULL_NATIVE(context)
            result = [UNIVERSE findShipsMatchingPredicate:JSEntityIsDemoShipPredicate
                                                parameter:NULL
                                                  inRange:-1
                                                 ofEntity:nil];
            OOJS_END_FULL_NATIVE
        ...
        case kSystem_allVisualEffects:
            OOJS_BEGIN_FULL_NATIVE(context)
            result = [UNIVERSE findVisualEffectsMatchingPredicate:JSEntityIsJavaScriptSearchablePredicate
                                                        parameter:NULL
                                                          inRange:-1
                                                         ofEntity:nil];
            OOJS_END_FULL_NATIVE
...
static JSBool SystemSetProperty(JSContext *context, JSObject *this, jsid propID, JSBool strict, jsval *value) {
    ...
    switch (JSID_TO_INT(propID)) {
        case kSystem_name:
            ...
            if (stringValue != nil) {
                [UNIVERSE setSystemDataForGalaxy:galaxy
                                          planet:system
                                             key:KEY_NAME
                                           value:stringValue
...
static JSBool SystemToString(JSContext *context, uintN argc, jsval *vp) {
    ...
    systemDesc = [NSString stringWithFormat:@ "[System %u:%u \"%@\"]",
                                            [player currentGalaxyID],
                                            [player currentSystemID],
                                            [[UNIVERSE currentSystemData] objectForKey:KEY_NAME]];
...
static JSBool SystemCountShipsWithPrimaryRole(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(role == nil)) {
        OOJSReportBadArguments(
            context, @ "System", @ "countShipsWithPrimaryRole", MIN(argc, 1U), OOJS_ARGV, nil, @ "string (role)");
        return NO;
    }
    ...
    if (EXPECT_NOT(!GetRelativeToAndRange(context, @ "countShipsWithPrimaryRole", &argc, &argv, &relativeTo, &range)))
        return NO;
...
static JSBool SystemCountShipsWithRole(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(role == nil)) {
        OOJSReportBadArguments(
            context, @ "System", @ "countShipsWithRole", MIN(argc, 1U), OOJS_ARGV, nil, @ "string (role)");
        return NO;
    }
    ...
    argc -= 1;
    jsval *argv = OOJS_ARGV + 1;
    if (EXPECT_NOT(!GetRelativeToAndRange(context, @ "countShipsWithRole", &argc, &argv, &relativeTo, &range)))
        return NO;
...
static JSBool SystemShipsWithPrimaryRole(JSContext *context, uintN argc, jsval *vp) {
    ...
    argc -= 1;
    jsval *argv = OOJS_ARGV + 1;
    if (EXPECT_NOT(!GetRelativeToAndRange(context, @ "shipsWithPrimaryRole", &argc, &argv, &relativeTo, &range)))
        return NO;
...
static JSBool SystemCountEntitiesWithScanClass(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (scanClass == CLASS_NOT_SET) {
        OOJSReportBadArguments(
            context, @ "System", @ "countEntitiesWithScanClass", MIN(argc, 1U), OOJS_ARGV, nil, @ "string (scan class)");
        return NO;
    }
    ...
    argc -= 1;
    jsval *argv = OOJS_ARGV + 1;
    if (EXPECT_NOT(!GetRelativeToAndRange(context, @ "countEntitiesWithScanClass", &argc, &argv, &relativeTo, &range)))
        return NO;
...
static JSBool SystemEntitiesWithScanClass(JSContext *context, uintN argc, jsval *vp) {
    ...
    argc -= 1;
    jsval *argv = OOJS_ARGV + 1;
    if (EXPECT_NOT(!GetRelativeToAndRange(context, @ "entitiesWithScanClass", &argc, &argv, &relativeTo, &range)))
        return NO;
...
static JSBool SystemFilteredEntities(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 2 || !OOJSValueIsFunction(context, OOJS_ARGV[1]) || !JS_ValueToObject(context, OOJS_ARGV[0], &jsThis)) {
        OOJSReportBadArguments(context,
                               ...
                               OOJS_ARGV,
                               nil,
                               @ "this, predicate function, and optional reference entity and range");
        return NO;
    }
...
static JSBool SystemLegacyAddShips(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc > 0) role = OOStringFromJSValue(context, OOJS_ARGV[0]);
    if (EXPECT_NOT(role == nil || !JS_ValueToInt32(context, OOJS_ARGV[1], &count) || argc < 2 || count < 1 ||
                   64 < count)) {
        OOJSReportBadArguments(context,
                               @ "System",
                               @ "legacy_addShips",
                               ...
                               OOJS_ARGV,
                               nil,
                               @ "role and positive count no greater than 64");
        return NO;
    }
...
static JSBool SystemLegacyAddSystemShips(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc > 0) role = OOStringFromJSValue(context, OOJS_ARGV[0]);
    if (EXPECT_NOT(role == nil || !JS_ValueToInt32(context, OOJS_ARGV[1], &count) || count < 1 || 64 < count ||
                   argc < 3 || !JS_ValueToNumber(context, OOJS_ARGV[2], &position))) {
        OOJSReportBadArguments(context,
                               @ "System",
                               ...
                               OOJS_ARGV,
                               nil,
                               @ "role, positive count no greater than 64, and position along route");
        return NO;
    }
...
// legacy_addShipsWithinRadius(role : String, count : Number, coordScheme : String, coords : vectorExpression, radius :
// Number)
static JSBool SystemLegacyAddShipsWithinRadius(JSContext *context, uintN argc, jsval *vp) {
    ...
    OOJS_BEGIN_FULL_NATIVE(context)
    arg = [NSString
        stringWithFormat:@ "%@ %d %@ %f %f %f %f", role, count, coordScheme, where.x, where.y, where.z, radius];
...
static JSBool SystemLegacySpawnShip(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (key == nil) {
        OOJSReportBadArguments(
            context, @ "System", @ "legacy_spawnShip", MIN(argc, 1U), OOJS_ARGV, nil, @ "string (ship key)");
        return NO;
    }
...
static JSBool SystemStaticSystemNameForID(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 1 || !JS_ValueToInt32(context, OOJS_ARGV[0], &systemID) || systemID < -1 ||
        kOOMaximumSystemID < systemID)  // -1 interstellar space!
    {
...
static JSBool SystemStaticInfoForSystem(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 2 || !JS_ValueToInt32(context, OOJS_ARGV[0], &galaxyID) ||
        !JS_ValueToInt32(context, OOJS_ARGV[1], &systemID)) {
    ...
    if (galaxyID < 0 || galaxyID > kOOMaximumGalaxyID) {
        OOJSReportBadArguments(context,
                               ...
                               OOJS_ARGV,
                               @ "Invalid galaxy ID",
                               [NSString stringWithFormat:@ "number in the range 0 to %u", kOOMaximumGalaxyID]);
        return NO;
    }
    ...
    if (systemID < kOOMinimumSystemID || systemID > kOOMaximumSystemID) {
        OOJSReportBadArguments(
            ...
            OOJS_ARGV + 1,
            @ "Invalid system ID",
            [NSString stringWithFormat:@ "number in the range %i to %i", kOOMinimumSystemID, kOOMaximumSystemID]);
        return NO;
    }
...
static JSBool SystemAddVisualEffect(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (key == nil) {
        OOJSReportBadArguments(
            context, @ "System", @ "addVisualEffect", MIN(argc, 1U), &OOJS_ARGV[0], nil, @ "string (key)");
        return NO;
    }

    if (!VectorFromArgumentListNoError(context, argc - 1, OOJS_ARGV + 1, &where, &consumed)) {
        OOJSReportBadArguments(
            context, @ "System", @ "addVisualEffect", MIN(argc - 1, 1U), &OOJS_ARGV[1], nil, @ "vector");
        return NO;
    }
...
static JSBool SystemSetPopulator(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 1) {
        OOJSReportBadArguments(
            context, @ "System", @ "setPopulator", MIN(argc, 0U), &OOJS_ARGV[0], nil, @ "string (key), object (settings)");
        return NO;
    }
    key = OOStringFromJSValue(context, OOJS_ARGV[0]);
    if (key == nil) {
        OOJSReportBadArguments(
            context, @ "System", @ "setPopulator", MIN(argc, 0U), &OOJS_ARGV[0], nil, @ "key, settings");
        return NO;
    }
    if (argc < 2 || JSVAL_IS_NULL(OOJS_ARGV[1])) {
        ...
        if (!JS_ValueToObject(context, OOJS_ARGV[1], &params)) {
            OOJSReportBadArguments(
                context, @ "System", @ "setPopulator", MIN(argc, 1U), OOJS_ARGV, NULL, @ "key, settings");
            return NO;
        }
        ...
        if (JS_GetProperty(context, params, "callback", &callback) == JS_FALSE || JSVAL_IS_VOID(callback)) {
            OOJSReportBadArguments(context,
                                   ...
                                   OOJS_ARGV,
                                   NULL,
                                   @ "settings must have a 'callback' property.");
            return NO;
        }
        ...
        if (JS_GetProperty(context, params, "coordinates", &coords) != JS_FALSE && !JSVAL_IS_VOID(coords)) {
            ...
            if (JSValueToVector(context, coords, &coordinates)) {
                ...
                [settings setObject:[NSArray arrayWithObjects:[NSNumber numberWithFloat:coordinates.x],
                                                              [NSNumber numberWithFloat:coordinates.y],
                                                              [NSNumber numberWithFloat:coordinates.z],
                                                              nil]
                             forKey:@ "coordinates"];
            }
        }
...
static JSBool SystemSetWaypoint(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 4 || JSVAL_IS_NULL(OOJS_ARGV[3])) {
        ...
        [settings setObject:[NSArray arrayWithObjects:[NSNumber numberWithDouble:position.x],
                                                      [NSNumber numberWithDouble:position.y],
                                                      [NSNumber numberWithDouble:position.z],
                                                      nil]
                     forKey:@ "position"];
        [settings setObject:[NSArray arrayWithObjects:[NSNumber numberWithDouble:orientation.w],
                                                      [NSNumber numberWithDouble:orientation.x],
                                                      [NSNumber numberWithDouble:orientation.y],
                                                      [NSNumber numberWithDouble:orientation.z],
                                                      nil]
                     forKey:@ "orientation"];
...
static JSBool SystemAddShipsOrGroup(JSContext *context, uintN argc, jsval *vp, BOOL isGroup) {
    ...
    if (argc < 2 || !JS_ValueToInt32(context, OOJS_ARGV[1], &count) || count < 1 || 64 < count) {
        OOJSReportBadArguments(context,
                               ...
                               &OOJS_ARGV[1],
                               nil,
                               @ "number (positive count no greater than 64)");
        return NO;
    }
        ...
        if (argc > 2 + consumed) {
            if (!JS_ValueToNumber(context, OOJS_ARGV[2 + consumed], &radius)) {
                OOJSReportBadArguments(context,
                                       @ "System",
                                       func,
                                       MIN(argc - 2 - consumed, 1U),
                                       &OOJS_ARGV[2 + consumed],
                                       nil,
                                       @ "number (radius)");
                return NO;
            }
    ...
    if (isGroup) {
        ...
        if ([array count] > 0)
            result = [(ShipEntity *)[array objectAtIndex:0] group];
        else
            result = nil;
    }
    OOJS_END_FULL_NATIVE
...
static JSBool SystemAddShipsOrGroupToRoute(JSContext *context, uintN argc, jsval *vp, BOOL isGroup) {
    ...
    if (argc > 2) {
        if (!JS_ValueToNumber(context, OOJS_ARGV[2], &where) || !isfinite(where) || where < 0.0f || where > 1.0f) {
            OOJSReportBadArguments(
                context, @ "System", func, MIN(argc - 2, 1U), &OOJS_ARGV[2], nil, @ "number (position along route)");
            return NO;
        }
        ...
        if (argc > 3) {
            ...
            if (route == nil || ![validRoutes containsObject:route]) {
                OOJSReportBadArguments(
                    context, @ "System", func, MIN(argc - 3, 1U), &OOJS_ARGV[3], nil, @ "string (route specifier)");
                return NO;
            }
        }
    }
...
static NSArray *FindJSVisibleEntities(EntityFilterPredicate predicate,
    ...
    BinaryOperationPredicateParameter param = {JSEntityIsJavaScriptSearchablePredicate, NULL, predicate, parameter};

    result = [UNIVERSE findEntitiesMatchingPredicate:ANDPredicate parameter:&param inRange:range ofEntity:relativeTo];
...
static NSComparisonResult CompareEntitiesByDistance(id a, id b, void *relativeTo) {
    ...
    Entity *ea = a, *eb = b, *r = (id)relativeTo;
    float d1, d2;
