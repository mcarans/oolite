...
JSClass gOOEntityJSClass = {"Entity",
                            JSCLASS_HAS_PRIVATE,
...
enum {
    ...
    kEntity_owner,  // owner, Entity, read-only. (Parent ship for subentities, station for defense ships, launching ship
                    // for missiles etc)
...
void InitOOJSEntity(JSContext *context, JSObject *global) {
    gOOEntityJSPrototype = JS_InitClass(context,
                                        global,
                                        NULL,
                                        &gOOEntityJSClass,
    ...
    if (EXPECT_NOT(!JSValueToEntity(context, argv[0], outEntity))) {
        ...
        if (scriptClass != nil && function != nil) {
            OOJSReportWarning(context,
                              ...
                              scriptClass,
                              function,
                              [NSString stringWithJavaScriptParameters:argv count:1 inContext:context]);
            return NO;
        }
    }
...
static JSBool EntityGetProperty(JSContext *context, JSObject *this, jsid propID, jsval *value) {
    ...
    if (OOIsStaleEntity(entity)) {
        if (JSID_TO_INT(propID) == kEntity_isValid)
            *value = JSVAL_FALSE;
        else {
            *value = JSVAL_VOID;
        }
    ...
    switch (JSID_TO_INT(propID)) {
        ...
        case kEntity_scanClass:
            if ([entity isShip] && ![entity isPlayer]) {
                ...
                if (newClass == CLASS_NOT_SET || newClass == CLASS_NO_DRAW || newClass == CLASS_TARGET ||
                    newClass == CLASS_WORMHOLE || newClass == CLASS_PLAYER || newClass == CLASS_VISUAL_EFFECT) {
