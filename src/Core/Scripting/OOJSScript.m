...
static JSScript *LoadScriptWithName(
    JSContext *context, NSString *path, JSObject *object, JSObject **outScriptObject, NSString **outErrorMessage);
...
static JSClass sScriptClass = {"Script",
                               JSCLASS_HAS_PRIVATE,
    ...
    if (self == nil)
        ...
        if (JS_IsExceptionPending(context)) {
            ...
            OOLogERR(
                ...
                @ "Prior to loading script %@, there was a pending JavaScript exception, which has been cleared. This is an internal error, please report it.",
                path);
        }
        ...
        RunningStack stackElement = {.back = sRunningStack, .current = self};
        ...
        */
        ...
        if (!problem) {
            ...
            OOLog(@ "script.javaScript.load.success",
                  @ "Loaded JavaScript: %@ -- %@",
                  [self displayName],
                  description ? description : (NSString *)@ "(no description)");
        }
...
- (BOOL)callMethod:(jsid)methodID
    ...
    if (EXPECT(JS_GetMethodById(context, _jsSelf, methodID, &root, &method) && !JSVAL_IS_VOID(method))) {
        ...
        if (JS_IsExceptionPending(context)) {
            OOLog(
                @ "script.internalBug",
                @ "Exception pending on context before calling method in %s, clearing. This is an internal error, please report it.",
                __PRETTY_FUNCTION__);
            JS_ClearPendingException(context);
        }
...
- (NSString *)scriptNameFromPath:(NSString *)path {
    ...
    if (path == nil)
        theName = [NSString stringWithFormat:@ "%p", self];
    else {
        lastComponent = [path lastPathComponent];
        if (![lastComponent hasPrefix:@ "script."])
...
- (NSDictionary *)defaultPropertiesFromPath:(NSString *)path {
    ...
    NSString *manifestPath = [[[path stringByDeletingLastPathComponent] stringByDeletingLastPathComponent]
        stringByAppendingPathComponent:@ "manifest.plist"];
...
void InitOOJSScript(JSContext *context, JSObject *global) {
    sScriptPrototype = JS_InitClass(
        context, global, NULL, &sScriptClass, OOJSUnconstructableConstruct, 0, NULL, sScriptMethods, NULL, NULL);
    ...
    if (JSID_IS_STRING(propID)) {
        ...
        if (JS_StringEqualsAscii(context, propName, "tickle", &match) && match) {
            ...
            OOJSReportWarning(context,
                              @ "Script %@ appears to use the tickle() event handler, which is no longer supported.",
                              [thisScript name]);
        }
    }
    ...
    *outErrorMessage = nil;
    ...
    if (script == NULL) {
        ...
        if (fileContents != nil) {
             ...
             * that here. */
            if ([fileContents rangeOfString:@ "\"use strict\";"].location == NSNotFound &&
                [fileContents rangeOfString:@ "'use strict';"].location == NSNotFound) {
        ...
        if (data == nil)
            *outErrorMessage = @ "could not load file";
        else {
            script = JS_CompileUCScript(
                context, object, [data bytes], [data length] / sizeof(unichar), [path UTF8String], 1);
            if (script != NULL)
                *outScriptObject = JS_NewScriptObject(context, script);
            else
                *outErrorMessage = @ "compilation failed";
        }
