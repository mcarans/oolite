...
static JSPropertySpec sEquipmentInfoStaticProperties[] = {
    {"allEquipment", 0, OOJS_PROP_READONLY_CB, EquipmentInfoGetAllEqipment}, {0}};
...
static JSClass sEquipmentInfoClass = {"EquipmentInfo",
                                      JSCLASS_HAS_PRIVATE,
...
void InitOOJSEquipmentInfo(JSContext *context, JSObject *global) {
    sEquipmentInfoPrototype = JS_InitClass(context,
                                           global,
                                           NULL,
                                           &sEquipmentInfoClass,
    ...
    switch (JSID_TO_INT(propID)) {
        ...
        case kEquipmentInfo_calculatedPrice:
            if ([[eqType identifier] isEqual:@ "EQ_FUEL"]) {
                return JS_NewNumberValue(context,
                                         (PLAYER_MAX_FUEL - [OOPlayerForScripting() fuel]) * [eqType price] *
                                             [OOPlayerForScripting() fuelChargeRate],
                                         value);
                ...
                return JS_NewNumberValue(context,
                                         [OOPlayerForScripting() adjustPriceByScriptForEqKey:[eqType identifier]
                                                                                 withCurrent:[eqType price]],
                                         value);
            }
    ...
    switch (JSID_TO_INT(propID)) {
        ...
        case kEquipmentInfo_effectiveTechLevel:
            OOStandardsDeprecated(
                [NSString stringWithFormat:@ "TL99 for variable tech level is deprecated for %@", [eqType identifier]]);
            if (!OOEnforceStandards() && [eqType techLevel] == kOOVariableTechLevel) {
                if (JSVAL_IS_NULL(*value)) {
                    // reset mission variable
                    [OOPlayerForScripting()
                        setMissionVariable:nil
                ...
                if (JS_ValueToInt32(context, *value, &iValue)) {
                    ...
                    [OOPlayerForScripting()
                        setMissionVariable:[NSString stringWithFormat:@ "%u", iValue]
                                    forKey:[@ "mission_TL_FOR_" stringByAppendingString:[eqType identifier]]];
                    return YES;
                }
            } else {
                OOJSReportWarning(context,
                                  @ "Cannot modify effective tech level for %@, because its base tech level is not 99.",
                                  [eqType identifier]);
                return YES;
            }
            break;
