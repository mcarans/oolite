...
#define INCLUDE_RAW_STRING \
    OOLITE_DEBUG  // If nonzero, raw condition strings are included; if zero, a placeholder is used.
...
static OOOperationType ClassifyLHSConditionSelector(NSString *selectorString,
                                                    NSString **outSanitizedMethod,
                                                    SanStackElement *stack);
static NSString *SanitizeQueryMethod(
    NSString *selectorString);  // Checks aliases and whitelist, returns nil if whitelist fails.
static NSString *SanitizeActionMethod(
    NSString *selectorString, BOOL allowAIMethods);  // Checks aliases and whitelist, returns nil if whitelist fails.
...
static NSArray *OOSanitizeLegacyScriptInternal(NSArray *script, SanStackElement *stack, BOOL allowAIMethods) {
    ...
    for (statementEnum = [script objectEnumerator]; (statement = [statementEnum nextObject]);) {
        SanStackElement subStack = {stack, nil, index++};
        ...
        if ([statement isKindOfClass:[NSDictionary class]]) {
            ...
            OOLog(@ "script.syntax.statement.invalidType",
...
static NSArray *OOSanitizeLegacyScriptConditionsInternal(NSArray *conditions, SanStackElement *stack) {
    ...
    for (conditionEnum = [conditions objectEnumerator]; (condition = [conditionEnum nextObject]);) {
        ...
        if (![condition isKindOfClass:[NSString class]]) {
            OOLog(@ "script.syntax.condition.notString",
...
static NSArray *SanitizeCondition(NSString *condition, SanStackElement *stack) {
    ...
    if (tokenCount < 1) {
        OOLog(@ "script.debug.syntax.scriptCondition.noneSpecified",
    ...
    if (opType >= OP_INVALID) {
        OOLog(@ "script.unpermittedMethod",
              @ "***** SCRIPT ERROR: in %@ (\"%@\"), method \"%@\" not allowed.",
              StringFromStack(stack),
              condition,
              selectorString);
        return nil;
    ...
    if (tokenCount > 1) {
        ...
        if ([comparatorString isEqualToString:@ "equal"])
            comparatorValue = COMPARISON_EQUAL;
        else if ([comparatorString isEqualToString:@ "notequal"])
            comparatorValue = COMPARISON_NOTEQUAL;
        else if ([comparatorString isEqualToString:@ "lessthan"])
            comparatorValue = COMPARISON_LESSTHAN;
        else if ([comparatorString isEqualToString:@ "greaterthan"])
            comparatorValue = COMPARISON_GREATERTHAN;
        else if ([comparatorString isEqualToString:@ "morethan"])
            comparatorValue = COMPARISON_GREATERTHAN;
        else if ([comparatorString isEqualToString:@ "oneof"])
            comparatorValue = COMPARISON_ONEOF;
        else if ([comparatorString isEqualToString:@ "undefined"])
            comparatorValue = COMPARISON_UNDEFINED;
        else {
            OOLog(@ "script.debug.syntax.badComparison",
                  @ "***** SCRIPT ERROR: in %@ (\"%@\"), unknown comparison operator \"%@\", will return NO.",
                  StringFromStack(stack),
                  condition,
                  comparatorString);
            return nil;
        ...
        /*	In the direct interpreter, having no operator resulted in an
                ...
                has the same effect.
         */
        OOLog(@ "script.debug.syntax.noOperator",
              @ "----- WARNING: SCRIPT in %@ -- No operator in expression \"%@\", will always evaluate as false.",
              StringFromStack(stack),
              condition);
        return nil;
    }
    ...
    if (opType == OP_NUMBER && comparatorValue == COMPARISON_UNDEFINED) {
        OOLog(@ "script.debug.syntax.invalidOperator",
              @ "***** SCRIPT ERROR: in %@ (\"%@\"), comparison operator \"%@\" is not valid for %@.",
              StringFromStack(stack),
              condition,
              @ "undefined",
              @ "numbers");
    ...
    */
    if (tokenCount > 2) {
        ...
        for (i = 2; i < tokenCount; i++) {
            ...
            if (rhsSelector != nil) {
                ...
                if (stringSegment == nil)
                    stringSegment = rhsItem;
                else
                    stringSegment = [NSString stringWithFormat:@ "%@ %@", stringSegment, rhsItem];
            }
        }
    ...
    return [NSArray arrayWithObjects:[NSNumber numberWithUnsignedInt:opType],
                                     rawString,
...
static NSArray *SanitizeConditionalStatement(NSDictionary *statement, SanStackElement *stack, BOOL allowAIMethods) {
    ...
    if (conditions == nil) {
        OOLog(@ "script.syntax.noConditions",
              @ "***** SCRIPT ERROR: in %@, conditions array contains no \"conditions\" entry, ignoring.",
              StringFromStack(stack));
        return nil;
    }
...
static NSArray *SanitizeActionStatement(NSString *statement, SanStackElement *stack, BOOL allowAIMethods) {
    ...
    if ([selectorString hasSuffix:@ ":"]) {
        ...
        argument = [argument stringByReplacingOccurrencesOfString:@ "[credits_number]"
                                                       withString:@ "[_oo_legacy_credits_number]"];
    }
...
static OOOperationType ClassifyLHSConditionSelector(NSString *selectorString,
    ...
    *outSanitizedSelector = SanitizeQueryMethod(selectorString);
    ...
    OOLog(@ "script.sanitize.unclassifiedSelector",
          @ "***** ERROR: Whitelisted query method \"%@\" has no type suffix, treating as invalid.",
          selectorString);
    return OP_INVALID;
}
...
static NSString *SanitizeQueryMethod(NSString *selectorString) {
    ...
    if (whitelist == nil) {
        whitelist =
            [[NSSet alloc] initWithArray:[[ResourceManager whitelistDictionary] oo_arrayForKey:@ "query_methods"]];
...
static NSString *StringFromStack(SanStackElement *topOfStack) { return StringFromStackInternal(topOfStack); }
