...
static BOOL ValidateContracts(JSContext *context,
                              uintN argc,
                              jsval *vp,
                              BOOL isCargo,
                              OOSystemID *start,
...
static JSClass sPlayerShipClass = {"PlayerShip",
                                   JSCLASS_HAS_PRIVATE,
...
enum {
    ...
    kPlayerShip_galacticHyperspaceBehaviour,    // can be standard, all systems reachable or fixed coordinates, integer,
                                                // read-only
    kPlayerShip_galacticHyperspaceFixedCoords,  // used when fixed coords behaviour is selected, Vector3D, read/write
    kPlayerShip_galacticHyperspaceFixedCoordsInLY,  // used when fixed coords behaviour is selected, Vector3D,
                                                    // read/write
    ...
    kPlayerShip_reticleColorTargetSensitive,  // Reticle color for targets picked up when sensitive mode is on, array,
                                              // read/write
    ...
    kPlayerShip_reticleTargetSensitive,       // target box changes color when primary target in crosshairs, boolean,
                                              // read/write
...
void InitOOJSPlayerShip(JSContext *context, JSObject *global) {
    sPlayerShipPrototype = JS_InitClass(context,
                                        global,
                                        JSShipPrototype(),
                                        &sPlayerShipClass,
                                        OOJSUnconstructableConstruct,
                                        0,
                                        sPlayerShipProperties,
                                        sPlayerShipMethods,
                                        NULL,
                                        NULL);
    ...
    PlayerEntity *player = [PlayerEntity
        sharedPlayer];  // NOTE: at time of writing, this creates the player entity. Don't use PLAYER here.

    // Create ship object as a property of the player object.
    sPlayerShipObject =
        JS_DefineObject(context, JSPlayerObject(), "ship", &sPlayerShipClass, sPlayerShipPrototype, OOJS_PROP_READONLY);
...
JSClass *JSPlayerShipClass(void) { return &sPlayerShipClass; }

JSObject *JSPlayerShipPrototype(void) { return sPlayerShipPrototype; }

JSObject *JSPlayerShipObject(void) { return sPlayerShipObject; }
...
static JSBool PlayerShipGetProperty(JSContext *context, JSObject *this, jsid propID, jsval *value) {
    ...
    if (EXPECT_NOT(OOIsPlayerStale() || this == sPlayerShipPrototype)) {
        *value = JSVAL_VOID;
        return YES;
    }
    ...
    switch (JSID_TO_INT(propID)) {
        ...
        case kPlayerShip_galacticHyperspaceFixedCoordsInLY:
            return VectorToJSValue(
                context, OOGalacticCoordinatesFromInternal([player galacticHyperspaceFixedCoords]), value);
        ...
        case kPlayerShip_price:
            return JS_NewNumberValue(
                context, [UNIVERSE tradeInValueForCommanderDictionary:[player commanderDataDictionary]], value);
    ...
    switch (JSID_TO_INT(propID)) {
        ...
        case kPlayerShip_chartHightlightMode:
            ...
            if (sValue != nil) {
                ...
                if (chartMode > OOLRC_MODE_UNKNOWN) {
                    ...
                    [player doScriptEvent:OOJSID("chartHighlightModeChanged")
                             withArgument:OOStringFromLongRangeChartMode([player longRangeChartMode])];
                    return YES;
                } else {
                    OOJSReportError(
                        context,
        ...
        case kPlayerShip_compassTarget:
            ...
            if (!JSVAL_IS_NULL(*value) && JSValueToEntity(context, *value, &eValue)) {
                ...
                [player validateCompassTarget];
                // cycle the targets until we either get back to the start (entity not found) or we find the one we're
                // looking for
...
static JSBool PlayerShipUseSpecialCargo(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(name == nil)) {
        OOJSReportBadArguments(context,
                               ...
                               OOJS_ARGV,
                               nil,
                               @ "string (special cargo description)");
        return NO;
    }
...
static JSBool PlayerShipRequestDockingClearance(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (stationForDocking == nil) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", @ "requestDockingClearance", MIN(argc, 1U), OOJS_ARGV, nil, @ "station");
        return NO;
    }
...
static JSBool PlayerShipCancelDockingRequest(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (stationForDocking == nil) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", @ "cancelDockingRequest", MIN(argc, 1U), OOJS_ARGV, nil, @ "station");
        return NO;
    }
...
static JSBool PlayerShipAddPassenger(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 5) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", @ "addPassenger", argc, OOJS_ARGV, nil, @ "name, start, destination, ETA, fee");
        return NO;
    }
    ...
    if (!ValidateContracts(context, argc, vp, NO, &start, &destination, &eta, &fee, &advance, @ "addPassenger", &risk))
        return NO;  // always go through validate contracts (passenger)
...
static JSBool PlayerShipAddParcel(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 5) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", @ "addParcel", argc, OOJS_ARGV, nil, @ "name, start, destination, ETA, fee");
        return NO;
    }
    ...
    if (!ValidateContracts(context, argc, vp, NO, &start, &destination, &eta, &fee, &premium, @ "addParcel", &risk))
        return NO;  // always go through validate contracts (passenger/parcel mode)
...
static JSBool PlayerShipAwardContract(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (!JS_ValueToInt32(context, OOJS_ARGV[0], &qty)) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", @ "awardContract", 1, &OOJS_ARGV[0], nil, @ "positive integer (cargo quantity)");
        return NO;
    }
    ...
    if (EXPECT_NOT(key == nil)) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", @ "awardContract", 1, &OOJS_ARGV[1], nil, @ "string (commodity identifier)");
        return NO;
    }

    if (!ValidateContracts(context, argc, vp, YES, &start, &destination, &eta, &fee, &premium, @ "awardContract", NULL))
        return NO;  // always go through validate contracts (cargo)

    BOOL OK = [player awardContract:qty
                          commodity:key
                              start:start
...
static JSBool PlayerShipRemoveContract(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 2) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", @ "removeContract", argc, OOJS_ARGV, nil, @ "commodity, destination");
        return NO;
    }
    ...
    if (EXPECT_NOT(key == nil)) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", @ "removeContract", 1, &OOJS_ARGV[0], nil, @ "string (commodity identifier)");
        return NO;
    }
...
static JSBool PlayerShipSetCustomView(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 2) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", @ "setCustomView", argc, OOJS_ARGV, nil, @ "position, orientiation, [weapon]");
        return NO;
    }
    ...
    NSString *orientationstr =
        [[NSString alloc] initWithFormat:@ "%f %f %f %f", orientation.w, orientation.x, orientation.y, orientation.z];
...
static JSBool PlayerShipBeginGalacticHyperspaceCountdown(JSContext *context, uintN argc, jsval *vp) {
    ...
    if ([player hasEquipmentItemProviding:@ "EQ_GAL_DRIVE"] && [player status] == STATUS_IN_FLIGHT &&
        [player witchJumpChecklist:true]) {
        ...
        // say it!
        [UNIVERSE addMessage:[NSString stringWithFormat:DESC(@ "witch-galactic-in-f-seconds"), witchspaceSpinUpTime]
                    forCount:1.0];
        begun = YES;
    }
    OOJS_RETURN_BOOL(begun);
...
static JSBool PlayerShipSetPrimedEquipment(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (key == nil) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", @ "setPrimedEquipment", MIN(argc, 1U), OOJS_ARGV, nil, @ "string (key)");
        return NO;
    }
    if (argc > 1 && EXPECT_NOT(!JS_ValueToBoolean(context, OOJS_ARGV[1], &showMsg))) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", @ "setPrimedEquipment", MIN(argc, 2U), OOJS_ARGV, nil, @ "boolean");
        return NO;
    }
...
static JSBool PlayerShipSetCustomHUDDial(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (key == nil) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", @ "setCustomHUDDial", MIN(argc, 1U), OOJS_ARGV, nil, @ "string (key), value]");
        return NO;
    }
    if (argc > 1) {
...
static JSBool PlayerShipHideHUDSelector(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (key == nil) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", @ "hideHUDSelector", MIN(argc, 1U), OOJS_ARGV, nil, @ "string (selector)");
        return NO;
    }
...
static BOOL ValidateContracts(JSContext *context,
    ...
    NSCParameterAssert(context != NULL && vp != NULL && start != NULL && destination != NULL && eta != NULL &&
                       fee != NULL);
    ...
    *destination = iValue;
    ...
    if (!JS_ValueToNumber(context, OOJS_ARGV[offset + 2], &fValue) || !isfinite(fValue) ||
        fValue <= [PLAYER clockTime]) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", functionName, 1, &OOJS_ARGV[offset + 2], nil, @ "number (future time)");
        return NO;
    }
    *eta = fValue;
    ...
    if (!JS_ValueToNumber(context, OOJS_ARGV[offset + 3], &fValue) || !isfinite(fValue) || fValue < 0.0) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", functionName, 1, &OOJS_ARGV[offset + 3], nil, @ "number (credits quantity)");
        return NO;
    }
    *fee = fValue;

    if (argc > offset + 4 && JS_ValueToNumber(context, OOJS_ARGV[offset + 4], &fValue) && isfinite(fValue) &&
        fValue >= 0.0) {
        *premium = fValue;
    }

    if (!isCargo) {
        if (argc > offset + 5 && JS_ValueToECMAUint32(context, OOJS_ARGV[offset + 5], &uValue) &&
            isfinite((double)uValue)) {
            *risk = uValue;
        }
    }
