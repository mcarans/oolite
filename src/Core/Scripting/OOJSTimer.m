...
- (void)timerFired {
    ...
    [engine callJSFunction:_function forObject:_jsThis argc:0 argv:NULL result:&rval];
...
static JSClass sTimerClass = {"Timer",
                              JSCLASS_HAS_PRIVATE,
...
void InitOOJSTimer(JSContext *context, JSObject *global) {
    sTimerPrototype = JS_InitClass(
        context, global, NULL, &sTimerClass, TimerConstruct, 0, sTimerProperties, sTimerMethods, NULL, NULL);
        ...
        case kTimer_nextTime:
            if (JS_ValueToNumber(context, *value, &fValue)) {
                if (![timer setNextTime:fValue]) {
                    OOJSReportWarning(
                        context, @ "Ignoring attempt to change next fire time for running timer %@.", timer);
                }
                return YES;
            }
            break;
    ...
    if (timer != nil) {
        if ([timer isScheduled]) {
            OOLogWARN(
                ...
                @ "Timer %@ is being garbage-collected while still running. You must keep a reference to all running timers, or they will stop unpredictably!",
                timer);
        }
        [timer release];
...
static JSBool TimerConstruct(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (JS_ValueToFunction(context, function) == NULL) {
        OOJSReportBadArguments(
            context, nil, @ "Timer", 1, OOJS_ARGV + 1, @ "Invalid argument in constructor", @ "function");
        return NO;
    }
