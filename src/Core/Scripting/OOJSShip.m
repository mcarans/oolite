...
static JSBool ShipSetMaterialsInternal(
    JSContext *context, uintN argc, jsval *vp, ShipEntity *thisEnt, BOOL fromShaders);
...
void InitOOJSShip(JSContext *context, JSObject *global) {
    sShipPrototype = JS_InitClass(context,
                                  global,
                                  JSEntityPrototype(),
                                  &sShipClass,
...
JSClass *JSShipClass(void) { return &sShipClass; }
    ...
    switch (JSID_TO_INT(propID)) {
        ...
        case kShip_isFleeing:
            if ([entity isPlayer]) {
                ...
                *value = OOJSValueFromBOOL([entity behaviour] == BEHAVIOUR_FLEE_TARGET ||
                                           [entity behaviour] == BEHAVIOUR_FLEE_EVASIVE_ACTION);
            }
            return YES;
...
static JSBool ShipSetProperty(JSContext *context, JSObject *this, jsid propID, JSBool strict, jsval *value) {
    ...
    switch (JSID_TO_INT(propID)) {
            ...
            if (JS_ValueToInt32(context, *value, &iValue)) {
                if (iValue < 0 || iValue > (int32)ENTITY_PERSONALITY_MAX) {
                    OOJSReportError(context,
                                    @ "ship.%@ must be >= 0 and <= %u.",
                                    OOStringFromJSPropertyIDAndSpec(context, propID, sShipProperties),
        ...
        case kShip_injectorSpeedFactor:
            if (JS_ValueToNumber(context, *value, &fValue)) {
                if (fValue < 1) {
                    ...
                    OOJSReportError(context,
                                    @ "ship.injectorSpeedFactor cannot be higher than minimum torus speed factor (%f).",
                                    MIN_HYPERSPEED_FACTOR);
#else
                else if (fValue > HYPERSPEED_FACTOR) {
                    OOJSReportError(context,
                                    @ "ship.injectorSpeedFactor cannot be higher than torus speed factor (%f).",
                                    HYPERSPEED_FACTOR);
#endif
                    return NO;
                }
        ...
        case kShip_lightsActive:
            if (JS_ValueToBoolean(context, *value, &bValue)) {
                if (bValue)
                    [entity switchLightsOn];
                else
                    [entity switchLightsOff];
                return YES;
            }
            break;
        ...
        case kShip_maxEscorts:
            ...
            if (JS_ValueToInt32(context, *value, &iValue)) {
                if ((NSInteger)iValue < (NSInteger)[[entity escortGroup] count] - 1) {
                    OOJSReportError(context,
                                    @ "ship.%@ must be >= current escort numbers.",
                                    OOStringFromJSPropertyIDAndSpec(context, propID, sShipProperties));
                    return NO;
                }
...
playerReadOnly:
    OOJSReportError(
        context, @ "player.ship.%@ is read-only.", OOStringFromJSPropertyIDAndSpec(context, propID, sShipProperties));
    return NO;

    // Not used (yet)
    /*
    npcReadOnly:
            OOJSReportError(context, @"npc.ship.%@ is read-only.", OOStringFromJSPropertyIDAndSpec(context, propID,
    sShipProperties)); return NO;
    */
...
static JSBool ShipHasEquipmentProviding(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(equipment == nil)) {
        OOJSReportBadArguments(
            context, @ "Ship", @ "hasEquipmentProviding", MIN(argc, 1U), OOJS_ARGV, nil, @ "string (equipment)");
        return NO;
    }
...
static JSBool ShipAddCargoEntity(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT([thisEnt isPlayer] && [(PlayerEntity *)thisEnt isDocked])) {
        OOJSReportWarningForCaller(
            context, @ "PlayerShip", @ "addCargoEntity", @ "Can't add cargo entity while docked, ignoring.");
        return NO;
    }
    if (EXPECT_NOT(argc == 0 || JSVAL_IS_NULL(OOJS_ARGV[0]) || !JSVAL_IS_OBJECT(OOJS_ARGV[0]) ||
                   !JSShipGetShipEntity(context, JSVAL_TO_OBJECT(OOJS_ARGV[0]), &target))) {
        OOJSReportBadArguments(
            context, @ "PlayerShip", @ "addCargoEntity", MIN(argc, 1U), OOJS_ARGV, nil, @ "scoopable entity.");
        return NO;
    }
...
static JSBool ShipEjectSpecificItem(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(itemKey == nil)) {
        OOJSReportBadArguments(
            context, @ "Ship", @ "ejectSpecificItem", MIN(argc, 1U), OOJS_ARGV, nil, @ "string (ship key)");
        return NO;
    }
...
static JSBool ShipDumpCargo(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (!EXPECT_NOT([thisEnt isPlayer])) {
        ...
        if (EXPECT_NOT(!gotCount || count < 1 || count > 64)) {
            OOJSReportBadArguments(context,
                                   @ "Ship",
                                   @ "dumpCargo",
                                   MIN(argc, 1U),
                                   OOJS_ARGV,
                                   nil,
                                   @ "optional quantity (1 to 64), optional preferred commodity");
            return NO;
        }

        for (i = 1; i < count; i++) {
            [thisEnt performSelector:@selector(dumpCargo)
                          withObject:nil
...
static JSBool ShipSpawn(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(role == nil || !gotCount || count < 1 || count > 64)) {
        OOJSReportBadArguments(
            context, @ "Ship", @ "spawn", MIN(argc, 1U), OOJS_ARGV, nil, @ "role and optional quantity (1 to 64)");
        return NO;
    }
...
static JSBool ShipDealEnergyDamage(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc >= 3) {
        ...
        if (!gotVBias) {
            OOJSReportBadArguments(
                context, @ "Ship", @ "dealEnergyDamage", argc, OOJS_ARGV, nil, @ "velocity bias must be a number");
            return NO;
        }
    }
...
static JSBool ShipRunLegacyScriptActions(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(argc != 2 || !JSVAL_IS_OBJECT(OOJS_ARGV[0]) ||
                   !JSShipGetShipEntity(context, JSVAL_TO_OBJECT(OOJS_ARGV[0]), &target) ||
                   ![actions isKindOfClass:[NSArray class]])) {
        OOJSReportBadArguments(
            context, @ "Ship", @ "__runLegacyScriptActions", argc, OOJS_ARGV, nil, @ "target and array of actions");
        return NO;
    }
...
static JSBool ShipCommsMessage(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc > 0) message = OOStringFromJSValue(context, OOJS_ARGV[0]);
    if (EXPECT_NOT(message == nil ||
                   (argc > 1 && (JSVAL_IS_NULL(OOJS_ARGV[1]) || !JSVAL_IS_OBJECT(OOJS_ARGV[1]) ||
                                 !JSShipGetShipEntity(context, JSVAL_TO_OBJECT(OOJS_ARGV[1]), &target))))) {
        OOJSReportBadArguments(
            context, @ "Ship", @ "commsMessage", MIN(argc, 1U), OOJS_ARGV, nil, @ "message and optional target");
        return NO;
    }
...
static JSBool ShipFireECM(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (!OK) {
        OOJSReportWarning(context,
                          @ "Ship %@ was requested to fire ECM burst but does not carry ECM equipment.",
                          [thisEnt oo_jsDescription]);
    }
...
static JSBool ShipCanAwardEquipment(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(key == nil)) {
        OOJSReportBadArguments(
            context, @ "Ship", @ "canAwardEquipment", MIN(argc, 1U), OOJS_ARGV, nil, @ "equipment type");
        return NO;
    }
    ...
    if (exists) {
        if (argc > 1) {
            ...
            if (![ctx isEqualToString:@ "scripted"] && ![ctx isEqualToString:@ "purchase"] &&
                ![ctx isEqualToString:@ "newShip"] && ![ctx isEqualToString:@ "npc"]) {
                OOJSReportBadArguments(
                    context, @ "Ship", @ "canAwardEquipment", MIN(argc, 2U), OOJS_ARGV, nil, @ "context");
                return NO;
            }
        }
        result = YES;
...
static JSBool ShipRemoveEquipment(JSContext *context, uintN argc, jsval *vp) {
    ...
    OK = [thisEnt hasOneEquipmentItem:key includeMissiles:YES whileLoading:NO] ||
         ([key isEqualToString:@ "EQ_PASSENGER_BERTH"] && [thisEnt passengerCapacity] > 0);
    if (!OK) {
...
static JSBool ShipRestoreSubEntities(JSContext *context, uintN argc, jsval *vp) {
    ...
    if ([[thisEnt subEntitiesForScript] count] - subCount > 0)
        numSubEntitiesRestored = [[thisEnt subEntitiesForScript] count] - subCount;
...
static JSBool ShipSetEquipmentStatus(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 2) {
        OOJSReportBadArguments(
            context, @ "Ship", @ "setEquipmentStatus", argc, OOJS_ARGV, nil, @ "equipment type and status");
        return NO;
    }
    ...
    if (![status isEqualToString:@ "EQUIPMENT_OK"] && ![status isEqualToString:@ "EQUIPMENT_DAMAGED"]) {
        OOJSReportErrorForCaller(
            ...
            @ "Ship",
            @ "setEquipmentStatus",
            @ "Second parameter for setEquipmentStatus must be either \"EQUIPMENT_OK\" or \"EQUIPMENT_DAMAGED\".");
        return NO;
    }
    ...
    if ([eqType canBeDamaged]) {
        ...
        if ((setOK && hasDamaged) || (setDamaged && hasOK)) {
            ...
            if ([thisEnt isPlayer]) {
                ...
                [(PlayerEntity *)thisEnt addEquipmentItem:(setDamaged ? damagedKey : key)
                                           withValidation:NO
                                                inContext:@ "scripted"];
                if (setDamaged) {
        ...
        if (hasOK && ![status isEqualToString:@ "EQUIPMENT_OK"]) {
            OOJSReportWarningForCaller(
                context, @ "Ship", @ "setEquipmentStatus", @ "Equipment %@ cannot be damaged.", key);
            hasOK = NO;
        }
    }
    ...
    */
    ...
    if (EXPECT_NOT(key == nil)) {
        if (argc > 0 && JSVAL_IS_STRING(OOJS_ARGV[0])) {
            if (asDict) {
                dict =
                    [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:1], @ "EQUIPMENT_UNKNOWN", nil];
        ...
        OOJSReportBadArguments(
            context, @ "Ship", @ "equipmentStatus", MIN(argc, 1U), &OOJS_ARGV[0], nil, @ "equipment type");
        return NO;
    }
    ...
    if (asDict) {
        dict = [NSDictionary
            dictionaryWithObjectsAndKeys:
                [NSNumber numberWithUnsignedInteger:[thisEnt countEquipmentItem:key]],
                @ "EQUIPMENT_OK",
                [NSNumber
                    numberWithUnsignedInteger:[thisEnt countEquipmentItem:[key stringByAppendingString:@ "_DAMAGED"]]],
                @ "EQUIPMENT_DAMAGED",
                nil];
        OOJS_RETURN_OBJECT(dict);
    } else {
        if ([thisEnt hasEquipmentItem:key includeWeapons:YES whileLoading:NO])
            OOJS_RETURN(strOK);
        else if ([thisEnt hasEquipmentItem:[key stringByAppendingString:@ "_DAMAGED"]])
...
static JSBool ShipFireMissile(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc > 0)
        result = [thisEnt fireMissileWithIdentifier:OOStringFromJSValue(context, OOJS_ARGV[0])
                                          andTarget:[thisEnt primaryTarget]];
    else
        result = [thisEnt fireMissile];
...
static JSBool ShipSetCargo(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(commodity == nil || !gotCount || count < 1)) {
        OOJSReportBadArguments(
            context, @ "Ship", @ "setCargo", argc, OOJS_ARGV, nil, @ "cargo name and optional positive quantity");
        return NO;
    }
...
static JSBool ShipSetCargoType(JSContext *context, uintN argc, jsval *vp) {
    ...
    if ([thisEnt cargoType] != CARGO_NOT_CARGO) {
        OOJSReportBadArguments(
            ...
            OOJS_ARGV,
            nil,
            [NSString stringWithFormat:@ "Can only be used on cargo pod carriers, not cargo pods (%@)",
                                       [thisEnt shipDataKey]]);
        return NO;
    }
    BOOL ok = YES;
...
// setMaterials(params: dict, [shaders: dict])  // sets materials dictionary. Optional parameter sets the shaders
// dictionary too.
static JSBool ShipSetMaterials(JSContext *context, uintN argc, jsval *vp) {
    OOJS_NATIVE_ENTER(context)
...
static JSBool ShipSetShaders(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (JSVAL_IS_NULL(OOJS_ARGV[0]) || (!JSVAL_IS_NULL(OOJS_ARGV[0]) && !JSVAL_IS_OBJECT(OOJS_ARGV[0]))) {
        ...
        OOJSReportWarning(context,
                          @ "Ship.%@: expected %@ instead of '%@'.",
                          @ "setShaders",
                          @ "object",
                          OOStringFromJSValueEvenIfNull(context, OOJS_ARGV[0]));
        OOJS_RETURN_BOOL(NO);
    }
    ...
    if (argc > 1) {
        ...
        if (JSVAL_IS_NULL(OOJS_ARGV[1]) || (!JSVAL_IS_NULL(OOJS_ARGV[1]) && !JSVAL_IS_OBJECT(OOJS_ARGV[1]))) {
            OOJSReportWarning(context,
                              ...
                              OOStringFromJSValueEvenIfNull(context, OOJS_ARGV[1]));
            withShaders = NO;
        }
    }
...
static BOOL RemoveOrExplodeShip(JSContext *context, uintN argc, jsval *vp, BOOL explode) {
    ...
    if (EXPECT_NOT([thisEnt isPlayer])) {
        NSCAssert(explode,
                  @ "RemoveOrExplodeShip(): shouldn't be called for player with !explode.");  // player.ship.remove() is
                                                                                             // blocked by caller.
        PlayerEntity *player = (PlayerEntity *)thisEnt;
...
static JSBool ShipAddDefenseTarget(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(argc == 0 ||
                   (argc > 0 && (JSVAL_IS_NULL(OOJS_ARGV[0]) || !JSVAL_IS_OBJECT(OOJS_ARGV[0]) ||
                                 !JSShipGetShipEntity(context, JSVAL_TO_OBJECT(OOJS_ARGV[0]), &target))))) {
...
static JSBool ShipOfferToEscort(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(argc == 0 ||
                   (argc > 0 && (JSVAL_IS_NULL(OOJS_ARGV[0]) || !JSVAL_IS_OBJECT(OOJS_ARGV[0]) ||
                                 !JSShipGetShipEntity(context, JSVAL_TO_OBJECT(OOJS_ARGV[0]), &mother))))) {
...
static JSBool ShipEnterWormhole(JSContext *context, uintN argc, jsval *vp) {
    ...
    GET_THIS_SHIP(thisEnt);
    if (EXPECT_NOT(argc == 0 || (argc > 0 && !JSVAL_IS_NULL(OOJS_ARGV[0]) &&
                                 (!JSVAL_IS_OBJECT(OOJS_ARGV[0]) ||
                                  !OOJSEntityGetEntity(context, JSVAL_TO_OBJECT(OOJS_ARGV[0]), &hole))))) {
...
static JSBool ShipCheckCourseToDestination(JSContext *context, uintN argc, jsval *vp) {
    ...
    Entity *hazard = [UNIVERSE hazardOnRouteFromEntity:thisEnt
                                            toDistance:[thisEnt desiredRange]
                                             fromPoint:[thisEnt destination]];
...
static JSBool ShipGetSafeCourseToDestination(JSContext *context, uintN argc, jsval *vp) {
    ...
    HPVector waypoint = [UNIVERSE getSafeVectorFromEntity:thisEnt
                                               toDistance:[thisEnt desiredRange]
                                                fromPoint:[thisEnt destination]];
