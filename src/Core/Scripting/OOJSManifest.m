...
static JSClass sManifestClass = {"Manifest",
                                 JSCLASS_HAS_PRIVATE,
...
void InitOOJSManifest(JSContext *context, JSObject *global) {
    sManifestPrototype = JS_InitClass(context,
                                      global,
                                      NULL,
                                      &sManifestClass,
    ...
    sManifestObject = JS_DefineObject(
        context, JSPlayerShipObject(), "manifest", &sManifestClass, sManifestPrototype, OOJS_PROP_READONLY);
...
static JSBool ManifestSetProperty(JSContext *context, JSObject *this, jsid propID, JSBool strict, jsval *value) {
    ...
    if (JSID_IS_STRING(propID)) {
        ...
        if (unit == UNITS_TONS && [entity specialCargo]) {
            OOJSReportWarning(
                context, @ "PlayerShip.manifest['foo'] - cannot modify cargo tonnage when Special Cargo is in use.");
            return YES;
        }
...
static JSBool ManifestSetComment(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (good == nil || information == nil) {
        OOJSReportBadArguments(
            context, @ "Manifest", @ "setComment", MIN(argc, 2U), OOJS_ARGV, nil, @ "good and information text");
        return NO;
    }
...
static JSBool ManifestSetShortComment(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (good == nil || information == nil) {
        OOJSReportBadArguments(
            context, @ "Manifest", @ "setShortComment", MIN(argc, 2U), OOJS_ARGV, nil, @ "good and information text");
        return NO;
    }
