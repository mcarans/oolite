...
- (void)runWithTarget:(Entity *)target {
    ...
    [PLAYER runScriptActions:_script withContextName:[self name] forTarget:(ShipEntity *)target];
...
+ (NSArray *)scriptsFromDictionaryOfScripts:(NSDictionary *)dictionary filePath:(NSString *)filePath {
    ...
    for (keyEnum = [dictionary keyEnumerator]; (key = [keyEnum nextObject]);) {
        ...
        if ([key isKindOfClass:[NSString class]] && [scriptArray isKindOfClass:[NSArray class]] &&
                ...
                if (script != nil) {
                    ...
                    [cachedScripts
                        setObject:[NSDictionary
                                      dictionaryWithObjectsAndKeys:scriptArray, kKeyScript, metadata, kKeyMetadata, nil]
                           forKey:key];
...
- (id)initWithName:(NSString *)name scriptArray:(NSArray *)script metadata:(NSDictionary *)metadata {
    ...
    if (self != nil) {
        ...
        if (name != nil) {
            if (metadata == nil)
                metadata = [NSDictionary dictionaryWithObject:name forKey:kMDKeyName];
            else {
