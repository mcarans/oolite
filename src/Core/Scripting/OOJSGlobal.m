...
static JSClass sGlobalClass = {"Global",
...
static JSBool GlobalLog(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 2) {
        ...
        message = [NSString concatenationOfStringsFromJavaScriptValues:OOJS_ARGV + 1
                                                                 count:argc - 1
                                                             separator:@ ", "
                                                             inContext:context];
    }
...
#if OOJSENGINE_MONITOR_SUPPORT
    [[OOJavaScriptEngine sharedEngine] sendMonitorLogMessage:message withMessageClass:nil inContext:context];
#endif
...
static JSBool GlobalExpandDescription(JSContext *context, uintN argc, jsval *vp) {
    ...
    OOJS_BEGIN_FULL_NATIVE(context)
    string = OOExpandDescriptionString(
        kNilRandomSeed, string, overrides, nil, nil, kOOExpandForJavaScript | kOOExpandGoodRNG);
    OOJS_END_FULL_NATIVE
...
static JSBool GlobalExpandMissionText(JSContext *context, uintN argc, jsval *vp) {
    ...
    string = OOExpandDescriptionString(
        kNilRandomSeed, string, overrides, nil, nil, kOOExpandForJavaScript | kOOExpandBackslashN | kOOExpandGoodRNG);
...
static JSBool GlobalSetExtraGuiScreenKeys(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc < 2 || !JS_ValueToObject(context, OOJS_ARGV[1], &params)) {
        OOJSReportBadArguments(context,
                               ...
                               &OOJS_ARGV[1],
                               nil,
                               @ "key, definition: definition is not a valid dictionary.");
        return NO;
    }
    ...
    if (JS_GetProperty(context, params, "guiScreen", &value) == JS_FALSE || JSVAL_IS_VOID(value)) {
        OOJSReportBadArguments(context,
                               ...
                               &OOJS_ARGV[1],
                               nil,
                               @ "key, definition: must have a 'guiScreen' property.");
        return NO;
    }

    gui = OOGUIScreenIDFromJSValue(context, value);
    // gui will be 0 for invalid screen id's as well as GUI_SCREEN_MAIN
    if (gui == 0 || gui == GUI_SCREEN_LOAD || gui == GUI_SCREEN_SAVE || gui == GUI_SCREEN_STICKMAPPER ||
        gui == GUI_SCREEN_OXZMANAGER || gui == GUI_SCREEN_NEWGAME || gui == GUI_SCREEN_SAVE_OVERWRITE ||
        gui == GUI_SCREEN_KEYBOARD || gui == GUI_SCREEN_STICKPROFILE || gui == GUI_SCREEN_KEYBOARD_CONFIRMCLEAR ||
    ...
    if (JS_GetProperty(context, params, "registerKeys", &value) == JS_FALSE || JSVAL_IS_VOID(value)) {
        OOJSReportBadArguments(context,
                               ...
                               &OOJS_ARGV[1],
                               nil,
                               @ "key, definition: must have a 'registerKeys' property.");
        return NO;
    }
    if (!JSVAL_IS_NULL(value)) {
        if (JSVAL_IS_OBJECT(value)) {
            ...
            OOJSReportBadArguments(context,
                                   ...
                                   &OOJS_ARGV[1],
                                   nil,
                                   @ "key, definition: registerKeys is not a valid dictionary.");
            return NO;
        }
    }
    ...
    if (JS_GetProperty(context, params, "callback", &callback) == JS_FALSE || JSVAL_IS_VOID(callback)) {
        OOJSReportBadArguments(context,
                               ...
                               &OOJS_ARGV[1],
                               NULL,
                               @ "key, definition; must have a 'callback' property.");
        return NO;
    }
...
static JSBool GlobalSetScreenBackground(JSContext *context, uintN argc, jsval *vp) {
    ...
    if ([UNIVERSE viewDirection] == VIEW_GUI_DISPLAY) {
        ...
        NSDictionary *descriptor = [gui textureDescriptorFromJSValue:value
                                                           inContext:context
                                                   callerDescription:@ "setScreenBackground()"];
        ...
        if (result && [PLAYER guiScreen] == GUI_SCREEN_EQUIP_SHIP)
            [PLAYER setEquipScreenBackgroundDescriptor:descriptor];
    }
...
static JSBool GlobalSetScreenBackgroundForKey(JSContext *context, uintN argc, jsval *vp) {
    ...
    NSDictionary *descriptor = [gui textureDescriptorFromJSValue:value
                                                       inContext:context
                                               callerDescription:@ "setScreenBackgroundDefault()"];
...
static JSBool GlobalSetScreenOverlay(JSContext *context, uintN argc, jsval *vp) {
    ...
    if ([UNIVERSE viewDirection] == VIEW_GUI_DISPLAY) {
        ...
        NSDictionary *descriptor = [gui textureDescriptorFromJSValue:value
                                                           inContext:context
                                                   callerDescription:@ "setScreenOverlay()"];
...
static JSBool GlobalTakeSnapShot(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (argc > 0) {
        ...
        if (EXPECT_NOT(value == nil ||
                       [value rangeOfCharacterFromSet:[allowedChars invertedSet]].location != NSNotFound)) {
...
static JSBool GlobalAutoAIForRole(JSContext *context, uintN argc, jsval *vp) {
    ...
    NSDictionary *autoAIMap = [ResourceManager dictionaryFromFilesNamed:@ "autoAImap.plist"
                                                               inFolder:@ "Config"
                                                               andMerge:YES];
...
static JSBool GlobalPauseGame(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (player) {
        ...
        if (guiScreen != GUI_SCREEN_LONG_RANGE_CHART && guiScreen != GUI_SCREEN_MISSION &&
            guiScreen != GUI_SCREEN_REPORT && guiScreen != GUI_SCREEN_KEYBOARD_ENTRY && guiScreen != GUI_SCREEN_SAVE) {
            [UNIVERSE pauseGame];
            result = YES;
        }
    }
