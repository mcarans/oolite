...
// Call a JS function, setting up new contexts as necessary. Caller is responsible for ensuring the jsval passed really
// is a function.
...
OOINLINE JSContext *OOJSAcquireContext(void) {
    ...
    NSCAssert(gOOJSMainThreadContext != NULL,
...
OOINLINE jsval OOJSValueFromBOOL(int b) { return BOOLEAN_TO_JSVAL(b != NO); }
...
#define OOJSSTR(str)                                                                     \
    ({                                                                                   \
        static jsval strCache;                                                           \
        static BOOL inited;                                                              \
        if (EXPECT_NOT(!inited)) OOJSStrLiteralCachePRIVATE("" str, &strCache, &inited); \
        strCache;                                                                        \
    })
...
// YES for ships other than sub-entities and menu-display ships, and planets other than atmospheres and menu miniatures.
// Parameter: ignored.
...
#else
#define OOJSDumpStack(cx) \
    do {                  \
    } while (0)
#define OOJSDescribeLocation(cx, frame) \
    do {                                \
    } while (0)
#define OOJSMarkConsoleEvalLocation(cx, frame) \
    do {                                       \
    } while (0)
#endif
...
#define OOJS_RETURN_WITH_HELPER(helper, value)       \
    do {                                             \
        ...
        JS_SET_RVAL(context, vp, jsresult);          \
        return OK;                                   \
    } while (0)
