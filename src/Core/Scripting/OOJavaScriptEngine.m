...
- (void)sendMonitorError:(JSErrorReport *)errorReport withMessage:(NSString *)message inContext:(JSContext *)context;
...
static void ReportJSError(JSContext *context, const char *message, JSErrorReport *report) {
    ...
    JSErrorReport blankReport = {
        .filename = "<unspecified file>", .linebuf = "", .uclinebuf = empty, .uctokenptr = empty, .ucmessage = empty};
    ...
    while ([lineBuf hasSuffix:@ "\n"] || [lineBuf hasSuffix:@ "\r"])
        lineBuf = [lineBuf substringToIndex:[lineBuf length] - 1];

    // Get string for error number, for useful log message classes
    NSDictionary *errorNames = [ResourceManager dictionaryFromFilesNamed:@ "javascript-errors.plist"
                                                                inFolder:@ "Config"
                                                                andMerge:YES];
    ...
    if ((report->flags & JSREPORT_WARNING) == 0 || OOLogWillDisplayMessagesInClass(messageClass)) {
        ...
        if (showLocation && sErrorHandlerStackSkip == 0 && report->filename != NULL) {
            // Second line: where error occured, and line if provided. (The line is only provided for compile-time
        ...
        if (report->flags & JSREPORT_WARNING)
            ...
            dump = [jsEng dumpStackForErrors];
        if (dump) OOJSDumpStack(context);
#endif

#if OOJSENGINE_MONITOR_SUPPORT
        JSExceptionState *exState = JS_SaveExceptionState(context);
        [[OOJavaScriptEngine sharedEngine] sendMonitorError:report withMessage:messageText inContext:context];
...
- (id)init {
    ...
    */
    ...
    if (_runtime == NULL) {
        OOLog(@ "script.javaScript.init.error",
              @ "***** FATAL ERROR: failed to create JavaScript runtime with size %uMiB.",
              jsRuntimeInMiB);
        exit(1);
    }
...
- (void)createMainThreadContext {
    NSAssert(gOOJSMainThreadContext == NULL,
    ...
    // Run prefix scripts.
    [OOJSScript
        jsScriptFromFileNamed:@ "oolite-global-prefix.js"
...
#if JS_THREADSAFE
    // NSAssert(!JS_IsInRequest(gOOJSMainThreadContext), @"JavaScript processes still pending. Can't reset JavaScript
    // engine.");
...
- (BOOL)lookUpStandardClassPointers {
    ...
    if (EXPECT_NOT(
            !JS_ValueToObject(gOOJSMainThreadContext, JS_GetEmptyStringValue(gOOJSMainThreadContext), &templateObject)))
        return NO;
...
static JSTrapStatus DebuggerHook(JSContext *context, JSScript *script, jsbytecode *pc, jsval *rval, void *closure) {
    ...
    OOLog(@ "script.javaScript.debugger",
...
- (void)sendMonitorError:(JSErrorReport *)errorReport
    ...
    if ([_monitor respondsToSelector:@selector(jsEngine:context:error:stackSkip:showingLocation:withMessage:)]) {
        [_monitor jsEngine:self
                    context:theContext
                      error:errorReport
                  stackSkip:sErrorHandlerStackSkip
            showingLocation:[self showErrorLocations]
                withMessage:message];
    }
}
...
static void DumpVariable(JSContext *context, JSPropertyDesc *prop) {
    ...
    enum { kInterestingFlags = ~(JSPD_ENUMERATE | JSPD_PERMANENT | JSPD_VARIABLE | JSPD_ARGUMENT) };

    NSString *flagStr = @ "";
    if ((prop->flags & kInterestingFlags) != 0) {
        NSMutableArray *flags = [NSMutableArray array];
        if (prop->flags & JSPD_READONLY) [flags addObject:@ "read-only"];
        if (prop->flags & JSPD_ALIAS)
            [flags addObject:[NSString stringWithFormat:@ "alias (%@)", OOJSDescribeValue(context, prop->alias, YES)]];
...
void OOJSReportWrappedException(JSContext *context, id exception) {
    if (!JS_IsExceptionPending(context)) {
        if ([exception isKindOfClass:[NSException class]])
            OOJSReportError(context, @ "Native exception: %@", [exception reason]);
        else
            OOJSReportError(context, @ "Unidentified native exception");
    }
...
void OOJSReportBadPropertyValue(
    JSContext *context, JSObject *thisObj, jsid propID, JSPropertySpec *propertySpec, jsval value) {
    NSString *propName = OOStringFromJSPropertyIDAndSpec(context, propID, propertySpec);
    const char *className = OOJSGetClass(context, thisObj)->name;
    NSString *valueDesc = OOJSDescribeValue(context, value, YES);

    OOJSReportError(
        context, @ "Cannot set property %@ of instance of %s to invalid value %@.", propName, className, valueDesc);
}
...
void OOJSReportBadArguments(JSContext *context,
    ...
    @try {
        ...
        message = [NSString stringWithFormat:@ "%@ %@",
                                             message,
                                             [NSString stringWithJavaScriptParameters:argv
                                                                                count:argc
                                                                            inContext:context]];
        if (expectedArgsDescription != nil)
            message = [NSString stringWithFormat:@ "%@ -- expected %@", message, expectedArgsDescription];
...
void OOJSSetWarningOrErrorStackSkip(unsigned skip) { sErrorHandlerStackSkip = skip; }
...
BOOL OOJSArgumentListGetNumber(JSContext *context,
    ...
    if (OOJSArgumentListGetNumberNoError(context, argc, argv, outNumber, outConsumed)) {
        ...
        OOJSReportBadArguments(context, scriptClass, function, argc, argv, @ "Expected number, got", NULL);
        return NO;
    }
}

BOOL OOJSArgumentListGetNumberNoError(
    JSContext *context, uintN argc, jsval *argv, double *outNumber, uintN *outConsumed) {
    OOJS_PROFILE_ENTER
...
void OOJSStrLiteralCachePRIVATE(const char *string, jsval *strCache, BOOL *inited) {
    ...
    if (EXPECT_NOT(string == NULL)) {
        [NSException raise:NSGenericException
                    format:@ "Failed to initialize JavaScript string literal cache for \"%@\".",
                           [[NSString stringWithUTF8String:string] escapedForJavaScriptLiteral]];
    }
...
static NSString *DescribeValue(JSContext *context, jsval value, BOOL abbreviateObjects, BOOL recursing) {
    ...
    if (JSVAL_IS_STRING(value)) {
        ...
        result = [NSString
            stringWithFormat:@ "\"%@%@\"", [result escapedForJavaScriptLiteral], (length > kMaxLength) ? @ "..." : @ ""];
...
+ (NSString *)concatenationOfStringsFromJavaScriptValues:(jsval *)values
    ...
    for (i = 0; i != count; ++i) {
        ...
        if (result == nil)
            result = [[element mutableCopy] autorelease];
        else {
...
BOOL JSFunctionPredicate(Entity *entity, void *parameter) {
    ...
    args[0] = [entity oo_jsValueInContext:param->context];  // entity is required to be non-nil (asserted above), so
                                                            // oo_jsValueInContext: is safe.
...
BOOL JSEntityIsDemoShipPredicate(Entity *entity, void *parameter) {
    return ([entity isVisibleToScripts] && [entity isShip] && [entity status] == STATUS_COCKPIT_DISPLAY &&
            ![entity isSubEntity]);
}
...
void OOJSRegisterSubclass(JSClass *subclass, JSClass *superclass) {
    ...
    if (sRegisteredSubClasses == NULL) {
        sRegisteredSubClasses =
            NSCreateMapTable(NSNonOwnedPointerMapKeyCallBacks, NSNonOwnedPointerMapValueCallBacks, 0);
    }

    NSCAssert(NSMapGet(sRegisteredSubClasses, subclass) == NULL,
...
BOOL OOJSIsSubclass(JSClass *putativeSubclass, JSClass *superclass) {
    ...
    NSCAssert(sRegisteredSubClasses != NULL,
...
BOOL OOJSObjectGetterImplPRIVATE(JSContext *context,
    ...
    */
    ...
    if (EXPECT_NOT(!OOJSIsSubclass(actualClass, requiredJSClass))) {
        OOJSReportError(context,
                        @ "Native method expected %s, got %@.",
                        requiredJSClass->name,
                        OOStringFromJSValue(context, OBJECT_TO_JSVAL(object)));
        return NO;
    }
    NSCAssert(actualClass->flags & JSCLASS_HAS_PRIVATE,
    ...
    *outObject = [(id)JS_GetPrivate(context, object) weakRefUnderlyingObject];
    ...
    if (EXPECT_NOT(*outObject != nil && ![*outObject isKindOfClass:requiredObjCClass])) {
        OOJSReportError(context,
                        @ "Native method expected %@ from %s and got correct JS type but incorrect native object %@",
                        requiredObjCClass,
                        requiredJSClass->name,
                        *outObject);
        return NO;
...
static void UnregisterObjectConverters(void) { DESTROY(sObjectConverters); }
