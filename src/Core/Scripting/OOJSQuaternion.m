...
static JSClass sQuaternionClass = {"Quaternion",
                                   JSCLASS_HAS_PRIVATE,
...
static JSFunctionSpec sQuaternionMethods[] = {
    ...
    {
        "vectorForward",
        QuaternionVectorForward,
        0,
    },
    {
        "vectorRight",
        QuaternionVectorRight,
        0,
    },
    {
        "vectorUp",
...
void InitOOJSQuaternion(JSContext *context, JSObject *global) {
    sQuaternionPrototype = JS_InitClass(context,
                                        global,
                                        NULL,
                                        &sQuaternionClass,
                                        QuaternionConstruct,
                                        4,
                                        sQuaternionProperties,
                                        sQuaternionMethods,
                                        NULL,
                                        sQuaternionStaticMethods);
}
...
- (NSString *)reportJSQuaternionStatistics {
    ...
    return [NSString stringWithFormat:@ "quaternion-to-quaternion conversions: %lu (%g %%)\n"
                                       ...
                                       "                               total: %lu",
                                      (long)stats->quatCount,
                                      stats->quatCount * convFac,
...
#define COUNT(FIELD)                        \
    do {                                    \
        sQuaternionConversionStats.FIELD++; \
    } while (0)
...
static BOOL GetThisQuaternion(JSContext *context,
...
static BOOL QuaternionFromArgumentListNoErrorInternal(
    JSContext *context, uintN argc, jsval *argv, Quaternion *outQuaternion, uintN *outConsumed, BOOL permitNumberList) {
    OOJS_PROFILE_ENTER
...
// QuaternionFromArgumentListNoErrorInternal() directly).
BOOL QuaternionFromArgumentList(JSContext *context,
                                ...
                                Quaternion *outQuaternion,
                                uintN *outConsumed) {
    if (QuaternionFromArgumentListNoErrorInternal(context, argc, argv, outQuaternion, outConsumed, NO))
        return YES;
    else {
...
BOOL QuaternionFromArgumentListNoError(
    JSContext *context, uintN argc, jsval *argv, Quaternion *outQuaternion, uintN *outConsumed) {
...
static JSBool QuaternionMultiply(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(!QuaternionFromArgumentList(context, @ "Quaternion", @ "multiply", argc, OOJS_ARGV, &thatq, NULL)))
        return NO;
...
static JSBool QuaternionDot(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(!QuaternionFromArgumentList(context, @ "Quaternion", @ "dot", argc, OOJS_ARGV, &thatq, NULL)))
        return NO;
...
static JSBool QuaternionRotate(JSContext *context, uintN argc, jsval *vp) {
    ...
    argc -= consumed;
    if (argc > 0) {
        if (EXPECT_NOT(!OOJSArgumentListGetNumber(context, @ "Quaternion", @ "rotate", argc, argv, &angle, NULL)))
            return NO;
...
static JSBool QuaternionRotateX(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(!OOJSArgumentListGetNumber(context, @ "Quaternion", @ "rotateX", argc, OOJS_ARGV, &angle, NULL)))
        return NO;
...
static JSBool QuaternionRotateY(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(!OOJSArgumentListGetNumber(context, @ "Quaternion", @ "rotateY", argc, OOJS_ARGV, &angle, NULL)))
        return NO;
...
static JSBool QuaternionRotateZ(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (EXPECT_NOT(!OOJSArgumentListGetNumber(context, @ "Quaternion", @ "rotateZ", argc, OOJS_ARGV, &angle, NULL)))
        return NO;
...
static JSBool QuaternionToArray(JSContext *context, uintN argc, jsval *vp) {
    ...
    if (result != NULL) {
        ...
        if (JS_NewNumberValue(context, thisq.w, &nVal))
            JS_SetElement(context, result, 0, &nVal);
        else
            OK = NO;
        if (JS_NewNumberValue(context, thisq.x, &nVal))
            JS_SetElement(context, result, 1, &nVal);
        else
            OK = NO;
        if (JS_NewNumberValue(context, thisq.y, &nVal))
            JS_SetElement(context, result, 2, &nVal);
        else
            OK = NO;
        if (JS_NewNumberValue(context, thisq.z, &nVal))
            JS_SetElement(context, result, 3, &nVal);
        else
            OK = NO;
    }
