...
OOINLINE BOOL MethodExpectsParameter(MethodType type) {
    return type == kMethodTypeVoidObject || type == kMethodTypeObjectObject;
}

BOOL OOJSCallObjCObjectMethod(
    JSContext *context, id object, NSString *oo_jsClassName, uintN argc, jsval *argv, jsval *outResult) {
    OOJS_PROFILE_ENTER
    ...
    MethodType type;
    BOOL haveParameter = NO, error = NO;
    id result = nil;
    ...
    if (1 < argc && [selectorString hasSuffix:@ ":"]) {
        haveParameter = YES;
        paramString = [NSString concatenationOfStringsFromJavaScriptValues:argv + 1
                                                                     count:argc - 1
                                                                 separator:@ " "
                                                                 inContext:context];
    }
    ...
    if ([object respondsToSelector:selector]) {
        ...
        if (MethodExpectsParameter(type) && !haveParameter) {
            ...
            switch (type) {
                ...
                case kMethodTypeLongVoid:
                    result = [NSNumber
                        numberWithLongLong:OOCallIntegerMethod(object, selector, method, (OOShaderUniformType)type)];
                    break;

                case kMethodTypeUnsignedLongVoid:
                    result =
                        [NSNumber numberWithUnsignedLongLong:OOCallIntegerMethod(
                                                                 object, selector, method, (OOShaderUniformType)type)];
                    break;

                case kMethodTypeFloatVoid:
                case kMethodTypeDoubleVoid:
                    result = [NSNumber
                        numberWithDouble:OOCallFloatMethod(object, selector, method, (OOShaderUniformType)type)];
                    break;
                ...
                case kMethodTypeInvalid:
                    OOJSReportError(context,
                                    @ "%@.callObjC(): method %@ cannot be called from JavaScript.",
                                    oo_jsClassName,
                                    selectorString);
                    error = YES;
                    break;
        ...
        OOJSReportError(context,
                        @ "%@.callObjC(): %@ does not respond to method %@.",
                        oo_jsClassName,
