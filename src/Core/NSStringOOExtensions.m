...
+ (instancetype)stringWithContentsOfUnicodeFile:(NSString *)path {
    ...
    if (OK && 2 <= length && (length % sizeof(unichar)) == 0) {
        ...
        if (firstChar == 0xFFFE || firstChar == 0xFEFF) {
            ...
            result = [NSString stringWithCharacters:(unichar *)(bytes + sizeof(unichar))
                                             length:(length / sizeof(unichar)) - 1];
            if (result == nil) OK = NO;
        }
    }
    ...
    if (OK && result == nil) {
        ...
        result = [[[NSString alloc] initWithBytes:effectiveBytes length:effectiveLength
                                         encoding:NSUTF8StringEncoding] autorelease];
    }

    if (OK && result == nil) {
        // Not UTF-16 or UTF-8. Use ISO-Latin-1 (which should work for any byte sequence).
        result = [[[NSString alloc] initWithBytes:effectiveBytes
