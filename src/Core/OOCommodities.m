...
- (id)init {
    ...
    NSDictionary *rawCommodityLists = [ResourceManager dictionaryFromFilesNamed:@ "trade-goods.plist"
                                                                       inFolder:@ "Config"
    ...
    /* // TODO: validation of inputs
            ...
            NSMutableDictionary *validatedCommodityLists = [NSMutableDictionary
    dictionaryWithCapacity:[rawCommodityLists count]]; NSString *commodityName = nil; foreachkey (commodityName,
    rawCommodityLists)
            {
                    // validate
            }
...
- (NSDictionary *)modifyGood:(NSDictionary *)good
                  withScript:(OOScript *)script
                   atStation:(StationEntity *)station
                    inSystem:(OOSystemID)system
                   localMode:(BOOL)localMode {
    NSDictionary *result = nil;
    JSContext *context = OOJSAcquireContext();
    jsval rval;
    jsval args[] = {
        OOJSValueFromNativeObject(context, good), OOJSValueFromNativeObject(context, station), INT_TO_JSVAL(system)};
    BOOL OK = YES;
    ...
    if (!OK) {
        OOLog(@ "script.commodityScript.error",
              @ "Could not update %@ commodity definition for %@ - unable to call updateLocalCommodityDefinition",
              errorType,
              [good oo_stringForKey:kOOCommodityName]);
    ...
    if (!JSVAL_IS_OBJECT(rval)) {
        OOLog(@ "script.commodityScript.error",
              @ "Could not update %@ commodity definition for %@ - return value invalid",
              errorType,
              [good oo_stringForKey:kOOCommodityKey]);
        OOJSRelinquishContext(context);
        return good;
    }
...
- (OOCommodityMarket *)generateMarketForSystemWithEconomy:(OOEconomyID)economy andScript:(NSString *)scriptName {
    ...
    foreachkey(commodity, _commodityLists) {
        ...
        good = [self createDefinitionFrom:good
                                    price:p
                              andQuantity:q
                                   forKey:commodity
                                atStation:nil
                                 inSystem:[UNIVERSE currentSystemID]];

        if (script != nil) {
            good = [self modifyGood:good
                         withScript:script
                          atStation:nil
                           inSystem:[UNIVERSE currentSystemID]
                          localMode:YES];
...
- (OOCommodityMarket *)generateMarketForStation:(StationEntity *)station {
    ...
    foreachkey(commodity, _commodityLists) {
        good = [_commodityLists oo_dictionaryForKey:commodity];
        OOCargoQuantity baseCapacity = [good oo_unsignedIntForKey:kOOCommodityCapacity
                                                     defaultValue:MAIN_SYSTEM_MARKET_LIMIT];
        ...
        if (marketScript == nil) {
            NSDictionary *modifier = [self firstModifierForGood:commodity
                                                      inClasses:[good oo_arrayForKey:kOOCommodityClasses]
                                                       fromList:marketDefinition];
...
- (NSDictionary *)firstModifierForGood:(OOCommodityType)good
    ...
    for (i = 0; i < [definitions count]; i++) {
        ...
        if (definition != nil) {
            NSString *applicationType = [definition oo_stringForKey:kOOCommodityMarketType
                                                       defaultValue:kOOCommodityMarketTypeValueDefault];
            NSString *applicationName = [definition oo_stringForKey:kOOCommodityMarketName defaultValue:@ ""];

            if ([applicationType isEqualToString:kOOCommodityMarketTypeValueDefault] ||
                ([applicationType isEqualToString:kOOCommodityMarketTypeValueGood] &&
                 [applicationName isEqualToString:good]) ||
                ([applicationType isEqualToString:kOOCommodityMarketTypeValueClass] &&
                 [classes containsObject:applicationName])) {
                return definition;
            }
        }
    }
