...
- (NSString *)shortDescriptionComponents {
    return [NSString
        stringWithFormat:@ "%@:%@ / %@", stateMachineName, currentState, [stateMachine objectForKey:@ "jsScript"]];
}
...
- (void)reportStackOverflow {
    if (OOLogWillDisplayMessagesInClass(@ "ai.error.stackOverflow")) {
        ...
        OOLogERR(@ "ai.error.stackOverflow",
                 @ "AI stack overflow for %@ in %@: %@%@\n",
                 [_owner shortDescription],
                 stateMachineName,
                 currentState,
                 trailer);
        ...
        if (stackDump) {
            ...
            while (count--) {
                ...
                OOLog(@ "ai.error.stackOverflow.dump",
                      @ "%3llu: %@: %@",
                      count,
                      [preservedMachine name],
                      [preservedMachine state]);
            }
...
- (void)preserveCurrentStateMachine {
    ...
    if ([aiStack count] >= kStackLimiter) {
        ...
        [NSException raise:@ "OoliteException" format:@ "AI stack overflow for %@", _owner];
    }

    OOPreservedAIStateMachine *preservedMachine =
        [[OOPreservedAIStateMachine alloc] initWithStateMachine:stateMachine
...
- (void)restorePreviousStateMachine {
    ...
    [self directSetStateMachine:[preservedMachine stateMachine] name:[preservedMachine name]];
...
- (void)reactToMessage:(NSString *)message context:(NSString *)debugContext {
    ...
    */
    ...
    AIStackElement stackElement = {.back = sStack,
                                   .owner = owner,
    ...
    */
    if (recursionLimiter > kRecursionLimiter) {
        OOLogERR(
            @ "ai.error.recursion",
            @ "AI dispatch: hit stack depth limit in AI %@, state %@ handling message %@ in context \"%@\", aborting.",
            stateMachineName,
            currentState,
            message,
            debugContext);
        ...
        unsigned depth = 0;
        while (stack != NULL) {
            OOLog(@ "ai.error.recursion.stackTrace",
                  @ "%4u  %@ - %@:%@.%@ (%@)",
                  depth++,
                  [stack->owner shortDescription],
                  stack -> aiName,
                  stack -> state,
                  stack -> message,
                  stack -> context);
    ...
    if (currentState != nil && ![message isEqual:@ "UPDATE"] && [owner reportAIMessages]) {
        OOLog(@ "ai.message.receive",
              @ "AI %@ for %@ in state '%@' receives message '%@'. Context: %@, stack depth: %u",
              stateMachineName,
              ownerDesc,
              currentState,
              message,
    ...
    if ([actions count] > 0) {
        ...
        @try {
            ...
            OOLog(kOOLogException,
                  @ "Squashing exception %@:%@ in AI handler %@:%@.%@",
                  [exception name],
                  [exception reason],
                  stateMachineName,
                  currentState,
    ...
    if (tokenCount != 0) {
        ...
        if (owner != nil) {
            ...
            if ([owner respondsToSelector:selector]) {
                ...
                OOLogERR(@ "ai.takeAction.badSelector",
                         @ "in AI %@ in state %@: %@ does not respond to %@",
                         stateMachineName,
                         currentState,
                         ownerDesc,
                         selectorStr);
...
- (void)message:(NSString *)ms {
    ...
    if (EXPECT_NOT([pendingMessages count] > 32)) {
        // Generate the error, but don't crash Oolite! Fixes bug #18055 - Pending message overflow for thargoids, ->
        // crash !
        OOLogERR(
            @ "ai.message.failed.overflow",
            @ "AI message \"%@\" received by '%@' AI while pending messages stack full; message discarded. Pending messages:\n%@",
            ms,
            ownerDesc,
            pendingMessages);
    } else {
...
- (void)debugDumpPendingMessages {
    ...
    OOLog(
        @ "ai.debug.pendingMessages", @ "Pending messages for AI %@: %@", [self descriptionComponents], displayMessages);
}
...
- (OOTimeAbsolute)nextThinkTime {
    if (!stateMachine) return INFINITY;
...
- (void)performDeferredCall:(SEL)selector withObject:(id)object afterDelay:(NSTimeInterval)delay {
    ...
    if (selector != NULL) {
        ...
        [[AI class] performSelector:@selector(deferredCallTrampolineWithInfo:) withObject:info afterDelay:delay];
        [info release];
    }
}
...
- (NSDictionary *)loadStateMachine:(NSString *)smName jsName:(NSString *)script {
    ...
    if (![smName isEqualToString:@ "nullAI.plist"]) {
        ...
        if (newSM != nil && ![newSM isKindOfClass:[NSDictionary class]])
            return nil;  // catches use of @"nil" to indicate no AI found.
    }
        ...
        @try {
            ...
            if (newSM == nil) {
                ...
                OOLog(@ "ai.load.failed.unknownAI",
                      @ "Can't switch AI for %@%@ to \"%@\" - could not load file.",
                      [[self owner] shortDescription],
                      fromString,
                      smName);
                return nil;
            }
            ...
            for (stateEnum = [newSM keyEnumerator]; (stateKey = [stateEnum nextObject]);) {
                ...
                if (![stateHandlers isKindOfClass:[NSDictionary class]]) {
                    OOLogWARN(@ "ai.invalidFormat.state",
                              @ "State \"%@\" in AI \"%@\" is not a dictionary, ignoring.",
                              stateKey,
                              smName);
                    continue;
                }
...
- (NSDictionary *)cleanHandlers:(NSDictionary *)handlers forState:(NSString *)stateKey stateMachine:(NSString *)smName {
    ...
    for (handlerEnum = [handlers keyEnumerator]; (handlerKey = [handlerEnum nextObject]);) {
        ...
        if (![handlerActions isKindOfClass:[NSArray class]]) {
            OOLogWARN(@ "ai.invalidFormat.handler",
                      @ "Handler \"%@\" for state \"%@\" in AI \"%@\" is not an array, ignoring.",
                      handlerKey,
                      stateKey,
                      smName);
            continue;
...
- (NSArray *)cleanActions:(NSArray *)actions
    ...
    for (actionEnum = [actions objectEnumerator]; (action = [actionEnum nextObject]);) {
        if (![action isKindOfClass:[NSString class]]) {
            OOLogWARN(@ "ai.invalidFormat.action",
                      @ "An action in handler \"%@\" for state \"%@\" in AI \"%@\" is not a string, ignoring.",
                      handlerKey,
                      stateKey,
                      smName);
            continue;
        ...
        if (aliasedSelector != nil) {
            if ([aliasedSelector isKindOfClass:[NSString class]]) {
                ...
                if (spaceRange.location == NSNotFound)
                    action = aliasedSelector;
                else
                    action = [aliasedSelector stringByAppendingString:[action substringFromIndex:spaceRange.location]];
        ...
        if (![whitelist containsObject:selector]) {
            OOLog(@ "ai.unpermittedMethod",
                  @ "Handler \"%@\" for state \"%@\" in AI \"%@\" uses \"%@\", which is not a permitted AI method.",
                  handlerKey,
                  stateKey,
                  smName,
                  selector);
