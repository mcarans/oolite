...
@interface HeadUpDisplay : NSObject {
   @private
    ...
    NSString *hudName;
    NSString *deferredHudName;  // Usually it will be nil. If not nil, then it means that we have a deferred HUD waiting
                                // to be drawn This may happen for example when a script handler attempts to switch HUD
                                // while it is being rendered. - Nikos 20110628
    BOOL hudUpdating;

    GLfloat overallAlpha;

    BOOL reticleTargetSensitive;  // TO DO: Move this into the propertiesReticleTargetSensitive structure (Getafix -
                                  // 2010/08/21)
