    ...
    for (i = 0; i < OO_GALAXIES_AVAILABLE; i++) {
        ...
        for (j = 0; j < OO_SYSTEMS_PER_GALAXY; j++) {
            ...
            for (k = j + 1; k < OO_SYSTEMS_PER_GALAXY; k++) {
                ...
                if (distanceBetweenPlanetPositions(coordinatesCache[jIndex].x,
                                                   coordinatesCache[jIndex].y,
                                                   coordinatesCache[kIndex].x,
                                                   coordinatesCache[kIndex].y) <= MAX_JUMP_RANGE) {
...
- (void)setProperties:(NSDictionary *)properties forSystemKey:(NSString *)key {
    ...
    if ([tokens count] == 2 && [tokens oo_unsignedIntegerAtIndex:0] < OO_GALAXIES_AVAILABLE &&
        [tokens oo_unsignedIntegerAtIndex:1] < OO_SYSTEMS_PER_GALAXY) {
        ...
        if (index >= OO_SYSTEM_CACHE_LENGTH) {
            OOLog(@ "system.description.error",
                  @ "'%@' is an invalid system key. This is an internal error. Please report it.",
                  key);
        } else {
...
- (void)setProperty:(NSString *)property
    ...
    if ([tokens count] == 2 && [tokens oo_unsignedIntegerAtIndex:0] < OO_GALAXIES_AVAILABLE &&
        [tokens oo_unsignedIntegerAtIndex:1] < OO_SYSTEMS_PER_GALAXY) {
        [self saveScriptedChangeToProperty:property
                              forSystemKey:key
                                  andLayer:layer
                                   toValue:value
    ...
    else if ([tokens count] == 4 && [tokens oo_unsignedIntegerAtIndex:1] < OO_GALAXIES_AVAILABLE &&
             [tokens oo_unsignedIntegerAtIndex:2] < OO_SYSTEMS_PER_GALAXY &&
             [tokens oo_unsignedIntegerAtIndex:3] < OO_SYSTEMS_PER_GALAXY) {
        [self saveScriptedChangeToProperty:property
                              forSystemKey:key
                                  andLayer:layer
                                   toValue:value
...
- (void)saveScriptedChangeToProperty:(NSString *)property
    ...
    NSArray *overrideKey =
        [NSArray arrayWithObjects:manifest, key, property, [[NSNumber numberWithInt:layer] stringValue], nil];
    ...
    if (s >= 0) {
        ...
        if (index >= OO_SYSTEM_CACHE_LENGTH) {
            OOLog(
                ...
                @ "'%llu' is an invalid system index for the current system. This is an internal error. Please report it.",
                index);
...
- (NSDictionary *)getPropertiesForSystem:(OOSystemID)s inGalaxy:(OOGalaxyID)g {
    ...
    if (index >= OO_SYSTEM_CACHE_LENGTH) {
        OOLog(@ "system.description.error",
              @ "'%u, %u' is an invalid system. This is an internal error. Please report it.",
              g,
              s);
        return [NSDictionary dictionary];
    }
...
- (id)getProperty:(NSString *)property forSystem:(OOSystemID)s inGalaxy:(OOGalaxyID)g {
    if (s < 0) {
        OOLog(@ "system.description.error",
              @ "'%d %d' is an invalid system key. This is an internal error. Please report it.",
              g,
              s);
        return nil;
    }
...
- (id)validateProperty:(NSString *)property withValue:(id)value {
    if ([property isEqualToString:@ "coordinates"]) {
        ...
        if ([tokens count] != 2) {
            OOLog(@ "system.description.error",
                  @ "'%@' is not a valid format for coordinates (must have exactly two numbers)",
                  value);
            return nil;
        }
        ...
        if (![value isKindOfClass:[NSString class]]) {
            if ([value isKindOfClass:[NSNumber class]]) {
                ...
                OOLog(@ "system.description.error",
                      @ "'%@' is not a valid value for '%@' (string required)",
                      value,
                      property);
                return nil;
            }
        }
    } else if ([property isEqualToString:@ "inhabitant"] || [property isEqualToString:@ "inhabitants"] ||
               [property isEqualToString:@ "name"]) {
        // read in a context which expects a string
        if (![value isKindOfClass:[NSString class]]) {
            OOLog(
                @ "system.description.error", @ "'%@' is not a valid value for '%@' (string required)", value, property);
            return nil;
        }
    }
