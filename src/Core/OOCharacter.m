...
- (NSString *)descriptionComponents {
    return [NSString stringWithFormat:@ "%@, %@. bounty: %i insurance: %llu",
                                      [self name],
                                      [self shortDescription],
                                      [self legalStatus],
                                      [self insuranceCredits]];
}
...
- (NSString *)species {
    ...
    if (species == 3)
        speciesString = [UNIVERSE getSystemInhabitants:[self genSeed].e plural:NO];
    else
        speciesString = [UNIVERSE getSystemInhabitants:[self planetIDOfOrigin] plural:NO];
...
- (void)basicSetUp {
    ...
    if ([species hasPrefix:@ "human"]) {
        genName = [NSString
            stringWithFormat:@ "%@ %@", OOExpandWithSeed(genSeed, @ "%R"), OOExpandKeyWithSeed(genSeed, @ "nom")];
    } else {
    ...
    if (legalIndex == 3) {
        // criminal
        [self setLegalStatus:criminalTendency + criminalTendency * (gen_rnd_number() & 0x03) +
                             (gen_rnd_number() & gen_rnd_number() & 0x7f)];
    }
    legalIndex = 0;
...
- (BOOL)castInRole:(NSString *)role {
    ...
    if ([role hasPrefix:@ "pirate"]) {
        ...
        [self setLegalStatus:0];  // clean
        int insuranceIndex = gen_rnd_number() & 0x03;
        if (insuranceIndex == 3) [self setInsuranceCredits:500];
        specialSetUpDone = YES;
...
- (NSDictionary *)infoForScripting {
    return [NSDictionary dictionaryWithObjectsAndKeys:[self name],
                                                      @ "name",
                                                      [self shortDescription],
                                                      @ "description",
                                                      [self species],
                                                      @ "species",
                                                      [NSNumber numberWithInt:[self legalStatus]],
                                                      @ "legalStatus",
                                                      [NSNumber numberWithUnsignedLongLong:[self insuranceCredits]],
                                                      @ "insuranceCredits",
                                                      [NSNumber numberWithInt:[self planetIDOfOrigin]],
                                                      @ "homeSystem",
                                                      nil];
}
...
- (void)setCharacterFromDictionary:(NSDictionary *)dict {
    ...
    if ([dict oo_stringForKey:@ "short_description"])
