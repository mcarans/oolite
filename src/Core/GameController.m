...
- (id)init {
    if (sSharedController != nil) {
        ...
        [NSException raise:NSInternalInconsistencyException
                    format:@ "%s: expected only one GameController to exist at a time.", __PRETTY_FUNCTION__];
    }
    ...
    if ((self = [super init])) {
        ...
        _animationTimerInterval = [[NSUserDefaults standardUserDefaults] oo_doubleForKey:@ "animation_timer_interval"
                                                                            defaultValue:MINIMUM_ANIMATION_TICK];
...
- (void)setEcoQoS:(BOOL)efficiencyModeRequested {
    ...
    if ([[NSUserDefaults standardUserDefaults] oo_boolForKey:@ "ecoqos" defaultValue:YES]) {
        ...
        if (EXPECT_NOT(
                !SetPriorityClass(currentProcess, setEfficiencyMode ? IDLE_PRIORITY_CLASS : NORMAL_PRIORITY_CLASS))) {
        ...
        if (EXPECT_NOT(!SetProcessInformation(
                currentProcess, ProcessPowerThrottling, &powerThrottling, sizeof(powerThrottling)))) {
...
- (void)setMouseInteractionMode:(OOMouseInteractionMode)mode {
    ...
    _mouseMode = mode;
    OOLog(@ "input.mouseMode.changed",
          @ "Mouse interaction mode changed from %@ to %@",
          OOStringFromMouseInteractionMode(oldMode),
          OOStringFromMouseInteractionMode(mode));
...
- (void)setMouseInteractionModeForFlight {
    [self setMouseInteractionMode:[PLAYER isMouseControlOn] ? MOUSE_MODE_FLIGHT_WITH_MOUSE_CONTROL
                                                            : MOUSE_MODE_FLIGHT_NO_MOUSE_CONTROL];
}

- (void)setMouseInteractionModeForUIWithMouseInteraction:(BOOL)interaction {
    [self setMouseInteractionMode:interaction ? MOUSE_MODE_UI_SCREEN_WITH_INTERACTION
    ...
    @try {
        if (gameIsPaused)
            ...
            if (delta_t > MINIMUM_GAME_TICK)
                delta_t = MINIMUM_GAME_TICK;  // peg the maximum pause (at 0.5->1.0 seconds) to protect against when the
                                              // machine sleeps
        }
...
- (void)startAnimationTimer {
    if (timer == nil) {
        NSTimeInterval ti =
            _animationTimerInterval;  // default one two-hundredth of a second (should be a fair bit faster than
                                      // expected frame rate ~60Hz to avoid problems with phase differences)

        timer = [[NSTimer timerWithTimeInterval:ti
                                         target:self
                                       selector:@selector(performGameTick:)
                                       userInfo:nil
                                        repeats:YES] retain];
...
- (IBAction)showLogAction:sender {
    [[NSWorkspace sharedWorkspace]
        openFile:[OOLogHandlerGetLogBasePath() stringByAppendingPathComponent:@ "Previous.log"]];
}
...
static void RemovePreference(NSString *key) { [[NSUserDefaults standardUserDefaults] removeObjectForKey:key]; }
...
- (NSURL *)snapshotsURLCreatingIfNeeded:(BOOL)create {
    ...
    if (snapshotDirDict != nil) {
        url = JAURLFromPersistentFileReference(
            snapshotDirDict, kJAPersistentFileReferenceWithoutUI | kJAPersistentFileReferenceWithoutMounting, &stale);
        if (url != nil) {
            ...
            if ([existingName compare:name options:NSCaseInsensitiveSearch] != 0) {
                ...
                if (originalOldName == nil || [existingName compare:originalOldName
            ...
            OSStatus err =
                DetermineIfPathIsEnclosedByFolder(kOnAppropriateDisk, kTrashFolderType, utfPath, false, &inTrash);
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem {
    ...
    if (action == @selector(showLogAction:)) {
        ...
        return ([[NSFileManager defaultManager]
            fileExistsAtPath:[OOLogHandlerGetLogBasePath() stringByAppendingPathComponent:@ "Previous.log"]]);
    }
    ...
    if (action == @selector(showSnapshotsAction:)) {
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:[self snapshotsURLCreatingIfNeeded:NO].path
...
- (NSURL *)snapshotsURLCreatingIfNeeded:(BOOL)create {
    NSURL *url =
        [NSURL fileURLWithPath:[NSHomeDirectory() stringByAppendingPathComponent:DESC(@ "snapshots-directory-name")]];
...
- (void)setPlayerFileToLoad:(NSString *)filename {
    if (playerFileToLoad) [playerFileToLoad autorelease];
    playerFileToLoad = nil;
    if ([[[filename pathExtension] lowercaseString] isEqual:@ "oolite-save"]) playerFileToLoad = [filename copy];
}
...
- (void)reportUnhandledStartupException:(NSException *)exception {
    ...
    NSRunCriticalAlertPanel(@ "Oolite failed to start up, because an unhandled exception occurred.",
...
- (BOOL)suppressClangStuff {
    return pauseSelector && pauseTarget;
}
#endif
