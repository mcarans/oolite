...
/* The filename to temporarily store the downloaded OXZ. Has an OXZ extension since we might want to read its
 * manifest.plist out of it;  */
...
- (NSString *)extractionBasePathForIdentifier:(NSString *)identifier andVersion:(NSString *)version {
    ...
    return [[[basePath stringByAppendingPathComponent:[[rawMainDir componentsSeparatedByCharactersInSet:blacklist]
                                                          componentsJoinedByString:@ ""]] retain] autorelease];
}
...
- (NSString *)manifestPath {
    return [[[OOCacheManager sharedCache] cacheDirectoryPathCreatingIfNecessary:YES]
        stringByAppendingPathComponent:kOOOXZManifestCache];
}
...
- (NSString *)downloadPath {
    if (_interfaceState == OXZ_STATE_UPDATING) {
        return [[[OOCacheManager sharedCache] cacheDirectoryPathCreatingIfNecessary:YES]
            stringByAppendingPathComponent:kOOOXZTmpPlistPath];
    } else {
        return [[[OOCacheManager sharedCache] cacheDirectoryPathCreatingIfNecessary:YES]
            stringByAppendingPathComponent:kOOOXZTmpPath];
    }
}
...
- (NSArray *)applyCurrentFilter:(NSArray *)list {
    ...
    NSInvocation *invocation =
        [NSInvocation invocationWithMethodSignature:[[self class] instanceMethodSignatureForSelector:filterSelector]];
...
- (BOOL)applyFilterByKeyword:(NSDictionary *)manifest keyword:(NSString *)keyword {
    ...
    foreach (parameter, parameters) {
        if ([[manifest oo_stringForKey:parameter] rangeOfString:keyword options:NSCaseInsensitiveSearch].location !=
            NSNotFound) {
            return YES;
        }
    }
...
- (BOOL)validateFilter:(NSString *)input {
    ...
    if (([filter length] == 0)  // empty is valid
        || ([filter isEqualToString:kOOOXZFilterAll]) || ([filter isEqualToString:kOOOXZFilterUpdates]) ||
...
- (BOOL)beginDownload:(NSMutableURLRequest *)request {
    NSString *userAgent = [NSString
        stringWithFormat:@ "Oolite/%@", [[[NSBundle mainBundle] infoDictionary] objectForKey:@ "CFBundleVersion"]];
    ...
    if (download) {
        ...
        if (_interfaceState != OXZ_STATE_UPDATING) {
            ...
            label = [expectedManifest oo_stringForKey:kOOManifestTitle
                                         defaultValue:DESC(@ "oolite-oxzmanager-download-label-oxz")];
        }

        [self setCurrentDownload:download withLabel:label];  // retains it
        [download release];
        OOLog(kOOOXZDebugLog,
              @ "Download request received, using %@ and downloading to %@",
              [request URL],
              [self downloadPath]);
        return YES;
    } else {
...
- (BOOL)cancelUpdate {
    if (!(_interfaceState == OXZ_STATE_UPDATING || _interfaceState == OXZ_STATE_INSTALLING) ||
        _downloadStatus == OXZ_DOWNLOAD_NONE) {
        return NO;
    }
        ...
        foreach (filename, managedOXZs) {
            ...
            if (manifest != nil) {
                ...
                foreach (stored, _oxzList) {
                    if ([[stored oo_stringForKey:kOOManifestIdentifier]
                            isEqualToString:[manifest oo_stringForKey:kOOManifestIdentifier]]) {
                        if (foundInstallable == NO) {
                            [adjManifest setObject:[stored oo_stringForKey:kOOManifestVersion]
                                            forKey:kOOManifestAvailableVersion];
                            [adjManifest setObject:[stored oo_stringForKey:kOOManifestDownloadURL]
                                            forKey:kOOManifestDownloadURL];
...
- (BOOL)processDownloadedOXZ {
    ...
    NSDictionary *downloadedManifest =
        OODictionaryFromFile([[self downloadPath] stringByAppendingPathComponent:@ "manifest.plist"]);
    if (downloadedManifest == nil) {
        _downloadStatus = OXZ_DOWNLOAD_ERROR;
        OOLog(kOOOXZErrorLog,
    ...
    if (expectedManifest == nil ||
        (![[downloadedManifest oo_stringForKey:kOOManifestIdentifier]
            isEqualToString:[expectedManifest oo_stringForKey:kOOManifestIdentifier]]) ||
        (![[downloadedManifest oo_stringForKey:kOOManifestVersion]
            isEqualToString:[expectedManifest
                                oo_stringForKey:kOOManifestAvailableVersion
                                   defaultValue:[expectedManifest oo_stringForKey:kOOManifestVersion]]])) {
        _downloadStatus = OXZ_DOWNLOAD_ERROR;
        OOLog(
            kOOOXZErrorLog,
            @ "%@",
    ...
    if ([_dependencyStack count] > 0) {
        ...
        foreach (requirement, tempStack) {
            ...
            if (![ResourceManager manifest:downloadedManifest HasUnmetDependency:requirement logErrors:NO] &&
                ...
                [progress
                    appendFormat:DESC(@ "oolite-oxzmanager-progress-now-has-@"),
                                 [requirement
                                     oo_stringForKey:kOOManifestRelationDescription
                                        defaultValue:[requirement oo_stringForKey:kOOManifestRelationIdentifier]]];
                [_dependencyStack removeObject:requirement];
                OOLog(kOOOXZDebugLog, @ "%@", @ "Dependency stack: requirement met");
            } else if ([[requirement oo_stringForKey:kOOManifestRelationIdentifier]
                           isEqualToString:[downloadedManifest oo_stringForKey:kOOManifestIdentifier]]) {
    ...
    if (requires != nil) {
        foreach (requirement, requires) {
            if ([ResourceManager manifest:downloadedManifest HasUnmetDependency:requirement logErrors:NO]) {
                OOLog(kOOOXZDebugLog,
                      @ "Dependency stack: adding %@",
                      [requirement oo_stringForKey:kOOManifestRelationIdentifier]);
                [_dependencyStack addObject:requirement];
                [progress
                    appendFormat:DESC(@ "oolite-oxzmanager-progress-requires-@"),
                                 [requirement
                                     oo_stringForKey:kOOManifestRelationDescription
                                        defaultValue:[requirement oo_stringForKey:kOOManifestRelationIdentifier]]];
            }
    ...
    if ([_dependencyStack count] > 0) {
        ...
        do {
            ...
            foreach (availableDownload, _oxzList) {
                if ([[availableDownload oo_stringForKey:kOOManifestIdentifier] isEqualToString:needsIdentifier]) {
                    if ([ResourceManager matchVersions:requirement
                                           withVersion:[availableDownload oo_stringForKey:kOOManifestVersion]]) {
            ...
            if (foundDownload) {
                if ([self installableState:[_oxzList objectAtIndex:index]] == OXZ_UNINSTALLABLE_ALREADY) {
                    OOLog(kOOOXZDebugLog,
                          @ "Dependency stack: %@ is downloaded but not yet loadable, removing from list.",
                          [requirement oo_stringForKey:kOOManifestRelationIdentifier]);
        ...
        if (foundDownload) {
            ...
            if (_downloadAllDependencies) {
                ...
                if (![self installOXZ:index]) {
                    ...
                    [progress
                        appendFormat:DESC(@ "oolite-oxzmanager-progress-required-@-not-found"),
                                     [requirement
                                         oo_stringForKey:kOOManifestRelationDescription
                                            defaultValue:[requirement oo_stringForKey:kOOManifestRelationIdentifier]]];
                    [self setProgressStatus:progress];
                    OOLog(kOOOXZErrorLog,
                          @ "OXZ dependency %@ could not be found for automatic download.",
                          needsIdentifier);
                    _downloadStatus = OXZ_DOWNLOAD_ERROR;
                    OOLog(kOOOXZErrorLog, @ "%@", @ "Downloaded OXZ could not be installed.");
                    _interfaceState = OXZ_STATE_TASKDONE;
        ...
        // this is probably always the case, see above
            ...
            [progress
                appendFormat:DESC(@ "oolite-oxzmanager-progress-required-@-not-found"),
                             [requirement oo_stringForKey:kOOManifestRelationDescription
                                             defaultValue:[requirement oo_stringForKey:kOOManifestRelationIdentifier]]];
            [self setProgressStatus:progress];
...
- (OXZInstallableState)installableState:(NSDictionary *)manifest {
    ...
    if (installed != nil) {
        ...
        if ([[installed oo_stringForKey:kOOManifestVersion]
                isEqualToString:[manifest oo_stringForKey:kOOManifestAvailableVersion
                                             defaultValue:[manifest oo_stringForKey:kOOManifestVersion]]] &&
    ...
    if (installed != nil) {
        ...
        if (CompareVersions(ComponentsFromVersionString(installedVersion),
                            ComponentsFromVersionString(availableVersion)) == NSOrderedDescending) {
...
- (void)gui {
    ...
    switch (_interfaceState) {
        case OXZ_STATE_SETFILTER:
            ...
            [gui setText:[NSString stringWithFormat:DESC(@ "oolite-oxzmanager-currentfilter-is-@"), _currentFilter]
                  forRow:OXZ_GUI_ROW_FILTERCURRENT
                   align:GUI_ALIGN_LEFT];
            [gui addLongText:DESC(@ "oolite-oxzmanager-filterhelp")
                startingAtRow:OXZ_GUI_ROW_FILTERHELP
                        align:GUI_ALIGN_LEFT];

            return;  // don't do normal row selection stuff
        case OXZ_STATE_NODATA:
            if (_oxzList == nil) {
                [gui addLongText:DESC(@ "oolite-oxzmanager-firstrun")
                    startingAtRow:OXZ_GUI_ROW_FIRSTRUN
                            align:GUI_ALIGN_LEFT];
                ...
                // update data
                [gui addLongText:DESC(@ "oolite-oxzmanager-secondrun")
                    startingAtRow:OXZ_GUI_ROW_FIRSTRUN
                            align:GUI_ALIGN_LEFT];
                [gui setText:DESC(@ "oolite-oxzmanager-download-noupdate")
        ...
        case OXZ_STATE_RESTARTING:
            [gui addLongText:DESC(@ "oolite-oxzmanager-restart")
                startingAtRow:OXZ_GUI_ROW_FIRSTRUN
                        align:GUI_ALIGN_LEFT];
        ...
        case OXZ_STATE_PICK_REMOVE:
            if (_interfaceState != OXZ_STATE_MAIN) {
                [gui setText:[NSString stringWithFormat:DESC(@ "oolite-oxzmanager-currentfilter-is-@-@"),
                                                        OOExpand(@ "[oolite_key_oxzmanager_setfilter]"),
                                                        _currentFilter]
        ...
        case OXZ_STATE_INSTALLING:
            ...
            if (_downloadStatus == OXZ_DOWNLOAD_ERROR) {
                [gui addLongText:OOExpandKey(@ "oolite-oxzmanager-progress-error")
                    startingAtRow:OXZ_GUI_ROW_PROGRESS
                            align:GUI_ALIGN_LEFT];
            } else {
                [gui addLongText:[NSString stringWithFormat:DESC(@ "oolite-oxzmanager-progress-@-is-@-of-@"),
        ...
        case OXZ_STATE_DEPENDENCIES:
            ...
            [gui setText:DESC(@ "oolite-oxzmanager-dependencies-decision")
            ...
            [gui setText:DESC(@ "oolite-oxzmanager-dependencies-yes-all")
        ...
        case OXZ_STATE_REMOVING:
            [gui addLongText:DESC(@ "oolite-oxzmanager-removal-done")
                startingAtRow:OXZ_GUI_ROW_PROGRESS
                        align:GUI_ALIGN_LEFT];
        ...
        case OXZ_STATE_TASKDONE:
            if (_downloadStatus == OXZ_DOWNLOAD_COMPLETE) {
                [gui addLongText:[NSString stringWithFormat:DESC(@ "oolite-oxzmanager-progress-done-%u-%u"),
                                                            [_oxzList count],
        ...
        case OXZ_STATE_EXTRACT: {
            ...
            [gui setText:[NSString
                             stringWithFormat:DESC(@ "oolite-oxzmanager-infopage-title-@-version-@"), title, version]
                  forRow:0
                   align:GUI_ALIGN_LEFT];
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:path]) {
                ...
                [gui setText:DESC(@ "oolite-oxzmanager-extract-unavailable")
                      forRow:OXZ_GUI_ROW_PROCEED
                       align:GUI_ALIGN_CENTER];
                ...
                [gui addLongText:[NSString stringWithFormat:DESC(@ "oolite-oxzmanager-extract-to-@"), path]
                    startingAtRow:10
                            align:GUI_ALIGN_LEFT];
                startRow = OXZ_GUI_ROW_PROCEED;
                [gui setText:DESC(@ "oolite-oxzmanager-extract-proceed")
                      forRow:OXZ_GUI_ROW_PROCEED
                       align:GUI_ALIGN_CENTER];
                [gui setKey:@ "_PROCEED" forRow:OXZ_GUI_ROW_PROCEED];
            }
...
- (void)refreshTextInput:(NSString *)input {
    GuiDisplayGen *gui = [UNIVERSE gui];
    [gui setText:[NSString stringWithFormat:DESC(@ "oolite-oxzmanager-text-prompt-@"), input]
...
- (void)processFilterKey {
    ...
    if (_interfaceState == OXZ_STATE_PICK_INSTALL || _interfaceState == OXZ_STATE_PICK_INSTALLED ||
        _interfaceState == OXZ_STATE_PICK_REMOVE || _interfaceState == OXZ_STATE_MAIN) {
...
- (void)processShowInfoKey {
    if (_interfaceState == OXZ_STATE_PICK_INSTALL || _interfaceState == OXZ_STATE_PICK_INSTALLED ||
        ...
        if (_interfaceShowingOXZDetail) {
            ...
            [gui addLongText:[NSString stringWithFormat:DESC(@ "oolite-oxzmanager-infopage-tags-@"),
                                                        [[manifest oo_arrayForKey:kOOManifestTags]
                                                            componentsJoinedByString:@ ", "]]
                startingAtRow:4
                        align:GUI_ALIGN_LEFT];
            // description
            [gui addLongText:[NSString stringWithFormat:DESC(@ "oolite-oxzmanager-infopage-description-@"),
            ...
            [gui setText:[NSString stringWithFormat:DESC(@ "oolite-oxzmanager-infopage-infourl-@"), infoURLString]
                  forRow:25
                   align:GUI_ALIGN_LEFT];
...
- (void)processExtractKey {
    ...
    if (!_interfaceShowingOXZDetail &&
        (_interfaceState == OXZ_STATE_PICK_INSTALLED || _interfaceState == OXZ_STATE_PICK_REMOVE)) {
        GuiDisplayGen *gui = [UNIVERSE gui];
        OOGUIRow selection = [gui selectedRow];
...
- (OOGUIRow)showInstallOptions {
    ...
    foreach (manifest, options) {
        NSDictionary *installed =
            [ResourceManager manifestForIdentifier:[manifest oo_stringForKey:kOOManifestIdentifier]];
        NSString *localPath =
            [[[self installPath] stringByAppendingPathComponent:[manifest oo_stringForKey:kOOManifestIdentifier]]
                stringByAppendingPathExtension:@ "oxz"];
        if (installed == nil) {
        ...
        if (installed != nil) {
            installedVersion = [installed oo_stringForKey:kOOManifestVersion
                                             defaultValue:DESC(@ "oolite-oxzmanager-version-none")];
        }
        ...
        [gui setArray:[NSArray
                          arrayWithObjects:
                              [manifest oo_stringForKey:kOOManifestCategory
                                           defaultValue:DESC(@ "oolite-oxzmanager-missing-field")],
                              [manifest oo_stringForKey:kOOManifestTitle
                                           defaultValue:DESC(@ "oolite-oxzmanager-missing-field")],
                              installedVersion,
                              [manifest
        ...
        /* yellow for installable, orange for dependency issues, grey and unselectable for version issues, white and
         * unselectable for already installed (manually or otherwise) at the current version, red and unselectable for
         * already installed manually at a different version. */
        ...
        if (row == [gui selectedRow]) {
            ...
            if (infoUrl != nil) {
                [gui setArray:[NSArray arrayWithObjects:DESC(@ "oolite-oxzmanager-infoline-url"), infoUrl, nil]
            ...
            if (timestamp > 0) {
                ...
                [gui setArray:[NSArray arrayWithObjects:DESC(@ "oolite-oxzmanager-infoline-size"),
                                                        [self humanSize:size],
                                                        DESC(@ "oolite-oxzmanager-infoline-date"),
                                                        updatedDesc,
                                                        nil]
                ...
                [gui setArray:[NSArray
                                  arrayWithObjects:DESC(@ "oolite-oxzmanager-infoline-size"), [self humanSize:size], nil]
                       forRow:OXZ_GUI_ROW_LISTINFO2];
            }
        }
    ...
    if (!oxzLineSelected) {
        if (_interfaceState == OXZ_STATE_PICK_INSTALLED) {
            // installeD
            [gui addLongText:OOExpand(DESC(@ "oolite-oxzmanager-installed-nonepicked"))
                startingAtRow:OXZ_GUI_ROW_LISTDESC
                        align:GUI_ALIGN_LEFT];
        } else {
            // installeR
            [gui addLongText:OOExpand(DESC(@ "oolite-oxzmanager-installer-nonepicked"))
                startingAtRow:OXZ_GUI_ROW_LISTDESC
                        align:GUI_ALIGN_LEFT];
        }
    }
...
- (OOGUIRow)showRemoveOptions {
    ...
    GuiDisplayGen *gui = [UNIVERSE gui];
    if (options == nil) {
        [gui addLongText:DESC(@ "oolite-oxzmanager-nothing-removable")
            startingAtRow:OXZ_GUI_ROW_PROGRESS
                    align:GUI_ALIGN_LEFT];
        return startRow;
    }
    ...
    foreach (manifest, options) {
        [gui setArray:[NSArray arrayWithObjects:[manifest oo_stringForKey:kOOManifestCategory
                                                             defaultValue:DESC(@ "oolite-oxzmanager-missing-field")],
                                                [manifest oo_stringForKey:kOOManifestTitle
                                                             defaultValue:DESC(@ "oolite-oxzmanager-missing-field")],
                                                [manifest oo_stringForKey:kOOManifestVersion
    ...
    if (!oxzSelected) {
        [gui addLongText:DESC(@ "oolite-oxzmanager-remover-nonepicked")
            startingAtRow:OXZ_GUI_ROW_LISTDESC
                    align:GUI_ALIGN_LEFT];
    }
...
- (NSString *)extractOXZ:(NSUInteger)item {
    ...
    if (unzGoToFirstFile(uf) == UNZ_OK) {
        do {
            unzGetCurrentFileInfo64(uf, &file_info, rawComponentName, 512, NULL, 0, NULL, 0);
            NSString *componentName = [NSString stringWithUTF8String:rawComponentName];
            if ([componentName hasSuffix:@ "/"]) {
                // folder
                if (![fmgr oo_createDirectoryAtPath:[path stringByAppendingPathComponent:componentName]
                                         attributes:nil]) {
                ...
                if ([folder length] > 0 && ![fmgr fileExistsAtPath:folder] &&
                    ![fmgr oo_createDirectoryAtPath:folder attributes:nil]) {
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if (_interfaceState == OXZ_STATE_UPDATING) {
        ...
        OOLog(kOOOXZErrorLog,
              @ "Error: download completed in unexpected state %d. This is an internal error - please report it.",
              _interfaceState);
        _downloadStatus = OXZ_DOWNLOAD_ERROR;
    }
}
...
NSComparisonResult oxzSort(id m1, id m2, void *context) {
    NSComparisonResult result = [[m1 oo_stringForKey:kOOManifestCategory defaultValue:@ "zz"]
        localizedCompare:[m2 oo_stringForKey:kOOManifestCategory defaultValue:@ "zz"]];
    if (result == NSOrderedSame) {
        result = [[m1 oo_stringForKey:kOOManifestTitle
                         defaultValue:@ "zz"] localizedCompare:[m2 oo_stringForKey:kOOManifestTitle defaultValue:@ "zz"]];
        if (result == NSOrderedSame) {
            result = [[m2 oo_stringForKey:kOOManifestVersion defaultValue:@ "0"]
                localizedCompare:[m1 oo_stringForKey:kOOManifestVersion defaultValue:@ "0"]];
        }
    }
    return result;
}
