...
- (NSString *)description {
    return [NSString stringWithFormat:@ "<%@ %p>{ID: %d, %llu subregions, %u ents}",
                                      [self class],
                                      self,
                                      crid,
                                      [subregions count],
...
static BOOL positionIsWithinRegion(HPVector position, CollisionRegion *region) {
    ...
    if ((position.x < loc.x - r1) || (position.x > loc.x + r1) || (position.y < loc.y - r1) ||
        (position.y > loc.y + r1) || (position.z < loc.z - r1) || (position.z > loc.z + r1)) {
        return NO;
    }
...
static BOOL sphereIsWithinRegion(HPVector position, GLfloat rad, CollisionRegion *region) {
    ...
    if ((position.x - rad < loc.x - r1) || (position.x + rad > loc.x + r1) || (position.y - rad < loc.y - r1) ||
        (position.y + rad > loc.y + r1) || (position.z - rad < loc.z - r1) || (position.z + rad > loc.z + r1)) {
        return NO;
    }
...
- (void)findCollisions {
    ...
    for (i = 0; i < n_entities_to_test; i++) {
        ...
        while (e2 != nil) {
            checks_this_tick++;
            if (e1->isShip && e2->isShip && [(ShipEntity *)e1 collisionExceptedFor:(ShipEntity *)e2]) {
                // nothing happens
            } else {
                ...
                if (dist2 < PROXIMITY_WARN_DISTANCE2 * min_dist2) {
                    ...
                    if (gDebugFlags & DEBUG_COLLISIONS) {
                        OOLog(@ "collisionRegion.debug",
                              ...
                              e1,
                              (e1->collisionTestFilter == 3) ? @ "YES" : @ "NO",
                              e2,
                              (e2->collisionTestFilter == 3) ? @ "YES" : @ "NO");
                    }
#endif
                    checks_within_range++;

                    if (e1->isShip && e2->isShip) {
                        if ((dist2 < PROXIMITY_WARN_DISTANCE2 * r2 * r2) ||
                            (dist2 < PROXIMITY_WARN_DISTANCE2 * r1 * r1)) {
    ...
    if (gDebugFlags & DEBUG_COLLISIONS) {
        OOLog(@ "collisionRegion.debug",
              @ "Collision test checks %d, within range %d, for %d entities",
              checks_this_tick,
              checks_within_range,
              n_entities_to_test);
    }
...
- (void)findShadowedEntities {
    ...
    for (i = 0; i < ent_count; i++) {
        if (uni_entities[i]->isSunlit) {
            ...
            else if ([uni_entities[i] isShip] && [uni_entities[i] isVisible] &&
