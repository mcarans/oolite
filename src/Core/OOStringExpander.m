...
static NSString *Expand(OOStringExpansionContext *context,
                        NSString *string,
                        NSUInteger sizeLimit,
                        NSUInteger recursionLimit);

static NSString *ExpandKey(OOStringExpansionContext *context,
                           ...
                           NSUInteger sizeLimit,
                           NSUInteger recursionLimit);
static NSString *ExpandDigitKey(OOStringExpansionContext *context,
                                const unichar *characters,
                                NSUInteger keyStart,
                                NSUInteger keyLength,
                                NSUInteger sizeLimit,
                                NSUInteger recursionLimit);
static NSString *ExpandStringKey(OOStringExpansionContext *context,
                                 NSString *key,
                                 NSUInteger sizeLimit,
                                 NSUInteger recursionLimit);
...
static NSString *ExpandStringKeyFromDescriptions(OOStringExpansionContext *context,
                                                 NSString *key,
                                                 NSUInteger sizeLimit,
                                                 NSUInteger recursionLimit);
...
static NSString *ExpandPercentEscape(OOStringExpansionContext *context,
                                     const unichar *characters,
                                     NSUInteger size,
                                     NSUInteger idx,
                                     NSUInteger *replaceLength);
static NSString *ExpandSystemNameForGalaxyEscape(OOStringExpansionContext *context,
                                                 const unichar *characters,
                                                 NSUInteger size,
                                                 NSUInteger idx,
                                                 NSUInteger *replaceLength);
static NSString *ExpandSystemNameEscape(OOStringExpansionContext *context,
                                        const unichar *characters,
                                        NSUInteger size,
                                        NSUInteger idx,
                                        NSUInteger *replaceLength);
...
*/
static void SyntaxIssue(OOStringExpansionContext *context,
                        const char *function,
                        const char *fileName,
                        NSUInteger line,
                        NSString *logMessageClass,
                        NSString *prefix,
                        NSString *format,
                        ...) OO_TAKES_FORMAT_STRING(7, 8);
#define SyntaxError(CONTEXT, CLASS, FORMAT, ...) \
    SyntaxIssue(                                 \
        CONTEXT, OOLOG_FUNCTION_NAME, OOLOG_FILE_NAME, __LINE__, CLASS, OOLOG_WARNING_PREFIX, FORMAT, ##__VA_ARGS__)
...
NSString *OOExpandDescriptionString(Random_Seed seed,
    ...
    OOStringExpansionContext context = {.seed = seed,
    ...
    @try {
        // TODO: profile caching the results. Would need to keep track of whether we've done something nondeterministic
        // (array selection, %R etc).
...
Random_Seed OOStringExpanderDefaultRandomSeed(void) { return [[UNIVERSE systemManager] getRandomSeedForCurrentSystem]; }
    ...
    for (NSUInteger idx = 0; idx < size - 1; idx++) {
        ...
        */
        ...
        if (thisChar == '[') {
            ...
            SyntaxWarning(
                context, @ "strings.expand.warning.unbalancedClosingBracket", @ "%@", @ "Unbalanced ] in string.");
...
static NSString *ExpandKey(OOStringExpansionContext *context,
    ...
    for (end = idx + 1; end < size && balanceCount > 0; end++) {
        if (characters[end] == ']')
            ...
            if (characters[end] == '[')
                balanceCount++;
            else if (characters[end] == '|' && firstBar == 0)
                firstBar = end;
...
static NSString *Operator_cr(NSString *string, NSString *param) { return OOCredits([string doubleValue] * 10); }

static NSString *Operator_dcr(NSString *string, NSString *param) { return OOCredits([string longLongValue]); }

static NSString *Operator_icr(NSString *string, NSString *param) { return OOIntCredits([string longLongValue]); }
...
#define OPERATOR(name) [NSValue valueWithPointer:Operator_##name], @ #name
        operators = [[NSDictionary alloc] initWithObjectsAndKeys:OPERATOR(dcr),
                                                                 OPERATOR(cr),
                                                                 OPERATOR(icr),
                                                                 OPERATOR(idcr),
                                                                 OPERATOR(precision),
...
static NSString *ExpandDigitKey(OOStringExpansionContext *context,
    ...
    for (idx = keyStart; idx < (keyStart + keyLength); idx++) {
        NSCAssert2(isdigit(characters[idx]),
                   @ "%s called with non-numeric key [%@].",
                   __FUNCTION__,
                   [NSString stringWithCharacters:characters + keyStart length:keyLength]);
    ...
    if (EXPECT_NOT(entry == nil)) {
        if (keyValue >= context->sysDescCount) {
            SyntaxWarning(context,
                          ...
                          @ "Out-of-range system description expansion key [%@] in string.",
                          [NSString stringWithCharacters:characters + keyStart length:keyLength]);
        } else {
            // This is out of the scope of whatever triggered it, so shouldn't be a JS warning.
            OOLogERR(@ "strings.expand.invalidData",
                     @ "%@",
    ...
    if (value != nil) {
        ...
        if (![value isKindOfClass:[NSString class]] && ![value isKindOfClass:[NSNumber class]]) {
            SyntaxWarning(context,
                          ...
                          @ "String expansion override value %@ for [%@] is not a string or number.",
...
static NSString *ExpandStringKeySpecial(OOStringExpansionContext *context, NSString *key) {
    ...
    if (selector != NULL) {
        NSCAssert2([PLAYER respondsToSelector:selector],
                   @ "Special string expansion selector %@ for [%@] is not implemented.",
                   NSStringFromSelector(selector),
                   key);

        NSString *result = [PLAYER performSelector:selector];
        if (result != nil) {
            NSCAssert2([result isKindOfClass:[NSString class]],
                       @ "Special string expansion [%@] expanded to %@, but expected a string.",
                       key,
...
static NSMapTable *SpecialSubstitutionSelectors(void) {
    ...
    struct {
        NSString *key;
        SEL selector;
    } selectors[] = {{@ "commander_name", @selector(commanderName_string)},
...
static NSString *ExpandStringKeyFromDescriptions(OOStringExpansionContext *context,
    ...
    if (value != nil) {
        ...
        if (![value isKindOfClass:[NSString class]]) {
            // This is out of the scope of whatever triggered it, so shouldn't be a JS warning.
            OOLogERR(@ "strings.expand.invalidData",
                     @ "String expansion value %@ for [%@] from descriptions.plist is not a string or number.",
                     [value shortDescription],
                     key);
            return nil;
        }
...
static SEL LookUpLegacySelector(NSString *key) {
    ...
    if (selector == NULL) {
        ...
        if ([whitelist containsObject:selectorName]) {
            ...
            */
            NSCAssert1([PLAYER respondsToSelector:selector],
                       @ "Player does not respond to whitelisted query selector %@.",
                       key);
        }
        ...
        if (selector != NULL) {
            ...
            if (selectorCache == NULL) {
                selectorCache =
                    NSCreateMapTable(NSObjectMapKeyCallBacks, NSNonOwnedPointerMapValueCallBacks, [whitelist count]);
            }
...
static void ReportWarningForUnknownKey(OOStringExpansionContext *context, NSString *key) {
    if ([key hasSuffix:@ "_string"] || [key hasSuffix:@ "_number"] || [key hasSuffix:@ "_bool"]) {
        SyntaxError(
            context, @ "strings.expand.invalidSelector", @ "Unpermitted legacy script method [%@] in string.", key);
    } else {
        SyntaxWarning(
            context, @ "strings.expand.warning.unknownExpansion", @ "Unknown expansion key [%@] in string.", key);
    }
}
#endif
    ...
    switch (selector) {
        ...
        default:
            ...
            SyntaxWarning(
                ...
                @ "Unknown escape code in string: %%%lc. (To encode a %% sign without this warning, use %%%% - but prefer \"percent\" in prose writing.)",
                selector);
...
#define kInvalidGEscapeMessage @ "String escape code %G must be followed by six integers."
    ...
    if (!(isdigit(hundreds) && isdigit(tens) && isdigit(units) && isdigit(galHundreds) && isdigit(galTens) &&
          isdigit(galUnits))) {
        SyntaxError(context, @ "strings.expand.invalidJEscape", @ "%@", kInvalidGEscapeMessage);
        return nil;
    }
    ...
    if (sysID > kOOMaximumSystemID) {
        SyntaxError(context,
                    ...
                    @ "String escape code %%G%3u for system is out of range (must be less than %u).",
                    sysID,
    ...
    if (galID > kOOMaximumGalaxyID) {
        SyntaxError(context,
                    @ "strings.expand.invalidJEscape.range",
                    @ "String escape code %%G%3u for galaxy is out of range (must be less than %u).",
                    galID,
...
#define kInvalidJEscapeMessage @ "String escape code %J must be followed by three integers."
    ...
    if (sysID > kOOMaximumSystemID) {
        SyntaxError(context,
                    @ "strings.expand.invalidJEscape.range",
                    @ "String escape code %%J%3u is out of range (must be less than %u).",
                    sysID,
                    kOOMaximumSystemID + 1);
        return nil;
    }
...
static NSString *GetSystemNameIan(OOStringExpansionContext *context) {
    ...
    if (context->systemNameWithIan == nil) {
        context->systemNameWithIan = [OOExpandWithOptions(
            context->seed, kOOExpandDisallowPercentI | kOOExpandGoodRNG | kOOExpandKey, @ "planetname-possessive")
            retain];
    }
...
static NSString *NewRandomDigrams(OOStringExpansionContext *context) {
    ...
    if ((OO_EXPANDER_RANDOM % 5) < ((length == 1) ? 3 : 1))
        ++length;  // Make two-letter names rarer and 10-letter names happen sometimes
    NSString *digrams = [[UNIVERSE descriptions] objectForKey:@ "digrams"];
