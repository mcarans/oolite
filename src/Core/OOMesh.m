...
typedef enum { kNormalModePerFace, kNormalModeSmooth, kNormalModeExplicit } OOMeshNormalMode;
...
#define PROFILE(tag)                                                       \
    do {                                                                   \
        _stopwatchLastTime = Profile(tag, _stopwatch, _stopwatchLastTime); \
    } while (0)
...
+ (OOMaterial *)placeholderMaterial {
    ...
    if (placeholderMaterial == nil) {
        placeholderMaterial = [[OOBasicMaterial alloc]
             initWithName:@ "/placeholder/"
            configuration:[[ResourceManager materialDefaults] oo_dictionaryForKey:@ "no-textures-material"]];
    }
...
- (NSString *)descriptionComponents {
    return [NSString stringWithFormat:@ "\"%@\", %zu vertices, %zu faces, radius: %g m normals: %@",
                                      [self modelName],
                                      [self vertexCount],
                                      [self faceCount],
                                      [self collisionRadius],
                                      NormalModeDescription(_normalMode)];
}

- (id)copyWithZone:(NSZone *)zone {
    if (zone == [self zone])
        return [self retain];  // OK because we're immutable seen from the outside
    else
...
- (void)renderOpaqueParts {
    ...
    */
    ...
    if (_textureUnitCount <= 1) {
        ...
        */
        NSAssert2([[OOOpenGLExtensionManager sharedManager] textureCombinersSupported],
    ...
    @try {
        ...
        if ([[exception name] hasPrefix:@ "Oolite"])
            [UNIVERSE handleOoliteException:exception];  // handle these ourself
        else
            @throw exception;  // pass these on
    }
    ...
    if (materialCount != 0) {
        for (i = 0; i != materialCount; ++i) {
            ...
            if (![materialKeys[i] isEqualToString:@ "_oo_placeholder_material"]) {
                material =
                    [OOMaterial materialWithName:materialKeys[i]
                                        cacheKey:_cacheKey
...
- (unsigned)octreeDepth {
    ...
    if (size < kOctreeSmallSizeThreshold)
        result = kSmallOctreeDepth;
    else
        while (result < kMaxOctreeDepth) {
...
- (BoundingBox)findBoundingBoxRelativeToPosition:(Vector)opv
    ...
    if (EXPECT_NOT(vertexCount < 1)) {
        ...
        pv.z = rpos.z + si.z * _vertices[0].x + sj.z * _vertices[0].y +
               sk.z * _vertices[0].z;  // _vertices[0] position rel to opv
...
- (NSDictionary *)modelData {
    ...
    NSNumber *vertCnt = nil, *faceCnt = nil;
    NSData *vertData = nil, *normData = nil, *tanData = nil, *faceData = nil;
    NSArray *mtlKeys = nil;
    ...
    if (vertCnt == nil || faceCnt == nil || vertData == nil || faceData == nil || mtlKeys == nil || normMode == nil) {
        return nil;
    }
    ...
    return [NSDictionary dictionaryWithObjectsAndKeys:vertCnt,
                                                      @ "vertex count",
                                                      vertData,
                                                      @ "vertex data",
                                                      faceCnt,
...
- (BOOL)setModelFromModelData:(NSDictionary *)dict name:(NSString *)fileName {
    ...
    // Ensure we have all the required data elements.
    if (vertData == nil || faceData == nil || mtlKeys == nil) {
    ...
    if (includeNormals) {
        ...
        if (normData == nil || tanData == nil) {
            OOLog(
                @ "mesh.load.error.badCacheData", @ "Ignoring bad normal/tangent cache data for mesh \"%@\".", fileName);
            return NO;
        }
    }
...
- (BOOL)loadData:(NSString *)filename scaleFactor:(float)scale {
    ...
    if (!using_preloaded) {
        ...
        if (![self allocateVertexBuffersWithCount:vertexCount]) {
            OOLog(kOOLogAllocationFailure,
                  @ "***** ERROR: failed to allocate memory for model %@ (%u vertices).",
                  filename,
                  vertexCount);
            return NO;
        }
        ...
        if (faceRefs == NULL || ![self allocateFaceBuffersWithCount:faceCount]) {
            OOLog(kOOLogAllocationFailure,
                  @ "***** ERROR: failed to allocate memory for model %@ (%u vertices, %u faces).",
                  filename,
                  vertexCount,
                  faceCount);
            return NO;
            ...
            for (j = 0; j < faceCount; j++) {
                ...
                if (!failFlag) {
                    ...
                    if (!failFlag) {
                        ...
                        failString = [NSString
                            stringWithFormat:@ "%@Failed to read a color for face[%d] in FACES\n", failString, j];
                    }
                    ...
                    if (!failFlag) {
                        ...
                        failString = [NSString
                            stringWithFormat:@ "%@Failed to read a normal for face[%d] in FACES\n", failString, j];
                    }
                    ...
                    if ([scanner scanInt:&n_v]) {
                        if (n_v < 3) {
                            failFlag = YES;
                            failString = [NSString
                                stringWithFormat:@ "%@Face[%u] has fewer than three vertices.\n", failString, j];
                        } else if (n_v > 3) {
                            OOLogWARN(@ "mesh.load.warning.nonTriangular",
                                      @ "Face[%u] of %@ has %u vertices specified. Only the first three will be used.",
                                      j,
                                      baseFile,
                                      n_v);
                            n_v = 3;
                ...
                if (!failFlag) {
                    ...
                    if (![scanner scanUpToCharactersFromSet:whitespaceCharSet intoString:&materialKey]) {
                        ...
                        if (index != nil) {
                            ...
                            if (materialCount == kOOMeshMaxMaterials) {
                                OOLog(kOOLogMeshTooManyMaterials,
                                      @ "***** ERROR: model %@ has too many materials (maximum is %d)",
                                      filename,
                                      kOOMeshMaxMaterials);
                                return NO;
                            }
                    ...
                    if (!failFlag) {
                        ...
                        if (failFlag)
                            failString = [NSString
                                stringWithFormat:
                                    @ "%@Failed to read texture size for max_x and max_y in face[%d] in TEXTURES\n",
                                    failString,
                                    j];
                    }
                    ...
                    if (!failFlag) {
                        for (i = 0; i < 3; i++) {
                            ...
                            if (!failFlag) {
                                ...
                                failString = [NSString
                                    stringWithFormat:
                                        @ "%@Failed to read s t coordinates for vertex[%d] in face[%d] in TEXTURES\n",
                                        failString,
                                        i,
                                        j];
                        }
        ...
        if (failFlag) {
            OOLog(@ "mesh.error",
                  ...
                  failString,
                  filename,
                  (using_preloaded) ? @ "(from preloaded data)" : @ "(from file)");
        }
    }
    ...
    for (i = 0; i < faceCount; i++) {
        ...
        */
        ...
        if (dsAC * dtAB > dsAB * dtAC) {
            face->tangent = vector_normal(
                vector_subtract(vector_multiply_scalar(vProjAC, dtAB), vector_multiply_scalar(vProjAB, dtAC)));
        } else {
            face->tangent = vector_normal(
                vector_subtract(vector_multiply_scalar(vProjAB, dtAC), vector_multiply_scalar(vProjAC, dtAB)));
        }
    }
...
- (void)calculateVertexNormalsAndTangentsWithFaceRefs:(VertexFaceRef *)faceRefs {
    ...
    NSAssert1(
        _normals != NULL && _tangents != NULL, @ "Normal/tangent buffers not allocated in %s", __PRETTY_FUNCTION__);
    ...
    for (i = 0; i < vertexCount; i++) {
        ...
        tangent_sum =
            vector_subtract(tangent_sum, vector_multiply_scalar(normal_sum, dot_product(tangent_sum, normal_sum)));
...
- (void)calculateVertexTangentsWithFaceRefs:(VertexFaceRef *)faceRefs {
    ...
    */
    ...
    for (i = 0; i < vertexCount; i++) {
        ...
        tangent_sum =
            vector_subtract(tangent_sum, vector_multiply_scalar(_normals[i], dot_product(_normals[i], tangent_sum)));
        tangent_sum = vector_normal_or_fallback(tangent_sum, kBasisXVector);
...
- (void)getNormal:(Vector *)outNormal
    ...
    for (j = 0; j < faceCount; j++) {
        if (_faces[j].smoothGroup == smoothGroup) {
            if ((_faces[j].vertex[0] == v_index) || (_faces[j].vertex[1] == v_index) ||
                (_faces[j].vertex[2] == v_index)) {
            ...
            if (_faces[fi].materialIndex == mi) {
                for (vi = 0; vi < 3; vi++) {
                    ...
                    if (IsPerVertexNormalMode(_normalMode)) {
                        if (is_edge_vertex[v]) {
                            [self getNormal:&normal
                                   andTangent:&tangent
                                    forVertex:v
                                inSmoothGroup:_faces[fi].smoothGroup];
                        } else {
                            NSAssert1(_normals != NULL && _tangents != NULL,
                                      @ "Normal/tangent buffers not allocated in %s",
                                      __PRETTY_FUNCTION__);
...
- (void)calculateBoundingVolumes {
    ...
    result = 0.0f;
    if (vertexCount)
        bounding_box_reset_to_vector(&boundingBox, _vertices[0]);
    else
    ...
    d_squared = (length_longest_axis + length_shortest_axis) * (length_longest_axis + length_shortest_axis) *
                0.25;  // square of average length
    maxDrawDistance =
        d_squared * NO_DRAW_DISTANCE_FACTOR * NO_DRAW_DISTANCE_FACTOR;  // no longer based on the collision radius
...
- (BOOL)allocateVertexArrayBuffersWithCount:(NSUInteger)count {
    _displayLists.indexArray = [self allocateBytesWithSize:sizeof *_displayLists.indexArray
                                                     count:count * 3
                                                       key:@ "indexArray"];
    _displayLists.textureUVArray = [self allocateBytesWithSize:sizeof *_displayLists.textureUVArray
                                                         count:count * 6
                                                           key:@ "textureUVArray"];
    _displayLists.vertexArray = [self allocateBytesWithSize:sizeof *_displayLists.vertexArray
                                                      count:count * 3
                                                        key:@ "vertexArray"];
    _displayLists.normalArray = [self allocateBytesWithSize:sizeof *_displayLists.normalArray
                                                      count:count * 3
                                                        key:@ "normalArray"];
    _displayLists.tangentArray = [self allocateBytesWithSize:sizeof *_displayLists.tangentArray
                                                       count:count * 3
                                                         key:@ "tangentArray"];

    return _faces != NULL && _displayLists.indexArray != NULL && _displayLists.textureUVArray != NULL &&
           _displayLists.vertexArray != NULL && _displayLists.normalArray != NULL && _displayLists.tangentArray != NULL;
}
...
static NSUInteger VFRGetFaceAtIndex(VertexFaceRef *vfr, NSUInteger index) {
    ...
    if (index < vfr->internCount)
        return vfr->internFaces[index];
    else
        return [vfr->extra oo_unsignedIntegerAtIndex:index - vfr->internCount];
}
