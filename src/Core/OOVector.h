...
Vector OOVectorRandomSpatial(OOScalar maxLength);  // Random vector uniformly distributed in radius-maxLength sphere.
                                                   // (Longer vectors are more common.)
Vector OOVectorRandomRadial(OOScalar maxLength);   // Random vector with uniform distribution of direction and radius in
                                                   // radius-maxLength sphere. (Causes clustering at centre.)
// only needed in high-precision forms so far
// Vector OORandomPositionInCylinder(Vector centre1, OOScalar exclusion1, Vector centre2, OOScalar exclusion2, OOScalar
// radius); Vector OORandomPositionInShell(Vector centre, OOScalar inner, OOScalar outer);
#endif
...
OOINLINE Vector OOVectorInterpolate(Vector a, Vector b, OOScalar where) {
    return make_vector(OOLerp(a.x, b.x, where), OOLerp(a.y, b.y, where), OOLerp(a.z, b.z, where));
}

OOINLINE Vector OOVectorTowards(Vector a, Vector b, OOScalar where) {
    return make_vector(a.x + b.x * where, a.y + b.y * where, a.z + b.z * where);
}
...
OOINLINE Vector vector_flip(Vector v) { return vector_subtract(kZeroVector, v); }

OOINLINE bool vector_equal(Vector a, Vector b) { return a.x == b.x && a.y == b.y && a.z == b.z; }

OOINLINE OOScalar magnitude2(Vector vec) { return vec.x * vec.x + vec.y * vec.y + vec.z * vec.z; }

OOINLINE OOScalar magnitude(Vector vec) { return sqrt(magnitude2(vec)); }
...
OOINLINE Vector vector_normal_or_xbasis(Vector vec) { return vector_normal_or_fallback(vec, kBasisXVector); }

OOINLINE Vector vector_normal_or_ybasis(Vector vec) { return vector_normal_or_fallback(vec, kBasisYVector); }

OOINLINE Vector vector_normal_or_zbasis(Vector vec) { return vector_normal_or_fallback(vec, kBasisZVector); }

OOINLINE Vector vector_normal(Vector vec) { return vector_normal_or_fallback(vec, kZeroVector); }

OOINLINE OOScalar distance2(Vector v1, Vector v2) { return magnitude2(vector_subtract(v1, v2)); }

OOINLINE OOScalar distance(Vector v1, Vector v2) { return magnitude(vector_subtract(v1, v2)); }
...
OOINLINE Vector cross_product(Vector first, Vector second) { return vector_normal(true_cross_product(first, second)); }
