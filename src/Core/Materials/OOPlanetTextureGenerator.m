...
#define POLAR_CAPS 1
#define ALBEDO_FACTOR \
    0.7f  // Overall darkening of everything, allowing better contrast for snow and specular highlights.
...
#define FREE(x)                                       \
    do {                                              \
        if (0) {                                      \
            void *x__ = x;                            \
            x__ = x__;                                \
        } /* Preceeding is for type checking only. */ \
        void **x_ = (void **)&(x);                    \
        free(*x_);                                    \
        *x_ = NULL;                                   \
    } while (0)

#define PLANET_TEXTURE_OPTIONS \
    (kOOTextureMinFilterLinear | kOOTextureMagFilterLinear | kOOTextureRepeatS | kOOTextureNoShrink)

enum { kRandomBufferSize = 128 };
...
static float GetQ(
    float *qbuffer, int x, int y, unsigned width, unsigned height, unsigned widthMask, unsigned heightMask);
...
- (id)initWithPlanetInfo:(NSDictionary *)planetInfo {
    ...
    if ((self = [super initWithPath:[NSString stringWithFormat:@ "OOPlanetTexture@%p", self]
                            options:kOOTextureAllowCubeMap])) {
...
- (NSString *)cacheKeyForType:(NSString *)type {
    return [NSString
        stringWithFormat:@ "OOPlanetTextureGenerator-%@@%u\n%u,%u/%g/%u,%u/%f,%f,%f/%f,%f,%f/%f,%f,%f/%f,%f,%f",
                         ...
                         _info.seaColor.g,
                         _info.seaColor.b,
                         _info.paleLandColor.r,
                         _info.paleLandColor.g,
                         _info.paleLandColor.b,
                         _info.polarSeaColor.r,
                         _info.polarSeaColor.g,
                         _info.polarSeaColor.b];
}

- (OOPlanetNormalMapGenerator *)normalMapGenerator {
    if (_nMapGenerator == nil) {
        _nMapGenerator = [[OOPlanetNormalMapGenerator alloc] initWithCacheKey:[self cacheKeyForType:@ "normal"]
                                                                         seed:_info.seed];
    }
...
- (OOPlanetAtmosphereGenerator *)atmosphereGenerator {
    if (_atmoGenerator == nil) {
        _atmoGenerator = [[OOPlanetAtmosphereGenerator alloc] initWithCacheKey:[self cacheKeyForType:@ "atmo"]
                                                                          seed:_info.seed
                                                                     andParent:self];
    }
...
- (BOOL)getResult:(OOPixMap *)outData
    ...
    if (waiting) {
        ...
        OOLog(@ "texture.planet.generate.dequeue",
              @ "%s generator %@ without waiting.",
              result ? "Dequeued" : "Failed to dequeue",
              self);
    }
...
#define FAIL_IF_NULL(x) FAIL_IF((x) == NULL)
    ...
    // The second parameter is the temperature fraction. Most favourable: 1.0f,  little ice. Most unfavourable: 0.0f,
    // frozen planet. TODO: make it dependent on ranrot / planetinfo key...
    ...
    for (y = (int)_height - 1, fy = (float)y; y >= 0; y--, fy--) {
        ...
        for (x = (int)_width - 1; x >= 0; x--) {
            ...
            yN = GetQ(_info.qBuffer,
                      x,
                      y - 1,
                      _width,
                      _height,
                      widthMask,
                      heightMask);  // recalculates x & y if they go out of bounds.
            ...
            if (generateNormalMap) {
                ...
                */
                shade +=
                    color.a - color.a * shade;  // equivalent to - but slightly faster than - previous implementation.
            }
...
END:
    ...
    if (success) {
        NSString *diffuseName =
            [NSString stringWithFormat:@ "planet-%u-%u-diffuse-new", _info.seed.high, _info.seed.low];
...
- (void)dumpNoiseBuffer:(float *)noise {
    ...
    [[UNIVERSE gameView] dumpGrayToFileNamed:noiseName bytes:noisePx width:_width height:_height rowBytes:_width];
    FREE(noisePx);
}
...
static FloatRGB Blend(float fraction, FloatRGB a, FloatRGB b) {
    return (FloatRGB){Lerp(b.r, a.r, fraction), Lerp(b.g, a.g, fraction), Lerp(b.b, a.b, fraction)};
}

static float BlendAlpha(float fraction, float a, float b) { return Lerp(b, a, fraction); }
...
#define RECIP_DEEPS (1.0f / DEEPS)
    ...
    FloatRGB diffuse;
    // windows specular 'fix': 0 was showing pitch black continents when on the dark side, 0.01 shows the same shading
    // as on Macs.
    ...
    if (q <= 0.0f) {
        ...
        if (q > -SHALLOWS) {
            ...
            // Open sea.
            if (q > -DEEPS)
                diffuse = Blend(-q * RECIP_DEEPS, info->deepSeaColor, info->seaColor);
            else
                diffuse = info->deepSeaColor;
...
static FloatRGB FloatRGBFromDictColor(NSDictionary *dictionary, NSString *key) {
    ...
    return (FloatRGB){[color redComponent] * ALBEDO_FACTOR,
                      [color greenComponent] * ALBEDO_FACTOR,
                      [color blueComponent] * ALBEDO_FACTOR};
}

OOINLINE float Hermite(float q) { return 3.0f * q * q - 2.0f * q * q * q; }
...
enum {
    ...
    // Number of different gradient vectors used. The most important thing is that the gradients are evenly distributed
    // and sum to 0.
    kGradientCount = 12
};
...
#if !LUT_DOT
static const Vector kGradients[kGradientCount] = {{1, 1, 0},
                                                  {-1, 1, 0},
                                                  {1, -1, 0},
                                                  {-1, -1, 0},
                                                  {1, 0, 1},
                                                  ...
                                                  {0, -1, -1}};
#else
static const uint8_t kGradients[kGradientCount][3] = {{2, 2, 1},
                                                      {0, 2, 1},
                                                      {2, 0, 1},
                                                      {0, 0, 1},
                                                      {2, 1, 2},
...
static void AddNoise(OOPlanetTextureGeneratorInfo *info,
                     float *randomBuffer,
                     float octave,
                     unsigned octaveMask,
                     float scale,
...
static void SetMixConstants(OOPlanetTextureGeneratorInfo *info, float temperatureFraction) {
    ...
    info->mix_polarCap =
        temperatureFraction *
        (0.28f +
         0.24f * info->landFraction);  // landmasses make the polar cap proportionally bigger, but not too much bigger.
}
...
- (id)initWithCacheKey:(NSString *)cacheKey seed:(RANROTSeed)seed {
    // AllowCubeMap not used yet but might be in future
    if ((self = [super initWithPath:[NSString stringWithFormat:@ "OOPlanetNormalTexture@%p", self]
                            options:kOOTextureAllowCubeMap])) {
        _enqueued = NO;
...
- (id)initWithCacheKey:(NSString *)cacheKey seed:(RANROTSeed)seed andParent:(OOPlanetTextureGenerator *)parent {
    ...
    // AllowCubeMap not used yet but might be in future
    if ((self = [super initWithPath:[NSString stringWithFormat:@ "OOPlanetAtmoTexture@%p", self]
                            options:kOOTextureAllowCubeMap])) {
        _cacheKey = [cacheKey copy];
        _seed = seed;
...
- (BOOL)getResult:(OOPixMap *)result
    ...
    return [super getResult:result format:outFormat originalWidth:outWidth originalHeight:outHeight];
}
