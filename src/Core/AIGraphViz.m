...
static NSString *HandlerToken(NSString *state,
                              NSString *handler,
                              NSMutableDictionary *handlerKeys,
                              NSMutableSet *uniqueSet);
static void HandleOneCommand(NSMutableString *graphViz,
                             NSString *stateKey,
                             NSString *handlerKey,
                             NSMutableDictionary *handlerKeys,
                             NSArray *handlerCommands,
                             NSUInteger commandIter,
                             NSUInteger commandCount,
                             NSMutableSet *specialNodes,
                             NSMutableSet *uniqueSet,
                             BOOL *haveSetOrSwichAI);
static void AddSimpleSpecialNodeLink(NSMutableString *graphViz,
                                     NSString *handlerToken,
                                     NSString *name,
                                     NSString *shape,
                                     NSString *color,
                                     NSMutableSet *specialNodes);
static void AddExitAINode(NSMutableString *graphViz,
                          NSString *handlerToken,
                          NSString *message,
                          NSMutableSet *specialNodes);
static void AddChangeAINode(NSMutableString *graphViz,
                            NSString *handlerToken,
                            NSString *method,
                            NSArray *components,
                            NSArray *handlerCommands,
...
void GenerateGraphVizForAIStateMachine(NSDictionary *stateMachine, NSString *smName) {
    ...
    NSMutableString *graphViz = [NSMutableString
        stringWithFormat:@ "digraph ai_flow\n{\n"
                          "\tgraph [charset=\"UTF-8\", label=\"%@ transition diagram\", labelloc=t, labeljust=l "
                          "rankdir=LR compound=true nodesep=0.1 ranksep=2.5 fontname=Helvetica]\n"
                          ...
                          "[lhead=\"cluster_GLOBAL\" color=\"#0000A0\"]\n",
                         EscapedGraphVizString(smName),
                         HandlerToken(@ "GLOBAL", @ "ENTER", handlerKeys, uniqueSet)];
    ...
    while ((stateKey = [stateKeyEnum nextObject])) {
        ...
        [graphViz appendFormat:@ "\t\n\tsubgraph cluster_%@\n\t{\n\t\tlabel=\"%@\"\n",
                               stateKey,
                               EscapedGraphVizString(stateKey)];
        ...
        while ((handlerKey = [handlerKeyEnum nextObject])) {
            [graphViz appendFormat:@ "\t\t%@ [label=\"%@\"]\n",
                                   HandlerToken(stateKey, handlerKey, handlerKeys, uniqueSet),
                                   EscapedGraphVizString(handlerKey)];
        }
        ...
        if ([state objectForKey:@ "ENTER"] == nil) {
            [graphViz
                appendFormat:
                    @ "\t\t%@ [label=\"ENTER (implicit)\"] // No ENTER handler in file, but it's still the target of any incoming transitions.\n",
...
static void HandleOneCommand(NSMutableString *graphViz,
    ...
    if (!*haveSetOrSwichAI && [method isEqualToString:@ "setStateTo:"]) {
        if ([components count] > 1) {
            ...
            BOOL constraint = YES;
            if ([targetState isEqualToString:stateKey])
                constraint = NO;
            else if ([targetState isEqualToString:@ "GLOBAL"])
                constraint = NO;
            ...
            [specialNodes
                addObject:
                    @ "\tspecial_brokenSetStateTo [label=\"Broken setStateTo: command!\\n(No target state specified.)\" color=\"#C00000\" shape=diamond]\n"];
        ...
        AddSimpleSpecialNodeLink(
            graphViz, handlerToken, @ "performHyperSpaceExitWithoutReplacing", @ "box", @ "008080", specialNodes);
        ...
        AddSimpleSpecialNodeLink(
            graphViz, handlerToken, @ "becomeUncontrolledThargon", @ "ellipse", @ "804000", specialNodes);
        ...
        *haveSetOrSwichAI = YES;
        AddChangeAINode(
            graphViz, handlerToken, method, components, handlerCommands, commandIter, commandCount, specialNodes);
    }
}
...
static void AddExitAINode(NSMutableString *graphViz,
    ...
    [specialNodes
        addObject:[NSString stringWithFormat:@ "\t%@ [label=\"%@\" color=\"#0000A0\" shape=ellipse]\n", token, label]];
...
static void AddChangeAINode(NSMutableString *graphViz,
    ...
    if ([components count] > 1) {
        ...
        [specialNodes
            addObject:[NSString
                          stringWithFormat:@ "\t%@ [label=\"%@\" color=\"#408000\" shape=ellipse]\n", token, label]];
