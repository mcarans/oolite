...
static GLfloat docked_light_specular[4] = {
    DOCKED_ILLUM_LEVEL, DOCKED_ILLUM_LEVEL, DOCKED_ILLUM_LEVEL * 0.75f, (GLfloat)1.0f};  // yellow-white

// Weight of sun in ambient light calculation. 1.0 means only sun's diffuse is used for ambient, 0.0 means only sky
// colour is used.
...
- (int)nextColorblindMode:(int)index {
    if (++index > OO_POSTFX_COLORBLINDNESS_TRITAN) index = OO_POSTFX_NONE;

    return index;
}

- (int)prevColorblindMode:(int)index {
    if (--index < OO_POSTFX_NONE) index = OO_POSTFX_COLORBLINDNESS_TRITAN;
...
- (void)initTargetFramebufferWithViewSize:(NSSize)viewSize {
    ...
    OOGL(glTexImage2DMultisample(
        GL_TEXTURE_2D_MULTISAMPLE, 4, GL_RGBA16F, (GLsizei)viewSize.width, (GLsizei)viewSize.height, GL_TRUE));
    ...
    OOGL(glRenderbufferStorageMultisample(
        GL_RENDERBUFFER, 4, GL_DEPTH_COMPONENT32F, (GLsizei)viewSize.width, (GLsizei)viewSize.height));
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        OOLogERR(
            @ "initTargetFramebufferWithViewSize.result", @ "%@", @ "***** Error: Multisample framebuffer not complete");
    }
    ...
    OOGL(glTexImage2D(
        GL_TEXTURE_2D, 0, GL_RGBA16F, (GLsizei)viewSize.width, (GLsizei)viewSize.height, 0, GL_RGBA, GL_FLOAT, NULL));
    ...
    OOGL(glRenderbufferStorage(
        GL_RENDERBUFFER, GL_DEPTH_COMPONENT32F, (GLsizei)viewSize.width, (GLsizei)viewSize.height));
    ...
    // passthrough buffer
    // This is a framebuffer whose sole purpose is to pass on the texture rendered from the game to the blur and the
    // final bloom shaders. We need it in order to be able to use the OpenGL 3.3 layout (location = x) out vec4
    // outVector; construct, which allows us to perform multiple render target operations needed for bloom. The
    // alternative would be to not use this and change all our shaders to be OpenGL 3.3 compatible, but given how Oolite
    // synthesizes them and the work needed to port them over, well yeah no, not doing it at this time - Nikos 20220814.
    ...
    for (unsigned int i = 0; i < 2; i++) {
        ...
        OOGL(glTexImage2D(GL_TEXTURE_2D,
                          0,
                          GL_RGBA16F,
                          (GLsizei)viewSize.width,
                          (GLsizei)viewSize.height,
                          0,
                          GL_RGBA,
                          GL_FLOAT,
                          NULL));
        ...
        OOGL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE));
        OOGL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE));
        OOGL(glFramebufferTexture2D(
            GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, GL_TEXTURE_2D, passthroughTextureID[i], 0));
    }
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        OOLogERR(
            @ "initTargetFramebufferWithViewSize.result", @ "%@", @ "***** Error: Passthrough framebuffer not complete");
    }
    OOGL(glBindFramebuffer(GL_FRAMEBUFFER, defaultDrawFBO));
    ...
    for (unsigned int i = 0; i < 2; i++) {
        ...
        OOGL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR));
        OOGL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR));
        OOGL(glTexParameteri(
            GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE));  // we clamp to the edge as the blur filter would
        ...
        if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
            OOLogERR(
                @ "initTargetFramebufferWithViewSize.result", @ "%@", @ "***** Error: Pingpong framebuffers not complete");
        }
    }
    OOGL(glBindFramebuffer(GL_FRAMEBUFFER, defaultDrawFBO));
    ...
    */
    ...
    if (![[OOOpenGLExtensionManager sharedManager] shadersForceDisabled]) {
        ...
        // shader for blurring the over-threshold brightness image generated from the previous step using Gaussian
        // filter
        ...
        finalProgram = [[OOShaderProgram
            shaderProgramWithVertexShaderName:@ "oolite-final.vertex"
#if OOLITE_WINDOWS
                           fragmentShaderName:[[UNIVERSE gameView] hdrOutput] ? @ "oolite-final-hdr.fragment"
...
- (void)drawTargetTextureIntoDefaultFramebuffer {
    ...
    for (unsigned int i = 0; i < amount; i++) {
        ...
        OOGL(
            glBindTexture(GL_TEXTURE_2D, firstIteration ? passthroughTextureID[1] : pingpongColorbuffers[!horizontal]));
...
- (id)initWithGameView:(MyOpenGLView *)inGameView {
    if (gSharedUniverse != nil) {
        ...
        [NSException raise:NSInternalInconsistencyException
                    format:@ "%s: expected only one Universe to exist at a time.", __PRETTY_FUNCTION__];
    }
    ...
    OOLog(@ "speech.synthesis",
          @ "Spoken messages are %@.",
          ([prefs oo_boolForKey:@ "speech_on" defaultValue:NO] ? @ "on" : @ "off"));
#endif
    ...
    missiontext = [[ResourceManager dictionaryFromFilesNamed:@ "missiontext.plist" inFolder:@ "Config"
                                                    andMerge:YES] retain];
...
- (void)setDoProcedurallyTexturedPlanets:(BOOL)value {
    ...
    [[NSUserDefaults standardUserDefaults] setBool:doProcedurallyTexturedPlanets
                                            forKey:@ "procedurally-textured-planets"];
}
...
- (void)debugDumpEntities {
    ...
    OOLog(@ "universe.objectDump",
          @ "DEBUG: Entity Dump - [entities count] = %llu,\tn_entities = %u",
          [entities count],
          n_entities);
...
- (void)carryPlayerOn:(StationEntity *)carrier inWormhole:(WormholeEntity *)wormhole {
    ...
    ShipScriptEvent(context,
                    ...
                    STRING_TO_JSVAL(JS_InternString(context, [[player jumpCause] UTF8String])),
                    INT_TO_JSVAL(dest));
    ...
    if (![wormhole withMisjump]) {
        ...
        if (randf() < 0.1)
            [player erodeReputation];  // once every 10 misjumps - should be much rarer than successful jumps!
    }
...
- (void)setUpUniverseFromStation {
    if (![self sun]) {
        ...
        BOOL interstel =
            [dockedStation interstellarUndockingAllowed];  // && (s_seed.d != coords.x || s_seed.b != coords.y); - Nikos
                                                           // 20110623: Do we really need the commented out check?
        [player setPreviousSystemID:[player currentSystemID]];

        // remove everything except the player and the docked station
        if (dockedStation && !interstel) {  // jump to the nearest system
            ...
            if (dockedStation == nil)
                [self removeAllEntitiesExceptPlayer];  // get rid of witchspace sky etc. if still extant
        }
        ...
        if (!dockedStation || !interstel) {
            ...
            if (dockedStation) {
                if ([dockedStation maxFlightSpeed] > 0)  // we are a carrier: exit near the WitchspaceExitPosition
                {
                    ...
                    if (abs((int)d1) < 2750) {
                        d1 += ((d1 > 0.0) ? 2750.0f
                                          : -2750.0f);  // no closer than 2750m. Carriers are bigger than player ships.
                    }
...
- (void)setUpUniverseFromWitchspace {
    ...
    [comm_log_gui
        printLongText:[NSString stringWithFormat:@ "%@ %@", [self getSystemName:systemID], [player dial_clock_adjusted]]
                align:GUI_ALIGN_CENTER
                color:[OOColor whiteColor]
             fadeTime:0
                  key:nil
           addToArray:[player commLog]];
...
- (void)setUpWitchspaceBetweenSystem:(OOSystemID)s1 andSystem:(OOSystemID)s2 {
    ...
    [system_repopulator release];
    system_repopulator = [[systeminfo oo_stringForKey:@ "repopulator"
                                         defaultValue:@ "interstellarSpaceWillRepopulate"] retain];
    JSContext *context = OOJSAcquireContext();
    [PLAYER doWorldScriptEvent:OOJSIDFromString(populator)
                     inContext:context
                 withArguments:NULL
                         count:0
    ...
    if (script_actions != nil) {
        OOStandardsDeprecated(
            [NSString stringWithFormat:@ "The script_actions system info key is deprecated for %@.", override_key]);
...
- (OOPlanetEntity *)setUpPlanet {
    ...
    NSMutableDictionary *planetDict =
        [NSMutableDictionary dictionaryWithDictionary:[systemManager getPropertiesForCurrentSystem]];
    [planetDict oo_setBool:YES forKey:@ "mainForLocalSystem"];
    OOPlanetEntity *a_planet = [[OOPlanetEntity alloc] initFromDictionary:planetDict
                                                           withAtmosphere:[planetDict oo_boolForKey:@ "has_atmosphere"
                                                                                       defaultValue:YES]
                                                                  andSeed:systemSeed
                                                                forSystem:systemID];
    ...
    if ([allPlanets count] >
        0)  // F7 sets [UNIVERSE planet], which can lead to some trouble! TODO: track down where exactly that happens!
    {
...
#define FPROP(PROP) OOLog(@ "planetinfo.record", @ #PROP " = %f;", [systeminfo oo_floatForKey:@ "" #PROP]);
    ...
    do {
        sun_distance *= 2.0;
        sunPos = HPvector_subtract([a_planet position], HPvector_multiply_scalar(sun_dir, sun_distance));
    ...
    */
    if (![a_station isStation] || ![a_station validForAddToUniverse]) {
        if (a_station == nil) {
            ...
            OOLog(@ "universe.setup.badStation",
                  @ "Failed to set up a ship for role \"%@\" as system station, trying again with \"%@\".",
                  stationDesc,
                  defaultStationDesc);
        } else {
            OOLog(
                @ "universe.setup.badStation",
                @ "***** ERROR: Attempt to use non-station ship of type \"%@\" for role \"%@\" as system station, trying again with \"%@\".",
                [a_station name],
                stationDesc,
                defaultStationDesc);
        }
        ...
        if (![a_station isStation] || ![a_station validForAddToUniverse]) {
            if (a_station == nil) {
                OOLog(
                    @ "universe.setup.badStation",
                    @ "On retry, failed to set up a ship for role \"%@\" as system station. Trying to fall back to built-in Coriolis station.",
                    stationDesc);
            } else {
                OOLog(
                    @ "universe.setup.badStation",
                    @ "***** ERROR: On retry, rolled non-station ship of type \"%@\" for role \"%@\". Non-station ships should not have this role! Trying to fall back to built-in Coriolis station.",
                    [a_station name],
                    stationDesc);
            }
            [a_station release];
...
- (void)populateNormalSpace {
    ...
    if ([PLAYER status] != STATUS_START_GAME) {
        NSString *populator =
            [systeminfo oo_stringForKey:@ "populator"
                           defaultValue:(sunGoneNova) ? @ "novaSystemWillPopulate" : @ "systemWillPopulate"];
        [system_repopulator release];
        system_repopulator =
            [[systeminfo oo_stringForKey:@ "repopulator"
                            defaultValue:(sunGoneNova) ? @ "novaSystemWillRepopulate" : @ "systemWillRepopulate"] retain];
    ...
    NSArray *script_actions = [systeminfo oo_arrayForKey:@ "script_actions"];
    if (script_actions != nil) {
        OOStandardsDeprecated([NSString stringWithFormat:@ "The script_actions system info key is deprecated for %@.",
...
- (HPVector)locationByCode:(NSString *)code withSun:(OOSunEntity *)sun andPlanet:(OOPlanetEntity *)planet {
    ...
    // past this point, can assume non-nil sun, planet
        ...
        if ([code isEqualToString:@ "LANE_WPS"]) {
            ...
            result = OORandomPositionInCylinder(
                kZeroHPVector, SCANNER_MAX_RANGE, [planet position], [planet radius] * 3, LANE_WIDTH);
        } else if ([code isEqualToString:@ "LANE_WS"]) {
            result = OORandomPositionInCylinder(
                kZeroHPVector, SCANNER_MAX_RANGE, [sun position], [sun radius] * 3, LANE_WIDTH);
        } else if ([code isEqualToString:@ "LANE_PS"]) {
            result = OORandomPositionInCylinder(
                [planet position], [planet radius] * 3, [sun position], [sun radius] * 3, LANE_WIDTH);
        } else if ([code isEqualToString:@ "STATION_AEGIS"]) {
            do {
                result = OORandomPositionInShell(
                    [[self station] position], [[self station] collisionRadius] * 1.2, SCANNER_MAX_RANGE * 2.0);
            ...
            do {
                ...
                result = HPvector_add(HPvector_multiply_scalar([planet position], r),
                                      HPvector_multiply_scalar([sun position], s));
            }
            // make sure at least 3 radii from vertices
            while (HPdistance2(result, [sun position]) < [sun radius] * [sun radius] * 9.0 ||
                   HPdistance2(result, [planet position]) < [planet radius] * [planet radius] * 9.0 ||
                   HPmagnitude2(result) < SCANNER_MAX_RANGE2 * 9.0);
        } else if ([code isEqualToString:@ "INNER_SYSTEM"]) {
            do {
                result = OORandomPositionInShell(
                    [sun position], [sun radius] * 3.0, HPdistance([sun position], [planet position]));
                result =
                    OOProjectHPVectorToPlane(result, kZeroHPVector, HPcross_product([sun position], [planet position]));
            ...
            result = OORandomPositionInShell(
                [sun position],
                HPdistance([sun position], [planet position]),
                HPdistance([sun position], [planet position]) * 10.0);  // no more than 10 AU out
            result =
                OOProjectHPVectorToPlane(result, kZeroHPVector, HPcross_product([sun position], [planet position]));
            result = HPvector_add(
                result, OOHPVectorRandomSpatial(0.01 * HPdistance(result, [sun position])));  // within 1% of plane
            ...
            OOLog(kOOLogUniversePopulateError,
                  @ "Named populator region %@ is not implemented, falling back to WITCHPOINT",
                  code);
            result = OOHPVectorRandomSpatial(SCANNER_MAX_RANGE);
        }
    }
...
- (void)setLighting {
    ...
    */
    ...
    if (the_sun) {
        ...
        // witchspace
        stars_ambient[0] = 0.05;
        stars_ambient[1] = 0.20;
        stars_ambient[2] = 0.05;
        stars_ambient[3] = 1.0;
        sun_diffuse[0] = 0.85;
        sun_diffuse[1] = 1.0;
        sun_diffuse[2] = 0.85;
...
- (ShipEntity *)addShipWithRole:(NSString *)desc launchPos:(HPVector)launchPos rfactor:(GLfloat)rfactor {
    ...
    if (ship) {
        ...
        if (![ship crew] && ![ship isUnpiloted])
            [ship setCrew:[NSArray arrayWithObject:[OOCharacter randomCharacterWithRole:desc
...
- (HPVector)coordinatesForPosition:(HPVector)pos
    ...
    /*	the point is described using a system selected by a string
            ...
            So:
                    ps:		z axis = (planet -> sun)		y axis = normal to (planet - sun - witchpoint)
       x axis = normal to y and z axes pw:		z axis = (planet -> witchpoint)	y axis = normal to (planet -
       witchpoint - sun)	x axis = normal to y and z axes sp:		z axis = (sun -> planet)
       y axis = normal to (sun - planet - witchpoint)	x axis = normal to y and z axes sw:		z axis = (sun ->
       witchpoint)	y axis = normal to (sun - witchpoint - planet)	x axis = normal to y and z axes wp:
       z axis = (witchpoint -> planet)	y axis = normal to (witchpoint - planet - sun)	x axis = normal to y and z axes
                    ws:		z axis = (witchpoint -> sun)	y axis = normal to (witchpoint - sun - planet)	x axis =
       normal to y and z axes

            The third letter denotes the units used:
                    m:		meters
                    p:		planetary radii
                    s:		solar radii
                    u:		distance between first two features indicated (eg. spu means that u = distance from sun
       to the planet)
    ...
    */
    ...
    if ([l_sys length] != 3) return kZeroHPVector;
    ...
    HPVector v =
        HPvector_normal_or_xbasis(HPvector_subtract(p2, p0));  // temporary vector in plane of 'forward' and 'right'
...
- (HPVector)coordinatesFromCoordinateSystemString:(NSString *)system_x_y_z {
    ...
    GLfloat dummy;
    return [self
        coordinatesForPosition:make_HPvector(
                                   [tokens oo_floatAtIndex:1], [tokens oo_floatAtIndex:2], [tokens oo_floatAtIndex:3])
          withCoordinateSystem:[tokens oo_stringAtIndex:0]
               returningScalar:&dummy];
}
...
- (BOOL)addShipWithRole:(NSString *)desc nearPosition:(HPVector)pos withCoordinateSystem:(NSString *)system {
    ...
    if (rfactor > SCANNER_MAX_RANGE) rfactor = SCANNER_MAX_RANGE;
    if (rfactor < 1000) rfactor = 1000;
...
- (BOOL)addShips:(int)howMany
    ...
    bounding_box_reset_to_vector(&launch_bbox,
                                 make_vector(launchPos.x - rfactor, launchPos.y - rfactor, launchPos.z - rfactor));
...
- (BOOL)addShips:(int)howMany withRole:(NSString *)desc intoBoundingBox:(BoundingBox)bbox {
    ...
    if (howMany > 1) {
        ...
        return ([self addShips:h0 withRole:desc intoBoundingBox:bbox0] && [self addShips:h1
                                                                                     withRole:desc
                                                                              intoBoundingBox:bbox1]);
    }
...
- (BOOL)spawnShip:(NSString *)shipdesc {
    ...
    OOStandardsDeprecated([NSString
        stringWithFormat:@ "'spawn' via legacy script is deprecated as a way of adding ships for %@", shipdesc]);
...
- (void)witchspaceShipWithPrimaryRole:(NSString *)role {
    ...
    if (ship) {
        ...
        if ([role isEqual:@ "trader"]) {
            ...
            if ([ship hasRole:@ "sunskim-trader"] &&
                randf() < 0.25)  // select 1/4 of the traders suitable for sunskimming.
            {
        ...
        if ([role isEqual:@ "pirate"]) {
            ...
            [ship setBounty:(Ranrot() & 7) + (Ranrot() & 7) + ((randf() < 0.05) ? 63 : 23)
                 withReason:kOOLegalStatusReasonSetup];  // they already have a price on their heads
        }
        if ([ship crew] == nil && ![ship isUnpiloted])
            [ship setCrew:[NSArray arrayWithObject:[OOCharacter randomCharacterWithRole:role
                                                                      andOriginalSystem:Ranrot() & 255]]];
        // The following is set inside leaveWitchspace: AI state GLOBAL, STATUS_EXITING_WITCHSPACE, ai message:
        // EXITED_WITCHSPACE, then STATUS_IN_FLIGHT
...
- (ShipEntity *)spawnShipWithRole:(NSString *)desc near:(Entity *)entity {
    ...
    spawn_pos = HPvector_add([entity position],
                             vectorToHPVector(vector_multiply_scalar(vector_forward_from_quaternion(spawn_q), offset)));
...
- (ShipEntity *)addShipAt:(HPVector)pos withRole:(NSString *)role withinRadius:(GLfloat)radius {
    ...
    if (ship != nil) {
        ...
        if (trader) {
            ...
            if (randf() > 0.5f) {
                [ship setCargoFlag:(randf() < 0.66f ? CARGO_FLAG_FULL_PLENTIFUL
                                                    : CARGO_FLAG_FULL_SCARCE)];  // most of them will carry the cargo
                                                                                 // produced in-system.
            }
...
- (NSArray *)addShipsToRoute:(NSString *)route
    ...
    if ([route isEqualTo:@ "wp"] || [route isEqualTo:@ "pw"]) {
        ...
        return nil;  // no route specifier? We shouldn't be here!
    ...
    if (isGroup) {
        ...
        while (count--) {
            ship = [self addShipAt:pos
                          withRole:role
                      withinRadius:0];  // no radius because pos is already randomised with SCANNER_MAX_RANGE.
...
- (void)setUpBreakPattern:(HPVector)pos orientation:(Quaternion)q forDocking:(BOOL)forDocking {
    ...
    if (colorDesc != nil) {
        ...
        if (color != nil)
            ...
            OOLogWARN(@ "hyperspaceTunnel.fromDict", @ "could not interpret \"%@\" as a colour.", colorDesc);
    }
...
- (void)setupIntroFirstGo:(BOOL)justCobra {
    ...
    if (justCobra) {
        ...
        if (!demo_ship)
            ship = [self newShipWithName:[[[demo_ships oo_arrayAtIndex:demo_ship_index]
                                             oo_dictionaryAtIndex:demo_ship_subindex] oo_stringForKey:kOODemoShipKey]
                          usePlayerProxy:NO];
...
- (void)setLibraryTextForDemoShip {
    ...
    if (![librarySettings oo_boolForKey:kOODemoShipShipData
                           defaultValue:[[librarySettings oo_stringForKey:kOODemoShipClass
                                                             defaultValue:@ "ship"] isEqualToString:@ "ship"]]) {
        descRow = 3;
    } else {
    ...
    field1 =
        [NSString stringWithFormat:@ "<-- %@",
                                   OOShipLibraryCategoryPlural([[[demo_ships
                                       objectAtIndex:((demo_ship_index + [demo_ships count] - 1) % [demo_ships count])]
                                       objectAtIndex:0] oo_stringForKey:kOODemoShipClass])];
    field2 = OOShipLibraryCategoryPlural(
        [[[demo_ships objectAtIndex:demo_ship_index] objectAtIndex:0] oo_stringForKey:kOODemoShipClass]);
    field3 = [NSString stringWithFormat:@ "%@ -->",
                                        OOShipLibraryCategoryPlural(
                                            [[[demo_ships objectAtIndex:((demo_ship_index + 1) % [demo_ships count])]
                                                objectAtIndex:0] oo_stringForKey:kOODemoShipClass])];
...
- (StationEntity *)station {
    if (cachedSun != nil && cachedStation == nil) {
        cachedStation = [self findOneEntityMatchingPredicate:IsCandidateMainStationPredicate parameter:nil];
    }
...
- (void)setNextBeacon:(Entity<OOBeaconEntity> *)beaconShip {
    if ([beaconShip isBeacon]) {
        ...
        OOLog(@ "universe.beacon.error",
              @ "***** ERROR: Universe setNextBeacon '%@'. The ship has no beacon code set.",
              beaconShip);
    }
}
    ...
    */
    ...
    if ((profSlowPath % 10) == 0)  // Only print every tenth slow path, to reduce spamminess.
    {
        OOLog(
            ...
            @ "Hit slow path in ship selection for role \"%@\", having selected ship \"%@\". Now %lu of %lu on slow path (%f%%).",
            role,
            shipKey,
            profSlowPath,
            profTotal,
            ((double)profSlowPath) / ((double)profTotal) * 100.0f);
    }
#endif
        ...
        if (ship != nil) {
            ...
            if ([shipInfo oo_fuzzyBooleanForKey:@ "auto_ai" defaultValue:YES]) {
                ...
                if (autoAI != nil) {
                    ...
                    if ([role isEqualToString:@ "pirate"])
                        [ship setBounty:20 + randf() * 50 withReason:kOOLegalStatusReasonSetup];
                ...
                if ([role isEqualToString:@ "thargoid"])
                    [ship setScanClass:CLASS_THARGOID];  // thargoids are not on the autoAIMap
            }
        }
    }
    ...
    @try {
        ...
        if ([[exception name] isEqual:OOLITE_EXCEPTION_DATA_NOT_FOUND]) {
            OOLog(kOOLogException,
                  @ "***** Oolite Exception : '%@' in [Universe newVisualEffectWithName: %@ ] *****",
                  [exception reason],
                  effectKey);
        } else
            @throw exception;
...
- (ShipEntity *)newShipWithName:(NSString *)shipKey
    ...
    @try {
        ...
        if ([[exception name] isEqual:OOLITE_EXCEPTION_DATA_NOT_FOUND]) {
            OOLog(kOOLogException,
                  @ "***** Oolite Exception : '%@' in [Universe newShipWithName: %@ ] *****",
                  [exception reason],
                  shipKey);
        } else
            @throw exception;
...
- (DockEntity *)newDockWithName:(NSString *)shipDataKey andScaleFactor:(float)scale {
    ...
    @try {
        ...
        if ([[exception name] isEqual:OOLITE_EXCEPTION_DATA_NOT_FOUND]) {
            OOLog(kOOLogException,
                  @ "***** Oolite Exception : '%@' in [Universe newDockWithName: %@ ] *****",
                  [exception reason],
                  shipDataKey);
        } else
            @throw exception;
    ...
    */
    ...
    for (i = 0; i < how_many; i++) {
        ...
        if (container != nil) {
            ...
            OOLog(@ "universe.createContainer.failed",
                  @ "***** ERROR: failed to find a container to fill with %@ (%llu).",
                  [goodsKeys oo_stringAtIndex:co_type],
                  co_type);
        }
    }
...
- (NSArray *)getContainersOfCommodity:(OOCommodityType)commodity_name:(OOCargoQuantity)how_much {
    ...
    while (how_much > 0) {
        if (container) {
            ...
            OOLog(@ "universe.createContainer.failed",
                  @ "***** ERROR: failed to find a container to fill with %@",
                  commodity_name);
        }
...
- (NSDictionary *)gameSettings {
    ...
    NSDictionary *gameWindow =
        [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:[gameView viewSize].width],
                                                   @ "width",
                                                   [NSNumber numberWithFloat:[gameView viewSize].height],
                                                   @ "height",
                                                   [NSNumber numberWithBool:[[self gameController] inFullScreenMode]],
                                                   @ "fullScreen",
                                                   nil];
...
- (void)useGUILightSource:(BOOL)GUILight {
    if (GUILight != demo_light_on) {
        ...
        // making the following line necessary.
            ...
            OOGL(glEnable(GL_LIGHT1));  // make sure we have a light, even with shaders (!)
...
- (BOOL)viewFrustumIntersectsSphereAt:(Vector)position withRadius:(GLfloat)radius {
    ...
    int p;
    for (p = 0; p < 6; p++) {
        if (frustum[p][0] * position.x + frustum[p][1] * position.y + frustum[p][2] * position.z + frustum[p][3] <=
            -radius) {
            return NO;
        }
    }
...
- (void)drawUniverse {
    ...
    BOOL hudSeparateRenderPass =
        [self useShaders] && (currentPostFX == OO_POSTFX_NONE ||
                              ((currentPostFX == OO_POSTFX_CLOAK || currentPostFX == OO_POSTFX_CRTBADSIGNAL) &&
                               [self colorblindMode] == OO_POSTFX_NONE));
    NSSize viewSize = [gameView viewSize];
    OOLog(@ "universe.profile.draw", @ "%@", @ "Begin draw");

    if (!no_update) {
        if ((int)targetFramebufferSize.width != (int)viewSize.width ||
            (int)targetFramebufferSize.height != (int)viewSize.height) {
        ...
        @try {
            ...
            if (!displayGUI && wasDisplayGUI) {
                ...
                if (cachedSun)
                    [UNIVERSE
                        setMainLightPosition:HPVectorToVector([cachedSun position])];  // the main light is the sun.
                else
                    [UNIVERSE setMainLightPosition:kZeroVector];
            }
                    ...
                    for (i = furthest; i >= nearest; i--) {
                              ...
                              demoShipMode))  // either demo ship mode or in flight
                        {
                            ...
                            if (fogging) {
                                ...
                                fog_blend = OOClamp_0_1_f((magnitude([drawthing cameraRelativePosition]) - half_scale) /
                                                          half_scale);
                                [drawthing setAtmosphereFogging:[OOColor colorWithRed:skyClearColor[0]
                                                                                green:skyClearColor[1]
                                                                                 blue:skyClearColor[2]
                                                                                alpha:fog_blend]];
                            }
                            ...
                            // atmospheric fog
                            if (fogging) {
                                [drawthing setAtmosphereFogging:[OOColor colorWithRed:0.0
                                                                                green:0.0
                                                                                 blue:0.0
                                                                                alpha:0.0]];
                        ...
                        if (!((d_status == STATUS_COCKPIT_DISPLAY) ^
                              demoShipMode))  // either in flight or in demo ship mode
                        {
                            OOGLPushModelView();
                            if (EXPECT(drawthing != player)) {
                                // translate the object
            ...
            // If the HUD has a non-nil deferred name string, it means that a HUD switch was requested while it was
            // being rendered. If so, execute the deferred HUD switch now - Nikos 20110628
...
- (void)prepareToRenderIntoDefaultFramebuffer {
    ...
    if ([self useShaders]) {
        if ([gameView msaa]) {
            ...
            OOGL(glBlitFramebuffer(0,
                                   0,
                                   (GLint)viewSize.width,
                                   (GLint)viewSize.height,
                                   0,
                                   0,
                                   (GLint)viewSize.width,
                                   (GLint)viewSize.height,
...
- (void)drawWatermarkString:(NSString *)watermarkString {
    ...
    OODrawString(watermarkString,
                 MAIN_GUI_PIXEL_WIDTH / 2 - watermarkStringSize.width + 80,
                 MAIN_GUI_PIXEL_HEIGHT / 2 - watermarkStringSize.height,
                 [gameView display_z],
                 NSMakeSize(10, 10));
}

- (id)entityForUniversalID:(OOUniversalID)u_id {
    if (u_id == 100) return PLAYER;  // the player

    if (MAX_ENTITY_UID < u_id) {
        OOLog(
            @ "universe.badUID",
            @ "Attempt to retrieve entity for out-of-range UID %u. (This is an internal programming error, please report it.)",
            u_id);
        return nil;
    }

    if ((u_id == NO_TARGET) || (!entity_for_uid[u_id])) return nil;
...
static BOOL MaintainLinkedLists(Universe *uni) {
    ...
    if (uni->n_entities > 0) {
        ...
        if ((checkEnt) || (n > 0)) {
            OOExtraLog(
                kOOLogEntityVerificationError, @ "Broken *** broken y_next %@ list (%d) ***", uni->y_list_start, n);
            result = NO;
        }
        ...
        if ((checkEnt) || (n > 0)) {
            ...
            if (result) {
                ...
                NSCAssert(
                    checkEnt != nil,
                    @ "Expected z-list to be non-empty.");  // Previously an implicit assumption. -- Ahruman 2011-01-25
...
- (BOOL)addEntity:(Entity *)entity {
    if (entity) {
        ...
        if ([entities containsObject:entity]) return YES;

        if (n_entities >= UNIVERSE_MAX_ENTITIES - 1) {
            // throw an exception here...
            OOLog(@ "universe.addEntity.failed",
                  @ "***** Universe cannot addEntity:%@ -- Universe is full (%d entities out of %d)",
                  entity,
                  n_entities,
        ...
        if (![entity isEffect]) {
            ...
            while (entity_for_uid[next_universal_id] != nil)  // skip allocated numbers
            {
                ...
                if (limiter-- == 0) {
                    // Every slot has been tried! This should not happen due to previous test, but there was a problem
                    // here in 1.70.
                    OOLog(@ "universe.addEntity.failed",
                          @ "***** Universe cannot addEntity:%@ -- Could not find free slot for entity.",
                          entity);
                    return NO;
                }
            }
                ...
                if ([se isStation]) {
                    ...
                    if ([(StationEntity *)se isRotatingStation]) {
                        ...
                        if (definedRoll != nil) {
                            ...
                            stationRoll = [[self currentSystemData] oo_doubleForKey:@ "station_roll"
                                                                       defaultValue:STANDARD_STATION_ROLL];
                        }
                    ...
                    if ([se maxFlightSpeed] > 0)
                        se->isExplicitlyNotMainStation = YES;  // we never want carriers to become main stations.
                }
...
- (void)ensureEntityReallyRemoved:(Entity *)entity {
    if ([entity universalID] != NO_TARGET) {
        OOLog(
            ...
            @ "Entity %@ dealloced without being removed from universe! (This is an internal programming error, please report it.)",
            entity);
...
- (void)removeAllEntitiesExceptPlayer {
    ...
    if (!(p0->isPlayer)) {
        OOLog(kOOLogInconsistentState,
              @ "%@",
              @ "***** First entity is not the player in Universe.removeAllEntitiesExceptPlayer - exiting.");
        exit(EXIT_FAILURE);
    }
#endif
...
- (BOOL)isVectorClearFromEntity:(Entity *)e1 toDistance:(double)dist fromPoint:(HPVector)p2 {
    ...
    v1.x -= p1.x;
    v1.y -= p1.y;
    v1.z -= p1.z;  // vector from entity to p2

    double nearest = sqrt(v1.x * v1.x + v1.y * v1.y + v1.z * v1.z) - dist;  // length of vector

    if (nearest < 0.0) return YES;  // within range already!

    int i;
    int ent_count = n_entities;
    Entity *my_entities[ent_count];
    for (i = 0; i < ent_count; i++) my_entities[i] = [sortedEntities[i] retain];  //	retained
    ...
    for (i = 0; i < ent_count; i++) {
        ...
        if ((e2 != e1) && ([e2 canCollide])) {
            ...
            epos.x -= p1.x;
            epos.y -= p1.y;
            epos.z -= p1.z;  // epos now holds vector from p1 to this entities position
            ...
            if ((d_forward > 0) && (d_forward < nearest)) {
                ...
                if (dist2 < cr * cr) {
                    for (i = 0; i < ent_count; i++) [my_entities[i] release];  //	released
                    return NO;
                }
            }
        }
...
- (Entity *)hazardOnRouteFromEntity:(Entity *)e1 toDistance:(double)dist fromPoint:(HPVector)p2 {
    ...
    if (nearest < 0.0) return nil;  // within range already!
...
- (HPVector)getSafeVectorFromEntity:(Entity *)e1 toDistance:(double)dist fromPoint:(HPVector)p2 {
    ...
    if (!e1) {
        OOLog(kOOLogParameterError,
              @ "%@",
              @ "***** No entity set in Universe getSafeVectorFromEntity:toDistance:fromPoint:");
        return kZeroHPVector;
    }
        ...
        if ((e2 != e1) && ([e2 canCollide])) {
            ...
            if ((d_forward > 0) && (d_forward < nearest)) {
                ...
                if (dist2 < cr * cr) {
                    ...
                    nearest_point.x += d_forward * f1.x;
                    nearest_point.y += d_forward * f1.y;
                    nearest_point.z += d_forward * f1.z;
                    ...
                    if (outward.x || outward.y || outward.z)
                        ...
                        outward.y = 1.0;
                    // outward holds unit vector through the nearest point on the line from the center of incident
                    // object
                    ...
                    result.x += 0.25 * (outward.x * current_distance) +
                                0.75 * (backward.x * current_distance);  // push 'out' by this amount
...
- (void)addLaserHitEffectsAt:(HPVector)pos against:(ShipEntity *)target damage:(float)damage color:(OOColor *)color {
    ...
    if ([target showDamage] && [target energy] < [target maxEnergy] / 2) {
        ...
        if ([target energy] * randf() < damage) {
            ShipEntity *wreck = [self addWreckageFrom:target
                                             withRole:@ "oolite-wreckage-chunk"
                                                   at:pos
                                                scale:0.05
                                             lifetime:(125.0 + (randf() * 200.0))];
            if (wreck) {
...
- (ShipEntity *)firstShipHitByLaserFromShip:(ShipEntity *)srcEntity
    ...
    if (parent) {
        ...
        HPVector midfrontplane =
            make_HPvector(0.5 * (bbox.max.x + bbox.min.x), 0.5 * (bbox.max.y + bbox.min.y), bbox.max.z);
...
- (Entity *)firstEntityTargetedByPlayer {
    ...
    HPVector p1 = HPvector_add([player position],
                               vectorToHPVector(OOVectorMultiplyMatrix(offset, OOMatrixFromBasisVectors(r1, u1, f1))));
...
- (Entity *)firstEntityTargetedByPlayerPrecisely {
    ...
    return [self firstShipHitByLaserFromShip:PLAYER
                                 inDirection:targetFacing
                                      offset:laserPortOffset
                           gettingRangeFound:NULL];
}

- (NSArray *)entitiesWithinRange:(double)range ofEntity:(Entity *)entity {
    if (entity == nil) return nil;

    return [self findShipsMatchingPredicate:YESPredicate parameter:NULL inRange:range ofEntity:entity];
}

- (unsigned)countShipsWithRole:(NSString *)role inRange:(double)range ofEntity:(Entity *)entity {
    return [self countShipsMatchingPredicate:HasRolePredicate parameter:role inRange:range ofEntity:entity];
}
...
- (unsigned)countShipsWithPrimaryRole:(NSString *)role inRange:(double)range ofEntity:(Entity *)entity {
    return [self countShipsMatchingPredicate:HasPrimaryRolePredicate parameter:role inRange:range ofEntity:entity];
}
...
- (unsigned)countEntitiesMatchingPredicate:(EntityFilterPredicate)predicate
    ...
    for (i = 0; i < n_entities; i++) {
        ...
        if (e2 != e1 && predicate(e2, parameter)) {
            if (range < 0)
                distance = -1;  // Negative range means infinity
            else {
...
- (unsigned)countShipsMatchingPredicate:(EntityFilterPredicate)predicate
    ...
    if (predicate != NULL) {
        BinaryOperationPredicateParameter param = {IsShipPredicate, NULL, predicate, parameter};

        return [self countEntitiesMatchingPredicate:ANDPredicate parameter:&param inRange:range ofEntity:entity];
    } else {
        return [self countEntitiesMatchingPredicate:IsShipPredicate parameter:NULL inRange:range ofEntity:entity];
    }
}
...
- (NSMutableArray *)findEntitiesMatchingPredicate:(EntityFilterPredicate)predicate
    ...
    for (i = 0; i < n_entities; i++) {
        ...
        if (e1 != e2 && EntityInRange(p1, e2, range) && predicate(e2, parameter)) {
            [result addObject:e2];
        }
    }
...
- (id)findOneEntityMatchingPredicate:(EntityFilterPredicate)predicate parameter:(void *)parameter {
    unsigned i;
...
- (NSMutableArray *)findShipsMatchingPredicate:(EntityFilterPredicate)predicate
    ...
    if (predicate != NULL) {
        ...
        return [self findEntitiesMatchingPredicate:ANDPredicate parameter:&param inRange:range ofEntity:entity];
    } else {
        return [self findEntitiesMatchingPredicate:IsShipPredicate parameter:NULL inRange:range ofEntity:entity];
    }
}
...
- (NSMutableArray *)findVisualEffectsMatchingPredicate:(EntityFilterPredicate)predicate
                                             parameter:(void *)parameter
                                              ...
                                              ofEntity:(Entity *)entity {
    if (predicate != NULL) {
        BinaryOperationPredicateParameter param = {IsVisualEffectPredicate, NULL, predicate, parameter};
...
- (id)nearestEntityMatchingPredicate:(EntityFilterPredicate)predicate
    ...
    for (i = 0; i < n_entities; i++) {
        ...
        if (entity != e2 && distanceToReferenceEntitySquared < rangeSq && predicate(e2, parameter)) {
            result = e2;
...
- (id)nearestShipMatchingPredicate:(EntityFilterPredicate)predicate
    ...
    if (predicate != NULL) {
        ...
        return [self nearestEntityMatchingPredicate:ANDPredicate parameter:&param relativeToEntity:entity];
    } else {
        return [self nearestEntityMatchingPredicate:IsShipPredicate parameter:NULL relativeToEntity:entity];
    }
}
...
- (void)setViewDirection:(OOViewID)vd {
    ...
    if ((viewDirection == vd) && (vd != VIEW_CUSTOM) && (!displayGUI)) return;
    ...
    if (viewDirection != vd || viewDirection == VIEW_CUSTOM) {
        ...
        // if (EXPECT(viewDirection == VIEW_GUI_DISPLAY || vd == VIEW_GUI_DISPLAY )) [PLAYER
        // noteViewDidChangeFrom:viewDirection toView:vd];
        viewDirection = vd;
        if (ms && !gamePaused) {
...
- (void)enterGUIViewModeWithMouseInteraction:(BOOL)mouseInteraction {
    ...
    if (viewDirection != vd) {
        ...
        ShipScriptEvent(context,
                        ...
                        OOJSValueFromViewID(context, viewDirection),
                        OOJSValueFromViewID(context, vd));
        OOJSRelinquishContext(context);
    }
...
- (NSString *)soundNameForCustomSoundKey:(NSString *)key {
        ...
        if (object == nil || ([result hasPrefix:@ "["] && [result hasSuffix:@ "]"])) {
            for (;;) {
                ...
                if ([seen containsObject:result]) {
                    OOLogERR(@ "sound.customSounds.recursion",
                             @ "recursion in customsounds.plist for '%@' (at '%@'), no sound will be played.",
                             key,
                             result);
                    result = nil;
                    break;
    ...
    if ([result isEqualToString:@ "__oolite-no-sound"]) {
        OOLog(@ "sound.customSounds",
              @ "Could not resolve sound name in customsounds.plist for '%@', no sound will be played.",
              key);
        result = nil;
    }
    return result;
...
- (NSDictionary *)screenTextureDescriptorForKey:(NSString *)key {
    ...
    if ([value isKindOfClass:[NSString class]])
        value = [NSDictionary dictionaryWithObject:value forKey:@ "name"];
        ...
        if (speechArray != nil) {
            ...
            for (speechEnumerator = [speechArray objectEnumerator]; (thePair = [speechEnumerator nextObject]);) {
                ...
                if (![replacement_phrase isEqualToString:@ "_"]) {
                    spokenText = [spokenText stringByReplacingOccurrencesOfString:original_phrase
                                                                       withString:replacement_phrase];
                }
            }
...
- (void)addCommsMessage:(NSString *)text
    ...
    if (![currentMessage isEqualToString:expandedMessage] || universal_time >= messageRepeatTime) {
        ...
        if (!logOnly) {
            ...
            [self showGUIMessage:expandedMessage
                      withScroll:YES
                        andColor:[message_gui textCommsColor]
                    overDuration:count];
        ...
        [comm_log_gui printLongText:expandedMessage
...
- (void)showGUIMessage:(NSString *)text
    ...
    if (scroll) {
        [message_gui printLongText:text
                             align:GUI_ALIGN_CENTER
                             color:selectedColor
                          fadeTime:how_long
                               key:nil
        ...
        [message_gui printLineNoScroll:text
                                 align:GUI_ALIGN_CENTER
                                 color:selectedColor
                              fadeTime:how_long
                                   key:nil
...
- (void)repopulateSystem {
    ...
    JSContext *context = OOJSAcquireContext();
    [PLAYER doWorldScriptEvent:OOJSIDFromString(system_repopulator)
                     inContext:context
                 withArguments:NULL
                         count:0
                     timeLimit:kOOJSLongTimeLimit];
        ...
        @try {
            ...
            if (EXPECT_NOT([player showDemoShips] && [player guiScreen] == GUI_SCREEN_SHIPLIBRARY)) {
                ...
                if (universal_time >= demo_stage_time) {
                    if (ent_count > 1) {
                        ...
                        switch (demo_stage) {
                            ...
                            case DEMO_FLY_OUT:
                                ...
                                demo_ship_subindex =
                                    (demo_ship_subindex + 1) % [[demo_ships objectAtIndex:demo_ship_index] count];
                                demo_ship = [self newShipWithName:[[self demoShipData] oo_stringForKey:kOODemoShipKey]
                                                   usePlayerProxy:NO];
                                ...
                                if (demo_ship != nil) {
                                    ...
                                    [demo_ship
                                        setStatus:STATUS_COCKPIT_DISPLAY];  // prevents it getting escorts on addition
                                    ...
                                    if ([self addEntity:demo_ship]) {
                                        ...
                                        [demo_ship
                                            setDestination:make_HPvector(
                                                               0.0f, 0.0f, demo_start_z * 0.01f)];  // ideal position
                    ...
                    [demo_ship setPositionX:0.0f
                                          y:[demo_ship destination].y * delta
                                          z:demo_start_z + ([demo_ship destination].z - demo_start_z) * delta];
                }
            }
            ...
            for (i = 0; i < ent_count; i++) {
                ...
                update_stage = @ "update:entity [%@]";
#endif
                // Game Over code depends on regular delta_t updates to the dead player entity. Ignore the player
                // entity, even when dead.
                ...
                while (index > 0 && z_distance < sortedEntities[index - 1]->zero_distance) {
                    sortedEntities[index] =
                        sortedEntities[index - 1];  // bubble up the list, usually by just one position
            ...
            if (zombies != nil) {
                ...
                for (zombieEnum = [zombies objectEnumerator]; (zombie = [zombieEnum nextObject]);) {
                    OOLogERR(
                        @ "universe.zombie",
                        @ "Found dead entity %@ in active entity list, removing. This is an internal error, please report it.",
                        zombie);
...
#ifndef NDEBUG
                if (update_stage_param != nil)
                    update_stage = [NSString stringWithFormat:update_stage, update_stage_param];
#endif
                OOLog(kOOLogException,
                      @ "***** Exception during [%@] in [Universe update:] : %@ : %@ *****",
                      update_stage,
                      [exception name],
                      [exception reason]);
                @throw exception;
...
- (void)setTimeAccelerationFactor:(double)newTimeAccelerationFactor {
    if (newTimeAccelerationFactor < TIME_ACCELERATION_FACTOR_MIN ||
        newTimeAccelerationFactor > TIME_ACCELERATION_FACTOR_MAX) {
...
- (void)filterSortedLists {
    ...
    */
    ...
    while (e0) {
        ...
        next = e0->z_next;
        while ((next) && (next->collisionTestFilter ==
                          3))  // next has been eliminated from the list of possible colliders - so skip it
            next = next->z_next;
        if (next) {
            ...
            if (next_start < finish) {
                ...
                while ((next) && (next_start < finish)) {
                    ...
                    if (next_finish > finish) finish = next_finish;
                    e0 = next;
                    next = e0->z_next;
                    while ((next) && (next->collisionTestFilter == 3))  // next has been eliminated - so skip it
                        next = next->z_next;
                    if (next) next_start = next->position.z - 2.0f * next->collision_radius;
                }
    ...
    while (e0) {
        ...
        finish = start - 4.0f * e0->collision_radius;
        prev = e0->z_previous;
        while ((prev) && (prev->collisionTestFilter ==
                          3))  // next has been eliminated from the list of possible colliders - so skip it
        ...
        if (prev) {
            ...
            if (prev_start > finish) {
                ...
                while ((prev) && (prev_start > finish)) {
                    ...
                    if (prev_finish < finish) finish = prev_finish;
                    e0 = prev;
                    prev = e0->z_previous;
                    while ((prev) && (prev->collisionTestFilter == 3))  // next has been eliminated - so skip it
                        prev = prev->z_previous;
                    if (prev) prev_start = prev->position.z + 2.0f * prev->collision_radius;
                }
        ...
        if (next) {
            ...
            if (next_start < finish) {
                ...
                while ((next) && (next_start < finish)) {
                    ...
                    while ((next) && (next->collisionTestFilter == 3))  // next has been eliminated - so skip it
                        next = next->y_next;
                    if (next) next_start = next->position.y - 2.0f * next->collision_radius;
                }
                // now either (next == nil) or (next_start >= finish)-which would imply a gap!
            } else {
                // e0 is a singleton
        ...
        if (prev) {
            ...
            if (prev_start > finish) {
                ...
                while ((prev) && (prev_start > finish)) {
                    ...
                    while ((prev) && (prev->collisionTestFilter == 3))  // next has been eliminated - so skip it
                        prev = prev->y_previous;
                    if (prev) prev_start = prev->position.y + 2.0f * prev->collision_radius;
                }
                // now either (prev == nil) or (prev_start <= finish)-which would imply a gap!
            } else {
                // e0 is a singleton
        ...
        if (next) {
            ...
            if (next_start < finish) {
                ...
                while ((next) && (next_start < finish)) {
                    ...
                    while ((next) && (next->collisionTestFilter == 3))  // next has been eliminated - so skip it
                        next = next->x_next;
                    if (next) next_start = next->position.x - 2.0f * next->collision_radius;
                }
                // now either (next == nil) or (next_start >= finish)-which would imply a gap!
            } else {
                // e0 is a singleton
        ...
        if (prev) {
            ...
            if (prev_start > finish) {
                ...
                while ((prev) && (prev_start > finish)) {
                    ...
                    while ((prev) && (prev->collisionTestFilter == 3))  // next has been eliminated - so skip it
                        prev = prev->x_previous;
                    if (prev) prev_start = prev->position.x + 2.0f * prev->collision_radius;
                }
                // now either (prev == nil) or (prev_start <= finish)-which would imply a gap!
            } else {
                // e0 is a singleton
                        ...
                        if (prev->collision_chain == nil) {
                            ...
                            if (e0->collision_chain == nil) {
                                ...
                                OOLog(
                                    ...
                                    @ "Unexpected state in collision chain builder prev=%@, prev->c=%@, e0=%@, e0->c=%@",
                                    prev,
...
- (NSDictionary *)descriptions {
    if (_descriptions == nil) {
        ...
        _descriptions = [NSDictionary
            dictionaryWithContentsOfFile:[[[ResourceManager builtInPath] stringByAppendingPathComponent:@ "Config"]
...
static void VerifyDescString(NSString *key, NSString *desc) {
    if ([desc rangeOfString:@ "%n"].location != NSNotFound) {
        OOLog(@ "descriptions.verify.percentN",
              @ "***** FATAL: descriptions.plist entry \"%@\" contains the dangerous control sequence %%n.",
              key);
        exit(EXIT_FAILURE);
    }
}
...
static void VerifyDesc(NSString *key, id desc) {
    if ([desc isKindOfClass:[NSString class]]) {
        ...
        OOLogERR(@ "descriptions.verify.badType",
                 @ "***** FATAL: descriptions.plist entry for \"%@\" is neither a string nor an array.",
                 key);
        exit(EXIT_FAILURE);
    }
}
...
- (void)verifyDescriptions {
    ...
    */
    ...
    foreachkey(key, _descriptions) { VerifyDesc(key, [_descriptions objectForKey:key]); }
}

- (void)loadDescriptions {
    [_descriptions autorelease];
    _descriptions = [[ResourceManager dictionaryFromFilesNamed:@ "descriptions.plist" inFolder:@ "Config"
                                                      andMerge:YES] retain];
    [self verifyDescriptions];
}
...
- (NSDictionary *)currentSystemData {
    ...
    if (![self inInterstellarSpace]) {
        ...
        if (interstellarDict == nil) {
            ...
            interstellarDict = [[NSDictionary alloc] initWithObjectsAndKeys:interstellarName,
                                                                            KEY_NAME,
                                                                            minusOne,
                                                                            KEY_GOVERNMENT,
                                                                            minusOne,
...
- (void)setSystemDataKey:(NSString *)key value:(NSObject *)object fromManifest:(NSString *)manifest {
    [self setSystemDataForGalaxy:galaxyID
                          planet:systemID
                             key:key
                           value:object
                    fromManifest:manifest
                        forLayer:OO_LAYER_OXP_DYNAMIC];
}
...
- (void)setSystemDataForGalaxy:(OOGalaxyID)gnum
    ...
    if ([key isEqualToString:KEY_RADIUS] && sameGalaxy &&
        sameSystem)  // buggy if we allow this key to be set while in the system
    {
        ...
        if ([key isEqualToString:KEY_TECHLEVEL]) {
            if ([self station]) {
                ...
                [[self station] setLocalShipyard:[self shipsForSaleForSystem:systemID
                                                                      withTL:[object intValue]
                                                                      atTime:[PLAYER clockTime]]];
            }
                ...
                if ([key isEqualToString:@ "sun_color"]) {
                    ...
                    for (i = n_entities - 1; i > 0; i--)
                        if ((sortedEntities[i]) && ([sortedEntities[i] isKindOfClass:[DustEntity class]]))
                            [(DustEntity *)sortedEntities[i]
                                setDustColor:[color blendedColorWithFraction:0.5 ofColor:[OOColor whiteColor]]];
                }
            }
    ...
    sysdataLocked = YES;
    [PLAYER doScriptEvent:OOJSID("systemInformationChanged")
            withArguments:[NSArray arrayWithObjects:[NSNumber numberWithInt:gnum],
                                                    [NSNumber numberWithInt:pnum],
                                                    key,
                                                    object,
                                                    nil]];
    sysdataLocked = NO;
...
- (NSMutableArray *)nearbyDestinationsWithinRange:(double)range {
    ...
    for (unsigned short i = 0; i < 256; i++) {
        ...
        if (dist <= range &&
            (i != systemID || [self inInterstellarSpace]))  // if we are in interstellar space, it's OK to include the
                                                            // system we (mis)jumped from
        {
            [result addObject:[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithDouble:dist],
                                                                         @ "distance",
                                                                         [NSNumber numberWithInt:i],
                                                                         @ "sysID",
                                                                         [[self generateSystemData:i]
                                                                             oo_stringForKey:@ "sun_gone_nova"
                                                                                defaultValue:@ "0"],
                                                                         @ "nova",
                                                                         nil]];
        }
...
- (OOSystemID)findNeighbouringSystemToCoords:(NSPoint)coords withGalaxy:(OOGalaxyID)g {
    ...
    BOOL connected[256];
    for (i = 0; i < 256; i++) connected[i] = NO;
...
- (NSDictionary *)routeFromSystem:(OOSystemID)start toSystem:(OOSystemID)goal optimizedBy:(OORouteType)optimizeBy {
    ...
    [curr addObject:cheapest[start] = [RouteElement elementWithLocation:start
                                                                 parent:-1
                                                                   cost:0
                                                               distance:0
                                                                   time:0
        ...
        for (i = 0; i < [curr count]; i++) {
            ...
            for (j = 0; j < [ns count]; j++) {
                ...
                if (cost < maxCost && (cheapest[n] == nil || [cheapest[n] cost] > cost)) {
                    RouteElement *e = [RouteElement elementWithLocation:n
                                                                 parent:c
                                                                   cost:cost
                                                               distance:distance
                                                                   time:time
                    ...
                    if (n == goal && cost < maxCost) maxCost = cost;
                }
            }
        }
        [curr setArray:next];
    ...
    [c_route release];
    c_route = [[NSDictionary alloc] initWithObjectsAndKeys:route,
                                                           @ "route",
                                                           [NSNumber numberWithDouble:[cheapest[goal] distance]],
                                                           @ "distance",
                                                           nil];

    return c_route;
#else
    return [NSDictionary dictionaryWithObjectsAndKeys:route,
                                                      @ "route",
                                                      [NSNumber numberWithDouble:[cheapest[goal] distance]],
                                                      @ "distance",
                                                      [NSNumber numberWithDouble:[cheapest[goal] time]],
                                                      @ "time",
                                                      [NSNumber numberWithInt:[cheapest[goal] jumps]],
                                                      @ "jumps",
                                                      nil];
#endif
}
...
- (NSString *)shortTimeDescription:(double)interval {
    ...
    if (interval <= 0.0) return DESC(@ "contracts-no-time");
...
- (NSArray *)shipsForSaleForSystem:(OOSystemID)s withTL:(OOTechLevelID)specialTL atTime:(OOTimeAbsolute)current_time {
    ...
    for (i = 0; i < 256; i++) {
        ...
        if (ship_sold_time < 0) ship_sold_time += 0x1000000;  // wraparound
        ...
        double chance =
            1.0 - pow(1.0 - [ship_info oo_doubleForKey:KEY_CHANCE], MAX((OOTechLevelID)1, techlevel - ship_techlevel));
        ...
        if ((days_until_sale > 0.0) && (days_until_sale < 30.0) && (ship_techlevel <= techlevel) &&
            (randf() < chance) && (shipBaseDict != nil)) {
            ...
            // NSString* brochure_desc = [self brochureDescriptionWithDictionary: ship_dict standardEquipment: extras
            // optionalEquipment: options]; NSLog(@"%@ Brochure description : \"%@\"", [ship_dict
            // objectForKey:KEY_NAME], brochure_desc);

            [shortShipDescription appendFormat:@ "%@:", shipName];

            OOWeaponFacingSet availableFacings = [ship_info oo_unsignedIntForKey:KEY_WEAPON_FACINGS
                                                                    defaultValue:VALID_WEAPON_FACINGS] &
            ...
            while ((randf() < chance) && ([options count])) {
                chance *=
                    chance;  // decrease the chance of a further customisation (unless it is 1, which might be a bug)
                ...
                if (item != nil) {
                    ...
                    OOCreditsQuantity eqPrice =
                        [item price] / 10;  // all amounts are x/10 due to being represented in tenths of credits.
                    ...
                    if (condition_script != nil) {
                        ...
                        if (condScript != nil)  // should always be non-nil, but just in case
                        {
                            ...
                            jsval args[] = {OOJSValueFromNativeObject(JScontext, equipmentKey),
                                            OOJSValueFromNativeObject(JScontext, testship),
                                            OOJSValueFromNativeObject(JScontext, @ "newShip")};
                    ...
                    if ([equipmentKey hasPrefix:@ "EQ_WEAPON"]) {
                        ...
                        if (availableFacings & WEAPON_FACING_FORWARD &&
                            [new_weapon weaponThreatAssessment] > [fwdWeapon weaponThreatAssessment]) {
                            ...
                            if (availableFacings & WEAPON_FACING_AFT &&
                                (isWeaponNone(aftWeapon) ||
                                 [new_weapon weaponThreatAssessment] > [aftWeapon weaponThreatAssessment])) {
            ...
            if (passengerBerthCount) {
                NSString *npb =
                    (passengerBerthCount > 1) ? [NSString stringWithFormat:@ "%d ", passengerBerthCount] : (id) @ "";
                NSString *ppb = DESC_PLURAL(@ "passenger-berth", passengerBerthCount);
                NSString *extraPassengerBerthsDescription =
            ...
            NSDictionary *ship_info_dictionary =
                [NSDictionary dictionaryWithObjectsAndKeys:shipID,
                                                           SHIPYARD_KEY_ID,
                                                           ship_key,
                                                           ...
                                                           [NSNumber numberWithUnsignedShort:personality],
...
- (OOCreditsQuantity)tradeInValueForCommanderDictionary:(NSDictionary *)dict {
    ...
    if (shipyard_info == nil) {
        OOLogERR(
            ...
            @ "Shipyard dictionary entry for ship %@ required for trade in value calculation, but does not exist. Setting ship value to 0.",
            ship_desc);
    } else {
    ...
    OOWeaponType ship_fwd_weapon =
        [OOEquipmentType equipmentTypeWithIdentifier:[dict oo_stringForKey:@ "forward_weapon"]];
    ...
    OOWeaponType ship_starboard_weapon =
        [OOEquipmentType equipmentTypeWithIdentifier:[dict oo_stringForKey:@ "starboard_weapon"]];
    ...
    NSMutableArray *ship_extra_equipment =
        [NSMutableArray arrayWithArray:[[dict oo_dictionaryForKey:@ "extra_equipment"] allKeys]];
    ...
    NSMutableArray *base_extra_equipment =
        [NSMutableArray arrayWithArray:[basic_info oo_arrayForKey:KEY_EQUIPMENT_EXTRAS]];
    ...
    if (base_weapon_key != nil) base_weapons_value += [UNIVERSE getEquipmentPriceForKey:base_weapon_key] / 10;
    ...
    long long extra_equipment_value =
        ship_max_passengers * [UNIVERSE getEquipmentPriceForKey:@ "EQ_PASSENGER_BERTH"] / 10;
    ...
    if (ship_aft_weapon) {
        ...
        if (base_weapon_key != nil)  // aft weapon was defined as a base weapon
        {
            ship_main_weapons_value +=
                [UNIVERSE getEquipmentPriceForKey:weapon_key] / 10;  // take weapon downgrades into account
        } else {
...
- (NSString *)brochureDescriptionWithDictionary:(NSDictionary *)dict
    ...
    if (max_cargo) {
        ...
        if (canExpand) [desc appendFormat:@ " (expandable to %dt at most starports)", max_cargo + extra_cargo];
        [desc appendString:@ "."];
    }
...
- (HPVector)getSunSkimStartPositionForShip:(ShipEntity *)ship {
    ...
    v1.x -= v0.x;
    v1.y -= v0.y;
    v1.z -= v0.z;  // vector from sun to ship
    if (v1.x || v1.y || v1.z)
...
- (HPVector)getSunSkimEndPositionForShip:(ShipEntity *)ship {
    ...
    v1.x += 15000 * v3.x;
    v1.y += 15000 * v3.y;
    v1.z += 15000 * v3.z;  // point 15000m at a tangent to sun from v1
    v1.x -= v0.x;
    v1.y -= v0.y;
    v1.z -= v0.z;
    if (v1.x || v1.y || v1.z)
...
- (void)handleOoliteException:(NSException *)exception {
    if (exception != nil) {
        if ([[exception name] isEqual:OOLITE_EXCEPTION_FATAL]) {
            ...
            NSString *exception_msg = [NSString
                stringWithFormat:@ "Exception : %@ : %@ Please take a screenshot and/or press esc or Q to quit.",
                                 [exception name],
                                 [exception reason]];
...
- (void)startSpeakingString:(NSString *)text {
    [speechSynthesizer
        startSpeakingString:[NSString stringWithFormat:@ "[[volm %.3f]]%@", 0.3333333f * [OOSound masterVolume], text]];
}
...
- (void)startSpeakingString:(NSString *)text {
    ...
    if (utf8 != nil)  // we have a valid UTF-8 string
    {
        ...
        espeak_Synth(stringToSay,
                     strlen(stringToSay) + 1 /* inc. NULL */,
                     0,
                     POS_CHARACTER,
                     0,
...
- (unsigned int)nextVoice:(unsigned int)index {
    if (++index >= espeak_voice_count) index = 0;
    return index;
}

- (unsigned int)prevVoice:(unsigned int)index {
    if (--index >= espeak_voice_count) index = espeak_voice_count - 1;
    return index;
}

- (unsigned int)setVoice:(unsigned int)index withGenderM:(BOOL)isMale {
    if (index == UINT_MAX) index = [self voiceNumber:DESC(@ "espeak-default-voice")];
...
- (void)setUpSettings {
    ...
    // message_gui and comm_log_gui defaults are set up inside [hud resetGuis:] ( via [player deferredInit], called from
    // the code that calls this method).
    [message_gui autorelease];
    message_gui = [[GuiDisplayGen alloc] initWithPixelSize:NSMakeSize(480, 160)
                                                   columns:1
                                                      rows:9
                                                 rowHeight:19
                                                  rowStart:20
    ...
    comm_log_gui = [[GuiDisplayGen alloc] initWithPixelSize:NSMakeSize(360, 120)
                                                    columns:1
                                                       rows:10
                                                  rowHeight:12
                                                   rowStart:12
    ...
    speechArray = [[ResourceManager arrayFromFilesNamed:@ "speech_pronunciation_guide.plist"
                                               inFolder:@ "Config"
                                               andMerge:YES] retain];
#endif
    ...
    characters = [[ResourceManager dictionaryFromFilesNamed:@ "characters.plist" inFolder:@ "Config"
                                                   andMerge:YES] retain];

    [customSounds autorelease];
    customSounds = [[ResourceManager dictionaryFromFilesNamed:@ "customsounds.plist" inFolder:@ "Config"
                                                     andMerge:YES] retain];

    [globalSettings autorelease];
    globalSettings = [[ResourceManager dictionaryFromFilesNamed:@ "global-settings.plist"
                                                       inFolder:@ "Config"
    ...
    screenBackgrounds = [[ResourceManager dictionaryFromFilesNamed:@ "screenbackgrounds.plist"
                                                          inFolder:@ "Config"
                                                          andMerge:YES] retain];
    ...
    explosionSettings = [[ResourceManager dictionaryFromFilesNamed:@ "explosions.plist" inFolder:@ "Config"
                                                          andMerge:YES] retain];
}
...
- (void)verifyEntitySessionIDs {
    ...
    for (i = 0; i < n_entities; i++) {
        ...
        if ([entity sessionID] != _sessionID) {
            OOLogERR(@ "universe.sessionIDs.verify.failed",
                     @ "Invalid entity %@ (came from session %llu, current session is %llu).",
...
- (void)setUpInitialUniverse {
    ...
    [player setUpShipFromDictionary:[[OOShipRegistry sharedRegistry]
                                        shipInfoForKey:[player shipDataKey]]];  // the standard cobra at this point
    [player baseMass];  // bootstrap the base mass used in all fuel charge calculations.
    OO_DEBUG_POP_PROGRESS();

    // Player init above finishes initialising all standard player ship properties. Now that the base mass is set, we
    // can run setUpSpace!
    [self setUpSpace];

    [self
        setDockingClearanceProtocolActive:[[self currentSystemData] oo_boolForKey:@ "stations_require_docking_clearance"
                                                                     defaultValue:YES]];
    ...
    if (index >= 0) {
        if (sortedEntities[index] != entity) {
            OOLog(
                ...
                @ "DEBUG: Universe removeEntity:%@ ENTITY IS NOT IN THE RIGHT PLACE IN THE ZERO_DISTANCE SORTED LIST -- FIXING...",
                entity);
            unsigned i;
            index = -1;
            for (i = 0; (i < n_entities) && (index == -1); i++)
                if (sortedEntities[i] == entity) index = i;
            if (index == -1)
                OOLog(
                    kOOLogInconsistentState,
                    @ "DEBUG: Universe removeEntity:%@ ENTITY IS NOT IN THE ZERO_DISTANCE SORTED LIST -- CONTINUING...",
                    entity);
        }
        if (index != -1) {
            while ((unsigned)index < n_entities) {
                ...
                */
                sortedEntities[index] =
                    sortedEntities[index + n];  // copy entity[index + n] -> entity[index] (preserves sort order)
            ...
            if (n > 1)
                OOLog(kOOLogInconsistentState,
                      @ "DEBUG: Universe removeEntity: REMOVED %d EXTRA COPIES OF %@ FROM THE ZERO_DISTANCE SORTED LIST",
                      n - 1,
                      entity);
            while (n--) {
                n_entities--;
...
- (void)populateSpaceFromActiveWormholes {
    ...
    while ([activeWormholes count]) {
        ...
        @try {
            ...
            if (![whole isScanned] && NSEqualPoints([PLAYER galaxy_coordinates], [whole destinationCoordinates])) {
            ...
            OOLog(kOOLogException,
                  @ "Squashing exception during wormhole unpickling (%@: %@).",
                  [exception name],
                  [exception reason]);
        }
        [pool release];
...
- (void)dumpSystemDescriptionGraphViz {
    ...
    keyMap = [ResourceManager dictionaryFromFilesNamed:@ "sysdesc_key_table.plist" inFolder:@ "Config" andMerge:NO];

    graphViz = [NSMutableString
        stringWithString:@ "// System description grammar:\n\n"
                          "digraph system_descriptions\n"
                          "{\n"
                          "\tgraph [charset=\"UTF-8\", label=\"System description grammar\", labelloc=t, labeljust=l "
                          "rankdir=LR compound=true nodesep=0.02 ranksep=1.5 concentrate=true fontname=Helvetica]\n"
                          "\tedge [arrowhead=dot]\n"
    ...
    [graphViz appendString:@ "\tpercent_I [label=\"%I\\nInhabitants\" shape=diamond]\n"
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (label == nil)
            label = [NSString stringWithFormat:@ "[%llu]", i];
        else
            label = [NSString stringWithFormat:@ "[%llu] (%@)", i, label];
    ...
    // Write file
    [graphViz appendString:@ "\t}\n"];
    [ResourceManager writeDiagnosticData:[graphViz dataUsingEncoding:NSUTF8StringEncoding]
                             toFileNamed:@ "SystemDescription.dot"];
}
...
- (void)addNumericRefsInString:(NSString *)string
    ...
    for (;;) {
        ...
        // Each node gets a colour for its incoming edges. The multiplication and mod shuffle them to avoid adjacent
        // nodes having similar colours.
        [graphViz appendFormat:@ "\t%@ -> n%u_0 [color=\"%f,0.75,0.8\" lhead=cluster_%u]\n",
                               fromNode,
                               i,
                               ((float)(i * 511 % nodeCount)) / ((float)nodeCount),
                               i];
    }
...
- (void)runLocalizationTools {
    ...
    for (argEnum = [arguments objectEnumerator]; (arg = [argEnum nextObject]);) {
        if ([arg isEqual:@ "--compile-sysdesc"])
            compileSysDesc = YES;
        else if ([arg isEqual:@ "--export-sysdesc"])
            exportSysDesc = YES;
        else if ([arg isEqual:@ "--xml"])
            xml = YES;
        else if ([arg isEqual:@ "--openstep"])
            xml = NO;
    }
...
- (void)loadConditionScripts {
    ...
    [self addConditionScripts:[[[OOCacheManager sharedCache] objectForKey:@ "equipment conditions"
                                                                  inCache:@ "condition scripts"] objectEnumerator]];

    [self addConditionScripts:[[[OOCacheManager sharedCache] objectForKey:@ "ship conditions"
                                                                  inCache:@ "condition scripts"] objectEnumerator]];

    [self addConditionScripts:[[[OOCacheManager sharedCache] objectForKey:@ "demoship conditions"
                                                                  inCache:@ "condition scripts"] objectEnumerator]];
}
...
NSComparisonResult equipmentSort(id a, id b, void *context) {
    ...
    OOCreditsQuantity comp1 =
        [[one oo_dictionaryAtIndex:EQUIPMENT_EXTRA_INFO_INDEX] oo_unsignedLongLongForKey:@ "sort_order"
                                                                            defaultValue:1000];
    OOCreditsQuantity comp2 =
        [[two oo_dictionaryAtIndex:EQUIPMENT_EXTRA_INFO_INDEX] oo_unsignedLongLongForKey:@ "sort_order"
                                                                            defaultValue:1000];
...
NSComparisonResult equipmentSortOutfitting(id a, id b, void *context) {
    ...
    OOCreditsQuantity comp1 = [[one oo_dictionaryAtIndex:EQUIPMENT_EXTRA_INFO_INDEX]
        oo_unsignedLongLongForKey:@ "purchase_sort_order"
                     defaultValue:[[one oo_dictionaryAtIndex:EQUIPMENT_EXTRA_INFO_INDEX]
                                      oo_unsignedLongLongForKey:@ "sort_order"
                                                   defaultValue:1000]];
    OOCreditsQuantity comp2 = [[two oo_dictionaryAtIndex:EQUIPMENT_EXTRA_INFO_INDEX]
        oo_unsignedLongLongForKey:@ "purchase_sort_order"
                     defaultValue:[[two oo_dictionaryAtIndex:EQUIPMENT_EXTRA_INFO_INDEX]
                                      oo_unsignedLongLongForKey:@ "sort_order"
                                                   defaultValue:1000]];
    if (comp1 < comp2) return NSOrderedAscending;
    if (comp1 > comp2) return NSOrderedDescending;
...
NSString *OOLookUpPluralDescriptionPRIV(NSString *key, NSInteger count) {
    ...
    if (tmp != nil) {
        ...
        if (![warned containsObject:tmp]) {
            OOLogWARN(@ "localization.plurals",
                      @ "'%@' found in descriptions.plist, should be '%@%%0'. Localization data needs updating.",
                      key,
                      key);
    ...
    for (index = i = 0; i < [conditions count]; ++index, ++i) {
        ...
        for (;;) {
            ...
            switch (command) {
                ...
                case '%':
                    if (param < 2) break;  // ouch - fail this!
                    input %= param;
                    continue;

                case '=':
                    if (flag ^ (input == param)) continue;
                    break;
                case '!':
                    if (flag ^ (input != param)) continue;
                    break;
