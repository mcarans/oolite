...
- (void)setFunctionForAxis:(int)axis function:(int)function stick:(int)stickNum;

- (void)setFunctionForButton:(int)button function:(int)function stick:(int)stickNum;
...
- (void)saveProfileForAxis:(int)axis {
    ...
    if ([profile isKindOfClass:[OOJoystickStandardAxisProfile class]]) {
        ...
        for (i = 0; i < [controlPoints count]; i++) {
            ...
            [points addObject:[NSArray arrayWithObjects:[NSNumber numberWithFloat:point.x],
    ...
    if ([type isEqualToString:@ "Standard"]) {
        ...
        for (i = 0; i < [points count]; i++) {
            ...
            if ([pointArray count] >= 2) {
                point =
                    NSMakePoint([[pointArray objectAtIndex:0] floatValue], [[pointArray objectAtIndex:1] floatValue]);
...
- (NSDictionary *)axisFunctions {
    ...
    for (i = 0; i < MAX_AXES; i++) {
        for (j = 0; j < MAX_STICKS; j++) {
            if (axismap[j][i] >= 0) {
                NSDictionary *fnDict = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithBool:YES],
                                                                                  STICK_ISAXIS,
                                                                                  ...
                                                                                  STICK_AXBUT,
                                                                                  nil];
                [fnList setValue:fnDict forKey:ENUMKEY(axismap[j][i])];
            }
        }
    }
    return fnList;
...
- (NSDictionary *)buttonFunctions {
    ...
    for (i = 0; i < MAX_BUTTONS; i++) {
        for (j = 0; j < MAX_STICKS; j++) {
            if (buttonmap[j][i] >= 0) {
                NSDictionary *fnDict = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithBool:NO],
                                                                                  STICK_ISAXIS,
                                                                                  [NSNumber numberWithInt:j],
                                                                                  STICK_NUMBER,
                                                                                  [NSNumber numberWithInt:i],
                                                                                  STICK_AXBUT,
                                                                                  nil];
                [fnList setValue:fnDict forKey:ENUMKEY(buttonmap[j][i])];
            }
        }
    }
    return fnList;
...
- (void)setFunction:(int)function withDict:(NSDictionary *)stickFn {
    ...
    if (isAxis) {
        [self setFunctionForAxis:stickAxBt function:function stick:stickNum];
    } else {
        [self setFunctionForButton:stickAxBt function:function stick:stickNum];
    }
}
...
- (void)decodeAxisEvent:(JoyAxisEvent *)evt {
    ...
    if (cbObject && (cbHardware & HW_AXIS)) {
        ...
        if (axisvalue > AXCBTHRESH) {
            NSDictionary *fnDict = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithBool:YES],
                                                                              STICK_ISAXIS,
                                                                              [NSNumber numberWithInt:evt->which],
                                                                              STICK_NUMBER,
                                                                              [NSNumber numberWithInt:evt->axis],
                                                                              STICK_AXBUT,
                                                                              nil];
            cbHardware = 0;
...
- (void)decodeButtonEvent:(JoyButtonEvent *)evt {
    ...
    if (cbObject && (cbHardware & HW_BUTTON)) {
        NSDictionary *fnDict = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithBool:NO],
                                                                          ...
                                                                          [NSNumber numberWithInt:evt->which],
                                                                          STICK_NUMBER,
                                                                          [NSNumber numberWithInt:evt->button],
                                                                          STICK_AXBUT,
                                                                          nil];
        cbHardware = 0;
        [cbObject performSelector:cbSelector withObject:fnDict];
    ...
    if (evt->type == JOYBUTTONDOWN) {
        bs = YES;
        if (function == BUTTON_PRECISION) precisionMode = !precisionMode;
    }
...
- (void)saveStickSettings {
    ...
    [defaults setObject:[self axisFunctions] forKey:AXIS_SETTINGS];
    [defaults setObject:[self buttonFunctions] forKey:BUTTON_SETTINGS];
...
- (void)loadStickSettings {
    ...
    if (axisSettings) {
        ...
        for (i = 0; i < [keys count]; i++) {
            ...
            [self setFunction:[key intValue] withDict:[axisSettings objectForKey:key]];
        }
    }
    if (buttonSettings) {
        ...
        for (i = 0; i < [keys count]; i++) {
            NSString *key = [keys objectAtIndex:i];
            [self setFunction:[key intValue] withDict:[buttonSettings objectForKey:key]];
        }
    } else {
