...
RANROTSeed MakeRanrotSeed(uint32_t seed) {
    RANROTSeed result = {.low = seed, .high = ~seed};
...
RANROTSeed RanrotSeedFromRandomSeed(Random_Seed seed) {
    ...
    RNG_Seed s = {.a = seed.c, .b = seed.d, .c = seed.e, .d = seed.f};
...
OORandomState OOSaveRandomState(void) {
    return (OORandomState){.ranrot = sRANROT, .rnd = rnd_seed};
}
