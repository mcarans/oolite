...
HPVector OORandomUnitHPVector(void) {
    ...
    do {
        ...
        m = HPmagnitude2(v);
             ...
             m == 0.0);  // We're confining to a sphere of radius 0.5 using the sqared magnitude; 0.5 squared is 0.25.
...
HPVector OORandomPositionInCylinder(
    ...
    if (HPdistance(centre1, centre2) < (exclusion1 + exclusion2) * 1.2) {
        OOLog(@ "position.cylinder.error",
              @ "Trying to generate cylinder position in range %f long with exclusions %f and %f",
              HPdistance(centre1, centre2),
              exclusion1,
              exclusion2);
    }
    HPVector result;
...
HPVector OOProjectHPVectorToPlane(HPVector point, HPVector plane, HPVector normal) {
    return HPvector_subtract(point,
                             HPvector_multiply_scalar(normal, HPdot_product(HPvector_subtract(point, plane), normal)));
}
