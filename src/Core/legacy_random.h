...
double cunningFee(double value,
                  double precision);  // precision is the fraction below which numbers become insignificant.
...
#define ranrot_rand() \
    ((int)Ranrot())  // Some uses perform arithmetic that does weird things if result is unsigned -- DustEntity.m, for
                     // instance.
float randf(void);
float bellf(int n);
...
OOINLINE int equal_seeds(Random_Seed seed1, Random_Seed seed2) {
    return ((seed1.a == seed2.a) && (seed1.b == seed2.b) && (seed1.c == seed2.c) && (seed1.d == seed2.d) &&
