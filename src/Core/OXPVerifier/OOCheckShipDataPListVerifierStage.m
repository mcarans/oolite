...
- (BOOL)shouldRun {
    ...
    return [fileScanner fileExists:@ "shipdata.plist" inFolder:@ "Config" referencedFrom:nil checkBuiltIn:NO];
}
...
- (void)run {
    ...
    fileScanner = [[self verifier] fileScannerStage];
    _shipdataPList = [fileScanner plistNamed:@ "shipdata.plist" inFolder:@ "Config" referencedFrom:nil checkBuiltIn:NO];

    if (_shipdataPList == nil) return;

    // Get AI verifier stage (may be nil).
    _aiVerifierStage = [[self verifier]
        stageWithName:[OOAIStateMachineVerifierStage nameForReverseDependencyForVerifier:[self verifier]]];

    ooliteShipData = [ResourceManager dictionaryFromFilesNamed:@ "shipdata.plist" inFolder:@ "Config" andMerge:YES];
    ...
    _schemaVerifier =
        [OOPListSchemaVerifier verifierWithSchema:[ResourceManager dictionaryFromFilesNamed:@ "shipdataEntrySchema.plist"
                                                                                   inFolder:@ "Schemata"
                                                                                   andMerge:NO]];
    ...
    for (shipEnum = [shipList objectEnumerator]; (shipKey = [shipEnum nextObject]);) {
        ...
        if (shipInfo == nil) {
            OOLog(@ "verifyOXP.shipdata.badType",
                  @ "***** ERROR: shipdata.plist entry for \"%@\" is not a dictionary.",
                  shipKey);
        } else {
...
- (void)checkKeys {
    ...
    if (_isPlayer)
        referenceSet = _playerKeys;
    else if (_isStation)
        referenceSet = _stationKeys;
    else
        referenceSet = _basicKeys;
...
- (void)checkModel {
    id model = nil, materials = nil, shaders = nil;
    ...
    if (model != nil) {
        if (![[[self verifier] modelVerifierStage] modelNamed:model
                                                ...
                                                withMaterials:materials
                                                   andShaders:shaders]) {
            [self message:@ "----- WARNING: model \"%@\" could not be found in %@ or in Oolite.",
                          model,
...
- (BOOL)verifier:(OOPListSchemaVerifier *)verifier
    ...
    [self verboseMessage:@ "- Skipping verification for type %@ at %@.%@.",
                         typeKey,
                         _name,
                         [OOPListSchemaVerifier descriptionForKeyPath:keyPath]];
    return YES;
}
...
- (BOOL)verifier:(OOPListSchemaVerifier *)verifier
    ...
    [self message:@ "***** ERROR: verification of ship \"%@\" failed at \"%@\": %@",
                  name,
