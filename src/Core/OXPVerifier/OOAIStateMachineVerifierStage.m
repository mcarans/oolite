...
- (void)run {
    ...
    [whitelist
        addObjectsFromArray:[[[ResourceManager whitelistDictionary] oo_dictionaryForKey:@ "ai_method_aliases"] allKeys]];
...
- (void)stateMachineNamed:(NSString *)name usedByShip:(NSString *)shipName {
    ...
    if (![fileScanner fileExists:name
        ...
        OOLog(
            ...
            @ "----- WARNING: AI state machine \"%@\" referenced in shipdata.plist entry \"%@\" could not be found in %@ or in Oolite.",
            name,
            shipName,
...
- (void)validateAI:(NSString *)aiName {
    ...
    // Attempt to load AI.
    path = [[[self verifier] fileScannerStage] pathForFile:aiName
                                                  inFolder:@ "AIs"
                                            referencedFrom:@ "AI list"
                                              checkBuiltIn:NO];
    if (path == nil) return;
    ...
    if (aiStateMachine == nil) {
        OOLog(@ "verifyOXP.validateAI.failed.notDictPlist",
              @ "***** ERROR: could not interpret \"%@\" as a dictionary.",
              path);
        return;
    }
    ...
    for (stateEnum = [aiStateMachine keyEnumerator]; (stateKey = [stateEnum nextObject]);) {
        ...
        if (![stateHandlers isKindOfClass:[NSDictionary class]]) {
            OOLog(@ "verifyOXP.validateAI.failed.invalidFormat.state",
                  @ "***** ERROR: state \"%@\" in AI \"%@\" is not a dictionary.",
                  stateKey,
                  aiName);
            continue;
        }
        ...
        for (handlerEnum = [stateHandlers keyEnumerator]; (handlerKey = [handlerEnum nextObject]);) {
            ...
            if (![handlerActions isKindOfClass:[NSArray class]]) {
                OOLog(@ "verifyOXP.validateAI.failed.invalidFormat.handler",
                      @ "***** ERROR: handler \"%@\" for state \"%@\" in AI \"%@\" is not an array, ignoring.",
                      handlerKey,
                      stateKey,
                      aiName);
                continue;
            ...
            for (actionEnum = [handlerActions objectEnumerator]; (action = [actionEnum nextObject]);) {
                ...
                if (![action isKindOfClass:[NSString class]]) {
                    OOLog(
                        ...
                        @ "***** ERROR: action %llu in handler \"%@\" for state \"%@\" in AI \"%@\" is not a string, ignoring.",
                        index - 1,
                        handlerKey,
                        stateKey,
                        aiName);
    ...
    if ([badSelectors count] != 0) {
        ...
        OOLog(@ "verifyOXP.validateAI.failed.badSelector",
              @ "***** ERROR: the AI \"%@\" uses %llu unpermitted method%s: %@",
              aiName,
              [badSelectors count],
              ([badSelectors count] == 1) ? "" : "s",
              badSelectorDesc);
    }
