...
- (BOOL)shouldRun {
    ...
    return [fileScanner fileExists:@ "requires.plist" inFolder:@ "Config" referencedFrom:nil checkBuiltIn:NO];
}
...
- (void)run {
    ...
    NSString *version = nil, *maxVersion = nil;
    NSArray *ooVersionComponents = nil, *versionComponents = nil, *maxVersionComponents = nil;

    fileScanner = [[self verifier] fileScannerStage];
    requiresPList = [fileScanner plistNamed:@ "requires.plist" inFolder:@ "Config" referencedFrom:nil checkBuiltIn:NO];
    ...
    if ([actualKeys count] != 0) {
        OOLog(
            ...
            @ "----- WARNING: requires.plist contains unknown keys. This OXP will not be loaded by this version of Oolite. Unknown keys are: %@.",
            [[actualKeys allObjects] componentsJoinedByString:@ ", "]);
    }
    ...
    if (version != nil || maxVersion != nil) {
        ...
        if (ooVersionComponents == nil) {
            OOLog(@ "verifyOXP.requiresPList.cantFindOoliteVersion",
                  @ "%@",
        ...
        if (version != nil) {
            ...
            if (versionComponents == nil) {
                OOLog(@ "verifyOXP.requiresPList.badValue",
                      @ "***** ERROR: could not interpret version string \"%@\" as version number.",
                      version);
            } else if (ooVersionComponents != nil) {
                if (CompareVersions(ooVersionComponents, versionComponents) == NSOrderedAscending) {
                    OOLog(@ "verifyOXP.requiresPList.oxpRequiresNewerOolite",
                          @ "----- WARNING: this OXP requires a newer version of Oolite (%@) to work.",
                          version);
                }
            }
        }
        if (maxVersion != nil) {
            ...
            if (maxVersionComponents == nil) {
                OOLog(@ "verifyOXP.requiresPList.badValue",
                      @ "***** ERROR: could not interpret max_version string \"%@\" as version number.",
                      maxVersion);
            } else if (ooVersionComponents != nil) {
                if (CompareVersions(ooVersionComponents, maxVersionComponents) == NSOrderedDescending) {
                    OOLog(@ "verifyOXP.requiresPList.oxpRequiresOlderOolite",
                          @ "----- WARNING: this OXP requires an older version of Oolite (%@) to work.",
                          maxVersion);
                }
            }
        }
        ...
        if (versionComponents != nil && maxVersionComponents != nil) {
            if (CompareVersions(versionComponents, maxVersionComponents) == NSOrderedDescending) {
                OOLog(@ "verifyOXP.requiresPList.noVersionsInRange",
                      @ "***** ERROR: this OXP's maximum version (%@) is less than its minimum version (%@).",
                      maxVersion,
                      version);
            }
        }
