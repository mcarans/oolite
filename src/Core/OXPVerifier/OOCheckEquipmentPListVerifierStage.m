...
- (BOOL)shouldRun {
    ...
    return [fileScanner fileExists:@ "equipment.plist" inFolder:@ "Config" referencedFrom:nil checkBuiltIn:NO];
}
...
- (void)run {
    ...
    equipmentPList = [fileScanner plistNamed:@ "equipment.plist" inFolder:@ "Config" referencedFrom:nil checkBuiltIn:NO];
...
- (void)runCheckWithEquipment:(NSArray *)equipmentPList {
    ...
    for (entryEnum = [equipmentPList objectEnumerator]; (entry = [entryEnum nextObject]);) {
        ...
        if (![entry isKindOfClass:[NSArray class]]) {
            OOLog(@ "verifyOXP.equipmentPList.entryNotArray",
                  @ "***** ERROR: equipment.plist entry %u of equipment.plist is not an array.",
                  entryIndex);
            continue;
        }
        ...
        if (EQUIPMENT_KEY_INDEX < elemCount)
            name = [entry oo_stringAtIndex:EQUIPMENT_KEY_INDEX];
        else
            name = nil;

        if (name != nil)
            entryDesc = [NSString stringWithFormat:@ "%u (\"%@\")", entryIndex, name];
        else
            entryDesc = [NSString stringWithFormat:@ "%u", entryIndex];

        // Check that the entry has an acceptable number of elements.
        if (elemCount < 5) {
            OOLog(@ "verifyOXP.equipmentPList.badEntrySize",
                  @ "***** ERROR: equipment.plist entry %@ has too few elements (%llu, should be 5 or 6).",
                  entryDesc,
                  elemCount);
            continue;
        }
        if (6 < elemCount) {
            OOLog(@ "verifyOXP.equipmentPList.badEntrySize",
                  @ "----- WARNING: equipment.plist entry %@ has too many elements (%llu, should be 5 or 6).",
                  entryDesc,
                  elemCount);
        }
        ...
        */
        if ([entry oo_longAtIndex:EQUIPMENT_TECH_LEVEL_INDEX defaultValue:-1] < 0) {
            OOLog(@ "verifyOXP.equipmentPList.badElementType",
                  @ "***** ERROR: tech level for entry %@ of equipment.plist is not a positive integer.",
                  entryDesc);
        }
        if ([entry oo_longAtIndex:EQUIPMENT_PRICE_INDEX defaultValue:-1] < 0) {
            OOLog(@ "verifyOXP.equipmentPList.badElementType",
                  @ "***** ERROR: price for entry %@ of equipment.plist is not a positive integer.",
                  entryDesc);
        }
        if ([entry oo_stringAtIndex:EQUIPMENT_SHORT_DESC_INDEX] == nil) {
            OOLog(@ "verifyOXP.equipmentPList.badElementType",
                  @ "***** ERROR: short description for entry %@ of equipment.plist is not a string.",
                  entryDesc);
        }
        if ([entry oo_stringAtIndex:EQUIPMENT_KEY_INDEX] == nil) {
            OOLog(@ "verifyOXP.equipmentPList.badElementType",
                  @ "***** ERROR: key for entry %@ of equipment.plist is not a string.",
                  entryDesc);
        }
        if ([entry oo_stringAtIndex:EQUIPMENT_LONG_DESC_INDEX] == nil) {
            OOLog(@ "verifyOXP.equipmentPList.badElementType",
                  @ "***** ERROR: long description for entry %@ of equipment.plist is not a string.",
                  entryDesc);
        }
        ...
        if (5 < elemCount) {
            if ([entry oo_dictionaryAtIndex:EQUIPMENT_EXTRA_INFO_INDEX] == nil) {
                OOLog(@ "verifyOXP.equipmentPList.badElementType",
                      @ "***** ERROR: equipment.plist entry %@'s extra information dictionary is not a dictionary.",
                      entryDesc);
            }
