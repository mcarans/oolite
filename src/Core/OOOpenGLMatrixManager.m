...
const char *ooliteStandardMatrixUniforms[] = {"ooliteModelView",
                                              "ooliteProjection",
...
- (void)lookAtWithEye:(Vector)eye center:(Vector)center up:(Vector)up {
    ...
    OOMatrix m1 =
        OOMatrixConstruct(i.x, j.x, k.x, 0.0, i.y, j.y, k.y, 0.0, i.z, j.z, k.z, 0.0, -eye.x, -eye.y, -eye.z, 1.0);
...
- (void)frustumLeft:(double)l right:(double)r bottom:(double)b top:(double)t near:(double)n far:(double)f {
    ...
    [self multProjection:OOMatrixConstruct(2 * n / (r - l),
                                           0.0,
                                           0.0,
                                           0.0,
                                           0.0,
...
- (void)orthoLeft:(double)l right:(double)r bottom:(double)b top:(double)t near:(double)n far:(double)f {
    ...
    [self multProjection:OOMatrixConstruct(2 / (r - l),
                                           0.0,
                                           0.0,
                                           0.0,
                                           0.0,
...
- (void)perspectiveFovy:(double)fovy aspect:(double)aspect zNear:(double)zNear zFar:(double)zFar {
    ...
    [self multProjection:OOMatrixConstruct(f / aspect,
                                           0.0,
                                           0.0,
                                           0.0,
                                           0.0,
...
- (OOMatrix)getMatrix:(int)which {
    ...
    switch (which) {
        case OOLITE_GL_MATRIX_MODELVIEW_PROJECTION:
            matrices[which] =
                OOMatrixMultiply(matrices[OOLITE_GL_MATRIX_MODELVIEW], matrices[OOLITE_GL_MATRIX_PROJECTION]);
            break;
    ...
    for (i = 0; i < OOLITE_GL_MATRIX_END; i++) {
        ...
        if (location >= 0) {
            if (i == OOLITE_GL_MATRIX_NORMAL) {
                [locationSet addObject:[NSArray arrayWithObjects:[NSNumber numberWithInt:location],
