...
+ (OOColor *)colorWithRGBAComponents:(OORGBAComponents)components {
    return [self colorWithRed:components.r green:components.g blue:components.b alpha:components.a];
}

+ (OOColor *)colorWithHSBAComponents:(OOHSBAComponents)components {
    return [self colorWithHue:components.h / 360.0f saturation:components.s brightness:components.b alpha:components.a];
}
...
+ (OOColor *)brightColorWithDescription:(id)description {
    ...
    return [OOColor colorWithHue:[color hueComponent] / 360.0f
...
+ (OOColor *)colorFromString:(NSString *)colorFloatString {
    ...
    return [OOColor colorWithRed:rgbaValue[0] * factor
                           green:rgbaValue[1] * factor
                            blue:rgbaValue[2] * factor
                           alpha:rgbaValue[3] * factor];
}
...
- (float)hueComponent {
    float maxrgb =
        (rgba[0] > rgba[1]) ? ((rgba[0] > rgba[2]) ? rgba[0] : rgba[2]) : ((rgba[1] > rgba[2]) ? rgba[1] : rgba[2]);
    float minrgb =
        (rgba[0] < rgba[1]) ? ((rgba[0] < rgba[2]) ? rgba[0] : rgba[2]) : ((rgba[1] < rgba[2]) ? rgba[1] : rgba[2]);
...
- (OOHSBAComponents)hsbaComponents {
    OOHSBAComponents c;
    [self getHue:&c.h saturation:&c.s brightness:&c.b alpha:&c.a];
    return c;
}
...
- (OOColor *)premultipliedColor {
    ...
    return [OOColor colorWithRed:rgba[0] * rgba[3] green:rgba[1] * rgba[3] blue:rgba[2] * rgba[3] alpha:1.0f];
}
...
NSString *OORGBAComponentsDescription(OORGBAComponents components) {
    return
        [NSString stringWithFormat:@ "{%.3g, %.3g, %.3g, %.3g}", components.r, components.g, components.b, components.a];
}

NSString *OOHSBAComponentsDescription(OOHSBAComponents components) {
    return [NSString
        stringWithFormat:@ "{%i, %.3g, %.3g, %.3g}", (int)components.h, components.s, components.b, components.a];
}
