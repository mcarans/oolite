...
+ (NSArray *)userRootPaths {
    ...
    if (sUserRootPaths == nil) {
        ...
        NSArray *defaultAddOnsPaths = [NSArray
            arrayWithObjects:
#if OOLITE_MAC_OS_X
                [[[[NSHomeDirectory() stringByAppendingPathComponent:@ "Library"]
                    stringByAppendingPathComponent:@ "Application Support"] stringByAppendingPathComponent:@ "Oolite"]
                ...
                nil];
        sUserRootPaths = [[[[OOOXZManager sharedManager] additionalAddOnsPaths]
            arrayByAddingObjectsFromArray:defaultAddOnsPaths] retain];
    }
...
+ (void)preloadFileLists {
    ...
    NSArray *folders = [NSArray arrayWithObjects:@ "AIs",
                                                 @ "Images",
                                                 @ "Models",
                                                 @ "Music",
                                                 @ "Scenarios",
...
+ (void)preloadFileListFromOXZ:(NSString *)path forFolders:(NSArray *)folders {
    ...
    if (unzGoToFirstFile(uf) == UNZ_OK) {
        do {
            unzGetCurrentFileInfo64(uf, NULL, componentName, 512, NULL, 0, NULL, 0);
...
+ (void)preloadFileListFromFolder:(NSString *)path forFolders:(NSArray *)folders {
    ...
    foreach (subFolder, folders) {
        ...
        foreach (fileName, fileList) {
            [self preloadFilePathFor:fileName
                            inFolder:subFolder
                              atPath:[subFolderPath stringByAppendingPathComponent:fileName]];
        }
    }
}
...
+ (NSString *)maskUserName:(NSString *)name inPath:(NSString *)path {
    ...
    if (result && name)
        [result replaceOccurrencesOfString:name
                                withString:@ "*"
...
// Given a path to an assumed OXP (or other location where files are permissible), check for a requires.plist or
// manifest.plist and add to search paths if acceptable.
+ (void)checkPotentialPath:(NSString *)path:(NSMutableArray *)searchPaths {
    ...
    if (manifest == nil) {
        if ([[[path pathExtension] lowercaseString] isEqualToString:@ "oxz"]) {
            ...
            manifest = [NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@ "__oolite.tmp.%@", path],
                                                                  kOOManifestIdentifier,
                                                                  @ "1",
                                                                  kOOManifestVersion,
                                                                  @ "OXP without manifest",
                                                                  kOOManifestTitle,
                                                                  @ "1",
                                                                  kOOManifestRequiredOoliteVersion,
                                                                  nil];
        }
    }
...
+ (BOOL)validateManifest:(NSDictionary *)manifest forOXP:(NSString *)path {
    ...
    if (duplicate != nil) {
        OOLog(@ "oxp.duplicate",
              @ "OXP %@ has the same identifier (%@) as %@ which has already been loaded.",
              path,
              identifier,
              [duplicate oo_stringForKey:kOOManifestFilePath]);
        [self addErrorWithKey:@ "oxp-manifest-duplicate"
                       param1:path
                       param2:[duplicate oo_stringForKey:kOOManifestFilePath]];
        return NO;
    }
...
+ (BOOL)checkVersionCompatibility:(NSDictionary *)manifest forOXP:(NSString *)title {
    ...
    if (maxRequired == nil || [maxRequired length] == 0) {
        return [self areRequirementsFulfilled:[NSDictionary dictionaryWithObjectsAndKeys:required, @ "version", nil]
                                       forOXP:title
                                      andFile:@ "manifest.plist"];
    } else {
        return [self areRequirementsFulfilled:[NSDictionary dictionaryWithObjectsAndKeys:required,
                                                                                         @ "version",
                                                                                         maxRequired,
                                                                                         @ "max_version",
                                                                                         nil]
...
+ (BOOL)areRequirementsFulfilled:(NSDictionary *)requirements forOXP:(NSString *)path andFile:(NSString *)file {
    ...
    if (ooVersionComponents == nil) {
        ooVersionComponents =
            ComponentsFromVersionString([[[NSBundle mainBundle] infoDictionary] objectForKey:@ "CFBundleVersion"]);
    ...
    if (OK) {
        ...
        if (requiredVersion != nil) {
            ...
            if ([requiredVersion isKindOfClass:[NSString class]]) {
                ...
                OOLog(@ "requirements.wrongType",
                      @ "Expected %@ entry \"%@\" to be string, but got %@ in OXP %@.",
                      file,
                      @ "version",
                      [requirements class],
    ...
    if (OK && conditionsHandled < [requirements count]) {
        ...
        OOLog(@ "requirements.unknown",
              @ "requires.plist for OXP %@ contains unknown keys, rejecting.",
              [path lastPathComponent]);
        OK = NO;
    }
    ...
    if (conflicts != nil && [conflicts count] > 0) {
        ...
        foreach (conflicting, conflicts) {
            ...
            if (conflictManifest != nil) {
                // then check versions
                if ([self matchVersions:conflicting
                            withVersion:[conflictManifest oo_stringForKey:kOOManifestVersion]]) {
                    if (logErrors) {
                        [self addErrorWithKey:@ "oxp-conflict"
                                       param1:[manifest oo_stringForKey:kOOManifestTitle]
                                       param2:[conflictManifest oo_stringForKey:kOOManifestTitle]];
                        OOLog(@ "oxp.conflict",
                              @ "OXP %@ conflicts with %@ and was removed from the loading list",
                              [[manifest oo_stringForKey:kOOManifestFilePath] lastPathComponent],
                              [[conflictManifest oo_stringForKey:kOOManifestFilePath] lastPathComponent]);
                    }
                    return YES;
                }
            }
...
+ (BOOL)manifest:(NSDictionary *)manifest HasUnmetDependency:(NSDictionary *)required logErrors:(BOOL)logErrors {
    ...
    if (!requirementsMet) {
        if (logErrors) {
            [self addErrorWithKey:@ "oxp-required"
                           param1:[manifest oo_stringForKey:kOOManifestTitle]
                           param2:[required oo_stringForKey:kOOManifestRelationDescription
                                               defaultValue:[required oo_stringForKey:kOOManifestRelationIdentifier]]];
            OOLog(@ "oxp.requirementMissing",
                  @ "OXP %@ had unmet requirements and was removed from the loading list",
                  [[manifest oo_stringForKey:kOOManifestFilePath] lastPathComponent]);
        }
        return YES;
    }
    ...
    foreach (uaoBit, sUseAddOnsParts) {
        if ([uaoBit hasPrefix:SCENARIO_OXP_DEFINITION_BYID]) {
            result |= [ResourceManager
                manifestAllowedByScenario:manifest
                           withIdentifier:[uaoBit substringFromIndex:[SCENARIO_OXP_DEFINITION_BYID length]]];
        } else if ([uaoBit hasPrefix:SCENARIO_OXP_DEFINITION_BYTAG]) {
            result |= [ResourceManager
                manifestAllowedByScenario:manifest
                                  withTag:[uaoBit substringFromIndex:[SCENARIO_OXP_DEFINITION_BYTAG length]]];
        }
    }
    return result;
}
...
+ (BOOL)checkCacheUpToDateForPaths:(NSArray *)searchPaths {
    ...
    */
    ...
    if (EXPECT_NOT([[NSUserDefaults standardUserDefaults] boolForKey:@ "always-flush-cache"])) {
        OOLog(kOOLogCacheExplicitFlush,
              @ "%@",
              @ "Cache explicitly flushed with always-flush-cache preference. Rebuilding from scratch.");
        upToDate = NO;
    ...
    if (upToDate && ![oldPaths isEqual:searchPaths]) {
        ...
        if (oldPaths != nil)
            OOLog(
                kOOLogCacheStalePaths, @ "%@", @ "Cache is stale (search paths have changed). Rebuilding from scratch.");
        upToDate = NO;
    }
    ...
    if (upToDate && ![[cacheMgr objectForKey:kOOCacheKeyModificationDates
                                     inCache:kOOCacheSearchPathModDates] isEqual:modDates]) {
        OOLog(kOOLogCacheStaleDates,
              @ "%@",
    ...
    if (!upToDate) {
        ...
        OOLog(kOOLogCacheUpToDate, @ "%@", @ "Data cache is up to date.");
...
+ (NSDictionary *)dictionaryFromFilesNamed:(NSString *)fileName
    ...
    return [ResourceManager dictionaryFromFilesNamed:fileName
                                            inFolder:folderName
                                           mergeMode:mergeFiles ? MERGE_BASIC : MERGE_NONE
                                               cache:YES];
}
...
+ (NSDictionary *)dictionaryFromFilesNamed:(NSString *)fileName
    ...
    if (mergeType == nil) {
        OOLog(kOOLogParameterError,
              @ "Unknown dictionary merge mode %u for %@. (This is an internal programming error, please report it.)",
              mergeMode,
              fileName);
        return nil;
    }
    ...
    if (mergeMode == MERGE_NONE) {
        ...
        for (enumerator = [results objectEnumerator]; (dict = [enumerator nextObject]);) {
            if (mergeMode == MERGE_SMART)
                [result mergeEntriesFromDictionary:dict];
            else
...
+ (NSArray *)arrayFromFilesNamed:(NSString *)fileName
    ...
    if (useCache) {
        cacheKey = [NSString
            stringWithFormat:@ "%@%@ merge:%@",
                             (folderName != nil) ? [folderName stringByAppendingString:@ "/"] : (NSString *)@ "",
                             fileName,
                             mergeFiles ? @ "yes" : @ "no"];
    ...
    if (!mergeFiles) {
        ...
        for (enumerator = [ResourceManager pathEnumerator]; (path = [enumerator nextObject]);) {
            ...
            if (folderName != nil) {
                ...
                if ([array count] != 0 && [[array objectAtIndex:0] isKindOfClass:[NSArray class]]) {
                    if ([[fileName lowercaseString] isEqualToString:@ "equipment.plist"])
                        [self handleEquipmentListMerging:results
                                          forLookupIndex:3];  // Index 3 is the role string (EQ_*).
                }
            }
        }
...
+ (void)handleEquipmentListMerging:(NSMutableArray *)arrayToProcess forLookupIndex:(unsigned)lookupIndex {
    ...
    for (i = 0; i < [refArray count]; i++) {
        for (j = 0; j < [arrayToProcess count] - 1; j++) {
            ...
            for (k = 0; k < count; k++) {
                id processValue = [[[arrayToProcess oo_arrayAtIndex:j] oo_arrayAtIndex:k] oo_objectAtIndex:lookupIndex
                                                                                              defaultValue:nil];
            ...
            if ([equipKey isEqual:refValue]) {
                ...
                for (infoKeyEnum = [overridesEntry keyEnumerator]; (infoKey = [infoKeyEnum nextObject]);) {
                    ...
                    if ([infoKey isEqualToString:@ "techlevel"])
                        [equipArray replaceObjectAtIndex:EQUIPMENT_TECH_LEVEL_INDEX
                                              withObject:[overridesEntry objectForKey:infoKey]];
                    else if ([infoKey isEqualToString:@ "price"])
                        [equipArray replaceObjectAtIndex:EQUIPMENT_PRICE_INDEX
                                              withObject:[overridesEntry objectForKey:infoKey]];
                    else if ([infoKey isEqualToString:@ "short_description"])
                        [equipArray replaceObjectAtIndex:EQUIPMENT_SHORT_DESC_INDEX
                                              withObject:[overridesEntry objectForKey:infoKey]];
                    else if ([infoKey isEqualToString:@ "name"])
                        [equipArray replaceObjectAtIndex:EQUIPMENT_SHORT_DESC_INDEX
                                              withObject:[overridesEntry objectForKey:infoKey]];
                    else if ([infoKey isEqualToString:@ "long_description"])
                        [equipArray replaceObjectAtIndex:EQUIPMENT_LONG_DESC_INDEX
                                              withObject:[overridesEntry objectForKey:infoKey]];
                        ...
                        [equipArray replaceObjectAtIndex:EQUIPMENT_EXTRA_INFO_INDEX
                                              withObject:[[extra copy] autorelease]];
                    }
                }
...
+ (NSDictionary *)whitelistDictionary {
    ...
    if (whitelistDictionary == nil) {
        NSString *path = [[[ResourceManager builtInPath] stringByAppendingPathComponent:@ "Config"]
            stringByAppendingPathComponent:@ "whitelist.plist"];
...
+ (NSDictionary *)logControlDictionary {
    ...
    NSString *key = nil;
    foreachkey(key, logControl) { [coreRoots addObject:LogClassKeyRoot(key)]; }

    NSArray *rootPaths = [self rootPaths];
    NSString *configPath = nil;
    NSDictionary *dict = nil;

    // Look for logcontrol.plists inside OXPs (but not in root paths). These are not allowed to define keys in
    // hierarchies used by the build-in one.
    NSEnumerator *pathEnum = [self pathEnumerator];
    while ((path = [pathEnum nextObject])) {
        if ([rootPaths containsObject:path]) continue;

        configPath =
            [[path stringByAppendingPathComponent:@ "Config"] stringByAppendingPathComponent:@ "logcontrol.plist"];
    ...
    // Now, look for logcontrol.plists in root paths, i.e. not within OXPs. These are allowed to override the built-in
    // copy.
    ...
    while ((path = [pathEnum nextObject])) {
        ...
        foreachkey(key, dict) { [logControl setObject:[dict objectForKey:key] forKey:key]; }
    }
...
+ (NSDictionary *)roleCategoriesDictionary {
    ...
    while ((path = [pathEnum nextObject])) {
        ...
        configPath =
            [[path stringByAppendingPathComponent:@ "Config"] stringByAppendingPathComponent:@ "role-categories.plist"];
    ...
    NSArray *pirateVictims = [ResourceManager arrayFromFilesNamed:@ "pirate-victim-roles.plist"
                                                         inFolder:@ "Config"
                                                         andMerge:YES];
    if (OOEnforceStandards() && [pirateVictims count] > 0) {
        OOStandardsDeprecated(@ "pirate-victim-roles.plist is still being used.");
    }
    [ResourceManager mergeRoleCategories:[NSDictionary dictionaryWithObject:pirateVictims
                                                                     forKey:@ "oolite-pirate-victim"]
                          intoDictionary:roleCategories];
...
+ (void)mergeRoleCategories:(NSDictionary *)catData intoDictionary:(NSMutableDictionary *)categories {
    ...
    foreachkey(key, catData) {
        ...
        OOLog(@ "shipData.load.roleCategories",
              ...
              (unsigned long)[catDataEntry count],
              key);
...
+ (OOSystemDescriptionManager *)systemDescriptionManager {
    ...
    while ((path = [pathEnum nextObject])) {
        ...
        configPath =
            [[path stringByAppendingPathComponent:@ "Config"] stringByAppendingPathComponent:@ "planetinfo.plist"];
        categories = OODictionaryFromFile(configPath);
        if (categories != nil) {
...
+ (NSDictionary *)shaderBindingTypesDictionary {
    ...
    if (shaderBindingTypesDictionary == nil) {
        ...
        NSString *path = [[[ResourceManager builtInPath] stringByAppendingPathComponent:@ "Config"]
            stringByAppendingPathComponent:@ "shader-uniform-bindings.plist"];
...
+ (NSString *)pathForFileNamed:(NSString *)fileName inFolder:(NSString *)folderName cache:(BOOL)useCache {
    ...
    if (cache) {
        if (folderName != nil)
            cacheKey = [NSString stringWithFormat:@ "%@/%@", folderName, fileName];
        else
            cacheKey = fileName;
...
+ (NSDictionary *)loadScripts {
            ...
            @try {
                ...
                if (results != nil) {
                    for (scriptEnum = [results objectEnumerator]; (script = [scriptEnum nextObject]);) {
                        name = [script name];
                        if (name != nil)
                            [loadedScripts setObject:script forKey:name];
                        else
                            OOLog(@ "script.load.unnamed", @ "Discarding anonymous script %@", script);
                    }
                }
            } @catch (NSException *exception) {
                OOLog(
                    @ "script.load.exception",
                    @ "***** %s encountered exception %@ (%@) while trying to load script from %@ -- ignoring this location.",
                    __PRETTY_FUNCTION__,
                    [exception name],
                    [exception reason],
                    path);
    ...
    if (OOLogWillDisplayMessagesInClass(@ "script.load.world.listAll")) {
        ...
        if (count != 0) {
            ...
            displayString = [[displayNames sortedArrayUsingSelector:@selector(caseInsensitiveCompare:)]
