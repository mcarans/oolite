...
// Values for _special
enum { kSpecialNone, kSpecialTheme, kSpecialDocking, kSpecialDocked, kSpecialMission };
...
#if OOLITE_MAC_OS_X
- (void)playiTunesPlaylist:(NSString *)playlistName {
    NSString *ootunesScriptString = [NSString stringWithFormat:@ "with timeout of 1 second\n"
    ...
    if (errDict) OOLog(@ "sound.music.iTunesIntegration.failed", @ "ootunes returned :%@", errDict);
}

- (void)pauseiTunes {
    NSString *ootunesScriptString = [NSString
        stringWithFormat:
