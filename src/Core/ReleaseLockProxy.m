...
- (void)release {
    ...
    if (_locked) {
        ...
        VerboseLog(@ "-- ReleaseLockProxy (%@): released while not locked; retain count going from %u to %u.",
                   [self rlpObjectDescription],
                   retainCount,
                   retainCount - 1);
    }
...
- (id)autorelease {
    VerboseLog(@ "-- ReleaseLockProxy (%@): autoreleased while %slocked and retain count at %u.",
               [self rlpObjectDescription],
               _locked ? "" : "not ",
               [self retainCount]);
...
- (NSMethodSignature *)methodSignatureForSelector:(SEL)selector {
    ...
    if (selector == @selector(initWithObject:name:) || selector == @selector(initWithRetainedObject:name:) ||
        selector == @selector(rlpAllowRelease) || selector == @selector(rlpObjectDescription)) {
