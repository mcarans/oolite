...
#define OctreeDebugLog(format, ...)                                                                        \
    do {                                                                                                   \
        if (EXPECT_NOT(gDebugFlags & DEBUG_OCTREE_LOGGING)) OOLog(@ "octree.debug", format, ##__VA_ARGS__); \
    } while (0)
#else
#define OctreeDebugLog(...) \
    do {                    \
    } while (0)
#endif
...
- (Octree_details)octreeDetails {
    Octree_details details = {.octree = _octree, .radius = _radius, .octree_collision = _collisionOctree};
    return details;
}
...
static Vector offsetForOctant(int oct, GLfloat r) {
    return make_vector((0.5f - (GLfloat)((oct >> 2) & 1)) * r,
                       (0.5f - (GLfloat)((oct >> 1) & 1)) * r,
                       (0.5f - (GLfloat)(oct & 1)) * r);
}
...
- (void)drawOctreeFromLocation:(uint32_t)loc:(GLfloat)scale:(Vector)offset {
    ...
    if (_octree[loc] == -1)  // full
    {
        ...
        [self drawOctreeFromLocation:loc + _octree[loc] +
                                   0:sc:make_vector(offset.x - sc, offset.y - sc, offset.z - sc)];
        OCTREE_COLOR(0.0f, 0.0f, 1.0f, 0.5f);
        [self drawOctreeFromLocation:loc + _octree[loc] +
                                   1:sc:make_vector(offset.x - sc, offset.y - sc, offset.z + sc)];
        OCTREE_COLOR(0.0f, 1.0f, 0.0f, 0.5f);
        [self drawOctreeFromLocation:loc + _octree[loc] +
                                   2:sc:make_vector(offset.x - sc, offset.y + sc, offset.z - sc)];
        OCTREE_COLOR(0.0f, 1.0f, 1.0f, 0.5f);
        [self drawOctreeFromLocation:loc + _octree[loc] +
                                   3:sc:make_vector(offset.x - sc, offset.y + sc, offset.z + sc)];
        OCTREE_COLOR(1.0f, 0.0f, 0.0f, 0.5f);
        [self drawOctreeFromLocation:loc + _octree[loc] +
                                   4:sc:make_vector(offset.x + sc, offset.y - sc, offset.z - sc)];
        OCTREE_COLOR(1.0f, 0.0f, 1.0f, 0.5f);
        [self drawOctreeFromLocation:loc + _octree[loc] +
                                   5:sc:make_vector(offset.x + sc, offset.y - sc, offset.z + sc)];
        OCTREE_COLOR(1.0f, 1.0f, 0.0f, 0.5f);
        [self drawOctreeFromLocation:loc + _octree[loc] +
                                   6:sc:make_vector(offset.x + sc, offset.y + sc, offset.z - sc)];
        OCTREE_COLOR(1.0f, 1.0f, 1.0f, 0.5f);
        [self drawOctreeFromLocation:loc + _octree[loc] +
                                   7:sc:make_vector(offset.x + sc, offset.y + sc, offset.z + sc)];
    }
}
...
- (void)drawOctreeCollisionFromLocation:(uint32_t)loc:(GLfloat)scale:(Vector)offset {
    ...
    if (_octree[loc] > 0) {
        GLfloat sc = 0.5f * scale;
        [self drawOctreeCollisionFromLocation:loc + _octree[loc] +
                                            0:sc:make_vector(offset.x - sc, offset.y - sc, offset.z - sc)];
        [self drawOctreeCollisionFromLocation:loc + _octree[loc] +
                                            1:sc:make_vector(offset.x - sc, offset.y - sc, offset.z + sc)];
        [self drawOctreeCollisionFromLocation:loc + _octree[loc] +
...
static BOOL isHitByLine(const int *octbuffer,
    ...
    int faces = face_hit;
    if (faces == 0) faces = lineCubeIntersection(u0, u1, rad);
    ...
    if (faces > 0) {
        ...
        if (CUBE_FACE_FRONT & faces) octantIntersected = ((vi.x < 0.0) ? 1 : 5) + ((vi.y < 0.0) ? 0 : 2);
        if (CUBE_FACE_BACK & faces) octantIntersected = ((vi.x < 0.0) ? 0 : 4) + ((vi.y < 0.0) ? 0 : 2);

        if (CUBE_FACE_RIGHT & faces) octantIntersected = ((vi.y < 0.0) ? 4 : 6) + ((vi.z < 0.0) ? 0 : 1);
        if (CUBE_FACE_LEFT & faces) octantIntersected = ((vi.y < 0.0) ? 0 : 2) + ((vi.z < 0.0) ? 0 : 1);

        if (CUBE_FACE_TOP & faces) octantIntersected = ((vi.x < 0.0) ? 2 : 6) + ((vi.z < 0.0) ? 0 : 1);
        if (CUBE_FACE_BOTTOM & faces) octantIntersected = ((vi.x < 0.0) ? 0 : 4) + ((vi.z < 0.0) ? 0 : 1);
    ...
    // first test the intersected octant, then the three adjacent, then the next three adjacent, the finally the
    // diagonal octant
    int oct0, oct1, oct2,
        oct3;  // test oct0 then oct1, oct2, oct3 then (7 - oct1), (7 - oct2), (7 - oct3) then (7 - oct0)
...
static BOOL isHitByOctree(Octree_details axialDetails,
    ...
    if (axialBuffer[0] == -1) {
        ...
        int nearest_oct =
            ((otherPosition.x > 0.0) ? 0 : 4) | ((otherPosition.y > 0.0) ? 0 : 2) | ((otherPosition.z > 0.0) ? 0 : 1);
    ...
    int nearest_oct =
        ((otherPosition.x > 0.0) ? 4 : 0) | ((otherPosition.y > 0.0) ? 2 : 0) | ((otherPosition.z > 0.0) ? 1 : 0);
...
- (NSDictionary *)dictionaryRepresentation {
    return [NSDictionary
        dictionaryWithObjectsAndKeys:_data, @ "octree", [NSNumber numberWithFloat:_radius], @ "radius", nil];
}
...
#ifndef NDEBUG
- (size_t)totalSize {
    return [self oo_objectSize] + _nodeCount * [_data oo_objectSize] + [_data length] +
           _nodeCount * sizeof *_collisionOctree;
}
#endif
...
OOINLINE struct OOOctreeBuildState *State(OOOctreeBuilder *self) { return &self->_stateStack[self->_level]; }
...
- (Octree *)buildOctreeWithRadius:(GLfloat)radius {
    NSAssert(State(self)->remaining == 0 && _level == 0,
    ...
    /*	Hand over the bytes to the data object, which will be used directly by
            the Octree.
    */
    NSData *data = [NSData dataWithBytesNoCopy:resized length:dataSize freeWhenDone:YES];
