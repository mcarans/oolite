...
- (NSString *)description {
    return [NSString stringWithFormat:@ "<%@ %p>{\"%@\", %u elements, prune threshold=%u, auto-prune=%s dirty=%s}",
                                      [self class],
                                      self,
                                      [self name],
                                      CacheGetCount(cache),
...
- (id)objectForKey:(id)key {
    ...
    // Note: while reordering the age list technically makes the cache dirty, it's not worth rewriting it just for that,
    // so we don't flag it.
...
static OOCacheNode *TreeCheckIntegrity(OOCacheImpl *cache,
                                       OOCacheNode *node,
                                       OOCacheNode *expectedParent,
                                       NSString *context);
#endif
...
static OOCacheImpl *CacheAllocate(void) { return calloc(sizeof(OOCacheImpl), 1); }
...
static NSArray *CacheArrayOfNodesByAge(OOCacheImpl *cache) {
    ...
    for (node = cache->oldest; node != NULL; node = node->younger) {
        [result addObject:[NSDictionary dictionaryWithObjectsAndKeys:node->key,
...
static NSString *CacheGetName(OOCacheImpl *cache) { return cache->name; }
...
static unsigned CacheGetCount(OOCacheImpl *cache) { return cache->count; }
...
// CacheNodeFree(): recursively delete a cache node and its children in the splay tree. To delete an individual node,
// first clear its child pointers.
static void CacheNodeFree(OOCacheImpl *cache, OOCacheNode *node) {
    id key, value;
...
static OOCacheNode *TreeInsert(OOCacheImpl *cache, id<OOCacheComparable> key, id value) {
    OOCacheNode *closest = NULL, *node = NULL;
    NSComparisonResult order;
    ...
    if (cache->root == NULL) {
        ...
        if (node != NULL) {
            ...
            if (order == NSOrderedAscending) {
                ...
                OOLog(@ "dataCache.inconsistency",
                      @ "%s() internal inconsistency for cache \"%@\", insertion failed.",
                      __PRETTY_FUNCTION__,
                      cache->name);
                CacheNodeFree(cache, node);
                return NULL;
...
// TreeCheckIntegrity(): verify the links and contents of a (sub-)tree. If successful, returns the root of the subtree
// (which could theoretically be changed), otherwise returns NULL.
static OOCacheNode *TreeCheckIntegrity(OOCacheImpl *cache,
                                       OOCacheNode *node,
                                       OOCacheNode *expectedParent,
                                       NSString *context) {
    ...
    if (OK && node->key == nil) {
        OOLog(kOOLogCacheIntegrityCheck,
              @ "Integrity check (%@ for \"%@\"): node \"%@\" has nil key; deleting subtree.",
              context,
              cache->name,
              CacheNodeGetDescription(node));
        OK = NO;
    }

    if (OK && node->value == nil) {
        OOLog(kOOLogCacheIntegrityCheck,
              @ "Integrity check (%@ for \"%@\"): node \"%@\" has nil value, deleting.",
              context,
              cache->name,
              CacheNodeGetDescription(node));
        OK = NO;
    ...
    if (OK && node->leftChild != NULL) {
        ...
        if (order != NSOrderedDescending) {
            OOLog(
                ...
                @ "Integrity check (%@ for \"%@\"): node %@'s left child %@ is not correctly ordered. Deleting subtree.",
                context,
                cache->name,
                CacheNodeGetDescription(node),
                CacheNodeGetDescription(node->leftChild));
            CacheNodeFree(cache, node->leftChild);
            node->leftChild = NULL;
    ...
    if (node->rightChild != NULL) {
        ...
        if (order != NSOrderedAscending) {
            OOLog(
                kOOLogCacheIntegrityCheck,
                @ "Integrity check (%@ for \"%@\"): node \"%@\"'s right child \"%@\" is not correctly ordered. Deleting subtree.",
                context,
                cache->name,
                CacheNodeGetDescription(node),
                CacheNodeGetDescription(node->rightChild));
            CacheNodeFree(cache, node->rightChild);
            node->rightChild = NULL;
            cache->count = kCountUnknown;
        } else {
...
static void AgeListCheckIntegrity(OOCacheImpl *cache, NSString *context) {
    ...
    if (node)
        for (;;) {
            ...
            if (next->younger != node) {
                OOLog(
                    kOOLogCacheIntegrityCheck,
                    @ "Integrity check (%@ for \"%@\"): node \"%@\" has invalid older link (should be \"%@\", is \"%@\"); repairing.",
                    context,
                    cache->name,
                    CacheNodeGetDescription(next),
                    CacheNodeGetDescription(node),
                    CacheNodeGetDescription(next->older));
                next->older = node;
            }
            node = next;
        }
    ...
    if (seenCount != cache->count) {
        // This is especially bad since this function is called just after verifying that the count field reflects the
        // number of objects in the tree.
        OOLog(kOOLogCacheIntegrityCheck,
              @ "Integrity check (%@ for \"%@\"): expected %u nodes, found %u. Cannot repair; clearing cache.",
              context,
              cache->name,
              cache->count,
              seenCount);
    ...
    if (node != cache->oldest) {
        OOLog(
            kOOLogCacheIntegrityCheck,
            @ "Integrity check (%@ for \"%@\"): oldest pointer in cache is wrong (should be \"%@\", is \"%@\"); repairing.",
            context,
            cache->name,
            CacheNodeGetDescription(node),
            CacheNodeGetDescription(cache->oldest));
        cache->oldest = node;
    }
}
...
- (void)appendNodesFromSubTree:(OOCacheNode *)subTree toString:(NSMutableString *)ioString {
    [ioString appendFormat:@ "\tn%p [label=\"<f0> | <f1> %@ | <f2>\"];\n",
                           subTree,
                           EscapedGraphVizString([subTree->key description])];
...
- (NSString *)generateGraphVizBodyWithRootNamed:(NSString *)rootName {
    ...
    [result appendFormat:@ "\t%@ [label=\"Cache \\\"%@\\\"\" shape=box];\n"
                          "\tnode [shape=record];\n\t\n",
                         rootName,
                         EscapedGraphVizString([self name])];
