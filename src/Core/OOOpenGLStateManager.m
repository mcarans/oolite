...
typedef enum { kStateFalse = false, kStateTrue = true, kStateMaybe } StateFlag;
...
*/
static inline void SetFunctionTracking(bool value) { sGLFunctionTracking = value; }
...
#define ITEM_STATEFLAG(NAME)                                                                      \
    do {                                                                                          \
        if (a->NAME != b->NAME && a->NAME != kStateMaybe && b->NAME != kStateMaybe) return false; \
    } while (0)
...
#if OO_CHECK_GL_HEAVY
            OOLog(kOOLogOpenGLVerifyDump,
                  @ "Previous OpenGL-using function: %s (line %u)",
                  sPreviousFunction,
                  sPreviousLine);
#endif
...
#define TEST_ITEM(NAME_, DISP_)                       \
    if (currentState.NAME_ != expectedState->NAME_) { \
        OOLog(kOOLogOpenGLVerifyDump,                 \
              @ "GL_%@ should be %@ but is %@.",       \
              @ #NAME_,                               \
              DISP_(expectedState->NAME_),            \
              DISP_(currentState.NAME_));             \
    }

#define ITEM_STATEFLAG(NAME)                  \
    if (expectedState->NAME != kStateMaybe) { \
...
#include "OOOpenGLStates.tbl"
            ...
            if (currentState.BLEND_SRC != expectedState->BLEND_SRC ||
                currentState.BLEND_DST != expectedState->BLEND_DST) {
                OOLog(kOOLogOpenGLVerifyDump,
                      @ "GL blend mode should be %@, %@ but is %@, %@.",
                      OOGLEnumToString(expectedState->BLEND_SRC),
                      OOGLEnumToString(expectedState->BLEND_DST),
                      OOGLEnumToString(currentState.BLEND_SRC),
                      OOGLEnumToString(currentState.BLEND_DST));
            }
...
#define ITEM_STATEFLAG(NAME)                                                          \
    if (sourceState->NAME != targetState->NAME && sourceState->NAME != kStateMaybe && \
        targetState->NAME != kStateMaybe) {                                           \
        if (targetState->NAME) {                                                      \
