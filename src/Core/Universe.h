...
@class GameController, CollisionRegion, MyOpenGLView, GuiDisplayGen, Entity, ShipEntity, StationEntity, OOPlanetEntity,
    OOSunEntity, OOVisualEffectEntity, PlayerEntity, OORoleSet, WormholeEntity, DockEntity, OOJSScript,
    OOWaypointEntity, OOSystemDescriptionManager;
...
@interface Universe : OOWeakRefObject {
   ...
   @private
    ...
    OOWeakReference *_firstBeacon, *_lastBeacon;
    ...
    OOTimeAbsolute countdown_messageRepeatTime;  // Getafix(4/Aug/2010) - Quickfix countdown messages colliding with
                                                 // weapon overheat messages.
                                                 //                       For proper handling of message dispatching,
    ...
    //	NSSet					*pirateVictimRoles;		// Roles listed in
    // pirateVictimRoles.plist.
    NSDictionary *
        roleCategories;  // Categories for roles from role-categories.plist, extending the old pirate-victim-roles.plist
...
- (ShipEntity *)newShipWithRole:(NSString *)role
    OO_RETURNS_RETAINED;  // Selects ship using role weights, applies auto_ai, respects conditions
- (ShipEntity *)newShipWithName:(NSString *)shipKey
    OO_RETURNS_RETAINED;  // Does not apply auto_ai or respect conditions
- (ShipEntity *)newSubentityWithName:(NSString *)shipKey
                      andScaleFactor:(float)scale OO_RETURNS_RETAINED;  // Does not apply auto_ai or respect conditions
...
- (ShipEntity *)newShipWithName:(NSString *)shipKey
    ...
    OO_RETURNS_RETAINED;  // If usePlayerProxy, non-carriers are instantiated as ProxyPlayerEntity.
- (ShipEntity *)newShipWithName:(NSString *)shipKey
                 usePlayerProxy:(BOOL)usePlayerProxy
                    isSubentity:(BOOL)isSubentity OO_RETURNS_RETAINED;
- (ShipEntity *)newShipWithName:(NSString *)shipKey
                 usePlayerProxy:(BOOL)usePlayerProxy
                    isSubentity:(BOOL)isSubentity
                 andScaleFactor:(float)scale OO_RETURNS_RETAINED;
...
- (void)enterGUIViewModeWithMouseInteraction:
    (BOOL)mouseInteraction;  // Use instead of setViewDirection:VIEW_GUI_DISPLAY
