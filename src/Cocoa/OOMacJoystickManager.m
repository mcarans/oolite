...
- (void)handleDeviceAttach:(IOHIDDeviceRef)device {
    ...
    if (OOLogWillDisplayMessagesInClass(@ "joystick.reject")) {
        ...
        unsigned usagePage =
            [(NSNumber *)IOHIDDeviceGetProperty(device, CFSTR(kIOHIDPrimaryUsagePageKey)) unsignedIntValue];
    ...
    if (OOLogWillDisplayMessagesInClass(@ "joystick.connect.element")) {
        ...
        if (elementList != NULL) {
            ...
            for (idx = 0; idx < count; idx++) {
                ...
                NSString *name =
                    (NSString *)IOHIDElementGetProperty(element, CFSTR(kIOHIDElementNameKey)) ?: @ "unnamed";
                OOLog(@ "joystick.connect.element",
                      @ "%@ - usage %d:%d, cookie %d, range %d-%d",
                      name,
                      usagePage,
                      usage,
                      (int)elementCookie,
    ...
    */
    ...
    if (0 <= value && value <= max) {
        if (max == 3) {
            ...
            const uint8_t valueMap8[8] = {JOYHAT_UP,
                                          JOYHAT_RIGHTUP,
                                          JOYHAT_RIGHT,
                                          JOYHAT_RIGHTDOWN,
                                          JOYHAT_DOWN,
...
- (void)handleInputEvent:(IOHIDValueRef)value {
    ...
    if (axisNum >= 0) {
        ...
        // Note: this is designed so gammaIndex == intValue if min == 0 and max == kJoystickGammaTableSize - 1 (the
        // common case).
