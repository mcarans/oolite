...
- (id)initWithFrame:(NSRect)frameRect {
    ...
    NSOpenGLPixelFormatAttribute attrs[] = {
        ...
        // We may be on a multi-display system (and each screen may be driven by a different renderer), so we need to
        // specify which screen we want to take over. For this demo, we'll specify the main screen.
        NSOpenGLPFAScreenMask,
        CGDisplayIDToOpenGLDisplayMask(kCGDirectMainDisplay),

        // Specifying "NoRecovery" gives us a context that cannot fall back to the software renderer.
        // This makes the View-based context a compatible with the fullscreen context, enabling us to use the
        // "shareContext" feature to share textures, display lists, and other OpenGL objects between the two.
...
- (void)initKeyMappingData {
    ...
    NSDictionary *kmap =
        [NSDictionary dictionaryWithDictionary:[ResourceManager dictionaryFromFilesNamed:@ "keymappings_mac.plist"
                                                                                inFolder:@ "Config"
...
- (void)noteMovedToBadDisplay {
    NSRunInformationalAlertPanel(
        DESC(@ "oolite-mac-bad-display"), @ "%@", nil, nil, nil, DESC(@ "oolite-mac-bad-display-2"));
}
...
- (void)viewDidMoveToWindow {
    ...
    */
    if (&NSWindowDidChangeBackingPropertiesNotification != NULL &&
...
- (BOOL)snapShot:(NSString *)filename {
    ...
    if (withFilename && [[NSFileManager defaultManager] fileExistsAtPath:pathToPic]) {
        OOLog(@ "screenshot.filenameExists",
              @ "Snapshot \"%@.png\" already exists - adding numerical sequence.",
              pathToPic);
        pathToPic = nil;
    }
    ...
    NSBitmapImageRep *bitmapRep =
        [[NSBitmapImageRep alloc] initWithBitmapDataPlanes:NULL  // --> let the class allocate it
                                                pixelsWide:w
                                                pixelsHigh:h
    ...
    [[bitmapRep representationUsingType:NSPNGFileType
                             properties:[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithBool:YES],
                                                                                   NSImageInterlaced,
                                                                                   NULL]]
        writeToFile:pathToPic
         atomically:YES];  // save PNG representation of image
...
- (void)keyUp:(NSEvent *)theEvent {
    ...
    */
    ...
    if ((key >= 0) && (key < [self numKeys]) && (keys[key])) {
        ...
        if (key > [self numKeys]) OOLog(kOOLogKeyCodeOutOfRange, @ "Translated key: %d out of range", key);
    }
}
...
- (void)dumpRGBAToFileNamed:(NSString *)name
    ...
    if (bitmap != nil) {
        ...
        [[bitmap representationUsingType:NSPNGFileType properties:[NSDictionary dictionary]] writeToFile:filepath
                                                                                              atomically:YES];
    }
}
