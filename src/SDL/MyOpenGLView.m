...
+ (NSMutableDictionary *)getNativeSize {
    ...
    if (SDL_GetWMInfo(&dpyInfo)) {
        ...
        OOLog(@"display.mode.list.native",
              @"X11 native resolution detected: %d x %d",
              nativeDisplayWidth,
              nativeDisplayHeight);
    } else {
        OOLog(
            @"display.mode.list.native.failed", @"%@", @"SDL_GetWMInfo failed, defaulting to 1024x768 for native size");
    }
#elif OOLITE_WINDOWS
    nativeDisplayWidth = GetSystemMetrics(SM_CXSCREEN);
    nativeDisplayHeight = GetSystemMetrics(SM_CYSCREEN);
    OOLog(@"display.mode.list.native",
          @"Windows native resolution detected: %d x %d",
          nativeDisplayWidth,
          nativeDisplayHeight);
#else
...
- (id)init {
    ...
    NSString *versionString = [NSString
        stringWithFormat:@"Oolite v%@", [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleVersion"]];
    ...
    _hdrToneMapper = OOHDRToneMapperFromString([prefs oo_stringForKey:@"hdr-tone-mapper"
                                                         defaultValue:@"OOHDR_TONEMAPPER_ACES_APPROX"]);
    ...
    _sdrToneMapper = OOSDRToneMapperFromString([prefs oo_stringForKey:@"sdr-tone-mapper"
                                                         defaultValue:@"OOSDR_TONEMAPPER_ACES"]);
        ...
        if (surface == NULL) {
            ...
            if (surface == NULL) {
                ...
                if (showSplashScreen) {
                    ...
                    OOLogWARN(@"display.mode.conflict",
                              @"Possible incompatibility between the splash screen and video drivers detected.");
                    OOLogWARN(@"display.mode.conflict",
                              @"Oolite will start without showing the splash screen from now on. Override with "
                              @"'oolite.exe -splash'");
                }
#endif
                exit(1);
    ...
    _mouseVirtualStickSensitivityFactor = OOClamp_0_1_f([prefs oo_floatForKey:@"mouse-flight-sensitivity"
                                                                 defaultValue:0.95f]);
...
#if OOLITE_WINDOWS
    NSDictionary *kmap =
        [NSDictionary dictionaryWithDictionary:[ResourceManager dictionaryFromFilesNamed:@"keymappings_windows.plist"
                                                                                inFolder:@"Config"
                                                                               mergeMode:MERGE_BASIC
                                                                                   cache:NO]];
#else
    NSDictionary *kmap =
        [NSDictionary dictionaryWithDictionary:[ResourceManager dictionaryFromFilesNamed:@"keymappings_linux.plist"
                                                                                inFolder:@"Config"
                                                                               mergeMode:MERGE_BASIC
                                                                                   cache:NO]];
#endif
...
- (void)dealloc {
    if (typedString) [typedString release];

    if (screenSizes) [screenSizes release];
    ...
    if (keyMappings_normal) [keyMappings_normal release];

    if (keyMappings_shifted) [keyMappings_shifted release];
...
- (void)setDisplayMode:(int)mode fullScreen:(BOOL)fsm {
    ...
    currentSize = mode;
    if (fullScreen) [self initialiseGLWithSize:[self modeAsSize:mode]];
}
...
- (void)setScreenSize:(int)sizeIndex {
    ...
    if (fullScreen) [self initialiseGLWithSize:[self modeAsSize:currentSize]];
}
...
- (NSSize)modeAsSize:(int)sizeIndex {
    ...
    return NSMakeSize([[mode objectForKey:kOODisplayWidth] intValue], [[mode objectForKey:kOODisplayHeight] intValue]);
}
...
#define OO_RESET_SDLKEY_MODIFIER(vkCode, kModCode, sdlkCode) \
    do {                                                     \
...
- (void)setWindowBorderless:(BOOL)borderless {
    ...
    if ((!borderless && (currentWindowStyle & WS_CAPTION)) || (borderless && !(currentWindowStyle & WS_CAPTION)))
        return;
    ...
    if (borderless) {
        ...
        SetWindowLong(
            SDL_Window, GWL_STYLE, currentWindowStyle | WS_CAPTION | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZEBOX);
        [self refreshDarKOrLightMode];
    }
...
- (BOOL)isDarkModeOn {
    ...
    HRESULT resultRegGetValue = RegGetValueW(HKEY_CURRENT_USER,
...
- (BOOL)isOutputDisplayHDREnabled {
    ...
    do {
        ...
        if (!pModeInfoArray) {
            if (pPathInfoArray) free(pPathInfoArray);
            OOLog(@"gameView.isOutputDisplayHDREnabled", @"Error! Code: -1");
            return NO;
        }
        ...
        if (dd.StateFlags & DISPLAY_DEVICE_PRIMARY_DEVICE) {
            ...
            while (EnumDisplayDevices(saveDeviceName, j, &dd, 0x00000001)) {
                if (EXPECT(dd.StateFlags & DISPLAY_DEVICE_ACTIVE)) {
                    ...
                    OOLogWARN(@"gameView.isOutputDisplayHDREnabled",
                              @"Primary monitor candidate %s (%s %s) not active.",
                              dd.DeviceName,
                              dd.DeviceString,
                              dd.DeviceID);
...
finished:
    ...
    for (i = 0; i < pathCount; i++) {
        ...
        if (tempResult == ERROR_SUCCESS)
            isAdvColorInfo2DetectionSuccess = YES;
        else {
            OOLogWARN(@"gameView.isOutputDisplayHDREnabled",
                      @"Received 0x%08lX while attempting to detect HDR mode using Advanced Color Info 2 API. Retrying "
                      @"detection using legacy API.",
                      HRESULT_FROM_WIN32(tempResult));
        ...
        if (isPrimaryDisplayDevice && ((isAdvColorInfo2DetectionSuccess && advColorInfo2.highDynamicRangeSupported &&
                                        advColorInfo2.activeColorMode == DISPLAYCONFIG_ADVANCED_COLOR_MODE_HDR) ||
                                       (!isAdvColorInfo2DetectionSuccess && advColorInfo.advancedColorSupported &&
                                        advColorInfo.advancedColorEnabled && !advColorInfo.wideColorEnforced))) {
            result = YES;
            break;
        }
    }
    ...
    OOLog(@"gameView.isOutputDisplayHDREnabled",
          @"HDR display output requested - checking availability: %@",
          result ? @"YES" : @"NO");
...
- (void)initialiseGLWithSize:(NSSize)v_size useVideoMode:(BOOL)v_mode {
    ...
    // are we attempting to go to a different screen resolution? Note: this also takes care of secondary monitor
    // situations because by design the only resolution available for fullscreen on a secondary display device is its
    // native one - Nikos 20150605
    BOOL changingResolution =
        ...
        ((fullScreen && (settings.dmPelsWidth != viewSize.width || settings.dmPelsHeight != viewSize.height)) ||
         (wasFullScreen &&
          (settings.dmPelsWidth != [[[screenSizes objectAtIndex:0] objectForKey:kOODisplayWidth] intValue] ||
   ...
   */
        ...
        if (lastWindowPlacementMaximized) {
            ...
            GetWindowRect(SDL_Window, &lastGoodRect);

        // ok, can go fullscreen now
        SetForegroundWindow(SDL_Window);
        if (changingResolution) {
            if (ChangeDisplaySettingsEx(monitorInfo.szDevice, &settings, NULL, CDS_FULLSCREEN, NULL) !=
        ...
        */
        ...
        if (!lastWindowPlacementMaximized) {
            MoveWindow(SDL_Window,
                       (monitorInfo.rcMonitor.right - monitorInfo.rcMonitor.left - (int)viewSize.width) / 2 +
                           monitorInfo.rcMonitor.left,
    ...
    if (!fullScreen && (bounds.size.width != wDC.right - wDC.left || bounds.size.height != wDC.bottom - wDC.top)) {
    ...
    if (v_mode == NO) videoModeFlags |= SDL_NOFRAME;
    if (fullScreen == YES) {
        videoModeFlags |= SDL_FULLSCREEN;
    } else {
        videoModeFlags |= SDL_RESIZABLE;
...
- (BOOL)snapShot:(NSString *)filename {
    ...
    if (withFilename && [[NSFileManager defaultManager] fileExistsAtPath:pathToPic]) {
        OOLog(@"screenshot.filenameExists",
              @"Snapshot \"%@%@\" already exists - adding numerical sequence.",
              pathToPic,
              extension);
        pathToPic = nil;
    }
    ...
    tmpSurface =
        SDL_CreateRGBSurfaceFrom(pixls, surface->w, surface->h, 24, surface->w * 3, 0xFF, 0xFF00, 0xFF0000, 0x0);
#if SNAPSHOTS_PNG_FORMAT
    ...
    if ([self hdrOutput]) {
        NSString *fileExtension = [[NSUserDefaults standardUserDefaults] oo_stringForKey:@"hdr-snapshot-format"
                                                                            defaultValue:SNAPSHOTHDR_EXTENSION_DEFAULT];

        // we accept file extension with or without a leading dot; if it is without, insert it at the beginning now
        if (![[fileExtension substringToIndex:1] isEqual:@"."])
        ...
        if (![fileExtension isEqual:SNAPSHOTHDR_EXTENSION_EXR] && ![fileExtension isEqual:SNAPSHOTHDR_EXTENSION_HDR]) {
            OOLog(@"screenshotHDR",
                  @"Unrecognized HDR file format requested, defaulting to %@",
        ...
        if (([fileExtension isEqual:SNAPSHOTHDR_EXTENSION_EXR] &&
             SaveEXRSnapshot(
                 [pathToPicHDR cStringUsingEncoding:NSUTF8StringEncoding], surface -> w, surface -> h, pixlsf) !=
                 0)  // TINYEXR_SUCCESS
            || ([fileExtension isEqual:SNAPSHOTHDR_EXTENSION_HDR] &&
                !stbi_write_hdr(
                    [pathToPicHDR cStringUsingEncoding:NSUTF8StringEncoding], surface -> w, surface -> h, 3, pixlsf))) {
...
#if SNAPSHOTS_PNG_FORMAT
// This method is heavily based on 'Mars, Land of No Mercy' SDL examples, by Angelo "Encelo" Theodorou, see
...
- (void)clearKeys {
    int i;
    lastKeyShifted = NO;
    for (i = 0; i < [self numKeys]; i++) keys[i] = NO;
}
        ...
        switch (event.type) {
            ...
            case SDL_KEYDOWN:
                ...
                switch (kbd_event->keysym.sym) {
                    ...
                    case SDLK_KP0:
                        key_id = (!allowingStringInput ? gvNumberPadKey0 : gvNumberKey0);
                        special_key = YES;
                        break;
                    case SDLK_KP1:
                        key_id = (!allowingStringInput ? gvNumberPadKey1 : gvNumberKey1);
                        special_key = YES;
                        break;
                    case SDLK_KP2:
                        key_id = (!allowingStringInput ? gvNumberPadKey2 : gvNumberKey2);
                        special_key = YES;
                        break;
                    case SDLK_KP3:
                        key_id = (!allowingStringInput ? gvNumberPadKey3 : gvNumberKey3);
                        special_key = YES;
                        break;
                    case SDLK_KP4:
                        key_id = (!allowingStringInput ? gvNumberPadKey4 : gvNumberKey4);
                        special_key = YES;
                        break;
                    case SDLK_KP5:
                        key_id = (!allowingStringInput ? gvNumberPadKey5 : gvNumberKey5);
                        special_key = YES;
                        break;
                    case SDLK_KP6:
                        key_id = (!allowingStringInput ? gvNumberPadKey6 : gvNumberKey6);
                        special_key = YES;
                        break;
                    case SDLK_KP7:
                        key_id = (!allowingStringInput ? gvNumberPadKey7 : gvNumberKey7);
                        special_key = YES;
                        break;
                    case SDLK_KP8:
                        key_id = (!allowingStringInput ? gvNumberPadKey8 : gvNumberKey8);
                        special_key = YES;
                        break;
                    case SDLK_KP9:
                        key_id = (!allowingStringInput ? gvNumberPadKey9 : gvNumberKey9);
                        special_key = YES;
                        break;
                    case SDLK_KP_PERIOD:
                        key_id = (!allowingStringInput ? gvNumberPadKeyPeriod : 46);
                        special_key = YES;
                        break;
                    case SDLK_KP_DIVIDE:
                        key_id = (!allowingStringInput ? gvNumberPadKeyDivide : 47);
                        special_key = YES;
                        break;
                    case SDLK_KP_MULTIPLY:
                        key_id = (!allowingStringInput ? gvNumberPadKeyMultiply : 42);
                        special_key = YES;
                        break;
                    case SDLK_KP_MINUS:
                        key_id = (!allowingStringInput ? gvNumberPadKeyMinus : 45);
                        special_key = YES;
                        break;
                    case SDLK_KP_PLUS:
                        key_id = (!allowingStringInput ? gvNumberPadKeyPlus : 43);
                        special_key = YES;
                        break;
                    case SDLK_KP_EQUALS:
                        key_id = (!allowingStringInput ? gvNumberPadKeyEquals : 61);
                        special_key = YES;
                        break;
                    case SDLK_KP_ENTER:
                        key_id = gvNumberPadKeyEnter;
                        special_key = YES;
                        break;
                    case SDLK_HOME:
                        key_id = gvHomeKey;
                    ...
                    case SDLK_UP:
                        ...
                        break;
                    case SDLK_DOWN:
                        key_id = gvArrowKeyDown;
                        special_key = YES;
                        break;
                    case SDLK_LEFT:
                        key_id = gvArrowKeyLeft;
                        special_key = YES;
                        break;
                    case SDLK_RIGHT:
                        key_id = gvArrowKeyRight;
                        special_key = YES;
                        break;
                    case SDLK_PAUSE:
                        key_id = gvPauseKey;
                    ...
                    case SDLK_DELETE:
                        ...
                        break;
                    case SDLK_F1:
                        key_id = gvFunctionKey1;
                        special_key = YES;
                        break;
                    case SDLK_F2:
                        key_id = gvFunctionKey2;
                        special_key = YES;
                        break;
                    case SDLK_F3:
                        key_id = gvFunctionKey3;
                        special_key = YES;
                        break;
                    case SDLK_F4:
                        key_id = gvFunctionKey4;
                        special_key = YES;
                        break;
                    case SDLK_F5:
                        key_id = gvFunctionKey5;
                        special_key = YES;
                        break;
                    case SDLK_F6:
                        key_id = gvFunctionKey6;
                        special_key = YES;
                        break;
                    case SDLK_F7:
                        key_id = gvFunctionKey7;
                        special_key = YES;
                        break;
                    case SDLK_F8:
                        key_id = gvFunctionKey8;
                        special_key = YES;
                        break;
                    case SDLK_F9:
                        key_id = gvFunctionKey9;
                        special_key = YES;
                        break;
                    case SDLK_F10:
                        key_id = gvFunctionKey10;
                        special_key = YES;
                        break;
                    case SDLK_F11:
                        key_id = gvFunctionKey11;
                        special_key = YES;
                        break;
                    case SDLK_F12:
                        key_id = 327;
                ...
                if (((!special_key && (ctrl || key_id == 0)) ||
                     ([self isCapsLockOn] && (!special_key && !allowingStringInput))) &&
                    !modifier_pressed)  //
                {
                ...
                if (key_id == 0) {
                    ...
                    if (!shift) {
                        NSString *keyNormal =
                            [keyMappings_normal objectForKey:[NSString stringWithFormat:@"%d", scan_code]];
                        if (keyNormal) key_id = [keyNormal integerValue];
                    } else {
                        NSString *keyShifted =
                            [keyMappings_shifted objectForKey:[NSString stringWithFormat:@"%d", scan_code]];
                ...
                OOLog(kOOLogKeyDown,
                      @"Keydown scancode = %d, unicode = %i, sym = %i, character = %c, shift = %d, ctrl = %d, alt = %d",
                      scan_code,
                      key_id,
            ...
            case SDL_KEYUP:
                ...
                // all the work should have been down on the keydown event, so all we need to do is get the unicode
                ...
                OOLog(kOOLogKeyUp,
                      @"Keyup scancode = %d, unicode = %i, sym = %i, character = %c, shift = %d, ctrl = %d, alt = %d",
                      scan_code,
                      key_id,
                      kbd_event->keysym.sym,
                      key_id,
                      ...
                      ctrl,
                      opt);
                // OOLog(kOOLogKeyUp, @"Keyup scancode = %d, shift = %d, ctrl = %d, alt = %d", scan_code, shift, ctrl,
                // opt);
                ...
                switch (kbd_event->keysym.sym) {
                    ...
                    default:
                        // OOLog(@"keys.test", @"Unhandled Keyup scancode with unicode = 0: %d",
...
`					*/
                        ...
                        if (!SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_TIME_CRITICAL)) {
                            ...
                            OOLog(@"wm_setfocus.message",
                                  @"Setting thread priority to time critical failed! (error code: %ld)",
                                  dwLastError);
                        }
                    ...
                    case WM_KILLFOCUS:
                        if (!SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_NORMAL)) {
                            dwLastError = GetLastError();
                            OOLog(@"wm_killfocus.message",
                                  @"Setting thread priority to normal failed! (error code: %ld)",
                                  dwLastError);
                        }
...
- (void)populateFullScreenModelist {
    ...
    for (i = 0; modes[i]; i++) {
        ...
        if (modes[i]->w != lastw || modes[i]->h != lasth) {
            ...
            [mode setValue:[NSNumber numberWithInt:(int)modes[i]->w] forKey:kOODisplayWidth];
            [mode setValue:[NSNumber numberWithInt:(int)modes[i]->h] forKey:kOODisplayHeight];
            [mode setValue:[NSNumber numberWithInt:0] forKey:kOODisplayRefreshRate];
...
- (NSSize)loadWindowSize {
    ...
    if ([defaults objectForKey:@"window_width"] && [defaults objectForKey:@"window_height"]) {
        windowSize = NSMakeSize([defaults integerForKey:@"window_width"], [defaults integerForKey:@"window_height"]);
    } else {
...
- (int)loadFullscreenSettings {
    ...
    if ([userDefaults objectForKey:@"display_width"]) width = [userDefaults integerForKey:@"display_width"];
    if ([userDefaults objectForKey:@"display_height"]) height = [userDefaults integerForKey:@"display_height"];
    if ([userDefaults objectForKey:@"display_refresh"]) refresh = [userDefaults integerForKey:@"display_refresh"];
    if ([userDefaults objectForKey:@"fullscreen"]) fullScreen = [userDefaults boolForKey:@"fullscreen"];
...
- (int)findDisplayModeForWidth:(unsigned int)d_width Height:(unsigned int)d_height Refresh:(unsigned int)d_refresh {
    ...
    OOLog(@"display.mode.found.failed",
          @"Failed to find mode: width=%d height=%d refresh=%d",
          d_width,
          d_height,
          d_refresh);
...
- (void)dumpRGBAToFileNamed:(NSString *)name
    ...
    SDL_Surface *tmpSurface =
        SDL_CreateRGBSurfaceFrom(bytes, width, height, 32, rowBytes, 0xFF, 0xFF00, 0xFF0000, 0xFF000000);
...
- (void)dumpRGBAToRGBFileNamed:(NSString *)rgbName
              andGrayFileNamed:(NSString *)grayName
                        ...
                        height:(NSUInteger)height
                      rowBytes:(NSUInteger)rowBytes {
    if ((rgbName == nil && grayName == nil) || bytes == NULL || width == 0 || height == 0 || rowBytes < width * 4)
        return;
    ...
    for (y = 0; y < height; y++) {
        ...
        for (x = 0; x < width; x++) {
            ...
            *rgbPx++ = *srcPx++;
            trivalAlpha =
                trivalAlpha && ((*srcPx == 0xFF) || (*srcPx == 0x00));  // Look for any "interesting" pixels in alpha.
            *grayPx++ = *srcPx++;
        }
    }

    [self dumpRGBToFileNamed:rgbName bytes:rgbBytes width:width height:height rowBytes:width * 3];
    free(rgbBytes);

    if (!trivalAlpha) {
        [self dumpGrayToFileNamed:grayName bytes:grayBytes width:width height:height rowBytes:width];
    }
    free(grayBytes);
}
#endif
