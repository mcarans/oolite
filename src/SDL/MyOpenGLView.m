...
- (id)init
{
    ...
    for (argEnum = [arguments objectEnumerator]; (arg = [argEnum nextObject]);) {
        ...
        if ([arg isEqual:@"-novsync"] || [arg isEqual:@"--novsync"])
            vSyncPreference = NO;

        if ([arg isEqual:@"-hdr"])
            bitsPerColorComponent = 16;
    ...
    [OOSound setUp];
    if (![OOSound isSoundOK])
        OOLog(@"sound.init", @"%@", @"Sound system disabled.");
    ...
    if (_mouseVirtualStickSensitivityFactor < 0.005f)
        _mouseVirtualStickSensitivityFactor = 0.005f;
...
- (void)getDisplayDimensions:(unsigned*)width height:(unsigned*)height
{
    ...
    if (SDL_GetDisplayBounds(displayID, &displaybounds)) {
        if (width)
            *width = (unsigned)displaybounds.w;
        if (height)
            *height = (unsigned)displaybounds.h;
    }
}
...
{
    if (newMaxBrightness < MIN_HDR_MAXBRIGHTNESS)
        newMaxBrightness = MIN_HDR_MAXBRIGHTNESS;
    if (newMaxBrightness > MAX_HDR_MAXBRIGHTNESS)
        newMaxBrightness = MAX_HDR_MAXBRIGHTNESS;
...
{
    if (newPaperWhiteBrightness < MIN_HDR_PAPERWHITE)
        newPaperWhiteBrightness = MIN_HDR_PAPERWHITE;
    if (newPaperWhiteBrightness > MAX_HDR_PAPERWHITE)
        newPaperWhiteBrightness = MAX_HDR_PAPERWHITE;
...
{
    if (newToneMapper > OOHDR_TONEMAPPER_REINHARD)
        newToneMapper = OOHDR_TONEMAPPER_REINHARD;
    if (newToneMapper < OOHDR_TONEMAPPER_NONE)
...
{
    if (newToneMapper > OOSDR_TONEMAPPER_REINHARD)
        newToneMapper = OOSDR_TONEMAPPER_REINHARD;
    if (newToneMapper < OOSDR_TONEMAPPER_NONE)
        newToneMapper = OOSDR_TONEMAPPER_NONE;
...
- (int)loadFullscreenSettings
{
    ...
    for (i = 0; i < [cmdline_arguments count]; i++) {
        if ([[cmdline_arguments objectAtIndex:i] isEqual:@"-fullscreen"])
            fullScreen = YES;
        if ([[cmdline_arguments objectAtIndex:i] isEqual:@"-windowed"])
            fullScreen = NO;
    }
...
{
    if (name == nil || bytes == NULL || width == 0 || height == 0 || rowBytes < width * 4)
        return;
...
- (void)dumpRGBToFileNamed:(NSString*)name
                  ...
                  rowBytes:(NSUInteger)rowBytes
{
    if (name == nil || bytes == NULL || width == 0 || height == 0 || rowBytes < width * 3)
        return;
...
- (void)dumpGrayToFileNamed:(NSString*)name
                   ...
                   rowBytes:(NSUInteger)rowBytes
{
    if (name == nil || bytes == NULL || width == 0 || height == 0 || rowBytes < width)
        return;
...
- (void)dumpGrayAlphaToFileNamed:(NSString*)name
                        ...
                        rowBytes:(NSUInteger)rowBytes
{
    if (name == nil || bytes == NULL || width == 0 || height == 0 || rowBytes < width * 2)
        return;
