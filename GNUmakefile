...
ifeq ($(GNUSTEP_HOST_OS),mingw32)
    ...
    ADDITIONAL_OBJC_LIBS         = -L$(WIN_DEPS_DIR)/lib -L$(JS_LIB_DIR) -lglu32 -lopengl32 -lopenal.dll -lpng.dll -lmingw32 -lSDLmain -lSDL -lvorbisfile.dll -lvorbis.dll -lz -lgnustep-base -l$(JS_IMPORT_LIBRARY) -lshlwapi -ldwmapi -lwinmm -mwindows
...
#     oolite_LIB_DIRS              += -L$(GNUSTEP_LOCAL_ROOT)/lib -L$(WIN_DEPS_DIR)/lib -L$(JS_LIB_DIR)
    ...
    ifeq ($(ESPEAK),yes)
        ADDITIONAL_OBJC_LIBS     += -lespeak-ng.dll
